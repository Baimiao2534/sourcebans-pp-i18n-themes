...
{else}
    {if NOT $groupbanning_enabled}
        <div class="card" data-testid="groupban-disabled">
            <div class="card__body">
                <h1 style="font-size:1.25rem;font-weight:600;margin:0">功能已禁用</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    分组封禁已在
                    <strong>config.enablegroupbanning</strong>
                    中关闭。请从设置选项卡重新启用以使用此功能。
                </p>
            </div>
        </div>
    {else}
        <section class="p-6" data-testid="groupban-section" style="max-width:48rem">
            <div class="mb-6">
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">分组封禁</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    一次性封禁 Steam 社区分组的所有成员。
                </p>
            </div>
            ...
            {if NOT $list_steam_groups}
                <form id="groupban-form"
                    ...
                    {csrf_field}
                    <div>
                        <label class="label" for="groupurl">Steam 社区分组 URL</label>
                        <input type="text"
                    ...
                    <div>
                        <label class="label" for="groupreason">分组封禁原因</label>
                        <textarea class="textarea"
                                  id="groupreason"
                                  name="groupreason"
                                  rows="4"
                                  placeholder="为什么要封禁此分组？"
            ...
            {else}
                <div class="card p-6 space-y-4" data-testid="groupban-from-player">
                    <p class="text-sm text-muted m-0">
                        {if $player_name}
                            <strong class="font-medium">{$player_name|escape}</strong> 所属的分组已加载到下方。
                        {else}
                            玩家所属的所有分组在此列出。
                        {/if}
                        勾选您想封禁的分组。
                    ...
                    <div id="steamGroupsText"
                         ...
                         class="text-sm text-muted">正在加载分组…</div>
                ...
                (function (friendid) {
                    sb.ready(function () {
                        sb.api.call(Actions.BansGetGroups, { friendid: friendid }).then(function (r) {
                            ...
                            if (!tbl) return;
                            if (groups.length === 0) {
                                sb.message.error('错误', "获取分组数据时出错。可能该玩家不属于任何分组，或其资料是私密的？", 'index.php?p=banlist');
                                var txt = sb.$id('steamGroupsText');
                                if (txt) txt.innerHTML = '<i>无分组...</i>';
                                return;
                            }
                            groups.forEach(function (g, i) {
                                ...
                                td2.appendChild(document.createTextNode(' 成员)'));
                            });
