...
<p class="lead">
    環境需求檢查 — 在我們接觸您的資料庫之前。
    {if $errors > 0}
        <strong>{$errors} 個阻斷性問題</strong>
        必須先解決才能繼續。
    {elseif $warnings > 0}
        <strong>{$warnings} 個警告</strong>。
        您可以繼續，但部分功能可能無法運作。
    {else}
        一切看起來正常。
    {/if}
                ...
                <tbody>
                    {foreach from=$group.rows item=row}
                        <tr>
                            ...
                            <td>
                                {if $row.status == 'ok'}
                                    <span class="install-pill install-pill--ok">通過</span>
                                {elseif $row.status == 'warn'}
                                    <span class="install-pill install-pill--warn">警告</span>
                                {else}
                                    <span class="install-pill install-pill--err">失敗</span>
                                {/if}
                            </td>
                            <td>{$row.detail}</td>
                        </tr>
