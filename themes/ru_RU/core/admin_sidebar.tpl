...
<aside class="admin-sidebar"
       ...
       aria-label="{if $sidebar_label == 'Admin sections'}Разделы администратора{elseif $sidebar_label == 'Bans sections'}Разделы банов{elseif $sidebar_label == 'Group sections'}Разделы групп{elseif $sidebar_label == 'MOD sections'}Разделы модов{elseif $sidebar_label == 'Server sections'}Разделы серверов{elseif $sidebar_label == 'Settings sections'}Разделы настроек{elseif $sidebar_label == 'Page sections'}Разделы страниц{else}{$sidebar_label}{/if}">
    <details class="admin-sidebar__details" open>
        <summary class="admin-sidebar__summary">
            <span class="admin-sidebar__summary-label">
                ...
                {if $sidebar_label == 'Admin sections'}Разделы администратора{elseif $sidebar_label == 'Bans sections'}Разделы банов{elseif $sidebar_label == 'Group sections'}Разделы групп{elseif $sidebar_label == 'MOD sections'}Разделы модов{elseif $sidebar_label == 'Server sections'}Разделы серверов{elseif $sidebar_label == 'Settings sections'}Разделы настроек{elseif $sidebar_label == 'Page sections'}Разделы страниц{else}{$sidebar_label}{/if}
            </span>
            <i data-lucide="chevron-down" class="admin-sidebar__chevron" style="width:14px;height:14px"></i>
        </summary>
        <nav class="admin-sidebar__nav" aria-label="{if $sidebar_label == 'Admin sections'}Разделы администратора{elseif $sidebar_label == 'Bans sections'}Разделы банов{elseif $sidebar_label == 'Group sections'}Разделы групп{elseif $sidebar_label == 'MOD sections'}Разделы модов{elseif $sidebar_label == 'Server sections'}Разделы серверов{elseif $sidebar_label == 'Settings sections'}Разделы настроек{elseif $sidebar_label == 'Page sections'}Разделы страниц{else}{$sidebar_label}{/if}">
            {foreach from=$tabs item=tab}
                <a class="sidebar__link admin-sidebar__link"
                   ...
                   {if $tab.slug == $active_tab}aria-current="page"{/if}>
                    ...
                    <span>{if $tab.name == 'Admins list'}Список администраторов{elseif $tab.name == 'Add admin'}Добавить администратора{elseif $tab.name == 'Add ban'}Добавить бан{elseif $tab.name == 'Ban protests'}Апелляции на бан{elseif $tab.name == 'Ban submissions'}Заявки на бан{elseif $tab.name == 'Import bans'}Импорт банов{elseif $tab.name == 'Groups'}Группы{elseif $tab.name == 'Groups list'}Список групп{elseif $tab.name == 'Add group'}Добавить группу{elseif $tab.name == 'Mods list'}Список модов{elseif $tab.name == 'Add mod'}Добавить мод{elseif $tab.name == 'Server list'}Список серверов{elseif $tab.name == 'Add server'}Добавить сервер{elseif $tab.name == 'Server admins'}Администраторы серверов{elseif $tab.name == 'Settings'}Настройки{elseif $tab.name == 'Features'}Возможности{elseif $tab.name == 'Logs'}Журналы{elseif $tab.name == 'Themes'}Темы{elseif $tab.name == 'Add comms block'}Добавить блокировку связи{else}{$tab.name}{/if}</span>
                </a>
            {/foreach}
        </nav>
    </details>
