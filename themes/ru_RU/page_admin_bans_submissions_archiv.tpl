...
{if NOT $permissions_submissions}
    <div class="card" data-testid="submissions-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">У вас нет прав на просмотр заявок на баны.</p>
        </div>
    </div>
{else}
        ...
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                ...
                <p class="text-sm text-muted m-0 mt-2">
                    Архивированные, принятые или отклонённые заявки.
                </p>
            </div>
        ...
        {if $submission_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="submissions-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">Архив заявок пуст</h2>
                    <p class="empty-state__body">Принятые, отклонённые или архивированные заявки будут перенесены сюда для протокола.</p>
                </div>
            </div>
        {else}
                    ...
                    <details class="queue-row ban-row ban-row--expired"
                        ...
                        <div class="p-4" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            <div class="text-sm font-medium mb-3">
                                Архивировано: {$sub.archive|escape}
                            </div>
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    ...
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.SteamId == ""}<span class="text-faint">SteamID отсутствует</span>{else}{$sub.SteamId|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">IP</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.sip == ""}<span class="text-faint">IP-адрес отсутствует</span>{else}{$sub.sip|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">Причина</dt>
                                    ...
                                    <dd style="margin:0" id="suba{$sub.subid}">
                                        {if $sub.hostname == ""}
                                            <i class="text-faint">Получение имени хоста</i>
                                        {else}
                                    ...
                                    <dt class="text-muted">Отправитель</dt>
                                    <dd style="margin:0">
                                        {if $sub.subname == ""}<span class="text-faint">имя отсутствует</span>{else}{$sub.subname|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">IP отправителя</dt>
                                    <dd class="font-mono" style="margin:0">{$sub.ip|escape}</dd>

                                    <dt class="text-muted">Архивировал</dt>
                                    <dd style="margin:0">
                                        {if !empty($sub.archivedby)}{$sub.archivedby|escape}{else}<i class="text-faint">Администратор удалён.</i>{/if}
                                    </dd>
                                </dl>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var msg;
        if (archiv === '2') msg = 'Восстановить заявку на бан «' + name + '» из архива?';
        else if (archiv === '1') msg = 'Переместить заявку на бан «' + name + '» в архив?';
        else msg = 'Удалить заявку на бан «' + name + '»?';
