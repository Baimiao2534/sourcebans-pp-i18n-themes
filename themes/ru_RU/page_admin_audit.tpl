...
<div class="audit-page">
    <header class="audit-page__header">
        <div>
            <h1 class="audit-page__title">Журнал аудита</h1>
            <p class="audit-page__subtitle">Каждое административное действие, записанное панелью ({$total_count} {if $total_count == 1}событие{else}событий{/if}).</p>
        </div>
    </header>
    ...
    <div class="audit-filters">
        <form method="get" action="index.php" role="search" aria-label="Поиск по журналу аудита" style="flex:1 1 16rem;max-width:22rem">
        ...
        <form method="get" action="index.php" class="flex items-center gap-2" style="flex-wrap:wrap" aria-label="Фильтр журнала аудита по важности">
            <input type="hidden" name="p" value="admin">
            <input type="hidden" name="c" value="audit">
            ...
            <div class="empty-state"
                ...
                <h2 class="empty-state__title">
                    {if $current_severity != '' || $search != ''}
                        Нет событий аудита, соответствующих этим фильтрам
                    {else}
                        События аудита ещё не записаны
                    {/if}
                </h2>
                <p class="empty-state__body">
                    {if $current_severity != '' || $search != ''}
                        Попробуйте другой поисковый запрос или уровень важности, либо сбросьте фильтры, чтобы увидеть все записанные действия администраторов.
                    {else}
                        Как только администраторы начнут модерировать через панель, каждое действие будет записано здесь для просмотра.
                    {/if}
                </p>
                {if $current_severity != '' || $search != ''}
    ...
    {if $page_count > 1 || $current_page > 1}
        <nav class="audit-pager" aria-label="Пагинация журнала аудита">
            <div class="audit-pager__status" aria-live="polite">
                Страница {$current_page} из {$page_count}
            </div>
