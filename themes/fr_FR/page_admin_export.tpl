...
<section class="p-6" data-testid="admin-export-section" style="max-width:56rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">Exportation complète des données</h1>
        <p class="text-sm text-muted m-0 mt-2">
            Téléchargez une archive ZIP de chaque ligne du panneau et de chaque démo téléversée.
            Utilisez-la pour les sauvegardes, la migration ou l&rsquo;analyse.
            Consultez la <a href="https://sbpp.github.io/configuring/data-export/" target="_blank" rel="noopener noreferrer">documentation d&rsquo;exportation des données</a>
            pour le format de transmission et la configuration par mode.
        </p>
        <p class="text-xs text-muted m-0 mt-2" data-testid="admin-export-rookhelm">
            Vous passez à un panneau hébergé ?
            <a href="https://rookhelm.com?utm_source=sourcebans-pp&amp;utm_medium=referral&amp;utm_content=panel-export"
               target="_blank"
               rel="noopener">RookHelm</a>
            importe cet export directement.
        </p>
    </div>

    <div class="card mb-4" data-testid="admin-export-summary-card">
        <div class="card__header">
            <h2 style="font-size:1.125rem;font-weight:600;margin:0">Contenu de l&rsquo;archive</h2>
            <p class="text-xs text-muted m-0 mt-1">
                Chaque exportation reçoit un nouvel identifiant d&rsquo;archive. Les compteurs ci-dessous sont un instantané au chargement de la page.
            </p>
        </div>
        <div class="card__body">
            <dl class="install-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(14rem,1fr));gap:0.75rem 1.5rem;margin:0">
                ...
                <div>
                    <dt class="text-xs text-muted">Limite (PUT S3)</dt>
            ...
            {if $row_counts|@count > 0}
                <details class="mt-4">
                    <summary class="text-sm" style="cursor:pointer">Compteurs par entité</summary>
    ...
    {if $exceeds_cap}
        ...
        <div class="card" data-testid="admin-export-cap-empty" data-filtered="false">
            <div class="empty-state">
                ...
                <h2 class="empty-state__title">L&rsquo;archive dépasse la limite PUT S3 de 5 Gio</h2>
                <p class="empty-state__body">
                    L&rsquo;archive estimée est de {($estimated_bundle_bytes / 1024 / 1024)|number_format:1} Mio.
                    Utilisez le formulaire de téléchargement ZIP ci-dessus (sans limite),
                    ou supprimez des démos / lignes et rechargez pour activer le formulaire S3.
                </p>
            </div>
        </div>
    {/if}
    ...
    <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(22rem,1fr));gap:1rem;margin-top:1rem">
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">Diffuser un téléchargement ZIP</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    Le navigateur télécharge l&rsquo;archive directement.
                    Gardez l&rsquo;onglet ouvert jusqu&rsquo;à la fin ; le fermer annule le transfert.
                </p>
                <p class="text-xs text-muted m-0">
                    Aucun fichier temporaire. La barre de progression avance en temps réel.
                </p>
            </div>
        ...
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">Téléverser vers une URL S3 pré-signée</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    Le panneau prépare l&rsquo;archive sur le disque, puis la téléverse vers votre URL.
                    Fermer l&rsquo;onglet annule le téléversement ; le journal d&rsquo;audit enregistre la tentative.
                </p>
                <label class="label mt-3" for="admin-export-s3-url">URL PUT pré-signée</label>
                <textarea class="textarea"
                          ...
                          required
                          pattern="^https://[^\s]+$"
                          title="Collez une URL PUT HTTPS pré-signée depuis votre client S3 (par ex. aws s3 presign --http-method PUT)"
                          placeholder="https://bucket.s3.region.amazonaws.com/path/sbpp-export.zip?X-Amz-..."
                          data-testid="admin-export-s3-url"></textarea>
                <p class="text-xs text-muted m-0 mt-1">
                    HTTPS uniquement. Utilisez une expiration courte (1 heure ou moins).
                    L&rsquo;URL est un identifiant d&rsquo;écriture à usage unique ; ne la collez pas dans le chat ou les journaux.
                </p>
            </div>
            <div class="card__header" style="border-top:1px solid var(--border);border-bottom:0;justify-content:flex-end">
                <button type="submit"
