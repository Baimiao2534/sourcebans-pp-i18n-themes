...
<form method="get"
    ...
    <div class="card__header">
        <div>
            ...
            <p>Каждая строка — отдельный критерий поиска. Кнопка &laquo;Поиск&raquo; справа отправляет запрос только со значением этой строки.</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-name" style="grid-column:1;align-self:end">Ник</label>
            <input class="input"
                   id="search-comms-name"
                   type="text"
                   placeholder="Частичное совпадение с ником игрока&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-reason" style="grid-column:1;align-self:end">Причина</label>
            <input class="input"
                   id="search-comms-reason"
                   type="text"
                   placeholder="Частичное совпадение с записанной причиной&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">Дата</span>
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">Длительность</span>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-length-op"
                        data-testid="search-comms-length-op"
                        style="width:5rem">
                    <option value="e"  title="равно">=</option>
                    <option value="h"  title="больше">&gt;</option>
                    <option value="l"  title="меньше">&lt;</option>
                    <option value="eh" title="больше или равно">&gt;=</option>
                    <option value="el" title="меньше или равно">&lt;=</option>
                </select>
                <select class="select"
                    ...
                    <option value="other">Другая длительность (минуты)&hellip;</option>
                </select>
                <input type="text"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-btype">Тип</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-btype"
                        data-testid="search-comms-btype">
                    <option value="1" selected>Мьют</option>
                    <option value="2">Гаг</option>
                </select>
            </div>
            <button type="submit"
                    class="btn btn--secondary btn--sm"
        ...
        {if NOT $hideadminname}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-admin">Администратор</label>
                <div class="flex items-center gap-2" style="flex-wrap:wrap">
                    <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-server">Сервер</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-server"
                        data-testid="search-comms-server">
                    <option value="0" selected>Веб-бан</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                data-sid="{$server.sid}"
                                data-ip="{$server.ip}"
                                data-port="{$server.port}"
                                data-server-host>Загрузка&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
            <button type="submit"
        ...
        {if $is_admin}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-comment" style="grid-column:1;align-self:end">Комментарий</label>
                <input class="input"
                       id="search-comms-comment"
                       type="text"
                       placeholder="Частичное совпадение с заметками администратора&hellip;"
