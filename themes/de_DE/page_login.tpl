        ...
        <div class="card__body">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <div class="text-xs text-muted">Administrationspanel</div>
                </div>
            </div>

            <h1 class="m-0" style="font-size:var(--fs-2xl);font-weight:600">Anmelden</h1>
            <p class="text-sm text-muted mt-2">Verwenden Sie Ihre Admin-Zugangsdaten oder melden Sie sich mit Steam an.</p>
            ...
            -{if !$normallogin_show and !$steamlogin_show}-
                <div class="toast"
                    ...
                    <div class="text-sm">
                        Die Anmeldung ist derzeit deaktiviert. Bitte wenden Sie sich an den Administrator.
                    </div>
                </div>
            -{/if}-
            ...
            -{if $steamlogin_show}-
                <a class="btn btn--secondary"
                   ...
                   style="width:100%;margin-top:1.5rem">
                    <i data-lucide="gamepad-2" aria-hidden="true"></i> Mit Steam fortfahren
                </a>
            -{/if}-

            -{if $steamlogin_show and $normallogin_show}-
                <div class="login-shell__divider" aria-hidden="true">
                    <span>Oder mit Zugangsdaten</span>
                </div>
            -{/if}-
            ...
            *}-
            -{if $normallogin_show}-
                <form id="loginForm"
                    ...
                    <div>
                        <label class="label" for="loginUsername">Benutzername</label>
                        <input class="input"
                               id="loginUsername"
                               name="username"
                               type="text"
                    ...
                    <button class="btn btn--primary"
                        ...
                        Anmelden <i data-lucide="arrow-right" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-4 text-xs" style="text-align:center">
                    <a href="index.php?p=lostpassword" data-testid="login-lostpwd">Passwort vergessen?</a>
                </div>
            -{/if}-
        </div>
    </div>
...
(function () {
    ...
    var lockedBody = 'Ihr Konto ist aufgrund zu vieler fehlgeschlagener Anmeldeversuche vorübergehend gesperrt. '
                   + 'Bitte versuchen Sie es in ungefähr ' + time + ' Minute'
                   + (time === 1 ? '' : 'n') + ' erneut.';
    var msgs = {
        no_access:    { kind: 'error', title: 'Kein Zugriff',          body: "Sie haben keine Berechtigung, auf diese Seite zuzugreifen. Bitte melden Sie sich mit einem Konto an, das Zugriff hat." },
        empty_pwd:    { kind: 'info',  title: 'Leeres Passwort',       body: 'Ihr Konto hat ein leeres Passwort. Setzen Sie es über den Link zum Zurücksetzen zurück oder bitten Sie einen Administrator darum.' },
        failed:       { kind: 'error', title: 'Anmeldung fehlgeschlagen', body: 'Der eingegebene Benutzername oder das Passwort war falsch. Wenn Sie Ihr Passwort vergessen haben, verwenden Sie den untenstehenden Link.' },
        steam_failed: { kind: 'error', title: 'Steam-Anmeldung fehlgeschlagen', body: "Die Steam-Anmeldung war erfolgreich, aber Ihre SteamID ist keinem Konto zugeordnet." },
        locked:       { kind: 'error', title: 'Konto gesperrt',        body: lockedBody }
    };
    var msg = msgs[m];
    if (!msg) return;
    function show() {
