...
<div class="p-6 space-y-6" style="max-width:64rem;margin:0 auto;width:100%">
    ...
    <header data-testid="account-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">アカウント</h1>
        <p class="text-sm text-muted m-0 mt-2">権限、パスワード、サーバーパスワード、メールアドレス。</p>
    </header>
    ...
    *}
                ...
                {if $server_permissions}
                    <div class="permissions-grid permissions-grid--server">
                        <section class="permissions-group"
                            ...
                            <h5 class="permissions-group__title">ゲームサーバーフラグ</h5>
                            <ul class="permissions-group__list">
    ...
    <section class="card" data-testid="account-change-password">
        <div class="card__header">
            <div>
                <h3>パスワード変更</h3>
                <p>{$min_pass_len} 文字以上で入力してください。変更成功後にログアウトします。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-password-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-current-password">現在のパスワード</label>
                ...
                <div>
                    <label class="label" for="account-new-password">新しいパスワード</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-password">新しいパスワード（確認）</label>
                    <input class="input" type="password"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-cancel">キャンセル</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-save">保存</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-server-password">
        <div class="card__header">
            <div>
                <h3>サーバーパスワード</h3>
                <p>ゲームサーバーで管理者権限を使用するために必要です。
                    <a href="https://wiki.alliedmods.net/Adding_Admins_%28SourceMod%29#Passwords"
                       target="_blank" rel="noopener"
                       style="color:var(--accent);text-decoration:underline">SourceMod ドキュメント</a>。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-srv-password-form"
                ...
                {if $srvpwset}
                    <div>
                        <label class="label" for="account-current-srv-password">現在のサーバーパスワード</label>
                        <input class="input" type="password"
                ...
                {/if}
                <div>
                    <label class="label" for="account-new-srv-password">新しいサーバーパスワード</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-srv-password">新しいサーバーパスワード（確認）</label>
                    <input class="input" type="password"
                ...
                {if $srvpwset}
                    <label class="flex items-center gap-2 text-sm">
                        ...
                        <span>既存のサーバーパスワードを削除（フィールドをクリア）。</span>
                    </label>
                {/if}
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-srv-cancel">キャンセル</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-srv-save">保存</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-change-email">
        <div class="card__header">
            <div>
                ...
                <p>現在のアドレス：
                    ...
                    {else}
                        <span class="text-sm text-muted" data-testid="account-current-email-empty">（未設定）</span>
                    {/if}
                </p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-email-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-email-password">パスワード</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-email">新しいメールアドレス</label>
                ...
                <div>
                    <label class="label" for="account-email-confirm">新しいメールアドレス（確認）</label>
                    <input class="input" type="email"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-email-cancel">キャンセル</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-email-save">保存</button>
                </div>
            </form>
        </div>
    </section>
...
(function () {
    ...
    function showFieldError(prefix, err, fieldMap) {
        ...
        setMsg(prefix + domId + '-msg', err.message || '無効な値です。');
        return true;
    }
    ...
    function flashSuccess(envelope) {
        ...
        showToast('success', m.title || '保存しました', m.body || '');
        if (m.redir) {
    ...
    function flashFailure(envelope) {
        var msg = (envelope && envelope.error && envelope.error.message)
            ...
            : 'リクエストに失敗しました。再度お試しください。';
        showToast('error', 'エラー', msg);
    }
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (next.length < minLen) {
                setMsg('account-new-password-msg', 'パスワードは ' + minLen + ' 文字以上でなければなりません。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-password-msg', 'パスワードが一致しません。');
                bad = true;
            }
            if (current.length === 0) {
    ...
    if (srvForm) {
        srvForm.addEventListener('submit', function (ev) {
            ...
            var bad = false;
            if (next.length < minLen) {
                setMsg('account-new-srv-password-msg', 'パスワードは ' + minLen + ' 文字以上でなければなりません。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-srv-password-msg', 'パスワードが一致しません。');
                bad = true;
            }
            ...
            if (hasExisting) {
                sb.api.call(Actions.AccountCheckSrvPassword, {
                    ...
                    if (!env.data.matches) {
                        setBusy(srvBtn, false);
                        setMsg('account-current-srv-password-msg', 'サーバーパスワードが正しくありません。');
                        return;
                    }
                    send();
                });
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', function (ev) {
            ...
            if (email.length === 0) {
                setMsg('account-email-msg', '新しいメールアドレスを入力してください。');
                bad = true;
            }
            if (confirm.length === 0) {
                setMsg('account-email-confirm-msg', '新しいメールアドレス（確認）を入力してください。');
                bad = true;
            }
            if (!bad && email !== confirm) {
                setMsg('account-email-confirm-msg', 'メールアドレスが一致しません。');
                bad = true;
            }
            if (pw.length === 0) {
                setMsg('account-email-password-msg', 'パスワードを入力してください。');
                bad = true;
            }
            if (bad) return;
