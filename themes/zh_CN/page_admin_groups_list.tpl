...
<div class="space-y-6">
    ...
    <section data-testid="web-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">网页管理员分组</h1>
                <p class="text-sm text-muted m-0 mt-2">网页面板分组的权限标志和免疫。共：{$web_group_count} 个。</p>
            </div>
        </div>
        ...
        {if $web_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-web" data-filtered="false">
                ...
                <h2 class="empty-state__title">暂无网页管理员分组</h2>
                <p class="empty-state__body">网页管理员分组为一批管理员打包面板权限。创建一个分组可同时为多名管理员分配相同的标志。</p>
                    ...
                    <a class="admin-groups-master-detail__row flex items-center gap-3 p-4"
                       ...
                       {if $selected_group && $selected_group.gid == $group.gid}aria-current="true"{/if}>
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="text-xs text-muted">{$web_admins[$smarty.foreach.web_group.index]} 名成员</div>
                ...
                <form class="card"
                    ...
                    <div class="card__header">
                        <div>
                            ...
                            <p>{$selected_group.member_count} 名成员</p>
                        </div>
                        {if $permission_deletegroup}
                            <button type="button"
                                    class="btn btn--ghost btn--sm"
                                    data-testid="group-delete"
                                    onclick="SbppGroupsDelete({$selected_group.gid}, '{$selected_group.name|escape:'javascript'}', this);">删除分组</button>
                        {/if}
                    </div>
                    <div class="card__body space-y-4">
                        ...
                        <div>
                            <label class="label" for="group-name-input">分组名称</label>
                            <input class="input"
                                   id="group-name-input"
                                   name="name"
                        ...
                        <div>
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <label class="label m-0">权限标志</label>
                                ...
                                <span class="text-xs text-muted" data-testid="flag-bitmask">{$selected_group.flags} 位掩码</span>
                            </div>
                        ...
                        {if $permission_editgroup}
                            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                                <a class="btn btn--ghost" href="?p=admin&c=groups">放弃</a>
                                <button class="btn btn--primary"
                                        type="submit"
                                        data-testid="group-save">保存更改</button>
                            </div>
                        {/if}
                    </div>
                </form>
            {else}
                <div class="card"><div class="card__body"><p class="text-muted m-0">选择左侧的分组以编辑其标志。</p></div></div>
            {/if}
        </div>
        {/if}
    </section>
    ...
    <section data-testid="server-admin-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">服务器管理员分组</h2>
                <p class="text-sm text-muted m-0 mt-2">SourceMod 管理员分组（游戏内标志）。共：{$server_admin_group_count} 个。</p>
            </div>
        </div>
        ...
        {if $server_admin_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server-admin" data-filtered="false">
                ...
                <h2 class="empty-state__title">暂无服务器管理员分组</h2>
                <p class="empty-state__body">服务器管理员分组携带 SourceMod 字符标志和免疫。创建一个分组可为一组管理员授予游戏内管理员权限。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                    ...
                    <article class="card" data-testid="server-admin-group-row" data-id="{$group.id}">
                        <div class="card__header">
                            <div>
                                ...
                                <p>{$server_admins[$smarty.foreach.server_admin_group.index]} 名成员 &middot; 免疫 {$group.immunity}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=srv&id={$group.id|escape:'url'}">编辑</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.id}, '{$group.name|escape:'javascript'}', 'srv', this);">删除</button>
                                {/if}
                            </div>
                        </div>
                        <div class="card__body space-y-3">
                            <div>
                                <div class="text-xs font-semibold text-muted mb-2">权限</div>
                                ...
                                {else}
                                    <p class="text-xs text-muted m-0"><em>无</em></p>
                                {/if}
                            </div>
                            {if $server_admins_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">成员</div>
                                        ...
                                        {foreach from=$server_admins_list[$smarty.foreach.server_admin_group.index] item="server_admin"}
                                            <li class="flex items-center justify-between gap-2 text-sm">
                                                ...
                                                {if $permission_editadmin}
                                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=admins&o=editgroup&id={$server_admin.aid|escape:'url'}">编辑</a>
                                                {/if}
                                            </li>
                                        {/foreach}
                                    </ul>
                            ...
                            {if $server_overrides_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">覆盖</div>
    ...
    <section data-testid="server-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">服务器分组</h2>
                <p class="text-sm text-muted m-0 mt-2">游戏服务器的分组（无权限标志）。共：{$server_group_count} 个。</p>
            </div>
        </div>
        ...
        {if $server_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server" data-filtered="false">
                ...
                <h2 class="empty-state__title">暂无服务器分组</h2>
                <p class="empty-state__body">服务器分组将游戏服务器打包在一起，便于一次性为多台服务器分配管理员。创建一个分组即可开始对服务器进行分组。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                ...
                {foreach from=$server_list item="group" name="server_group"}
                    <article class="card" data-testid="server-group-row" data-id="{$group.gid}">
                        <div class="card__header">
                            <div>
                                <h3>{$group.name|escape}</h3>
                                <p>{$server_counts[$smarty.foreach.server_group.index]} 台服务器</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=server&id={$group.gid|escape:'url'}">编辑</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.gid}, '{$group.name|escape:'javascript'}', 'server', this);">删除</button>
                                {/if}
                            </div>
                        </div>
                        {*
                                        ...
                                        <li class="flex items-center gap-2"
                                            ...
                                            {if !$server.enabled}
                                                <span class="pill pill--offline"
                                                      ...
                                                      title="已禁用 — 在公开列表中隐藏，并被每张卡片的探针跳过"
                                                      style="flex-shrink:0">已禁用</span>
                                            {/if}
                                        </li>
                                    {/foreach}
                                </ul>
...
function SbppGroupsSave(event) {
    ...
    sb.api.call(Actions.GroupsEdit, {
        ...
        SbppGroupsApplyResponse(r, { defaultTitle: '分组已更新' });
    });
    return false;
}

function SbppGroupsDelete(gid, name, btn) {
    if (!confirm('删除分组"' + name + '"？')) return;
    ...
    sb.api.call(Actions.GroupsRemove, { gid: Number(gid), type: 'web' })
        .then(function (r) {
            ...
            if (r && r.ok && (r.data && (r.data.reload || (r.data.message && r.data.message.redir)))) {
                SbppGroupsApplyResponse(r, { defaultTitle: '分组已删除' });
                return;
            }
...
(function () {
    ...
    grid.addEventListener('change', function (event) {
        ...
        preview.textContent = SbppFoldFlags(grid) + ' 位掩码';
    });
})();
{/literal}
</script>
