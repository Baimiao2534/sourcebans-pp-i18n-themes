...
<div class="p-6 space-y-6" style="max-width:1400px;margin:0 auto;width:100%">
    ...
    <header data-testid="dashboard-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">
            {if $dashboard_title}{$dashboard_title}{else}Tableau de bord{/if}
        </h1>
        <p class="text-sm text-muted m-0 mt-2">Activité sur l'ensemble de vos serveurs.</p>
    </header>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        <div class="card p-5" data-testid="dashboard-stat-total-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Total des bannissements</div>
            ...
            <div class="text-xs text-muted">au total</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-active-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Bannissements actifs</div>
            ...
            <div class="text-xs text-muted">en vigueur</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-comm-blocks">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Blocs de comm.</div>
        ...
        <div class="card p-5" data-testid="dashboard-stat-servers">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Serveurs</div>
            ...
            <div class="text-xs text-muted">configuré(s)</div>
        </div>
    ...
    {* -- Admins-only project announcement strip ----------------------
        ...
        hidden. *}
    {if $announcement}
    <aside class="announcement-strip" data-testid="dashboard-announcement" aria-label="Dernière annonce">
        <details>
            <summary>
            ...
            <a class="announcement-strip__link" data-testid="dashboard-announcement-link"
               href="{$announcement.url}" target="_blank" rel="noopener noreferrer">
                Lire la suite <i data-lucide="external-link" aria-hidden="true" style="width:13px;height:13px"></i>
            </a>
            {/if}
        </details>
    </aside>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))">
        ...
        <section class="card" data-testid="dashboard-recent-bans">
            <div class="card__header">
                <div>
                    <h3>Derniers bannissements</h3>
                    <p>Actions de modération les plus récentes</p>
                </div>
            ...
            <div>
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $b.short_name}{$b.short_name}{else}<span class="text-faint">aucun pseudonyme</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-bans-empty">
                    ...
                    <h4 class="empty-state__title">Aucun bannissement pour le moment</h4>
        ...
        <section class="card" data-testid="dashboard-servers">
            <div class="card__header">
                <div>
                    <h3>Serveurs</h3>
                    <p>{$total_servers|number_format} configuré(s)</p>
                </div>
            ...
            <div class="text-xs text-muted" style="border-bottom:1px solid var(--border);padding:0.625rem 1.25rem">
                ...
                Ouvrez la page Serveurs pour gérer les joueurs en temps réel.
            </div>
            {/if}
            {*
            ...
            *}
            <div style="padding:0.5rem"
                ...
                <div class="empty-state" data-testid="dashboard-servers-empty">
                    ...
                    <h4 class="empty-state__title">Aucun serveur configuré</h4>
                    <p class="empty-state__body">Ajoutez un serveur pour que les visiteurs puissent voir le statut en direct et s'y connecter depuis le panneau.</p>
                    {if $can_add_server}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(360px,1fr))">
        ...
        *}
        <section class="card" data-testid="dashboard-blocked-attempts" data-lognopopup="{if $dashboard_lognopopup}1{else}0{/if}">
            <div class="card__header">
                <div>
                    <h3>Dernières tentatives bloquées</h3>
                    <p>{$total_blocked|number_format} interceptions au total</p>
                </div>
            </div>
            <div>
                ...
                <a class="flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-blocked-row" data-id="{$p.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="font-medium text-sm truncate">
                            {if $p.short_name}{$p.short_name}{else}<span class="text-faint">aucun pseudonyme</span>{/if}
                        </div>
                ...
                <div class="empty-state" data-testid="dashboard-blocked-attempts-empty">
                    ...
                    <h4 class="empty-state__title">Aucune tentative bloquée pour le moment</h4>
                    <p class="empty-state__body">Lorsque des joueurs bannis tenteront de se reconnecter, le plugin SourceMod enregistrera chaque interception ici.</p>
                </div>
                {/foreach}
            </div>
        </section>
        ...
        <section class="card" data-testid="dashboard-recent-comms">
            <div class="card__header">
                <div>
                    <h3>Derniers blocs de comm.</h3>
                    <p>Mutes et gags les plus récents</p>
                </div>
            ...
            <div>
                ...
                <a class="ban-row ban-row--{$c.state} flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-comm-row" data-id="{$c.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $c.short_name}{$c.short_name}{else}<span class="text-faint">aucun pseudonyme</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-comms-empty">
                    ...
                    <h4 class="empty-state__title">Aucun bloc de comm. pour le moment</h4>
                    <p class="empty-state__body">Les mutes et gags émis depuis le panneau ou en jeu apparaîtront ici.</p>
                    {if $can_add_ban}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
