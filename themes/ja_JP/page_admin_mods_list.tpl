...
{else}
    <div class="card">
        <div class="card__header">
            <div>
                <h3>サーバー MOD</h3>
                <p><span data-testid="mod-count">{$mod_count}</span> 件設定済み</p>
            </div>
        </div>
        {if $mod_count > 0}
            <table class="table" data-testid="mods-table">
                <thead>
                    <tr>
                        ...
                        <th>フォルダ</th>
                        <th><span title="SteamID Universe（STEAM_X:Y:Z の X）">SU</span></th>
                        <th>ステータス</th>
                ...
                <tbody>
                    {foreach from=$mod_list item=mod}
                        <tr id="mid_{$mod.mid}" data-testid="mod-row" data-id="{$mod.mid}">
                            ...
                            <td>
                                {if $mod.enabled}
                                    <span class="pill pill--online">有効</span>
                                {else}
                                    <span class="pill pill--offline">無効</span>
                                {/if}
                            </td>
                            {if $permission_editmods || $permission_deletemods}
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        {if $permission_deletemods}
                                            ...
                                            <button class="btn btn--ghost btn--sm"
                                                    ...
                                                    aria-label="MOD {$mod.name|escape} を削除">削除</button>
                                        {/if}
                                    </div>
                                </td>
                            {/if}
        ...
        {else}
            <div class="card__body">
                <p class="text-muted">設定済みの MOD はまだありません。</p>
            </div>
        {/if}
    </div>
    ...
    <dialog id="mod-delete-dialog"
        ...
        <form method="dialog" data-testid="mod-delete-form">
            <h2 id="mod-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">MOD を削除</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                この <strong data-testid="mod-delete-target">MOD</strong> を完全に削除しようとしています。この操作は取り消せません。この MOD に対して記録された過去の BAN や通信ブロックは行を保持しますが、ゲームの関連付けが失われ、BAN リストのサーバー / MOD 列には &ldquo;不明&rdquo; と表示されます。
            </p>
            <label class="label" for="mod-delete-reason">理由（任意）</label>
            ...
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                <button type="button" class="btn btn--secondary" data-testid="mod-delete-cancel" value="cancel">キャンセル</button>
                <button type="submit" class="btn btn--danger" data-testid="mod-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> MOD を削除
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.ModsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', 'MOD が削除されました', ctx.name + ' は削除されました。');
            });
        });
