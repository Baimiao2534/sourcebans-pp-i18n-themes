...
<div class="p-6 space-y-6" style="max-width:1400px;margin:0 auto;width:100%">
    ...
    <header data-testid="dashboard-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">
            {if $dashboard_title}{$dashboard_title}{else}Панель{/if}
        </h1>
        <p class="text-sm text-muted m-0 mt-2">Активность на ваших серверах.</p>
    </header>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        <div class="card p-5" data-testid="dashboard-stat-total-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Всего банов</div>
            ...
            <div class="text-xs text-muted">за всё время</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-active-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Активные баны</div>
            ...
            <div class="text-xs text-muted">действуют сейчас</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-comm-blocks">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Блокировки связи</div>
            ...
            <div class="text-xs text-muted">муты + гаги</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-servers">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Серверы</div>
            ...
            <div class="text-xs text-muted">настроено</div>
        </div>
    ...
    {* -- Admins-only project announcement strip ----------------------
        ...
        hidden. *}
    {if $announcement}
    <aside class="announcement-strip" data-testid="dashboard-announcement" aria-label="Последнее объявление">
        <details>
            <summary>
            ...
            <a class="announcement-strip__link" data-testid="dashboard-announcement-link"
               href="{$announcement.url}" target="_blank" rel="noopener noreferrer">
                Читать далее <i data-lucide="external-link" aria-hidden="true" style="width:13px;height:13px"></i>
            </a>
            {/if}
        </details>
    </aside>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))">
        ...
        <section class="card" data-testid="dashboard-recent-bans">
            <div class="card__header">
                <div>
                    ...
                    <p>Самые свежие меры модерации</p>
                </div>
                <a class="btn btn--ghost btn--sm" href="?p=banlist">
                    Показать все <i data-lucide="arrow-right" style="width:14px;height:14px"></i>
                </a>
            </div>
            <div>
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $b.short_name}{$b.short_name}{else}<span class="text-faint">без ника</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-bans-empty">
                    ...
                    <h4 class="empty-state__title">Банов пока нет</h4>
                    <p class="empty-state__body">Меры модерации появятся здесь, как только администраторы начнут работу.</p>
                    {if $can_add_ban}
        ...
        <section class="card" data-testid="dashboard-servers">
            <div class="card__header">
                <div>
                    <h3>Серверы</h3>
                    <p>{$total_servers|number_format} настроено</p>
                </div>
            ...
            <div class="text-xs text-muted" style="border-bottom:1px solid var(--border);padding:0.625rem 1.25rem">
                ...
                Откройте страницу серверов для управления игроками в реальном времени.
            </div>
            {/if}
            {*
            ...
            *}
            <div style="padding:0.5rem"
                ...
                <div class="empty-state" data-testid="dashboard-servers-empty">
                    ...
                    <h4 class="empty-state__title">Серверы не настроены</h4>
                    <p class="empty-state__body">Добавьте сервер, чтобы посетители видели статус в реальном времени и подключались с панели.</p>
                    {if $can_add_server}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(360px,1fr))">
        ...
        *}
        <section class="card" data-testid="dashboard-blocked-attempts" data-lognopopup="{if $dashboard_lognopopup}1{else}0{/if}">
            <div class="card__header">
                <div>
                    ...
                    <p>{$total_blocked|number_format} всего перехватов</p>
                </div>
            </div>
            <div>
                ...
                <a class="flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-blocked-row" data-id="{$p.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="font-medium text-sm truncate">
                            {if $p.short_name}{$p.short_name}{else}<span class="text-faint">без ника</span>{/if}
                        </div>
                ...
                <div class="empty-state" data-testid="dashboard-blocked-attempts-empty">
                    ...
                    <h4 class="empty-state__title">Перехватов пока нет</h4>
                    <p class="empty-state__body">Когда забаненные игроки попытаются повторно зайти, плагин SourceMod будет регистрировать каждый перехват здесь.</p>
                </div>
                {/foreach}
            </div>
        </section>
        ...
        <section class="card" data-testid="dashboard-recent-comms">
            <div class="card__header">
                <div>
                    <h3>Последние блокировки связи</h3>
                    <p>Свежие муты и гаги</p>
                </div>
            ...
            <div>
                ...
                <a class="ban-row ban-row--{$c.state} flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-comm-row" data-id="{$c.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $c.short_name}{$c.short_name}{else}<span class="text-faint">без ника</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-comms-empty">
                    ...
                    <h4 class="empty-state__title">Блокировок связи пока нет</h4>
