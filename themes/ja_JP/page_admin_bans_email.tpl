...
<section class="p-6" data-testid="banemail-section" style="max-width:48rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">プレイヤーへメール送信</h1>
        <p class="text-sm text-muted m-0 mt-2">
            送信先：<span class="font-mono" data-testid="banemail-addr">{$email_addr|escape}</span>。
        </p>
    </div>
    <form class="card p-6 space-y-4"
        ...
        <div>
            ...
            <textarea class="textarea"
                      ...
                      name="message"
                      rows="8"
                      placeholder="プレイヤーに伝えたい内容をご記入ください。"
        ...
        <div class="flex justify-end gap-2"
            ...
            <button type="button"
                    ...
                    data-testid="banemail-back">戻る</button>
...
(function () {
    ...
    window.CheckEmail = function (type, id) {
        ...
        if (!subject || !subject.value) { setMsg('subject.msg', 'メールの件名を入力してください。'); err++; }
        else { setMsg('subject.msg', ''); }
        if (!message || !message.value) { setMsg('message.msg', 'メールのメッセージを入力してください。'); err++; }
        ...
        a.call(A.SystemSendMail, {
            ...
            if (!r || r.ok === false) {
                toast('error', 'メール送信に失敗しました', (r && r.error && r.error.message) || '不明なエラー');
                return;
            }
            ...
            toast('success', (msg && msg.title) || 'メールを送信しました', (msg && msg.body) || 'メールは送信されました。');
            if (msg && msg.redir) {
