...
{if NOT $permission_addban}
    <div class="card" data-testid="addban-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">Sie haben keine Berechtigung, Banns hinzuzufügen.</p>
        </div>
    </div>
{else}
    <section class="p-6" data-testid="addban-section" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">Bann hinzufügen</h1>
            <p class="text-sm text-muted m-0 mt-2">
                Bann per SteamID oder IP-Adresse. Bewegen Sie den Mauszeiger über die Beschriftungen für Inline-Hilfe.
            </p>
        </div>
        <form id="addban-form"
            ...
            <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(14rem,1fr))">
                <div>
                    <label class="label" for="type">Banntyp</label>
                    ...
                    <select class="select"
                        ...
                        <option value="1"{if $prefill_type == 1} selected{/if}>IP-Adresse</option>
                    </select>
                </div>
                <div>
                    <label class="label" for="banlength">Banndauer</label>
                    <select class="select"
                            id="banlength"
            ...
            <div>
                ...
                <select class="select"
                    ...
                    <optgroup label="Verhalten">
                        ...
                        <option value="Inappropriate Spray">Unangebrachtes Spray</option>
                        <option value="Inappropriate Language">Unangebrachte Sprache</option>
                        <option value="Inappropriate Name">Unangebrachter Name</option>
                        <option value="Ignoring Admins">Administratoren ignorieren</option>
...
(function () {
    ...
    function validate(type) {
        ...
        if (!nick || !nick.value) { setMsg('nick.msg', 'Sie müssen den Spitznamen der Person angeben, die Sie bannen'); err++; }
        else { setMsg('nick.msg', ''); }

        var steam = $id('steam');
        if (type === 0) {
            if (!steam || !/(?:STEAM_[01]:[01]:\d+)|(?:\[U:1:\d+\])|(?:\d{17})/.test(steam.value)) {
                setMsg('steam.msg', 'Sie müssen eine gültige Steam-ID oder Community-ID eingeben'); err++;
        ...
        if (type === 1) {
            if (!ip || ip.value.length < 7) {
                setMsg('ip.msg', 'Sie müssen eine gültige IP-Adresse eingeben'); err++;
            } else { setMsg('ip.msg', ''); }
        } else { setMsg('ip.msg', ''); }

        if (!reason) {
            setMsg('reason.msg', 'Sie müssen einen Grund für diesen Bann auswählen oder eingeben.'); err++;
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.BansAdd, {
            ...
            if (r && r.ok && r.data && r.data.kickit) {
                ...
                var k = r.data.kickit;
                toast('success', 'Bann hinzugefügt',
                    'Der Bann wurde erfolgreich hinzugefügt.');
            ...
            setBusy(btn, false);
            if (!r || r.ok === false) {
                toast('error', 'Hinzufügen fehlgeschlagen', (r && r.error && r.error.message) || 'Unbekannter Fehler');
                return;
            }
            ...
            toast('success', (msg && msg.title) || 'Bann hinzugefügt', (msg && msg.body) || 'Der Bann wurde erfolgreich hinzugefügt');
            reset();
        });
    });
})();
