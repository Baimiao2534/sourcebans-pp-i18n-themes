...
{else}
    {if NOT $groupbanning_enabled}
        <div class="card" data-testid="groupban-disabled">
            <div class="card__body">
                <h1 style="font-size:1.25rem;font-weight:600;margin:0">Funktion deaktiviert</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Gruppen-Banns sind deaktiviert unter
                    <strong>config.enablegroupbanning</strong>.
                    Aktivieren Sie sie im Einstellungen-Tab, um diese Funktion zu nutzen.
                </p>
            </div>
        </div>
    {else}
        <section class="p-6" data-testid="groupban-section" style="max-width:48rem">
            <div class="mb-6">
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">Gruppen-Bann</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Alle Mitglieder einer Steam-Community-Gruppe auf einmal bannen.
                </p>
            </div>
            ...
            {if NOT $list_steam_groups}
                <form id="groupban-form"
                    ...
                    {csrf_field}
                    <div>
                        <label class="label" for="groupurl">Steam-Community-Gruppen-URL</label>
                        <input type="text"
                    ...
                    <div>
                        <label class="label" for="groupreason">Gruppen-Bann-Grund</label>
                        <textarea class="textarea"
                                  id="groupreason"
                                  name="groupreason"
                                  rows="4"
                                  placeholder="Warum wird diese Gruppe gebannt?"
            ...
            {else}
                <div class="card p-6 space-y-4" data-testid="groupban-from-player">
                    <p class="text-sm text-muted m-0">
                        {if $player_name}
                            Gruppen, zu denen <strong class="font-medium">{$player_name|escape}</strong> gehört, sind unten geladen.
                        {else}
                            Alle Gruppen, in denen der Spieler Mitglied ist, sind hier aufgelistet.
                        {/if}
                        Markieren Sie die, die Sie bannen möchten.
                    </p>

                    <div id="steamGroupsText"
                         name="steamGroupsText"
                         class="text-sm text-muted">Gruppen werden geladen…</div>
                    ...
                    <div id="steamGroups"
                        ...
                        <a class="btn btn--ghost btn--sm"
                           ...
                           data-action="groupban-select-all"
                           name="tickswitchlink"
                           id="tickswitchlink">Alle auswählen</a>
                ...
                (function (friendid) {
                    sb.ready(function () {
                        sb.api.call(Actions.BansGetGroups, { friendid: friendid }).then(function (r) {
                            ...
                            if (!tbl) return;
                            if (groups.length === 0) {
                                sb.message.error('Fehler', "Beim Abrufen der Gruppendaten ist ein Fehler aufgetreten. Vielleicht ist der Spieler in keiner Gruppe Mitglied oder sein Profil ist privat?", 'index.php?p=banlist');
                                var txt = sb.$id('steamGroupsText');
                                if (txt) txt.innerHTML = '<i>Keine Gruppen...</i>';
                                return;
                            }
                            groups.forEach(function (g, i) {
                                ...
                                td2.appendChild(document.createTextNode(' Mitglieder)'));
                            });
