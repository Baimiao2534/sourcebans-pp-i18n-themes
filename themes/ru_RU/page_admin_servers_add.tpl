    ...
    {if NOT $permission_addserver}
        <div class="card" data-testid="server-add-denied">
            <div class="card__body">
                <h3 style="margin:0 0 0.25rem">Доступ запрещён</h3>
                <p class="text-sm text-muted m-0">У вас нет прав на добавление серверов.</p>
            </div>
        </div>
    {else}
        <form id="addserver-form"
            ...
            <div class="card__header">
                <div>
                    <h3>{if $edit_server}Изменить сервер{else}Добавить сервер{/if}</h3>
                    <p>Подсказка: наведите курсор на вопросительные знаки для встроенной справки. RCON необязателен, но открывает живую консоль и сопоставление администраторов с серверами.</p>
                </div>
            </div>
            <div class="card__body" style="display:grid;gap:1rem">
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">IP-адрес / домен сервера</span>
                        <input type="text"
                               id="address"
                               name="address"
                        ...
                        <span class="text-xs text-muted">IPv4 / IPv6 / имя хоста. Имена хостов разрешаются игровым сервером во время подключения.</span>
                    </label>
                    <label class="block">
                        <span class="label">Порт сервера</span>
                        <input type="number"
                               id="port"
                               name="port"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">По умолчанию <code class="font-mono">27015</code>.</span>
                    </label>
                </div>

                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">Пароль RCON</span>
                        <input type="password"
                               id="rcon"
                               name="rcon"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">Находится в <code class="font-mono">server.cfg</code> рядом с <code class="font-mono">rcon_password</code>.</span>
                    </label>
                    <label class="block">
                        <span class="label">Подтверждение RCON</span>
                        <input type="password"
                               id="rcon2"
                               name="rcon2"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">Введите повторно, чтобы избежать опечаток.</span>
                    </label>
                </div>
                ...
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        ...
                        <select id="mod"
                            ...
                            {if !$edit_server}<option value="-2">Выберите мод…</option>{/if}
                    ...
                    <label class="flex items-center gap-2" style="align-self:end;padding:0.5rem 0">
                        ...
                        <span class="text-sm">Включить в публичном списке серверов</span>
                    </label>
                </div>

                <fieldset style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem 1rem">
                    <legend class="text-xs font-semibold" style="padding:0 0.5rem">Группы серверов</legend>
                    <p class="text-xs text-muted m-0 mb-2">Выберите группы SourceMod, от которых этот сервер наследует администраторов.</p>
                    {if $grouplist|@count == 0}
                        <p class="text-xs text-faint m-0">Группы серверов пока не заданы — посетите раздел «Администраторы → Группы», чтобы создать группу.</p>
                    {else}
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!api || !window.Actions) {
            setError('JS API не загружен; обновите страницу.');
        ...
        api.call(window.Actions.ServersAdd, {
            ...
            if (submit) submit.disabled = false;
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'Не удалось добавить сервер.';
                setError(msg);
                if (window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'Не удалось добавить сервер', body: msg });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast && d.message) {
                window.SBPP.showToast({ kind: 'success', title: d.message.title || 'Сервер добавлен', body: d.message.body || '' });
            }
