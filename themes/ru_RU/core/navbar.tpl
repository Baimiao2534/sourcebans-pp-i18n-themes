...
<aside class="sidebar" id="sidebar" data-mobile-open="false">
    ...
    <nav class="sidebar__nav" role="navigation" aria-label="Основная навигация">
        <div class="sidebar__section">
            <div class="sidebar__section-label">Общедоступное</div>
            {foreach from=$navbar item=nav}
                {if $nav.endpoint != 'admin'}
                    <a class="sidebar__link"
                       href="index.php?p={$nav.endpoint}"
                       data-testid="nav-{$nav.endpoint}"
                       title="{if $nav.endpoint == 'home'}На этой странице отображается обзор ваших банов и серверов.{elseif $nav.endpoint == 'servers'}Здесь можно просмотреть все серверы и их статус.{elseif $nav.endpoint == 'banlist'}Здесь можно просмотреть все записи о банах в базе данных.{elseif $nav.endpoint == 'commslist'}Здесь можно просмотреть все блокировки коммуникаций (текстовый и голосовой чат) в базе данных.{elseif $nav.endpoint == 'submit'}Здесь вы можете отправить демо-файлы или скриншоты с подозрением на использование читов. После отправки они будут рассмотрены администратором.{elseif $nav.endpoint == 'protest'}Здесь вы можете подать апелляцию на ваш бан и указать причины, по которым вас следует разбанить.{else}{$nav.description}{/if}"
                       {if $nav.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $nav.endpoint == 'home'}Панель{elseif $nav.endpoint == 'servers'}Серверы{elseif $nav.endpoint == 'banlist'}Список банов{elseif $nav.endpoint == 'commslist'}Блокировки связи{elseif $nav.endpoint == 'submit'}Сообщить о игроке{elseif $nav.endpoint == 'protest'}Апелляция на бан{else}{$nav.title}{/if}
                    </a>
                {/if}
            {/foreach}
        </div>
        ...
        {if $isAdmin}
            <div class="sidebar__section">
                <div class="sidebar__section-label">Администрирование</div>
                {foreach from=$navbar item=nav}
                    {if $nav.endpoint == 'admin'}
                        <a class="sidebar__link"
                           href="index.php?p={$nav.endpoint}"
                           data-testid="nav-{$nav.endpoint}"
                           title="Это панель управления SourceBans, где вы можете настраивать администраторов, добавлять серверы и т. д."
                           {if $nav.state == 'active'}aria-current="page"{/if}>
                ...
                {foreach from=$adminbar item=admin}
                    <a class="sidebar__link"
                       ...
                       {if $admin.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $admin.endpoint == 'admins'}Администраторы{elseif $admin.endpoint == 'servers'}Серверы{elseif $admin.endpoint == 'bans'}Баны{elseif $admin.endpoint == 'comms'}Коммуникации{elseif $admin.endpoint == 'groups'}Группы{elseif $admin.endpoint == 'settings'}Настройки{elseif $admin.endpoint == 'mods'}Моды{elseif $admin.endpoint == 'export'}Экспорт{else}{$admin.title}{/if}
                    </a>
                {/foreach}
            </div>
        {/if}
