...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">选择为每个访问者渲染面板 chrome 的主题。</p>
    </div>
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>已安装主题</h3>
                        <p>每张卡片代表 <code>web/themes/</code> 下一个含有 <code>theme.conf.php</code> 清单的目录。</p>
                    </div>
                    <div class="text-xs text-muted" style="text-align:right" data-testid="current-theme-summary">
                        当前使用：<br>
                        <strong>{$theme_name}</strong> v{$theme_version} 由 {$theme_author}{if $theme_link != ''} · <a href="{$theme_link}" target="_blank" rel="noopener" class="text-muted">主页</a>{/if}<br>
                ...
                <div class="card__body">
                    {if NOT $can_web_settings}
                        <p class="text-xs text-muted m-0 mb-4" style="padding:0.5rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--bg-muted)">
                            ...
                            你可以浏览主题，但需要 <code>ADMIN_WEB_SETTINGS</code> 权限才能切换当前主题。
                        </p>
                    {/if}
                            ...
                            <article class="theme-card"
                                ...
                                <div style="aspect-ratio:250 / 170;background:var(--bg-muted);display:grid;place-items:center;overflow:hidden">
                                    <img src="{$t.screenshot}"
                                         ...
                                         onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\'text-muted text-xs\'>暂无截图</span>'">
                                </div>
                                <div class="p-4 space-y-3" style="flex:1;display:flex;flex-direction:column">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            {if $t.active}
                                                <span class="pill pill--online" title="当前使用中">
                                                    <i data-lucide="check" style="width:0.75rem;height:0.75rem"></i> 使用中
                                                </span>
                                            {/if}
                                        </div>
                                        <p class="text-xs text-muted m-0 mt-2">由 {$t.author} · v{$t.version}{if $t.link != ''} · <a href="{$t.link}" target="_blank" rel="noopener" class="text-muted" title="打开主题主页"><i data-lucide="external-link" style="width:0.75rem;height:0.75rem;vertical-align:-1px"></i> 主页</a>{/if}</p>
                                        ...
                                        {if $can_web_settings}
                                            ...
                                            {else}
                                                <button type="button"
                                                    ...
                                                    <i data-lucide="check-circle"></i> 使用此主题
                                                </button>
                                            {/if}
                                        {/if}
                                    </div>
...
(function () {
    ...
    window.applyTheme = function (theme) {
        if (!theme) return;
        if (!window.confirm('将面板主题切换为 "' + theme + '"？每个访问者在下次请求时将看到新主题。')) return;
