...
{else}
    {if NOT $groupbanning_enabled}
        <div class="card" data-testid="groupban-disabled">
            <div class="card__body">
                <h1 style="font-size:1.25rem;font-weight:600;margin:0">Функция отключена</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Групповые баны отключены в
                    <strong>config.enablegroupbanning</strong>.
                    Включите функцию на вкладке «Настройки», чтобы использовать этот раздел.
                </p>
            </div>
        </div>
    {else}
        <section class="p-6" data-testid="groupban-section" style="max-width:48rem">
            <div class="mb-6">
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">Групповой бан</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Забанить всех участников группы Steam Community одновременно.
                </p>
            </div>
            ...
            {if NOT $list_steam_groups}
                <form id="groupban-form"
                    ...
                    {csrf_field}
                    <div>
                        <label class="label" for="groupurl">URL группы Steam Community</label>
                        <input type="text"
                    ...
                    <div>
                        <label class="label" for="groupreason">Причина группового бана</label>
                        <textarea class="textarea"
                                  id="groupreason"
                                  name="groupreason"
                                  rows="4"
                                  placeholder="Почему эта группа получает бан?"
            ...
            {else}
                <div class="card p-6 space-y-4" data-testid="groupban-from-player">
                    <p class="text-sm text-muted m-0">
                        {if $player_name}
                            Группы, в которых состоит <strong class="font-medium">{$player_name|escape}</strong>, загружены ниже.
                        {else}
                            Здесь перечислены все группы, в которых состоит игрок.
                        {/if}
                        Отметьте те, которые хотите забанить.
                    </p>

                    <div id="steamGroupsText"
                         name="steamGroupsText"
                         class="text-sm text-muted">Загрузка групп…</div>
                    ...
                    <div id="steamGroups"
                        ...
                        <a class="btn btn--ghost btn--sm"
                           ...
                           data-action="groupban-select-all"
                           name="tickswitchlink"
                           id="tickswitchlink">Выбрать все</a>
                ...
                (function (friendid) {
                    sb.ready(function () {
                        sb.api.call(Actions.BansGetGroups, { friendid: friendid }).then(function (r) {
                            ...
                            if (!tbl) return;
                            if (groups.length === 0) {
                                sb.message.error('Ошибка', "Не удалось получить данные группы. Возможно, игрок не состоит ни в одной группе или его профиль скрыт?", 'index.php?p=banlist');
                                var txt = sb.$id('steamGroupsText');
                                if (txt) txt.innerHTML = '<i>Нет групп...</i>';
                                return;
                            }
                            groups.forEach(function (g, i) {
                                ...
                                td2.appendChild(document.createTextNode(' Участников)'));
                            });
