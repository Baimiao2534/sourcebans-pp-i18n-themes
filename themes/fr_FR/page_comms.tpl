...
<div class="p-6 space-y-4" style="max-width:1700px;margin:0 auto;width:100%">
    ...
    <header class="flex items-center justify-between gap-4" style="flex-wrap:wrap" data-testid="comms-header">
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Blocs de communication</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <span class="tabular-nums" data-testid="comms-count">{$ban_list|@count|number_format}</span>
                sur <span class="tabular-nums">{$total_bans|number_format}</span> blocs
            </p>
        </div>
        <div class="flex gap-2">
            ...
            <a class="btn btn--secondary btn--sm"
               ...
               data-testid="toggle-hide-inactive">
                ...
                {if $is_active_only}Afficher inactifs{else}Masquer inactifs{/if}
            </a>
            {if $can_add_comm}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=comms"
               data-testid="comms-add-button">
                <i data-lucide="plus"></i> Ajouter un bloc
            </a>
            {/if}
        </div>
    </header>
    ...
    <form method="get" action="index.php"
        ...
        <div class="flex gap-3" style="flex-wrap:wrap">
            <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
                ...
                <input class="input input--with-icon"
                       ...
                       placeholder="Joueur, SteamID ou motif…"
                       data-testid="comms-search">
            </div>
            <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="comms-server-filter" aria-label="Filtrer par serveur">
                <option value="">Tous les serveurs</option>
                {foreach $servers as $s}
                    <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
                {/foreach}
            </select>
            <select class="select" name="time" style="width:auto" data-testid="comms-time-filter" aria-label="Filtrer par période">
        ...
        <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="Filtres rapides">
            ...
            <button class="chip" type="submit" name="type" value="mute"
                ...
                <i data-lucide="mic-off" style="width:12px;height:12px"></i> Muet
            </button>
            <button class="chip" type="submit" name="type" value="gag"
                ...
                <i data-lucide="message-square-off" style="width:12px;height:12px"></i> Bâillon
            </button>
        ...
        <table class="table" data-testid="comms-table">
            <thead>
                <tr>
                    ...
                    <th>Joueur</th>
                    <th>SteamID</th>
                    <th class="col-length col-tier-3">Durée</th>
                    <th class="col-tier-2">Serveur</th>
                    <th class="col-admin col-tier-2">Administrateur</th>
                    <th class="col-tier-3">Début</th>
            ...
            <tbody>
                {foreach $ban_list as $comm}
                    <tr class="ban-row ban-row--{$comm.state}"
                            ...
                            <div class="flex items-center gap-3" style="min-width:0">
                                ...
                                <div style="min-width:0">
                                    ...
                                    <a class="font-medium truncate"
                                       ...
                                       data-testid="drawer-trigger">{if $comm.name}{$comm.name|escape}{else}<i class="text-faint">pas de pseudo</i>{/if}</a>
                                    ...
                                    {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                        <div class="text-xs text-faint mt-1" data-testid="comm-unban-meta">
                                            {if !empty($comm.removedby)}Levé par <span class="font-medium">{$comm.removedby|escape}</span>{if !empty($comm.ureason)} : {/if}{/if}
                                    ...
                                    <details class="ban-comments-inline mt-1"
                                      ...
                                      <summary class="ban-comments-inline__summary"
                                               ...
                                               title="{$comm.commentdata|@count} commentaire{if $comm.commentdata|@count != 1}s{/if} sur ce bloc">
                                        ...
                                        <span class="ban-comments-inline__label">commentaire{if $comm.commentdata|@count != 1}s{/if}</span>
                                      </summary>
                        ...
                        <td class="col-status">
                            <span class="pill pill--{$comm.state}" style="text-transform:capitalize">{if $comm.state == 'active'}Actif{elseif $comm.state == 'permanent'}Permanent{elseif $comm.state == 'expired'}Expiré{elseif $comm.state == 'unmuted'}Débloqué{else}{$comm.state|escape}{/if}</span>
                        </td>
                            ...
                            <div class="row-actions">
                                ...
                                {if $can_unmute_gag && $comm.unmute_url}
                                    ...
                                    <button type="button"
                                        ...
                                        {if $comm.type == 'mute'}Démuter{elseif $comm.type == 'gag'}Débâillonner{else}Lever le bloc{/if}
                                    </button>
                    ...
                    <tr>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">Aucun bloc ne correspond à ces filtres</h2>
                                <p class="empty-state__body">Essayez un autre terme de recherche, serveur ou période, ou effacez les filtres actifs pour voir tous les mutes / bâillons enregistrés.</p>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">Aucun bloc enregistré</h2>
                                <p class="empty-state__body">Les mutes et bâillons émis depuis le panneau ou en jeu apparaîtront ici.</p>
                                {if $can_add_comm}
                                <div class="empty-state__actions">
                                    <a class="btn btn--primary btn--sm"
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-sm truncate">
                                    {if $comm.name}{$comm.name|escape}{else}<span class="text-faint">pas de pseudo</span>{/if}
                                </span>
                                <span class="pill pill--{$comm.state}">{if $comm.state == 'active'}Actif{elseif $comm.state == 'permanent'}Permanent{elseif $comm.state == 'expired'}Expiré{elseif $comm.state == 'unmuted'}Débloqué{else}{$comm.state|escape}{/if}</span>
                            </div>
                            <div class="text-xs text-muted truncate" style="margin-top:0.125rem">
                                <span style="text-transform:capitalize">{if $comm.type == 'mute'}Muet{elseif $comm.type == 'gag'}Bâillon{elseif $comm.type == 'silence'}Silence{else}{$comm.type|escape}{/if}</span>
                            ...
                            {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                ...
                                <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="comm-unban-meta-mobile"
                                     title="{if !empty($comm.removedby)}Levé par {$comm.removedby|escape}{if !empty($comm.ureason)} : {/if}{/if}{if !empty($comm.ureason)}{$comm.ureason|escape}{/if}">
                                    {if !empty($comm.removedby)}Levé par {$comm.removedby|escape}{if !empty($comm.ureason)} : {/if}{/if}
            ...
            {if $pagination.prev_url}
                <a class="btn btn--secondary btn--sm"
                   ...
                   data-testid="page-prev">
                    <i data-lucide="chevron-left"></i> Préc
                </a>
            {else}
            ...
            {if $pagination.next_url}
                <a class="btn btn--secondary btn--sm"
                   ...
                   data-testid="page-next">
                    Suiv <i data-lucide="chevron-right"></i>
                </a>
            {else}
                <span class="btn btn--secondary btn--sm"
                      aria-disabled="true"
...
<dialog id="comms-unblock-dialog"
  ...
  <form method="dialog" data-testid="comms-unblock-form">
    <h2 id="comms-unblock-dialog-title" data-testid="comms-unblock-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Lever le bloc</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      Pourquoi levez-vous le bloc sur <strong data-testid="comms-unblock-target">ce joueur</strong> ? Cette raison sera enregistrée et apparaîtra dans le journal d'audit.
    </p>
    <label class="label" for="comms-unblock-reason">Motif <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="Bloc par erreur, appel accepté, peine purgée, &hellip;"></textarea>
    <p class="text-xs" data-testid="comms-unblock-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="comms-unblock-cancel" value="cancel">Annuler</button>
      <button type="submit" class="btn btn--primary" data-testid="comms-unblock-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> Confirmer
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnmuted(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === prev || txt === 'active' || txt === 'permanent' || txt === 'expired' || txt === 'actif' || txt === 'expiré') {
                ...
                if (lastText) lastText.textContent = ' Débloqué';
                else pill.textContent = 'Débloqué';
            }
        });
        Array.prototype.forEach.call(
            ...
            function (btn) {
                ...
                a.innerHTML = '<i data-lucide="rotate-ccw" style="width:13px;height:13px"></i> Réappliquer';
    ...
    function copyForType(type) {
        if (type === 'mute')    return { title: 'Démuter le joueur',  verb: 'démuter' };
        if (type === 'gag')     return { title: 'Débâillonner le joueur', verb: 'débâillonner' };
        if (type === 'silence') return { title: 'Lever le silence',   verb: 'lever le silence sur' };
        return                          { title: 'Lever le bloc',      verb: 'lever le bloc sur' };
    }
    ...
    function openUnblockDialog(ctx) {
        ...
        if (prompt) prompt.textContent = ctx.name || ('bloc #' + ctx.bid);
        ...
        if (p && p.firstChild && p.firstChild.nodeType === 3) {
            p.firstChild.textContent = 'Pourquoi ' + copy.verb + ' ';
        }
    ...
    document.addEventListener('click', function (e) {
        ...
        var bid = btn.getAttribute('data-bid') || '';
        var name = btn.getAttribute('data-name') || ('bloc #' + bid);
        ...
        if (act === 'comms-delete') {
            if (!window.confirm('Supprimer le bloc pour « ' + name + ' » ?')) return;
            setBusy(btn, true);
            a.call(A.CommsDelete, { bid: Number(bid) }).then(function (r) {
                ...
                decrementCount();
                toast('success', 'Bloc supprimé', 'Le bloc pour ' + name + ' a été supprimé.');
            });
            return;
        }
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.CommsUnblock, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            ...
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'Erreur inconnue';
                showError(msg);
                toast('error', 'Échec du déblocage', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnmuted);
            closeUnblockDialog();
            toast('success', 'Bloc levé', ctx.name + ' a été débloqué.');
        });
    });
