        ...
        <div class="card__body">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <div class="text-xs text-muted">管理パネル</div>
                </div>
            </div>

            <h1 class="m-0" style="font-size:var(--fs-2xl);font-weight:600">ログイン</h1>
            <p class="text-sm text-muted mt-2">管理者資格情報を使用するか、Steam でログインしてください。</p>
            ...
            -{if $steamlogin_show}-
                <a class="btn btn--secondary"
                   ...
                   style="width:100%;margin-top:1.5rem">
                    <i data-lucide="gamepad-2" aria-hidden="true"></i> Steam でログイン
                </a>
            -{/if}-
            ...
            *}-
            -{if $normallogin_show}-
                <form id="loginForm"
                    ...
                    <div>
                        <label class="label" for="loginUsername">ユーザー名</label>
                        <input class="input"
                               id="loginUsername"
                               name="username"
                               type="text"
                    ...
                    <div>
                        <label class="label" for="loginPassword">パスワード</label>
                        <input class="input"
                               id="loginPassword"
                               name="password"
                               type="password"
                    ...
                    <button class="btn btn--primary"
                        ...
                        ログイン <i data-lucide="arrow-right" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-4 text-xs" style="text-align:center">
                    <a href="index.php?p=lostpassword" data-testid="login-lostpwd">パスワードをお忘れですか？</a>
                </div>
            -{/if}-
        </div>
    </div>
...
(function () {
    ...
    var lockedBody = 'ログイン失敗が多すぎたため、アカウントは一時的にロックされています。'
                   + '約 ' + time + ' 分後に再度お試しください。';
    var msgs = {
        no_access:    { kind: 'error', title: 'アクセス権限なし',     body: 'このページにアクセスする権限がありません。アクセス権限のあるアカウントでログインしてください。' },
        empty_pwd:    { kind: 'info',  title: '空のパスワード',         body: 'アカウントに空のパスワードが設定されています。パスワード再設定リンクからリセットするか、管理者に依頼してください。' },
        failed:       { kind: 'error', title: 'ログイン失敗',         body: '入力されたユーザー名またはパスワードが正しくありません。パスワードをお忘れの場合は、下のパスワード再設定リンクをご利用ください。' },
        steam_failed: { kind: 'error', title: 'Steam ログイン失敗',    body: 'Steam ログインは成功しましたが、SteamID がどのアカウントにも関連付けられていません。' },
        locked:       { kind: 'error', title: 'アカウントがロックされました', body: lockedBody }
    };
    var msg = msgs[m];
    if (!msg) return;
    function show() {
