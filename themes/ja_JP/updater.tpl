...
<head>
 ...
 <title>アップデーター | SourceBans++</title>
 <script>
 (function () {
 try {
...
<div class="p-6 space-y-6" style="max-width:48rem;margin:2.5rem auto;width:100%">
    ...
    <header class="flex items-center gap-3" data-testid="updater-header">
        ...
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">SourceBans++ アップデーター</h1>
            <p class="text-sm text-muted m-0 mt-2">データベース移行ログ。</p>
        </div>
    </header>
    ...
    <section class="card">
        ...
        <div class="card__body">
            ...
            {else}
                <p class="text-sm text-muted m-0" data-testid="updater-empty">更新は適用されませんでした。</p>
            {/if}
        </div>
    </section>
    ...
    <section class="card">
        <div class="card__header" data-testid="updater-cleanup">
            <div>
                <h3>次のステップ</h3>
                <p>実行完了後、管理者にパネルを公開する前に <code class="font-mono">/updater</code> ディレクトリを削除してください。</p>
            </div>
        </div>
        <div class="card__body">
