...
{else}
    <div class="card">
        <div class="card__header">
            <div>
                <h3>服务器模组</h3>
                <p><span data-testid="mod-count">{$mod_count}</span> 个已配置</p>
            </div>
        </div>
        {if $mod_count > 0}
            <table class="table" data-testid="mods-table">
                <thead>
                    <tr>
                        ...
                        <th>文件夹</th>
                        <th><span title="SteamID Universe（STEAM_X:Y:Z 中的 X）">SU</span></th>
                        <th>状态</th>
                ...
                <tbody>
                    {foreach from=$mod_list item=mod}
                        <tr id="mid_{$mod.mid}" data-testid="mod-row" data-id="{$mod.mid}">
                            ...
                            <td>
                                {if $mod.enabled}
                                    <span class="pill pill--online">已启用</span>
                                {else}
                                    <span class="pill pill--offline">已禁用</span>
                                {/if}
                            </td>
                            {if $permission_editmods || $permission_deletemods}
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        {if $permission_deletemods}
                                            ...
                                            <button class="btn btn--ghost btn--sm"
                                                    ...
                                                    aria-label="删除模组 {$mod.name|escape}">删除</button>
                                        {/if}
                                    </div>
                                </td>
                            {/if}
    ...
    <dialog id="mod-delete-dialog"
        ...
        <form method="dialog" data-testid="mod-delete-form">
            <h2 id="mod-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">删除模组</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                你即将永久删除 <strong data-testid="mod-delete-target">此模组</strong>。此操作无法撤销。针对该模组记录的历史封禁和通信屏蔽将保留行，但会丢失其游戏关联，因此这些条目的封禁列表中的"服务器 / 模组"列将显示为 &ldquo;未知&rdquo;。
            </p>
            <label class="label" for="mod-delete-reason">原因（可选）</label>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.ModsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', '模组已删除', ctx.name + ' 已被移除。');
            });
        });
