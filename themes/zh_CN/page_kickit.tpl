...
<div id="container" class="card" data-testid="kickit-container" data-mode="-{$mode}-">
    <div class="card__header">
        <div>
            <h3>正在所有服务器上搜索玩家&hellip;</h3>
            <p>每行实时轮询；行状态随服务器响应更新。</p>
        </div>
    </div>
    ...
    (function () {
        ...
        function setCounter(count) {
            ...
            if (srvcount === TOTAL || count === -1) {
                ...
                if (ctl) {
                    ctl.innerHTML = '<font color="green" style="font-size: 12px;"><b>搜索完成。</b></font>' + ctl.innerHTML;
        ...
        function processRow(sid, num) {
            sb.api.call(Actions.KickitKickPlayer, { check: CHECK, sid: sid, num: num, type: TYPE, mode: MODE })
                .then(function (r) {
                    if (!r || !r.ok || !r.data) {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>错误。</i></span>");
                        setCounter(1);
                        return;
                    }
                    var d = r.data;
                    ...
                    if (d.status === 'no_connect') {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>无法连接到服务器。</i></span>");
                        setCounter(1);
                    } else if (d.status === 'kicked') {
                        sb.setHTML('srv_' + num, "<span class='text-xs font-semibold' style='color:var(--success)'><u>已找到并踢出玩家！</u></span>");
                        setCounter(-1);
                    } else {
        ...
        window.addEventListener('load', function () {
            ...
            sb.api.call(Actions.KickitLoadServers, {}).then(function (r) {
                ...
                r.data.servers.forEach(function (s) {
                    if (s.has_rcon) {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-muted">搜索中&hellip;</span>');
                        processRow(s.sid, s.num);
                    } else {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-faint">无 RCON 密码。</span>');
                        setCounter(1);
                    }
                });
            });
