        ...
        <span class="filters-details__summary-label">
            ...
            {if $active_filter_count > 0}
                <span class="filters-details__count"
                      ...
                      aria-label="{$active_filter_count} 個活躍篩選條件">
                    &middot; {$active_filter_count} 個活躍
                </span>
            {/if}
        </span>
...
<form method="get"
    ...
    *}
    <div class="card__header filters-details__header">
        <div>
            <p>組合下方任意篩選條件 — 伺服器會將管理員列表縮小至符合<strong>每個</strong>已填寫篩選條件的記錄（AND 邏輯）。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-name" style="grid-column:1;align-self:end">登入名稱</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <input class="input"
                       id="search-admins-name"
                       name="name"
                       type="text"
                       placeholder="與面板登入名稱比對&hellip;"
                ...
                <select class="select"
                        ...
                        aria-label="登入名稱比對模式"
                        style="width:9rem">
                    <option value="0"{if $active_filter_name_match == '0'} selected{/if}>完全比對</option>
                    <option value="1"{if $active_filter_name_match != '0'} selected{/if}>部分比對</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        aria-label="Steam ID 比對模式"
                        style="width:9rem">
                    <option value="0"{if $active_filter_steam_match != '1'} selected{/if}>完全比對</option>
                    <option value="1"{if $active_filter_steam_match == '1'} selected{/if}>部分比對</option>
                </select>
            </div>
        </div>
        ...
        {if $can_editadmin}
            <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
                <label class="label" for="search-admins-admemail" style="grid-column:1;align-self:end">電子郵件</label>
                <div class="flex gap-2" style="flex-wrap:wrap">
                    <input class="input"
                           id="search-admins-admemail"
                           name="admemail"
                           type="email"
                           placeholder="與面板電子郵件比對&hellip;"
                    ...
                    <select class="select"
                            ...
                            aria-label="電子郵件比對模式"
                            style="width:9rem">
                        <option value="0"{if $active_filter_admemail_match == '0'} selected{/if}>完全比對</option>
                        <option value="1"{if $active_filter_admemail_match != '0'} selected{/if}>部分比對</option>
                    </select>
                </div>
            </div>
        {/if}
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-webgroup">網頁群組</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvadmgroup">SourceMod 管理員群組</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvgroup">伺服器群組</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admwebflag">網頁權限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admsrvflag">伺服器權限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-server">伺服器</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
                    ...
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                ...
                                data-server-host{if $active_filter_server == $server.sid && $active_filter_server != ''} selected{/if}>載入中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
        </div>
        ...
        <div class="flex gap-2 justify-end" style="border-top:1px solid var(--border);padding-top:0.75rem">
            <a class="btn btn--ghost btn--sm"
               ...
               data-testid="search-admins-reset">清除篩選條件</a>
            <button type="submit"
                    class="btn btn--primary btn--sm"
                    data-testid="search-admins-submit">
                <i data-lucide="search" style="width:14px;height:14px"></i> 搜尋
            </button>
        </div>
    </div>
</form>
