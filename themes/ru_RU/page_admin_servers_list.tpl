    ...
    {if NOT $permission_list}
        <div class="card" data-testid="server-list-denied">
            <div class="card__body">
                ...
                <p class="text-sm text-muted m-0">У вас нет прав на просмотр серверов.</p>
            </div>
        </div>
    {else}
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Серверы</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    <span data-testid="server-count" id="srvcount">{$server_count}</span>
                    зарегистрирован{if $server_count == 1} сервер{else}о серверов{/if}.
                    Сведения о хосте / карте / количестве игроков обновляются после первой отрисовки.
                </p>
            </div>
        ...
        {if $server_count == 0}
            <div class="card" data-testid="server-list-empty">
                <div class="card__body text-sm text-muted">
                    ...
                    {if $permission_addserver}<a href="index.php?p=admin&amp;c=servers&amp;section=add">Добавить</a>, чтобы начать.{/if}
                </div>
            </div>
        {else}
            {*
                    ...
                    <article class="card{if !$server.enabled} card--disabled{/if}"
                            ...
                            {if $server.enabled}
                                ...
                                *}
                                <span class="pill pill--offline"
                                    ...
                                    <span data-status-label>Загрузка</span>
                                </span>
                            {else}
                                <span class="pill pill--offline" title="Выключен (скрыт из публичных списков)">Выключен</span>
                            {/if}
                        </header>
                        ...
                        *}
                        <dl class="text-xs text-muted" style="margin:0;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:0.25rem 0.5rem">
                            ...
                            *}
                            <dt style="font-weight:500;color:var(--text)">ID сервера</dt>
                            <dd style="margin:0;display:flex;align-items:center;gap:0.25rem" data-testid="server-id">
                                ...
                                <button class="btn btn--ghost btn--icon btn--xs"
                                        ...
                                        title="Копировать ID сервера"
                                        aria-label="Копировать ID сервера">
                                    <i data-lucide="copy" style="width:12px;height:12px"></i>
                                </button>
                            </dd>
                            <dt style="font-weight:500;color:var(--text)">Мод</dt>
                            <dd style="margin:0">{if isset($server.mod_name)}{$server.mod_name|escape}{else}<span class="text-faint">неизвестно</span>{/if}</dd>
                            <dt style="font-weight:500;color:var(--text)">Игроки</dt>
                            <dd style="margin:0" data-testid="server-players">—</dd>
                            <dt style="font-weight:500;color:var(--text)">Карта</dt>
                            ...
                            {if $server.enabled}
                                ...
                                *}
                                <button type="button"
                                        ...
                                        data-action="refresh"
                                        title="Повторный запрос этого сервера"
        ...
        {if $permission_addserver}
            <div class="text-xs text-muted mt-4" data-testid="server-list-mapimg-hint">
                Нужно загрузить скриншот карты? Поместите его в
                <code class="font-mono">web/images/maps/</code>, используя имя карты в качестве имени файла
                (например, <code class="font-mono">de_dust2.jpg</code>).
            </div>
        {/if}
    {/if}
</section>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var label = btn.dataset.label || ('Сервер №' + sid);
        if (!Number.isFinite(sid) || sid <= 0) return;
        if (!window.confirm('Удалить «' + label + '»?\n\nЭто удалит запись сервера и все сопоставления групп/администраторов. Баны, записанные от него, сохраняются.')) {
            return;
        }
        ...
        api.call(window.Actions.ServersRemove, { sid: sid }).then(function (r) {
            if (!r || r.ok === false) {
                ...
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'Не удалось удалить', body: r.error.message || 'Неизвестная ошибка' });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast) {
                window.SBPP.showToast({ kind: 'success', title: 'Сервер удалён', body: label });
            }
        });
    });
})();
