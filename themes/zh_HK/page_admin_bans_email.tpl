...
<section class="p-6" data-testid="banemail-section" style="max-width:48rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">寄送電子郵件給玩家</h1>
        <p class="text-sm text-muted m-0 mt-2">
            寄送至 <span class="font-mono" data-testid="banemail-addr">{$email_addr|escape}</span>。
        </p>
    </div>
    <form class="card p-6 space-y-4"
        ...
        {csrf_field}
        <div>
            <label class="label" for="subject">主旨</label>
            <input type="text"
                   class="input"
                   id="subject"
                   name="subject"
        ...
        <div>
            <label class="label" for="message">訊息</label>
            <textarea class="textarea"
                      id="message"
                      name="message"
                      rows="8"
                      placeholder="您想讓玩家知道什麼？"
        ...
        <div class="flex justify-end gap-2"
            ...
            <button type="button"
                    ...
                    data-testid="banemail-back">返回</button>
...
(function () {
    ...
    window.CheckEmail = function (type, id) {
        ...
        if (!subject || !subject.value) { setMsg('subject.msg', '您必須輸入電子郵件的主旨。'); err++; }
        else { setMsg('subject.msg', ''); }
        if (!message || !message.value) { setMsg('message.msg', '您必須輸入電子郵件的訊息。'); err++; }
        ...
        a.call(A.SystemSendMail, {
            ...
            if (!r || r.ok === false) {
                toast('error', '電子郵件發送失敗', (r && r.error && r.error.message) || '未知錯誤');
                return;
            }
            ...
            toast('success', (msg && msg.title) || '電子郵件已發送', (msg && msg.body) || '電子郵件已發送。');
            if (msg && msg.redir) {
