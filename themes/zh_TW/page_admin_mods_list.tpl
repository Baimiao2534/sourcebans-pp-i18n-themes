...
{else}
    <div class="card">
        <div class="card__header">
            <div>
                <h3>伺服器模組</h3>
                <p><span data-testid="mod-count">{$mod_count}</span> 個已設定</p>
            </div>
        </div>
        {if $mod_count > 0}
                ...
                <tbody>
                    {foreach from=$mod_list item=mod}
                        <tr id="mid_{$mod.mid}" data-testid="mod-row" data-id="{$mod.mid}">
                            ...
                            <td>
                                {if $mod.enabled}
                                    <span class="pill pill--online">已啟用</span>
                                {else}
                                    <span class="pill pill--offline">已停用</span>
                                {/if}
                            </td>
                            {if $permission_editmods || $permission_deletemods}
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        {if $permission_deletemods}
                                            ...
                                            <button class="btn btn--ghost btn--sm"
                                                    ...
                                                    aria-label="刪除模組 {$mod.name|escape}">刪除</button>
                                        {/if}
                                    </div>
                                </td>
                            {/if}
    ...
    <dialog id="mod-delete-dialog"
        ...
        <form method="dialog" data-testid="mod-delete-form">
            <h2 id="mod-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">刪除模組</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                您即將永久刪除 <strong data-testid="mod-delete-target">此模組</strong>。此操作無法復原。依此模組建立的歷史封禁與通訊封鎖記錄會保留，但會失去其遊戲關聯，因此封禁列表中這些項目的伺服器 / 模組欄位會顯示為「未知」。
            </p>
            <label class="label" for="mod-delete-reason">原因（選填）</label>
            ...
            <textarea class="textarea"
                      ...
                      maxlength="255"
                      autocomplete="off"
                      placeholder="僅供稽核記錄。留空則略過。"></textarea>
            ...
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                ...
                <button type="submit" class="btn btn--danger" data-testid="mod-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> 刪除模組
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.ModsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', '模組已刪除', ctx.name + ' 已被移除。');
            });
        });
