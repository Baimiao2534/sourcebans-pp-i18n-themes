...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Wählen Sie das Thema, das das Panel-Chrome für jeden Besucher rendert.</p>
    </div>
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>Installierte Themen</h3>
                        <p>Jede Karte steht für ein Verzeichnis unter <code>web/themes/</code> mit einem <code>theme.conf.php</code>-Manifest.</p>
                    </div>
                    <div class="text-xs text-muted" style="text-align:right" data-testid="current-theme-summary">
                        Aktuell verwendet:<br>
                        <strong>{$theme_name}</strong> v{$theme_version} von {$theme_author}{if $theme_link != ''} · <a href="{$theme_link}" target="_blank" rel="noopener" class="text-muted">Homepage</a>{/if}<br>
                ...
                <div class="card__body">
                    {if NOT $can_web_settings}
                        <p class="text-xs text-muted m-0 mb-4" style="padding:0.5rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--bg-muted)">
                            ...
                            Sie können Themen durchsuchen, benötigen jedoch <code>ADMIN_WEB_SETTINGS</code>, um das aktive Thema zu wechseln.
                        </p>
                    {/if}
                            ...
                            <article class="theme-card"
                                ...
                                <div style="aspect-ratio:250 / 170;background:var(--bg-muted);display:grid;place-items:center;overflow:hidden">
                                    <img src="{$t.screenshot}"
                                         alt="{$t.name} Thema-Screenshot"
                                         loading="lazy"
                                         style="width:100%;height:100%;object-fit:cover"
                                         onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\'text-muted text-xs\'>Kein Screenshot</span>'">
                                </div>
                                <div class="p-4 space-y-3" style="flex:1;display:flex;flex-direction:column">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            {if $t.active}
                                                <span class="pill pill--online" title="Aktiv">
                                                    <i data-lucide="check" style="width:0.75rem;height:0.75rem"></i> Aktiv
                                                </span>
                                            {/if}
                                        </div>
                                        <p class="text-xs text-muted m-0 mt-2">von {$t.author} · v{$t.version}{if $t.link != ''} · <a href="{$t.link}" target="_blank" rel="noopener" class="text-muted" title="Thema-Homepage öffnen"><i data-lucide="external-link" style="width:0.75rem;height:0.75rem;vertical-align:-1px"></i> Homepage</a>{/if}</p>
                                        ...
                                        {if $can_web_settings}
                                            {if $t.active}
                                                <button type="button" class="btn btn--secondary btn--sm" disabled aria-pressed="true">
                                                    <i data-lucide="check"></i> In Verwendung
                                                </button>
                                            {else}
                                                <button type="button"
                                                    ...
                                                    <i data-lucide="check-circle"></i> Dieses Thema verwenden
                                                </button>
                                            {/if}
                                        {/if}
                                    </div>
...
(function () {
    ...
    window.applyTheme = function (theme) {
        if (!theme) return;
        if (!window.confirm('Panel-Thema auf "' + theme + '" wechseln? Jeder Besucher sieht das neue Thema bei der nächsten Anfrage.')) return;
