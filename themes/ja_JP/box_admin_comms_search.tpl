...
<form method="get"
    ...
    <div class="card__header">
        <div>
            <h3>高度な検索</h3>
            <p>各行は独立した検索条件です。右側の &laquo;検索&raquo; ボタンはその行の値のみで送信します。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-name" style="grid-column:1;align-self:end">ニックネーム</label>
            <input class="input"
                   id="search-comms-name"
                   type="text"
                   placeholder="プレイヤーニックネームの部分一致&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        style="width:9rem">
                    <option value="0" selected>完全一致</option>
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-reason" style="grid-column:1;align-self:end">理由</label>
            <input class="input"
                   id="search-comms-reason"
                   type="text"
                   placeholder="記録された理由の部分一致&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">日付</span>
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">期間</span>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-length-op"
                        data-testid="search-comms-length-op"
                        style="width:5rem">
                    <option value="e"  title="等しい">=</option>
                    <option value="h"  title="より大きい">&gt;</option>
                    <option value="l"  title="より小さい">&lt;</option>
                    <option value="eh" title="以上">&gt;=</option>
                    <option value="el" title="以下">&lt;=</option>
                </select>
                <select class="select"
                    ...
                    <optgroup label="か月">
                        <option value="40320">1 か月</option>
                        <option value="80640">2 か月</option>
                        <option value="120960">3 か月</option>
                        <option value="241920">6 か月</option>
                        <option value="483840">12 か月</option>
                    </optgroup>
                    <option value="other">その他の期間 (分)&hellip;</option>
                </select>
                <input type="text"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-btype">タイプ</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-btype"
                        data-testid="search-comms-btype">
                    <option value="1" selected>ミュート</option>
                    <option value="2">チャット禁止</option>
                </select>
            </div>
            <button type="submit"
                    class="btn btn--secondary btn--sm"
        ...
        {if NOT $hideadminname}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-admin">管理者</label>
                <div class="flex items-center gap-2" style="flex-wrap:wrap">
                    <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-server">サーバー</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-server"
                        data-testid="search-comms-server">
                    <option value="0" selected>ウェブ BAN</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                data-sid="{$server.sid}"
                                data-ip="{$server.ip}"
                                data-port="{$server.port}"
                                data-server-host>読み込み中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
            <button type="submit"
        ...
        {if $is_admin}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-comment" style="grid-column:1;align-self:end">コメント</label>
                <input class="input"
                       id="search-comms-comment"
                       type="text"
                       placeholder="管理者コメントの部分一致&hellip;"
