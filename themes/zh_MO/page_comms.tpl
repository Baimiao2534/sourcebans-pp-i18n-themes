...
<div class="p-6 space-y-4" style="max-width:1700px;margin:0 auto;width:100%">
    ...
    <header class="flex items-center justify-between gap-4" style="flex-wrap:wrap" data-testid="comms-header">
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">通訊封禁列表</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <span class="tabular-nums" data-testid="comms-count">{$ban_list|@count|number_format}</span>
                / <span class="tabular-nums">{$total_bans|number_format}</span> 筆封禁
            </p>
        </div>
        <div class="flex gap-2">
            ...
            <a class="btn btn--secondary btn--sm"
               ...
               data-testid="toggle-hide-inactive">
                ...
                {if $is_active_only}顯示非活躍{else}隱藏非活躍{/if}
            </a>
            {if $can_add_comm}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=comms"
               data-testid="comms-add-button">
                <i data-lucide="plus"></i> 新增通訊封禁
            </a>
            {/if}
        </div>
    </header>
    ...
    <form method="get" action="index.php"
        ...
        <div class="flex gap-3" style="flex-wrap:wrap">
            <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
                ...
                <input class="input input--with-icon"
                       ...
                       placeholder="玩家、SteamID 或原因&hellip;"
                       data-testid="comms-search">
            </div>
            <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="comms-server-filter" aria-label="按伺服器篩選">
                <option value="">所有伺服器</option>
                {foreach $servers as $s}
                    <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
                {/foreach}
            </select>
            <select class="select" name="time" style="width:auto" data-testid="comms-time-filter" aria-label="按時間範圍篩選">
        ...
        <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="快速篩選">
            ...
            <button class="chip" type="submit" name="type" value="mute"
                ...
                <i data-lucide="mic-off" style="width:12px;height:12px"></i> 禁麥
            </button>
            <button class="chip" type="submit" name="type" value="gag"
                ...
                <i data-lucide="message-square-off" style="width:12px;height:12px"></i> 禁言
            </button>
            <button class="chip" type="submit" name="type" value="silence"
                ...
                <i data-lucide="volume-x" style="width:12px;height:12px"></i> 靜音
            </button>
        </div>
    </form>
        ...
        <table class="table" data-testid="comms-table">
            <thead>
                <tr>
                    ...
                    <th>玩家</th>
                    <th>SteamID</th>
                    <th class="col-length col-tier-3">時長</th>
                    <th class="col-tier-2">伺服器</th>
                    <th class="col-admin col-tier-2">管理員</th>
                    <th class="col-tier-3">開始時間</th>
                    <th class="col-status">狀態</th>
                    <th class="col-actions" aria-label="列操作"></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr class="ban-row ban-row--{$comm.state}"
                        ...
                        <td>
                            <span class="pill pill--{$comm.state}" style="text-transform:capitalize">
                                ...
                                {if $comm.type == 'mute'}禁麥{elseif $comm.type == 'gag'}禁言{elseif $comm.type == 'silence'}靜音{else}{$comm.type|escape}{/if}
                            </span>
                        </td>
                        <td>
                            <div class="flex items-center gap-3" style="min-width:0">
                                ...
                                <div style="min-width:0">
                                    ...
                                    <a class="font-medium truncate"
                                       ...
                                       data-testid="drawer-trigger">{if $comm.name}{$comm.name|escape}{else}<i class="text-faint">無暱稱</i>{/if}</a>
                                    ...
                                    {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                        <div class="text-xs text-faint mt-1" data-testid="comm-unban-meta">
                                            {if !empty($comm.removedby)}解除者 <span class="font-medium">{$comm.removedby|escape}</span>{if !empty($comm.ureason)}：{/if}{/if}
                                    ...
                                    <details class="ban-comments-inline mt-1"
                                      ...
                                      <summary class="ban-comments-inline__summary"
                                               ...
                                               title="{$comm.commentdata|@count} 則評論">
                                        ...
                                        <span class="ban-comments-inline__label">則評論</span>
                                      </summary>
                        ...
                        <td class="col-status">
                            <span class="pill pill--{$comm.state}" style="text-transform:capitalize">{if $comm.state == 'active'}活躍{elseif $comm.state == 'permanent'}永久{elseif $comm.state == 'expired'}已過期{elseif $comm.state == 'unmuted'}已解除{else}{$comm.state|escape}{/if}</span>
                        </td>
                            ...
                            <div class="row-actions">
                                ...
                                {if $can_unmute_gag && $comm.unmute_url}
                                    ...
                                    <button type="button"
                                        ...
                                        {if $comm.type == 'mute'}解除禁麥{elseif $comm.type == 'gag'}解除禁言{else}解除封禁{/if}
                                    </button>
                    ...
                    <tr>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">沒有通訊封禁符合這些篩選條件</h2>
                                <p class="empty-state__body">請嘗試其他搜尋字詞、伺服器或時間範圍，或清除目前篩選以查看所有禁麥 / 禁言紀錄。</p>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">目前沒有通訊封禁紀錄</h2>
                                <p class="empty-state__body">從面板或遊戲內發出的禁麥與禁言會顯示在這裡。</p>
                                {if $can_add_comm}
                                <div class="empty-state__actions">
                                    <a class="btn btn--primary btn--sm"
        ...
        <div class="ban-cards">
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-sm truncate">
                                    {if $comm.name}{$comm.name|escape}{else}<span class="text-faint">無暱稱</span>{/if}
                                </span>
                                <span class="pill pill--{$comm.state}">{if $comm.state == 'active'}活躍{elseif $comm.state == 'permanent'}永久{elseif $comm.state == 'expired'}已過期{elseif $comm.state == 'unmuted'}已解除{else}{$comm.state|escape}{/if}</span>
                            </div>
                            <div class="text-xs text-muted truncate" style="margin-top:0.125rem">
                                <span style="text-transform:capitalize">{if $comm.type == 'mute'}禁麥{elseif $comm.type == 'gag'}禁言{elseif $comm.type == 'silence'}靜音{else}{$comm.type|escape}{/if}</span>
                            ...
                            {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                ...
                                <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="comm-unban-meta-mobile"
                                     title="{if !empty($comm.removedby)}解除者 {$comm.removedby|escape}{if !empty($comm.ureason)}：{/if}{/if}{if !empty($comm.ureason)}{$comm.ureason|escape}{/if}">
                                    {if !empty($comm.removedby)}解除者 {$comm.removedby|escape}{if !empty($comm.ureason)}：{/if}{/if}
            ...
            {foreachelse}
                ...
                <div class="empty-state" data-testid="comms-empty-mobile" data-filtered="true">
                    ...
                    <h2 class="empty-state__title">沒有通訊封禁符合這些篩選條件</h2>
                    <p class="empty-state__body">請嘗試其他搜尋字詞或清除目前篩選。</p>
                    <div class="empty-state__actions">
            ...
            {if $pagination.prev_url}
                <a class="btn btn--secondary btn--sm"
                   ...
                   data-testid="page-prev">
                    <i data-lucide="chevron-left"></i> 上一頁
                </a>
            {else}
            ...
            {if $pagination.next_url}
                <a class="btn btn--secondary btn--sm"
                   ...
                   data-testid="page-next">
                    下一頁 <i data-lucide="chevron-right"></i>
                </a>
            {else}
                <span class="btn btn--secondary btn--sm"
                      aria-disabled="true"
...
<dialog id="comms-unblock-dialog"
  ...
  <form method="dialog" data-testid="comms-unblock-form">
    <h2 id="comms-unblock-dialog-title" data-testid="comms-unblock-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">解除封禁</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      您為什麼要解除 <strong data-testid="comms-unblock-target">此玩家</strong>？此原因將記錄在封禁中並顯示在稽核記錄中。
    </p>
    <label class="label" for="comms-unblock-reason">原因 <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="comms-unblock-cancel" value="cancel">取消</button>
      <button type="submit" class="btn btn--primary" data-testid="comms-unblock-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> 確認
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnmuted(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === prev || txt === 'active' || txt === 'permanent' || txt === 'expired' || txt === '活躍' || txt === '永久' || txt === '已過期') {
                ...
                if (lastText) lastText.textContent = ' 已解除';
                else pill.textContent = '已解除';
            }
        });
        Array.prototype.forEach.call(
            ...
            function (btn) {
                ...
                a.innerHTML = '<i data-lucide="rotate-ccw" style="width:13px;height:13px"></i> 重新套用';
    ...
    function copyForType(type) {
        if (type === 'mute')    return { title: '解除禁麥',     verb: '解除禁麥' };
        if (type === 'gag')     return { title: '解除禁言',      verb: '解除禁言' };
        if (type === 'silence') return { title: '解除靜音',      verb: '解除靜音' };
        return                          { title: '解除封禁',       verb: '解除封禁' };
    }
    ...
    function openUnblockDialog(ctx) {
        ...
        if (prompt) prompt.textContent = ctx.name || ('封禁 #' + ctx.bid);
        ...
        if (p && p.firstChild && p.firstChild.nodeType === 3) {
            p.firstChild.textContent = '您為什麼要' + copy.verb;
        }
    ...
    document.addEventListener('click', function (e) {
        ...
        var bid = btn.getAttribute('data-bid') || '';
        var name = btn.getAttribute('data-name') || ('封禁 #' + bid);
        ...
        if (act === 'comms-delete') {
            if (!window.confirm('永久刪除「' + name + '」的封禁？')) return;
            setBusy(btn, true);
            a.call(A.CommsDelete, { bid: Number(bid) }).then(function (r) {
                ...
                decrementCount();
                toast('success', '封禁已移除', '「' + name + '」的封禁已刪除。');
            });
            return;
        }
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.CommsUnblock, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            ...
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '未知錯誤';
                showError(msg);
                toast('error', '解除失敗', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnmuted);
            closeUnblockDialog();
            toast('success', '封禁已解除', ctx.name + ' 已解除封禁。');
        });
    });
