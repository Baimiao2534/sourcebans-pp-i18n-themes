...
{if NOT $permission_listmods}
    <div class="card">
        <div class="card__body">
            <p class="text-muted">Доступ запрещён.</p>
        </div>
    </div>
{else}
    <div class="card">
        <div class="card__header">
            <div>
                <h3>Серверные моды</h3>
                <p><span data-testid="mod-count">{$mod_count}</span> настроено</p>
            </div>
        </div>
        {if $mod_count > 0}
            <table class="table" data-testid="mods-table">
                <thead>
                    <tr>
                        ...
                        <th>Папка</th>
                        <th><span title="Вселенная SteamID (X в STEAM_X:Y:Z)">SU</span></th>
                        <th>Статус</th>
                ...
                <tbody>
                    {foreach from=$mod_list item=mod}
                        <tr id="mid_{$mod.mid}" data-testid="mod-row" data-id="{$mod.mid}">
                            ...
                            <td>
                                {if $mod.enabled}
                                    <span class="pill pill--online">Включён</span>
                                {else}
                                    <span class="pill pill--offline">Выключен</span>
                                {/if}
                            </td>
                            {if $permission_editmods || $permission_deletemods}
                                    ...
                                    <div class="flex justify-end gap-2">
                                        {if $permission_editmods}
                                            <a class="btn btn--ghost btn--sm"
                                               ...
                                               data-testid="editmod-link">Изменить</a>
                                        {/if}
                                        {if $permission_deletemods}
                                            ...
                                            <button class="btn btn--ghost btn--sm"
                                                    ...
                                                    aria-label="Удалить мод {$mod.name|escape}">Удалить</button>
                                        {/if}
                                    </div>
                                </td>
                            {/if}
        ...
        {else}
            <div class="card__body">
                <p class="text-muted">Моды пока не настроены.</p>
            </div>
        {/if}
    </div>
    ...
    <dialog id="mod-delete-dialog"
        ...
        <form method="dialog" data-testid="mod-delete-form">
            <h2 id="mod-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Удалить мод</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                Вы собираетесь навсегда удалить <strong data-testid="mod-delete-target">этот мод</strong>. Это действие необратимо. Исторические баны и блокировки связи, записанные для этого мода, сохраняют свои записи, но теряют ассоциацию с игрой, поэтому в списке банов для этих записей в столбце «Сервер / Мод» будет отображаться «Неизвестно».
            </p>
            <label class="label" for="mod-delete-reason">Причина (необязательно)</label>
            ...
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                <button type="button" class="btn btn--secondary" data-testid="mod-delete-cancel" value="cancel">Отмена</button>
                <button type="submit" class="btn btn--danger" data-testid="mod-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> Удалить мод
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        function openDeleteDialog(ctx) {
            ...
            if (target) target.textContent = ctx.name || ('мод №' + ctx.mid);
        ...
        document.addEventListener('click', function (e) {
            ...
            var name = btn.getAttribute('data-name') || ('мод №' + mid);
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.ModsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', 'Мод удалён', ctx.name + ' был удалён.');
            });
        });
