...
{else}
    <form method="post"
        ...
        <div class="card">
            <div class="card__header">
                <div>
                    <h3>MOD を追加</h3>
                    <p>BAN やサーバーに割り当て可能な新しいゲーム MOD を設定します。</p>
                </div>
            </div>
            <div class="card__body space-y-4" style="max-width:42rem">
                ...
                <div>
                    <label class="label" for="name">MOD 名</label>
                    <input class="input"
                           type="text"
                           id="name"
                           name="name"
                ...
                <div>
                    <label class="label" for="folder">MOD フォルダ</label>
                    <input class="input"
                           type="text"
                           id="folder"
                           name="folder"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        ディスク上のフォルダ名（例：Counter-Strike: Source の <span class="font-mono">cstrike</span>）。
                    </p>
                    <div id="folder.msg"
                         class="text-xs"
                         style="color:var(--danger);display:none;margin-top:0.25rem"></div>
                ...
                <div>
                    <label class="label" for="steam_universe">Steam universe 番号</label>
                    <input class="input"
                           type="number"
                           id="steam_universe"
                           name="steam_universe"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        この MOD でレンダリングされる <span class="font-mono">STEAM_X:Y:Z</span> の最初の数字（X）。デフォルトは 0。
                    </p>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox"
                               ...
                               value="1"
                               checked>
                        <span class="text-sm">有効 — BAN やサーバーに割り当て可能。</span>
                    </label>
                </div>

                <div>
                    <label class="label">MOD アイコン</label>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xs text-muted" data-testid="addmod-current-icon" hidden>
                            現在：<span class="font-mono" data-testid="addmod-current-icon-name"></span>
                        </span>
                    </div>
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        16x16 の GIF、PNG または JPG。アップロードポップアップが開きます。
                    </p>
                    <div id="icon.msg"
                         class="text-xs"
                ...
                <div class="flex justify-end gap-2"
                    ...
                    <a class="btn btn--ghost btn--sm"
                       ...
                       data-testid="addmod-cancel">戻る</a>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            var errors = 0;
            if (!name)   { showMsg('name.msg',   'MOD 名を入力してください。'); errors++; }
            if (!folder) { showMsg('folder.msg', 'MOD フォルダ名を入力してください。'); errors++; }
            if (!icon)   { showMsg('icon.msg',   'MOD のアイコンをアップロードしてください。'); errors++; }
            if (errors > 0) return;
            ...
            if (!a || !A) {
                ...
                showMsg('name.msg', 'JSON ディスパッチャーが見つかりません。ページをリロードして再試行してください。');
                return;
            }
            ...
            a.call(A.ModsAdd, {
                ...
                if (r.ok === false) {
                    ...
                    var slot = field === 'folder' ? 'folder.msg'
                        ...
                        : 'name.msg';
                    showMsg(slot, em);
                    toast('error', 'MOD の追加に失敗しました', em);
                    return;
                }
                var data = r.data || {};
                var msg = data.message || {};
                toast('success', msg.title || 'MOD が追加されました', msg.body || 'MOD が追加されました。');
                ...
                toast('error', 'MOD の追加に失敗しました', String(err && err.message ? err.message : err));
            });
        });
    })();
    </script>
