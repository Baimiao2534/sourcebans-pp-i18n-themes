...
<div style="max-width:48rem">
    <div class="mb-4">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Создать группу</h1>
        <p class="text-sm text-muted m-0 mt-2">Выберите имя и тип. Флаги прав можно изменить на вкладке <strong>Список групп</strong> после создания группы.</p>
    </div>
    <form class="card"
          method="post"
        ...
        <div class="card__body space-y-4">
            <div>
                <label class="label" for="add-group-name">Название группы</label>
                <input class="input"
                       id="add-group-name"
                       name="name"
                       data-testid="add-group-name"
                       autocomplete="off"
                       placeholder="напр. Старшие администраторы"
                       required>
                <p class="text-xs text-muted m-0 mt-2">Должно быть уникальным. Без запятых.</p>
            </div>

            <div>
                <label class="label" for="add-group-type">Тип группы</label>
                <select class="select"
                        id="add-group-type"
                        name="type"
                    ...
                    <option value="1">Веб-группа администратора</option>
                    <option value="2">Серверная группа администратора</option>
                    <option value="3">Серверная группа</option>
                </select>
                <p class="text-xs text-muted m-0 mt-2">Веб-администратор = права панели. Серверный администратор = char-флаги SourceMod. Серверная группа = объединение игровых серверов.</p>
            </div>

            <div data-testid="add-group-srvflags-block" id="add-group-srvflags-block" style="display:none">
                <label class="label" for="add-group-srvflags">Флаги SourceMod и иммунитет</label>
                <input class="input"
                ...
                <p class="text-xs text-muted m-0 mt-2">Строка флагов SourceMod. Добавьте <code>#&lt;immunity&gt;</code> для иммунитета (по умолчанию 0).</p>
            </div>
        </div>
        <div class="card__header" style="border-top:1px solid var(--border);border-bottom:0;justify-content:flex-end">
            <div class="flex gap-2">
                <a class="btn btn--ghost" href="?p=admin&c=groups&section=list" data-testid="add-group-cancel">Отмена</a>
                <button class="btn btn--primary" type="submit" data-testid="add-group-submit">Создать группу</button>
            </div>
        </div>
    </form>
</div>
    ...
    sb.api.call(Actions.GroupsAdd, {
        ...
        if (r.ok === false) {
            ...
            if (window.SBPP && typeof window.SBPP.showToast === 'function') {
                ...
                sb.message.error('Ошибка', em);
            }
            return;
        }
        var data = r.data || {};
        var body = (data.message && data.message.body) || 'Группа добавлена.';
        var title = (data.message && data.message.title) || 'Группа добавлена';
        if (window.SBPP && typeof window.SBPP.showToast === 'function') {
