...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Общая конфигурация сайта. Изменения вступают в силу немедленно.</p>
    </div>
    ...
    {else}
                <form action="?p=admin&amp;c=settings&amp;section=settings" method="post" class="space-y-4" id="form-settings-main">
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Общие</h3><p>Идентичность сайта, правила паролей и даты.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="template.title">
                                <label class="label" for="template_title">Название сайта</label>
                            ...
                            <div data-testid="setting-row" data-key="template.logo">
                                <label class="label" for="template_logo">Путь к логотипу</label>
                                    ...
                                    *}
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-warning-template.logo">
                                        <strong>Внимание:</strong> оформление в настоящее время отображает стандартный щит (<code class="font-mono">images/favicon.svg</code>), поскольку сохранённый путь не указывает на файл на диске в активной теме. Убедитесь, что файл существует в <code class="font-mono">themes/&lt;theme&gt;/</code>, или очистите поле ввода, чтобы удалить пользовательское значение.
                                    </p>
                                {/if}
                                <p class="settings-fieldset__help"
                                   id="template_logo_help"
                                   data-testid="setting-help-template.logo">
                                    Путь к логотипу боковой панели и страницы входа относительно каталога темы (<code class="font-mono">themes/&lt;theme&gt;/</code>). По умолчанию <code class="font-mono">images/favicon.svg</code>. Растровая версия 512&times;512 находится в <code class="font-mono">logos/sbpp_logo.png</code>.
                                </p>
                            </div>
                            <div data-testid="setting-row" data-key="config.password.minlength">
                                <label class="label" for="config_password_minlength">Минимальная длина пароля</label>
                            ...
                            <div data-testid="setting-row" data-key="config.dateformat">
                                <label class="label" for="config_dateformat">Формат даты <span class="text-muted text-xs">(<a href="https://www.php.net/manual/en/datetime.format.php" target="_blank" rel="noopener">PHP date()</a>)</span></label>
                            ...
                            <div data-testid="setting-row" data-key="config.debug">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Включить режим отладки</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="card">
                        <fieldset class="settings-fieldset"
                            ...
                            <legend class="settings-fieldset__legend">
                                <span class="settings-fieldset__title">Аутентификация</span>
                                <span class="settings-fieldset__hint">Срок жизни сеансовых токенов в минутах. Установите <code>0</code>, чтобы отключить способ входа.</span>
                            </legend>
                            <div class="settings-fieldset__body space-y-5">
                                ...
                                *}
                                <div data-testid="setting-row" data-key="auth.maxlife">
                                    <label class="label" for="auth_maxlife">Стандартный вход</label>
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife">
                                        Как долго длится обычный сеанс входа до автоматического выхода пользователя, в минутах.
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.remember">
                                    <label class="label" for="auth_maxlife_remember">&ldquo;Запомнить меня&rdquo; — вход</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.remember">
                                        Используется, когда пользователь устанавливает флажок &ldquo;Запомнить меня&rdquo; в форме входа. Обычно значительно дольше стандартного сеанса.
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.steam">
                                    <label class="label" for="auth_maxlife_steam">Вход через Steam</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.steam">
                                        Срок жизни сеанса, открытого через кнопку &ldquo;Войти через Steam&rdquo;.
                                    </p>
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Введение на главной панели</h3><p>Заголовок и текст публичной главной страницы.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="dash.intro.title">
                                <label class="label" for="dash_intro_title">Заголовок вступления</label>
                                ...
                                <label class="label" for="dash_intro_text">
                                    ...
                                    <span class="text-muted text-xs">
                                        ...
                                        (<a href="https://commonmark.org/help/"
                                            target="_blank"
                                            rel="noopener"
                                            data-testid="dash-intro-cheatsheet-link">шпаргалка по CommonMark</a>).
                                        HTML-теги экранируются при отображении.
                                    </span>
                                </label>
                                {*
                                ...
                                *}
                                <div class="dash-intro-editor"
                                    ...
                                    <div class="dash-intro-preview"
                                        ...
                                        <div class="dash-intro-preview__label">Предпросмотр</div>
                                        {*
                                        ...
                                        *}
                                        <div class="dash-intro-preview__body"
                                            ...
                                            {else}
                                                <p class="text-xs text-muted m-0" data-empty>Введите Markdown слева, чтобы увидеть, как он будет отображаться посетителям.</p>
                                            {/if}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div data-testid="setting-row" data-key="dash.lognopopup">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Отключить всплывающее окно журнала (использовать прямые ссылки)</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Публичные страницы</h3><p>Какие дополнительные страницы доступны посетителям.</p></div></div>
                        <div class="card__body space-y-3">
                            <div data-testid="setting-row" data-key="config.enableprotest">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Включить страницу протестов против банов</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablesubmit">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Включить страницу подачи заявок на бан</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablecomms">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Включить список блокировок связи</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="protest.emailonlyinvolved">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Отправлять письмо об исходном бане только администратору при новых протестах</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.defaultpage">
                                <label class="label" for="default_page">Страница по умолчанию</label>
                                <select class="select" id="default_page" name="default_page">
                                    <option value="0"{if $config_default_page == 0} selected{/if}>Главная панель</option>
                                    <option value="1"{if $config_default_page == 1} selected{/if}>Список банов</option>
                                    <option value="2"{if $config_default_page == 2} selected{/if}>Серверы</option>
                                    <option value="3"{if $config_default_page == 3} selected{/if}>Подать бан</option>
                                    <option value="4"{if $config_default_page == 4} selected{/if}>Опротестовать бан</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Список банов</h3><p>Пагинация и видимость.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="banlist.bansperpage">
                                <label class="label" for="banlist_bansperpage">Банов на странице</label>
                            ...
                            <div data-testid="setting-row" data-key="banlist.hideadminname">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Скрывать имя администратора в публичной информации о банах</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.hideplayerips">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Скрывать IP-адреса игроков в публичной информации о банах</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.nocountryfetch">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Пропустить определение страны по IP</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="bans.customreasons">
                                <label class="label">Свои причины банов</label>
                                <p class="text-xs text-muted m-0 mb-2">Каждая строка становится вариантом в раскрывающемся списке причин бана. Оставьте пустым, чтобы удалить.</p>
                                <div id="custom-reasons" class="space-y-3">
                                    ...
                                    {/foreach}
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="Добавить ещё причину…">
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="Добавить ещё причину…">
                                </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>SMTP</h3><p>Исходящая почта.</p></div></div>
                        <div class="card__body space-y-4">
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <div data-testid="setting-row" data-key="smtp.host">
                                    <label class="label" for="mail_host">Хост</label>
                            ...
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="smtp.user">
                                    <label class="label" for="mail_user">Имя пользователя</label>
                                ...
                                <div data-testid="setting-row" data-key="smtp.pass">
                                    <label class="label" for="mail_pass">Пароль <span class="text-muted text-xs">(оставьте пустым, чтобы сохранить текущий)</span></label>
                            ...
                            <div data-testid="setting-row" data-key="smtp.verify_peer">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Проверять TLS-сертификат узла</span>
                                </label>
                            </div>
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="config.mail.from_email">
                                    <label class="label" for="mail_from_email">От кого (email)</label>
                                ...
                                <div data-testid="setting-row" data-key="config.mail.from_name">
                                    <label class="label" for="mail_from_name">От кого (имя)</label>
                            ...
                            *}
                            <div data-testid="setting-row" data-key="smtp.test">
                                <label class="label" for="mail_test_to">Отправить тестовое письмо</label>
                                <div class="grid gap-2" style="grid-template-columns:1fr auto">
                                    ...
                                    <button type="button"
                                        ...
                                        <i data-lucide="send"></i> Отправить тестовое письмо
                                    </button>
                                </div>
                                <p class="text-xs text-muted mt-1">
                                    Отправляет проверочное письмо через сохранённые учётные данные SMTP. Сначала сохраните форму, если вы только что изменили значения выше — тест использует сохранённые данные, а не несохранённое содержимое полей ввода.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="settings-actions flex items-center justify-end gap-2">
                        <button type="button" class="btn btn--secondary btn--sm" onclick="clearCacheBtn();">
                            <i data-lucide="brush"></i> Очистить кэш
                        </button>
...
(function () {
    ...
    window.addCustomReason = function () {
        ...
        inp.placeholder = 'Добавить ещё причину…';
        box.appendChild(inp);
    };
    ...
    if (ta && preview && previewBody && window.sb && window.sb.api && window.Actions) {
        var emptyHtml = '<p class="text-xs text-muted m-0" data-empty>Введите Markdown слева, чтобы увидеть, как он будет отображаться посетителям.</p>';
    ...
    function humanizeMinutes(minutes) {
        if (!Number.isFinite(minutes) || minutes <= 0) return 'отключено';
        var n = Math.floor(minutes);
        if (n < 60) return n === 1 ? '1 минута' : n + ' мин.';
        if (n < 1440) {
            if (n % 60 === 0) {
                var h = n / 60;
    ...
    if (testBtn && window.sb && window.sb.api && window.Actions) {
        testBtn.addEventListener('click', function () {
            ...
            if (rawTo === '' || !testTo.checkValidity()) {
                ...
                toast('warn', 'Укажите получателя', 'Введите корректный email-адрес перед отправкой теста.');
                return;
            }
            ...
            window.sb.api.call(window.Actions.SystemTestEmail, { to: rawTo }).then(
                function (r) {
                    ...
                    if (r && r.ok && r.data && typeof r.data.to === 'string') {
                        toast('success', 'Тестовое письмо отправлено',
                            'Тестовое SMTP-сообщение было отправлено на адрес ' + r.data.to + '. Проверьте входящие для подтверждения доставки.');
                        return;
                    }
                    ...
                    var msg = err.message || 'Сервер вернул неожиданный ответ.';
                    if (code === 'smtp_not_configured') {
                        toast('warn', 'SMTP не настроен', msg);
                    } else if (code === 'rate_limited') {
                        toast('warn', 'Тестовое письмо отложено', msg);
                    } else if (code === 'validation') {
                        toast('warn', 'Проверьте получателя', msg);
                    } else if (code === 'mail_failed') {
                        toast('error', 'Ошибка тестового письма', msg);
                    } else {
                        toast('error', 'Ошибка тестового письма', msg);
                    }
                },
                function () {
                    setBusy(testBtn, false);
                    toast('error', 'Ошибка тестового письма', 'Не удалось связаться с API панели. Проверьте сеть и повторите попытку.');
                }
            );
        });
    }
