...
<div class="audit-page">
    <header class="audit-page__header">
        <div>
            <h1 class="audit-page__title">Audit-Protokoll</h1>
            <p class="audit-page__subtitle">Jede vom Panel erfasste administrative Aktion ({$total_count} {if $total_count == 1}Ereignis{else}Ereignisse{/if}).</p>
        </div>
    </header>
    ...
    <div class="audit-filters">
        <form method="get" action="index.php" role="search" aria-label="Audit-Protokoll durchsuchen" style="flex:1 1 16rem;max-width:22rem">
            ...
            <input type="search"
                   ...
                   name="search"
                   value="{$search}"
                   placeholder="Titel oder Detail suchen…"
                   data-testid="audit-search"
                   aria-label="Audit-Protokoll durchsuchen">
        </form>

        <form method="get" action="index.php" class="flex items-center gap-2" style="flex-wrap:wrap" aria-label="Audit-Protokoll nach Schweregrad filtern">
            <input type="hidden" name="p" value="admin">
            <input type="hidden" name="c" value="audit">
            ...
            <div class="empty-state"
                ...
                <h2 class="empty-state__title">
                    {if $current_severity != '' || $search != ''}
                        Keine Audit-Ereignisse entsprechen diesen Filtern
                    {else}
                        Noch keine Audit-Ereignisse erfasst
                    {/if}
                </h2>
                <p class="empty-state__body">
                    {if $current_severity != '' || $search != ''}
                        Versuchen Sie einen anderen Suchbegriff oder Schweregrad, oder entfernen Sie die Filter, um jede erfasste Administrator-Aktion zu sehen.
                    {else}
                        Sobald Administratoren mit der Moderation über das Panel beginnen, wird jede Aktion hier zur Überprüfung protokolliert.
                    {/if}
                </p>
                {if $current_severity != '' || $search != ''}
    ...
    {if $page_count > 1 || $current_page > 1}
        <nav class="audit-pager" aria-label="Audit-Protokoll Seitennavigation">
