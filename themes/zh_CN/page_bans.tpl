...
<div class="card" style="max-width:42rem;margin:1.5rem auto">
  <div class="card__header">
    <div>
      <h3>{$commenttype} 评论</h3>
      <p>仅管理员可见；启用公开评论时，已评论的话题也会对公众显示。</p>
    </div>
  </div>
  <div class="card__body">
    <form id="banlist-comment-form" data-bid="{$comment}" data-ctype="{$ctype}" data-cid="{$cid}" data-page="{$page}">
      <label class="label" for="banlist-comment-text">评论</label>
      <textarea class="textarea" id="banlist-comment-text" name="commenttext" rows="6" {if !$canedit}disabled{/if}>{$commenttext}</textarea>
      <div class="flex gap-2 mt-4">
        {if $canedit}
        <button class="btn btn--primary" type="submit">{$commenttype} 评论</button>
        {/if}
        <button class="btn btn--secondary" type="button" onclick="history.back()">返回</button>
      </div>
    </form>

    <div class="mt-6">
      {foreach from=$othercomments item=com name=othercomments}
        {if $smarty.foreach.othercomments.first}<h3 style="font-size:var(--fs-base);font-weight:600;margin:0 0 0.5rem">其他评论</h3>{/if}
        <div class="mt-4" style="border-top:1px solid var(--border);padding-top:0.75rem">
          <div class="flex items-center justify-between">
            ...
            {if $hideadminname}<i class="text-faint">已隐藏</i>{elseif !empty($com.comname)}<strong>{$com.comname|escape}</strong>{else}<i class="text-faint">已删除管理员</i>{/if}
          ...
          <div class="text-xs text-faint mt-2">最后编辑 {$com.edittime} 由 {if $hideadminname}<i class="text-faint">已隐藏</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>已删除管理员</i>{/if}</div>
          {/if}
        </div>
      {/foreach}
    </div>
...
<div id="banlist-root" class="p-6 space-y-4" style="max-width:1700px;margin:0 auto" data-loading="false">
  <div class="flex items-center justify-between gap-3" style="flex-wrap:wrap">
    <div>
      <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">封禁列表</h1>
      <p class="text-sm text-muted m-0 mt-2"><span class="tabular-nums">{$ban_list|@count}</span> / 共 <span class="tabular-nums">{$total_bans}</span> 条</p>
    </div>
    <div class="flex gap-2 items-center">
      {if $can_export}
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=steam" title="导出永久 SteamID 封禁">导出 Steam</a>
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=ip" title="导出永久 IP 封禁">导出 IP</a>
      {/if}
      ...
      <a class="btn btn--secondary btn--sm"
         ...
         data-testid="toggle-hide-inactive">{if $hidetext == 'Show'}显示{else}隐藏{/if}过期</a>
      {/if}
      {has_access flag=$smarty.const.ADMIN_ADD_BAN}
      <a class="btn btn--primary btn--sm" href="index.php?p=admin&c=bans">添加封禁</a>
      {/has_access}
    </div>
  </div>
  ...
  <form method="get" action="index.php" id="banlist-filters" style="position:sticky;top:3.5rem;z-index:20;background:var(--bg-page);padding:0.75rem 0;border-bottom:1px solid var(--border)">
    ...
    <div class="flex gap-3" style="flex-wrap:wrap">
      <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
        <input class="input" type="search" name="searchText" data-testid="bans-search"
          ...
          placeholder="玩家、SteamID 或 IP&hellip;" aria-label="搜索封禁">
      </div>
      <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="bans-server-filter" aria-label="按服务器筛选">
        <option value="">全部服务器</option>
        {foreach $server_list as $s}
          <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
        {/foreach}
      </select>
      <select class="select" name="time" style="width:auto" data-testid="bans-time-filter" aria-label="按时间范围筛选">
        <option value="">全部时间</option>
        <option value="1d"  {if $filters.time == '1d'}selected{/if}>今天</option>
        <option value="7d"  {if $filters.time == '7d'}selected{/if}>最近 7 天</option>
        <option value="30d" {if $filters.time == '30d'}selected{/if}>最近 30 天</option>
      </select>
      <button class="btn btn--secondary btn--sm" type="submit" data-testid="bans-filter-apply">
        <i data-lucide="filter"></i> 应用
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="按状态筛选">
      <a class="chip"
         href="index.php?p=banlist{$chip_base_link}"
         data-state-filter=""
         data-testid="filter-chip-all"
         data-active="{if $active_state == ''}true{else}false{/if}"
         aria-current="{if $active_state == ''}true{else}false{/if}">全部</a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#ef4444"></span>永久
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#f59e0b"></span>活跃
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:var(--zinc-300)"></span>过期
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#10b981"></span>已解封
      </a>
    </div>
  </form>
  ...
  <div class="card" style="overflow:hidden">
    ...
    <table class="table">
      <thead>
        ...
        <tr>
          ...
          <th scope="col">原因</th>
          <th scope="col" class="col-tier-2">服务器</th>
          {if !$hideadminname}<th scope="col" class="col-admin col-tier-2">管理员</th>{/if}
          <th scope="col" class="col-length col-tier-3">时长</th>
          <th scope="col" class="col-banned col-tier-3">封禁时间</th>
          <th scope="col" class="col-status">状态</th>
        ...
        <tr class="ban-row ban-row--{$ban.state}" data-state="{$ban.state}" data-id="{$ban.bid}" data-testid="ban-row">
          <td>
            <div class="flex items-center gap-3" style="min-width:0">
              ...
              <a class="font-medium truncate" href="?p=banlist&amp;id={$ban.bid}" data-drawer-href="?p=banlist&amp;c=details&amp;id={$ban.bid}" data-testid="drawer-trigger">{if empty($ban.name)}<i class="text-faint">无昵称</i>{else}{$ban.name|escape}{/if}</a>
            </div>
            ...
            <details class="ban-comments-inline"
              ...
              <summary class="ban-comments-inline__summary"
                       ...
                       title="此封禁有 {$ban.commentdata|@count} 条评论">
                ...
                <span class="ban-comments-inline__label">条评论</span>
              </summary>
              <ul class="ban-comments-inline__list" data-testid="ban-comments-list">
                ...
                <li class="ban-comments-inline__item" data-testid="ban-comment-item">
                  ...
                  {if !empty($com.edittime)}
                  <div class="ban-comments-inline__edit text-xs text-faint">最后编辑 {$com.edittime} 由 {if $hideadminname}<i class="text-faint">已隐藏</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>已删除管理员</i>{/if}</div>
                  {/if}
                </li>
                {/foreach}
              </ul>
          ...
          <td class="text-muted truncate" style="max-width:18rem"
            ...
            {if empty($ban.reason)}<i class="text-faint">无原因</i>{else}{$ban.reason|escape}{/if}
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint mt-1" data-testid="ban-unban-meta">
                {if !empty($ban.removedby)}
                  由 <span class="font-medium">{$ban.removedby|escape}</span> 解封{if !empty($ban.ureason)}：{/if}
                {/if}
          ...
          <td class="col-admin col-tier-2 text-muted">
            {if empty($ban.aname)}<i class="text-faint">已删除</i>{else}{$ban.aname|escape}{/if}
          </td>
          {/if}
          ...
          <td class="col-length col-tier-3 tabular-nums text-muted"
              {if $ban.length != 0}title="{$ban.length_human|escape}"{/if}>{if $ban.length == 0}永久{else}{$ban.length_human|escape}{/if}</td>
          ...
          <td class="col-status">
            ...
            <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}活跃{elseif $ban.state == 'permanent'}永久{elseif $ban.state == 'expired'}过期{elseif $ban.state == 'unbanned'}已解封{else}{$_pill_label|escape}{/if}</span>
          </td>
        ...
        <tr>
          ...
          <td colspan="10" style="padding:0">
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="true">
              ...
              <h2 class="empty-state__title">没有符合筛选条件的封禁</h2>
              <p class="empty-state__body">尝试其他搜索词或清除当前筛选以查看所有封禁记录。</p>
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="false">
              ...
              <h2 class="empty-state__title">尚无封禁记录</h2>
              <p class="empty-state__body">管理员开始执法后，封禁操作将显示在这里。</p>
              {if $can_add_ban}
              <div class="empty-state__actions">
    ...
    <div class="ban-cards">
        ...
        <a class="ban-card__summary flex items-center gap-3 p-4"
          ...
          <div style="flex:1;min-width:0">
            <div class="flex items-center gap-2">
              <span class="font-medium text-sm truncate">{if empty($ban.name)}<i class="text-faint">无昵称</i>{else}{$ban.name|escape}{/if}</span>
              {assign var=_m_pill_label value=$ban.state|capitalize}
              <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}活跃{elseif $ban.state == 'permanent'}永久{elseif $ban.state == 'expired'}过期{elseif $ban.state == 'unbanned'}已解封{else}{$_m_pill_label|escape}{/if}</span>
            </div>
            ...
            <div class="text-xs text-muted truncate" style="margin-top:0.125rem"
                 title="{if empty($ban.reason)}无原因{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永久{else}{$ban.length_human|escape}{/if}">{if empty($ban.reason)}无原因{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永久{else}{$ban.length_human|escape}{/if}</div>
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="ban-unban-meta-mobile"
                   title="{if !empty($ban.removedby)}由 {$ban.removedby|escape} 解封{if !empty($ban.ureason)}：{/if}{/if}{if !empty($ban.ureason)}{$ban.ureason|escape}{/if}">
                {if !empty($ban.removedby)}由 {$ban.removedby|escape} 解封{if !empty($ban.ureason)}：{/if}{/if}
      ...
      <div class="empty-state" data-testid="banlist-empty-mobile" data-filtered="true">
        ...
        <h2 class="empty-state__title">没有符合筛选条件的封禁</h2>
        <p class="empty-state__body">尝试其他搜索词或清除当前筛选。</p>
        <div class="empty-state__actions">
  ...
  <div class="card">
    <div class="card__body">
      <nav class="text-xs text-muted" aria-label="封禁列表分页">
...
<dialog id="bans-unban-dialog"
  ...
  <form method="dialog" data-testid="bans-unban-form">
    <h2 id="bans-unban-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">解封玩家</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      您为什么要解封 <strong data-testid="bans-unban-target">该玩家</strong>？此原因将记录在该封禁上并显示在审计日志中。
    </p>
    <label class="label" for="bans-unban-reason">解封原因 <span aria-hidden="true" style="color:var(--danger)">*</span></label>
...
(function () {
    ...
    function flipRowToUnbanned(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === 'Active' || txt === 'Permanent' || txt === 'Expired' || txt === '活跃' || txt === '永久' || txt === '过期') {
                if (lastText) lastText.textContent = '已解封';
                else pill.textContent = '已解封';
            }
        });
    ...
    function openUnbanDialog(ctx) {
        ...
        if (target) target.textContent = ctx.name || ('封禁 #' + ctx.bid);
    ...
    document.addEventListener('click', function (e) {
        ...
        var name = actionBtn.getAttribute('data-name') || ('封禁 #' + bid);
        ...
        if (act === 'bans-delete') {
            ...
            // page load.
            if (!fallback) return;
            if (!window.confirm('永久删除 "' + name + '" 的封禁？此操作无法撤销。')) return;
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.BansUnban, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            setBusy(submitBtn, false);
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '未知错误';
                showError(msg);
                toast('error', '解封失败', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnbanned);
            closeUnbanDialog();
            toast('success', '玩家已解封', ctx.name + ' 已解封。');
        });
    });
