...
<div class="audit-page">
    <header class="audit-page__header">
        <div>
            <h1 class="audit-page__title">監査ログ</h1>
            <p class="audit-page__subtitle">パネルによって記録されたすべての管理アクション（{$total_count} {if $total_count == 1}件{else}件{/if}）。</p>
        </div>
    </header>
    ...
    <div class="audit-filters">
        <form method="get" action="index.php" role="search" aria-label="監査ログを検索" style="flex:1 1 16rem;max-width:22rem">
            ...
            <input type="search"
                   ...
                   name="search"
                   value="{$search}"
                   placeholder="タイトルまたは詳細を検索…"
                   data-testid="audit-search"
                   aria-label="監査ログを検索">
        </form>

        <form method="get" action="index.php" class="flex items-center gap-2" style="flex-wrap:wrap" aria-label="重大度で絞り込み">
            <input type="hidden" name="p" value="admin">
            <input type="hidden" name="c" value="audit">
        ...
        {foreachelse}
            ...
            <div class="empty-state"
                ...
                <p class="empty-state__body">
                    {if $current_severity != '' || $search != ''}
                        別の検索語や重大度を試すか、フィルターを解除してすべての記録された管理アクションを表示してください。
                    {else}
                        管理者がパネルからモデレーションを開始すると、すべてのアクションがここに記録されます。
                    {/if}
                </p>
                {if $current_severity != '' || $search != ''}
    ...
    {if $page_count > 1 || $current_page > 1}
        <nav class="audit-pager" aria-label="監査ログのページネーション">
            <div class="audit-pager__status" aria-live="polite">
                {$page_count} ページ中 {$current_page} ページ目
            </div>
