...
<div class="p-6 space-y-6" style="max-width:64rem;margin:0 auto;width:100%">
    ...
    <header data-testid="account-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Ihr Konto</h1>
        <p class="text-sm text-muted m-0 mt-2">Berechtigungen, Passwort, Serverpasswort und E-Mail.</p>
    </header>
    ...
    *}
    <section class="card" data-testid="account-permissions">
        <div class="card__header">
            <div>
                <h3>Ihre Berechtigungen</h3>
                <p>Die Ihrem Administratorkonto derzeit gewährten Flags.</p>
            </div>
        </div>
                ...
                {else}
                    <p class="permissions-empty"
                       data-testid="account-permissions-web-empty">
                        <em>Keine Web-Berechtigungen erteilt.</em>
                    </p>
                {/if}
            </div>
                ...
                {if $server_permissions}
                    <div class="permissions-grid permissions-grid--server">
                        <section class="permissions-group"
                            ...
                            <h5 class="permissions-group__title">Gameserver-Flags</h5>
                            <ul class="permissions-group__list">
                ...
                {else}
                    <p class="permissions-empty"
                       data-testid="account-permissions-server-empty">
                        <em>Keine SourceMod-Flags erteilt.</em>
                    </p>
                {/if}
            </div>
        </div>
    ...
    <section class="card" data-testid="account-change-password">
        <div class="card__header">
            <div>
                ...
                <p>Verwenden Sie mindestens {$min_pass_len} Zeichen{if $min_pass_len != 1}{/if}. Sie werden nach einer erfolgreichen Änderung abgemeldet.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-password-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-current-password">Aktuelles Passwort</label>
                ...
                <div>
                    <label class="label" for="account-new-password">Neues Passwort</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-password">Neues Passwort bestätigen</label>
                    <input class="input" type="password"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-cancel">Abbrechen</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-save">Speichern</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-server-password">
        <div class="card__header">
            <div>
                <h3>Serverpasswort</h3>
                <p>Sie benötigen dieses auf dem Gameserver, um Ihre Admin-Rechte zu nutzen.
                    <a href="https://wiki.alliedmods.net/Adding_Admins_%28SourceMod%29#Passwords"
                       target="_blank" rel="noopener"
                       style="color:var(--accent);text-decoration:underline">SourceMod-Dokumentation</a>.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-srv-password-form"
                ...
                {if $srvpwset}
                    <div>
                        <label class="label" for="account-current-srv-password">Aktuelles Serverpasswort</label>
                        <input class="input" type="password"
                ...
                {/if}
                <div>
                    <label class="label" for="account-new-srv-password">Neues Serverpasswort</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-srv-password">Neues Serverpasswort bestätigen</label>
                    <input class="input" type="password"
                ...
                {if $srvpwset}
                    <label class="flex items-center gap-2 text-sm">
                        ...
                        <span>Das bestehende Serverpasswort entfernen (leert das Feld).</span>
                    </label>
                {/if}
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-srv-cancel">Abbrechen</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-srv-save">Speichern</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-change-email">
        <div class="card__header">
            <div>
                ...
                <p>Aktuelle Adresse:
                    ...
                    {else}
                        <span class="text-sm text-muted" data-testid="account-current-email-empty">(nicht festgelegt)</span>
                    {/if}
                </p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-email-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-email-password">Passwort</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-email">Neue E-Mail-Adresse</label>
                ...
                <div>
                    <label class="label" for="account-email-confirm">Neue E-Mail-Adresse bestätigen</label>
                    <input class="input" type="email"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-email-cancel">Abbrechen</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-email-save">Speichern</button>
                </div>
            </form>
        </div>
    </section>
...
(function () {
    ...
    function showFieldError(prefix, err, fieldMap) {
        ...
        setMsg(prefix + domId + '-msg', err.message || 'Ungültiger Wert.');
        return true;
    }
    ...
    function flashSuccess(envelope) {
        ...
        showToast('success', m.title || 'Gespeichert', m.body || '');
        if (m.redir) {
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (next.length < minLen) {
                setMsg('account-new-password-msg', 'Das Passwort muss mindestens ' + minLen + ' Zeichen lang sein.');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-password-msg', 'Die Passwörter stimmen nicht überein.');
                bad = true;
            }
            if (current.length === 0) {
                setMsg('account-current-password-msg', 'Geben Sie Ihr aktuelles Passwort ein.');
                bad = true;
            }
            if (bad) return;
    ...
    if (srvForm) {
        srvForm.addEventListener('submit', function (ev) {
            ...
            var bad = false;
            if (next.length < minLen) {
                setMsg('account-new-srv-password-msg', 'Das Passwort muss mindestens ' + minLen + ' Zeichen lang sein.');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-srv-password-msg', 'Die Passwörter stimmen nicht überein.');
                bad = true;
            }
            if (hasExisting && current.length === 0) {
                setMsg('account-current-srv-password-msg', 'Geben Sie Ihr aktuelles Serverpasswort ein.');
                bad = true;
            }
            if (bad) return;
            ...
            if (hasExisting) {
                sb.api.call(Actions.AccountCheckSrvPassword, {
                    ...
                    if (!env.data.matches) {
                        setBusy(srvBtn, false);
                        setMsg('account-current-srv-password-msg', 'Falsches Serverpasswort.');
                        return;
                    }
                    send();
                });
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', function (ev) {
            ...
            if (email.length === 0) {
                setMsg('account-email-msg', 'Geben Sie Ihre neue E-Mail-Adresse ein.');
                bad = true;
            }
            if (confirm.length === 0) {
                setMsg('account-email-confirm-msg', 'Bestätigen Sie Ihre neue E-Mail-Adresse.');
                bad = true;
            }
            if (!bad && email !== confirm) {
                setMsg('account-email-confirm-msg', 'Die E-Mail-Adressen stimmen nicht überein.');
                bad = true;
            }
            if (pw.length === 0) {
                setMsg('account-email-password-msg', 'Geben Sie Ihr Passwort ein.');
                bad = true;
            }
            if (bad) return;
