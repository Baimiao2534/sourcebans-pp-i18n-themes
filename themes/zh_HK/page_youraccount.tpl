...
<div class="p-6 space-y-6" style="max-width:64rem;margin:0 auto;width:100%">
    ...
    <header data-testid="account-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">您的帳號</h1>
        <p class="text-sm text-muted m-0 mt-2">權限、密碼、伺服器密碼與電子郵件。</p>
    </header>
    ...
    *}
    <section class="card" data-testid="account-permissions">
        ...
        <div class="card__body permissions-card__body">
            <div class="permissions-side permissions-side--web"
                ...
                <h4 class="permissions-side__heading">網頁</h4>
                ...
                {if $server_permissions}
                    <div class="permissions-grid permissions-grid--server">
                        <section class="permissions-group"
                            ...
                            <h5 class="permissions-group__title">遊戲伺服器旗標</h5>
                            <ul class="permissions-group__list">
    ...
    <section class="card" data-testid="account-change-password">
        <div class="card__header">
            <div>
                <h3>變更密碼</h3>
                <p>至少需使用 {$min_pass_len} 個字元。成功變更後您將被登出。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-password-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-current-password">目前密碼</label>
                ...
                <div>
                    <label class="label" for="account-new-password">新密碼</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-password">確認新密碼</label>
                    <input class="input" type="password"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-cancel">取消</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-save">儲存</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-server-password">
        <div class="card__header">
            <div>
                <h3>伺服器密碼</h3>
                <p>您需要在遊戲伺服器中使用此密碼來行使管理員權限。
                    <a href="https://wiki.alliedmods.net/Adding_Admins_%28SourceMod%29#Passwords"
                       target="_blank" rel="noopener"
                       style="color:var(--accent);text-decoration:underline">SourceMod 文件</a>。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-srv-password-form"
                ...
                {if $srvpwset}
                    <div>
                        <label class="label" for="account-current-srv-password">目前的伺服器密碼</label>
                        <input class="input" type="password"
                ...
                {/if}
                <div>
                    <label class="label" for="account-new-srv-password">新的伺服器密碼</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-srv-password">確認新的伺服器密碼</label>
                    <input class="input" type="password"
                ...
                {if $srvpwset}
                    <label class="flex items-center gap-2 text-sm">
                        ...
                        <span>移除現有的伺服器密碼（清空該欄位）。</span>
                    </label>
                {/if}
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-srv-cancel">取消</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-srv-save">儲存</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-change-email">
        ...
        <div class="card__body">
            <form id="account-email-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-email-password">密碼</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-email">新電子郵件地址</label>
                ...
                <div>
                    <label class="label" for="account-email-confirm">確認新電子郵件地址</label>
                    <input class="input" type="email"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-email-cancel">取消</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-email-save">儲存</button>
                </div>
            </form>
        </div>
    </section>
...
(function () {
    ...
    function showFieldError(prefix, err, fieldMap) {
        ...
        setMsg(prefix + domId + '-msg', err.message || '數值無效。');
        return true;
    }
    ...
    function flashSuccess(envelope) {
        ...
        showToast('success', m.title || '已儲存', m.body || '');
        if (m.redir) {
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (next.length < minLen) {
                setMsg('account-new-password-msg', '密碼至少需要 ' + minLen + ' 個字元。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-password-msg', '兩次輸入的密碼不一致。');
                bad = true;
            }
            if (current.length === 0) {
                setMsg('account-current-password-msg', '請輸入目前的密碼。');
                bad = true;
            }
            if (bad) return;
    ...
    if (srvForm) {
        srvForm.addEventListener('submit', function (ev) {
            ...
            var bad = false;
            if (next.length < minLen) {
                setMsg('account-new-srv-password-msg', '密碼至少需要 ' + minLen + ' 個字元。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-srv-password-msg', '兩次輸入的密碼不一致。');
                bad = true;
            }
            if (hasExisting && current.length === 0) {
                setMsg('account-current-srv-password-msg', '請輸入目前的伺服器密碼。');
                bad = true;
            }
            if (bad) return;
            ...
            if (hasExisting) {
                sb.api.call(Actions.AccountCheckSrvPassword, {
                    ...
                    if (!env.data.matches) {
                        setBusy(srvBtn, false);
                        setMsg('account-current-srv-password-msg', '伺服器密碼不正確。');
                        return;
                    }
                    send();
                });
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', function (ev) {
            ...
            if (email.length === 0) {
                setMsg('account-email-msg', '請輸入新的電子郵件地址。');
                bad = true;
            }
            if (confirm.length === 0) {
                setMsg('account-email-confirm-msg', '請確認新的電子郵件地址。');
                bad = true;
            }
            if (!bad && email !== confirm) {
                setMsg('account-email-confirm-msg', '兩次輸入的電子郵件地址不一致。');
                bad = true;
            }
            if (pw.length === 0) {
                setMsg('account-email-password-msg', '請輸入您的密碼。');
                bad = true;
            }
            if (bad) return;
