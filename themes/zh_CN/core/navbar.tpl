            ...
            {foreach from=$navbar item=nav}
                {if $nav.endpoint != 'admin'}
                    <a class="sidebar__link"
                       ...
                       title="{if $nav.endpoint == 'home'}此页面显示您的封禁和服务器的概览。{elseif $nav.endpoint == 'servers'}可以在此处查看所有服务器及其状态。{elseif $nav.endpoint == 'banlist'}可以在此处查看数据库中的所有封禁记录。{elseif $nav.endpoint == 'commslist'}可以在此处查看数据库中的所有通信封禁（如文字禁言和语音静音）。{elseif $nav.endpoint == 'submit'}您可以在此处提交疑似作弊者的演示文件或截图。提交后将由管理员审核。{elseif $nav.endpoint == 'protest'}您可以在此处申诉您的封禁，并说明您应该被解封的理由。{else}{$nav.description}{/if}"
                       {if $nav.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $nav.endpoint == 'home'}仪表盘{elseif $nav.endpoint == 'servers'}服务器{elseif $nav.endpoint == 'banlist'}封禁列表{elseif $nav.endpoint == 'commslist'}通信管理{elseif $nav.endpoint == 'submit'}举报玩家{elseif $nav.endpoint == 'protest'}申诉封禁{else}{$nav.title}{/if}
                    </a>
                {/if}
            {/foreach}
        </div>
                ...
                {foreach from=$navbar item=nav}
                    {if $nav.endpoint == 'admin'}
                        <a class="sidebar__link"
                           href="index.php?p={$nav.endpoint}"
                           data-testid="nav-{$nav.endpoint}"
                           title="这是 SourceBans 的控制面板，您可以在此设置新管理员、添加新服务器等。"
                           {if $nav.state == 'active'}aria-current="page"{/if}>
                ...
                {foreach from=$adminbar item=admin}
                    <a class="sidebar__link"
                       ...
                       {if $admin.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $admin.endpoint == 'admins'}管理员{elseif $admin.endpoint == 'servers'}服务器{elseif $admin.endpoint == 'bans'}封禁{elseif $admin.endpoint == 'comms'}通信{elseif $admin.endpoint == 'groups'}分组{elseif $admin.endpoint == 'settings'}设置{elseif $admin.endpoint == 'mods'}模组{elseif $admin.endpoint == 'export'}导出{else}{$admin.title}{/if}
                    </a>
                {/foreach}
            </div>
        {/if}
