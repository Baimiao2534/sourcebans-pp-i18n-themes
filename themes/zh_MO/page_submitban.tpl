...
<section class="p-6" style="max-width:48rem">
    <header class="mb-6">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">提交封禁申請</h1>
        <p class="text-sm text-muted m-0 mt-2">
            舉報玩家作弊、騷擾或其他違規行為。示範檔和截圖有助於管理員更快處理。
        </p>
    </header>
    ...
    <form class="card"
        ...
        <div class="card__body space-y-4">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    請提供 Steam ID 或 IP 地址（或兩者）。暱稱與清楚的事件描述為必填項目。
                </p>
            </div>
            ...
            *}
            <div data-testid="submitban-id-or-ip"
                ...
                <p id="submitban-id-or-ip-help"
                   ...
                   data-state="info">
                    ...
                    <span class="text-muted" data-id-or-ip-text>請提供 Steam ID 或 IP 地址（或兩者）。</span>
                </p>
                <div>
            ...
            <div>
                <label class="label" for="submitban-name">
                    玩家暱稱 <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <input type="text"
                       class="input"
                       id="submitban-name"
            ...
            <div>
                <label class="label" for="submitban-reason">
                    說明 <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                ...
                <p id="submitban-reason-help" class="text-xs text-muted m-0 mt-2">
                    請具體說明。僅寫「作弊」是不夠的。請描述您看到的情況、時間以及發生在哪個伺服器。
                </p>
            </div>
        </div>
        ...
        <div class="card__body space-y-4" style="border-top:1px solid var(--border)">
            ...
            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                ...
                <div>
                    <label class="label" for="submitban-reporter-email">
                        您的電子郵件 <span style="color:var(--danger)" aria-hidden="true">*</span>
                    </label>
                    <input type="email"
                           class="input"
            ...
            <div>
                <label class="label" for="submitban-server">
                    伺服器 <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <select class="select"
                        id="submitban-server"
                        name="server"
                    ...
                    <option value="-1">&mdash; 選擇伺服器 &mdash;</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"{if $server_selected == $server.sid} selected{/if}>{$server.hostname}</option>
                    {/foreach}
                    <option value="0"{if $server_selected == 0} selected{/if}>其他伺服器 / 未列於此處</option>
                </select>
            </div>

            <div>
                <label class="label" for="submitban-demo">上傳示範檔或證據</label>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    選填。允許的格式：<code class="font-mono">.dem</code>、
        ...
        <div class="card__body flex items-center justify-between gap-2"
            ...
            <p class="text-xs text-muted m-0">
                <span style="color:var(--danger)" aria-hidden="true">*</span> 必填欄位
            </p>
...
(function () {
    ...
    function setError(isError) {
        ...
        if (isError) {
            ...
            helpText.textContent = '請在提交前輸入 Steam ID 或 IP 地址。';
        } else {
