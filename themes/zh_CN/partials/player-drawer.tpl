...
<header class="drawer__header" style="display:flex;justify-content:space-between;align-items:center;padding:1rem 1.25rem;border-bottom:1px solid var(--border)">
    ...
    <button class="btn btn--ghost btn--icon" type="button" data-drawer-close aria-label="关闭">
        <i data-lucide="x"></i>
    </button>
</header>
...
{if $detail.comments_visible}
    <section style="padding:0 1.25rem 1.25rem">
        <h3 class="text-xs text-faint" style="text-transform:uppercase;letter-spacing:0.06em;margin:0 0 0.5rem">备注</h3>
        {if $detail.comments}
                ...
                {foreach $detail.comments as $c}
                    <li style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.625rem 0.75rem;background:var(--bg-surface)">
                        <div style="display:flex;justify-content:space-between;font-size:0.75rem;color:var(--text-muted);margin-bottom:0.25rem">
                            <span class="font-medium">{if $c.author_hidden}已隐藏{else}{$c.author|default:'未知'}{/if}</span>
        ...
        {else}
            <p class="text-sm text-muted" style="margin:0">暂无备注。</p>
        {/if}
    </section>
{/if}
