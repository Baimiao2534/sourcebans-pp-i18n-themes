...
{if NOT $permission_protests}
    <div class="card" data-testid="protests-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">您没有查看封禁申诉的权限。</p>
        </div>
    </div>
{else}
        ...
        {if $protest_list|@count == 0}
            ...
            <div class="card" data-testid="protests-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">没有待处理的申诉</h2>
                    <p class="empty-state__body">没有等待审查的内容。新的封禁申诉会在玩家提交时显示在此处。</p>
                </div>
            </div>
        {else}
                    ...
                    <details class="queue-row ban-row ban-row--active"
                            ...
                            <div class="row-actions">
                                {if $permission_editban}
                                    <button type="button"
                                            ...
                                            data-archiv="1">
                                        <span data-confirm-label>移除</span>
                                    </button>
                                {/if}
                        ...
                        <div class="p-4" id="ban_details_{$protest.pid}" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    ...
                                    <dd class="font-mono" style="margin:0">
                                        {if $protest.authid == ""}<span class="text-faint">无 SteamID</span>{else}{$protest.authid|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">IP 地址</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $protest.ip == 'none' OR $protest.ip == ''}<span class="text-faint">无 IP 地址</span>{else}{$protest.ip|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">执行时间</dt>
                                    <dd style="margin:0">{$protest.date|escape}</dd>

                                    <dt class="text-muted">到期时间</dt>
                                    <dd style="margin:0">
                                        {if $protest.ends == 'never'}<span class="text-faint">不适用。</span>{else}{$protest.ends|escape}{/if}
                                    </dd>
                            ...
                            {if $protest.commentdata != "None"}
                                <div class="mt-4">
                                    <h4 class="text-xs text-muted" style="font-weight:600;margin:0 0 0.5rem;text-transform:uppercase;letter-spacing:0.06em">评论</h4>
                                    <div class="space-y-3">
                                            ...
                                            <div class="card p-4">
                                                <div class="flex items-center justify-between gap-2 text-xs text-muted mb-2">
                                                    <strong style="color:var(--text);font-weight:600">
                                                        {if !empty($commenta.comname)}{$commenta.comname|escape}{else}<i class="text-faint">管理员已删除</i>{/if}
                                                    </strong>
                                                ...
                                                {if !empty($commenta.edittime)}
                                                    <div class="text-xs text-faint mt-2">
                                                        最后编辑 {$commenta.edittime|escape} 由
                                                        {if !empty($commenta.editname)}{$commenta.editname|escape}{else}<i>管理员已删除</i>{/if}
                                                    </div>
                                                {/if}
                                            </div>
                                        {/foreach}
...
(function () {
    ...
    function arm(btn) {
        ...
        labelEl.textContent = '点击确认';
        btn.setAttribute('data-pending', 'true');
        btn.setAttribute('aria-label', '在 3 秒内再次点击以确认移除');
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.ProtestsRemove, { pid: pid, archiv: '1' }).then(function (r) {
            if (!r || r.ok === false) {
                setBusy(btn, false);
                toast('error', '操作失败', (r && r.error && r.error.message) || '未知错误');
                return;
            }
            ...
            toast('success', '申诉已归档', '如需要，可从已归档的申诉页面恢复。');
        });
    });
})();
</script>
