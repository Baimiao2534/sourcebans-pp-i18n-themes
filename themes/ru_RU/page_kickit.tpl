...
<head>
    ...
    <title>-{if $mode === 'kick'}-Кикнуть игрока-{else}-Забанить игрока-{/if}-</title>
    <script>
    (function () {
        try {
...
<div id="container" class="card" data-testid="kickit-container" data-mode="-{$mode}-">
    <div class="card__header">
        <div>
            <h3>Поиск игрока на всех серверах&hellip;</h3>
            <p>Каждая строка опрашивается в реальном времени; строки обновляются по мере ответа серверов.</p>
        </div>
    </div>
                ...
                <tr data-testid="kickit-row--{$serv.num}-">
                    ...
                    <td>
                        <div id="srv_-{$serv.num}-"
                             ...
                             data-testid="kickit-status--{$serv.num}-">Ожидание&hellip;</div>
                    </td>
                </tr>
                -{/foreach}-
            </tbody>
    ...
    (function () {
        ...
        function setCounter(count) {
            ...
            if (srvcount === TOTAL || count === -1) {
                ...
                if (ctl) {
                    ctl.innerHTML = '<font color="green" style="font-size: 12px;"><b>Поиск завершён.</b></font>' + ctl.innerHTML;
        ...
        function processRow(sid, num) {
            sb.api.call(Actions.KickitKickPlayer, { check: CHECK, sid: sid, num: num, type: TYPE, mode: MODE })
                .then(function (r) {
                    if (!r || !r.ok || !r.data) {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>Ошибка.</i></span>");
                        setCounter(1);
                        return;
                    }
                    var d = r.data;
                    ...
                    if (d.status === 'no_connect') {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>Не удалось подключиться к серверу.</i></span>");
                        setCounter(1);
                    } else if (d.status === 'kicked') {
                        sb.setHTML('srv_' + num, "<span class='text-xs font-semibold' style='color:var(--success)'><u>Игрок найден и кикнут!</u></span>");
                        setCounter(-1);
                    } else {
                        sb.setHTML('srv_' + num, "<span class='text-xs text-muted'>Игрок не найден.</span>");
                        setCounter(1);
                    }
                });
        }
        ...
        window.addEventListener('load', function () {
            ...
            sb.api.call(Actions.KickitLoadServers, {}).then(function (r) {
                ...
                r.data.servers.forEach(function (s) {
                    if (s.has_rcon) {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-muted">Поиск&hellip;</span>');
                        processRow(s.sid, s.num);
                    } else {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-faint">Нет пароля RCON.</span>');
                        setCounter(1);
                    }
                });
            });
