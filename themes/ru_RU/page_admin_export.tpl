...
<section class="p-6" data-testid="admin-export-section" style="max-width:56rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">Полный экспорт данных</h1>
        <p class="text-sm text-muted m-0 mt-2">
            Скачайте ZIP-архив со всеми строками панели и загруженными демо.
            Используйте для резервного копирования, миграции или аналитики.
            См. <a href="https://sbpp.github.io/configuring/data-export/" target="_blank" rel="noopener noreferrer">документацию по экспорту данных</a>
            для формата и настройки каждого режима.
        </p>
        <p class="text-xs text-muted m-0 mt-2" data-testid="admin-export-rookhelm">
            Переезжаете на хостинг?
            <a href="https://rookhelm.com?utm_source=sourcebans-pp&amp;utm_medium=referral&amp;utm_content=panel-export"
               target="_blank"
               rel="noopener">RookHelm</a>
            напрямую импортирует этот экспорт.
        </p>
    </div>

    <div class="card mb-4" data-testid="admin-export-summary-card">
        <div class="card__header">
            <h2 style="font-size:1.125rem;font-weight:600;margin:0">Содержимое архива</h2>
            <p class="text-xs text-muted m-0 mt-1">
                Каждый экспорт получает новый ID архива. Счётчики ниже — снимок на момент загрузки страницы.
            </p>
        </div>
        <div class="card__body">
            <dl class="install-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(14rem,1fr));gap:0.75rem 1.5rem;margin:0">
                ...
                <div>
                    <dt class="text-xs text-muted">Лимит (S3 PUT)</dt>
            ...
            {if $row_counts|@count > 0}
                <details class="mt-4">
                    <summary class="text-sm" style="cursor:pointer">Количество строк по сущностям</summary>
    ...
    {if $exceeds_cap}
        ...
        <div class="card" data-testid="admin-export-cap-empty" data-filtered="false">
            <div class="empty-state">
                ...
                <h2 class="empty-state__title">Архив превышает лимит S3 PUT в 5 ГиБ</h2>
                <p class="empty-state__body">
                    Оценочный размер архива — {($estimated_bundle_bytes / 1024 / 1024)|number_format:1} MiB.
                    Используйте форму скачивания ZIP выше (без лимита),
                    либо удалите лишние демо / строки и перезагрузите страницу, чтобы включить форму S3.
                </p>
            </div>
        </div>
    {/if}
    ...
    <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(22rem,1fr));gap:1rem;margin-top:1rem">
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">Скачать ZIP потоком</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    Браузер скачивает архив напрямую.
                    Не закрывайте вкладку до завершения; закрытие прервёт передачу.
                </p>
                <p class="text-xs text-muted m-0">
                    Без промежуточного файла. Индикатор прогресса обновляется в реальном времени.
                </p>
            </div>
        ...
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">Загрузить по presigned URL S3</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    Панель размещает архив на диске, затем отправляет PUT по вашему URL.
                    Закрытие вкладки отменит загрузку; журнал аудита зафиксирует попытку.
                </p>
                ...
                <textarea class="textarea"
                          ...
                          required
                          pattern="^https://[^\s]+$"
                          title="Вставьте presigned HTTPS PUT URL из вашего S3-клиента (например, aws s3 presign --http-method PUT)"
                          placeholder="https://bucket.s3.region.amazonaws.com/path/sbpp-export.zip?X-Amz-..."
                          data-testid="admin-export-s3-url"></textarea>
                <p class="text-xs text-muted m-0 mt-1">
                    Только HTTPS. Используйте короткий срок действия (1 час или меньше).
                    URL является одноразовым учётным данным для записи; не вставляйте его в чат или логи.
                </p>
            </div>
            <div class="card__header" style="border-top:1px solid var(--border);border-bottom:0;justify-content:flex-end">
                <button type="submit"
