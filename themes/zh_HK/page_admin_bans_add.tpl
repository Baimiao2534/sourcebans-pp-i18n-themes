...
{if NOT $permission_addban}
    <div class="card" data-testid="addban-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">您沒有新增封禁的權限。</p>
        </div>
    </div>
{else}
    <section class="p-6" data-testid="addban-section" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">新增封禁</h1>
            <p class="text-sm text-muted m-0 mt-2">
                透過 SteamID 或 IP 位址進行封禁。將游標停留在標籤上以查看內嵌說明。
            </p>
        </div>
        <form id="addban-form"
            ...
            <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(14rem,1fr))">
                <div>
                    <label class="label" for="type">封禁類型</label>
                    ...
                    <select class="select"
                        ...
                        <option value="1"{if $prefill_type == 1} selected{/if}>IP 位址</option>
                    </select>
                </div>
                <div>
                    <label class="label" for="banlength">封禁時長</label>
                    <select class="select"
                            id="banlength"
            ...
            <div>
                <label class="label" for="ip">IP 位址</label>
...
(function () {
    ...
    function validate(type) {
        ...
        if (!nick || !nick.value) { setMsg('nick.msg', '您必須輸入要封禁的玩家暱稱'); err++; }
        else { setMsg('nick.msg', ''); }

        var steam = $id('steam');
        if (type === 0) {
            if (!steam || !/(?:STEAM_[01]:[01]:\d+)|(?:\[U:1:\d+\])|(?:\d{17})/.test(steam.value)) {
                setMsg('steam.msg', '您必須輸入有效的 STEAM ID 或 Community ID'); err++;
        ...
        if (type === 1) {
            if (!ip || ip.value.length < 7) {
                setMsg('ip.msg', '您必須輸入有效的 IP 位址'); err++;
            } else { setMsg('ip.msg', ''); }
        } else { setMsg('ip.msg', ''); }

        if (!reason) {
            setMsg('reason.msg', '您必須選擇或輸入此封禁的原因。'); err++;
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.BansAdd, {
            ...
            setBusy(btn, false);
            if (!r || r.ok === false) {
                toast('error', '新增封禁失敗', (r && r.error && r.error.message) || '未知錯誤');
                return;
            }
            ...
            toast('success', (msg && msg.title) || '封禁已新增', (msg && msg.body) || '封禁已成功新增');
            reset();
        });
    });
})();
