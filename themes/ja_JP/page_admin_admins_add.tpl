    ...
    {if !$can_add_admins}
        <div class="card">
            <div class="card__body">
                <p class="text-sm text-muted m-0">アクセスが拒否されました。</p>
            </div>
        </div>
    {else}
        <div class="mb-4">
            <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">新しい管理者を追加</h1>
            <p class="text-sm text-muted m-0 mt-2">ヘルプアイコンにマウスを合わせると、フィールドごとのガイダンスが表示されます。</p>
        </div>
        ...
        <div id="msg-green" class="card" style="display:none;border-left:3px solid var(--success)">
            <div class="card__body">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-semibold">管理者を追加しました</div>
                        <div class="text-sm text-muted">新しい管理者が正常に追加されました。リダイレクト中&hellip;</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form id="add-admin-form" method="post" action="" class="space-y-4" autocomplete="off">
            ...
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>識別情報</h3>
                        <p>ログイン名と Steam ID は、この管理者をパネルおよびゲームサーバーで識別するために使用されます。</p>
                    </div>
                </div>
                <div class="card__body space-y-3">
                    ...
                    <div>
                        <label class="label" for="email">メールアドレス <span class="text-faint" style="font-weight:400">(Web アクセスに必須)</span></label>
            ...
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>認証</h3>
                        <p>Web パネルのパスワードと、ゲーム内管理者パスワード（任意）。</p>
                    </div>
                </div>
                <div class="card__body space-y-3">
                    <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                        <div>
                            <label class="label" for="password">パスワード <span class="text-faint" style="font-weight:400">(Web アクセスに必須)</span></label>
                    ...
                    <div class="flex items-center gap-2 mt-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                        ...
                        <label for="a_useserverpass" class="text-sm font-medium" style="margin:0">ゲーム内管理者パスワードを設定</label>
            ...
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>サーバーアクセス</h3>
                        <p>管理者がゲーム内で管理できるサーバーおよびサーバーグループ。</p>
                    </div>
                </div>
                <div class="card__body space-y-3">
                    {if !$group_list && !$server_list}
                        <p class="text-sm text-muted m-0"><em>サーバーまたはサーバーグループがまだ追加されていません。</em></p>
                    {else}
                        {if $group_list}
                            <div class="text-xs text-faint" style="text-transform:uppercase;letter-spacing:0.06em;font-weight:600">サーバーグループ</div>
                        ...
                        {/if}
                        {if $server_list}
                            <div class="text-xs text-faint" style="text-transform:uppercase;letter-spacing:0.06em;font-weight:600;margin-top:0.5rem">個別サーバー</div>
                            {*
            ...
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>権限</h3>
                        <p>プリセットグループ、カスタムフラグ、または権限なし。新規グループを選択するとインラインエディタが開きます。</p>
                    </div>
                </div>
                <div class="card__body space-y-3">
                    <div>
                        <label class="label" for="serverg">サーバー管理者グループ</label>
                        ...
                        <select class="select" id="serverg" name="serverg" tabindex="8"
                            ...
                            <option value="-2">選択してください&hellip;</option>
                            <option value="-3">権限なし</option>
                            <option value="c">カスタム権限</option>
                            <option value="n">新規管理者グループ</option>
                        ...
                        <div id="serverperm" style="overflow:hidden">
                            <div id="server-new-name-block" hidden style="margin-top:0.75rem">
                                <label class="label" for="server-new-name">新規サーバー管理者グループ名</label>
                            ...
                            <div id="server-flags-block" hidden style="margin-top:0.75rem">
                                <label class="label" for="server-flags">SourceMod フラグ &amp; 免疫</label>
                                ...
                                <p class="text-xs text-muted m-0 mt-2">
                                    SourceMod フラグ文字列。免疫を設定するには <code>#&lt;免疫値&gt;</code> を追加します（デフォルトは 0）。
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <label class="label" for="webg">Web 管理者グループ</label>
                        ...
                        <div id="webperm" style="overflow:hidden">
                            <div id="web-new-name-block" hidden style="margin-top:0.75rem">
                                <label class="label" for="web-new-name">新規 Web 管理者グループ名</label>
                            ...
                            <div id="web-flags-block" hidden style="margin-top:0.75rem">
                                <fieldset style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem">
                                    <legend class="text-xs text-faint" style="padding:0 0.5rem;text-transform:uppercase;letter-spacing:0.06em;font-weight:600">Web 権限</legend>
                                    <p class="text-xs text-muted m-0 mb-2">この管理者（または新規グループ）が持つフラグを選択してください。所有者は他のすべての権限を付与します。</p>
                                    <div class="grid gap-2" style="grid-template-columns:repeat(auto-fill,minmax(16rem,1fr))">
                                        ...
                                        {if $can_grant_owner}
                                            {* #1402 adversarial review HIGH 1: OWNER is gated.
                                               ...
                                               affordance half. *}
                                            <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_OWNER" data-testid="admin-add-flag-owner"> <span class="text-sm">所有者</span></label>
                                        {/if}
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_LIST_ADMINS"> <span class="text-sm">管理者を表示</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_ADD_ADMINS"> <span class="text-sm">管理者を追加</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_ADMINS"> <span class="text-sm">管理者を編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_DELETE_ADMINS"> <span class="text-sm">管理者を削除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_LIST_SERVERS"> <span class="text-sm">サーバーを表示</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_ADD_SERVER"> <span class="text-sm">サーバーを追加</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_SERVERS"> <span class="text-sm">サーバーを編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_DELETE_SERVERS"> <span class="text-sm">サーバーを削除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_ADD_BAN"> <span class="text-sm">BAN を追加</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_OWN_BANS"> <span class="text-sm">自身の BAN を編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_GROUP_BANS"> <span class="text-sm">グループの BAN を編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_ALL_BANS"> <span class="text-sm">すべての BAN を編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_UNBAN"> <span class="text-sm">すべての BAN を解除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_UNBAN_OWN_BANS"> <span class="text-sm">自身の BAN を解除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_UNBAN_GROUP_BANS"> <span class="text-sm">グループの BAN を解除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_DELETE_BAN"> <span class="text-sm">すべての BAN を削除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_BAN_PROTESTS"> <span class="text-sm">BAN 申し立て</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_BAN_SUBMISSIONS"> <span class="text-sm">BAN 通報</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_BAN_IMPORT"> <span class="text-sm">BAN をインポート</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_LIST_GROUPS"> <span class="text-sm">グループを表示</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_ADD_GROUP"> <span class="text-sm">グループを追加</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_GROUPS"> <span class="text-sm">グループを編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_DELETE_GROUPS"> <span class="text-sm">グループを削除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_WEB_SETTINGS"> <span class="text-sm">Web 設定</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_LIST_MODS"> <span class="text-sm">MOD を表示</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_ADD_MODS"> <span class="text-sm">MOD を追加</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_EDIT_MODS"> <span class="text-sm">MOD を編集</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_DELETE_MODS"> <span class="text-sm">MOD を削除</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_NOTIFY_SUB"> <span class="text-sm">BAN 通報メール通知</span></label>
                                        <label class="flex items-center gap-2"><input type="checkbox" data-flag="ADMIN_NOTIFY_PROTEST"> <span class="text-sm">BAN 申し立てメール通知</span></label>
                                    </div>
                                </fieldset>
                            </div>
                        </div>
            ...
            <div class="flex justify-end gap-2">
                <button type="button" class="btn btn--ghost btn--sm"
                        onclick="history.go(-1);" data-testid="admin-add-back">戻る</button>
                <button type="submit" class="btn btn--primary btn--sm" id="aadmin"
                        data-testid="admin-add-submit"><i data-lucide="user-plus"></i> 管理者を追加</button>
            </div>
        </form>
        ...
        (function () {
            ...
            document.addEventListener('click', function (e) {
                ...
                a.call(A.AdminsGeneratePassword, {}).then(function (r) {
                    ...
                    setBusy(btn, false);
                    toast('error', 'パスワード生成に失敗しました', String(err && err.message ? err.message : err));
                });
            });
            ...
            document.addEventListener('submit', function (e) {
                ...
                a.call(A.AdminsAdd, {
                    ...
                    if (r.ok === false) {
                        ...
                        showMsg(slot, em);
                        toast('error', '管理者追加に失敗しました', em);
                        return;
                    }
                    var data = r.data || {};
                    var msg = data.message || {};
                    toast('success', msg.title || '管理者を追加しました', msg.body || '管理者が追加されました。');
                    ...
                    setBusy(submitBtn, false);
                    toast('error', '管理者追加に失敗しました', String(err && err.message ? err.message : err));
                });
            });
