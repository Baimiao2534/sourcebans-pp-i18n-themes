...
<section class="p-6" data-testid="admin-export-section" style="max-width:56rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">完全データエクスポート</h1>
        <p class="text-sm text-muted m-0 mt-2">
            パネルの全データとアップロード済みデモを含む ZIP アーカイブをダウンロードします。
            バックアップ、移行、分析に使用できます。
            転送形式および各モードの設定については
            <a href="https://sbpp.github.io/configuring/data-export/" target="_blank" rel="noopener noreferrer">データエクスポートドキュメント</a>
            を参照してください。
        </p>
        <p class="text-xs text-muted m-0 mt-2" data-testid="admin-export-rookhelm">
            ...
            はこのエクスポートを直接インポートできます。
        </p>
    </div>

    <div class="card mb-4" data-testid="admin-export-summary-card">
        <div class="card__header">
            <h2 style="font-size:1.125rem;font-weight:600;margin:0">アーカイブの内容</h2>
            <p class="text-xs text-muted m-0 mt-1">
                各エクスポートで新しいアーカイブ ID が生成されます。以下のカウントはページ読み込み時のスナップショットです。
            </p>
        </div>
        <div class="card__body">
            ...
            {if $row_counts|@count > 0}
                <details class="mt-4">
                    <summary class="text-sm" style="cursor:pointer">エンティティ別行数</summary>
    ...
    {if $exceeds_cap}
        ...
        <div class="card" data-testid="admin-export-cap-empty" data-filtered="false">
            <div class="empty-state">
                ...
                <h2 class="empty-state__title">アーカイブが 5 GiB の S3 PUT 制限を超過</h2>
                <p class="empty-state__body">
                    推定アーカイブサイズは {($estimated_bundle_bytes / 1024 / 1024)|number_format:1} MiB です。
                    上の ZIP ダウンロードフォーム（無制限）を使用するか、
                    デモ / 行数を整理してリロードし、S3 フォームを有効化してください。
                </p>
            </div>
        </div>
    {/if}
    ...
    <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(22rem,1fr));gap:1rem;margin-top:1rem">
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">ストリーミング ZIP ダウンロード</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    ブラウザが直接アーカイブをダウンロードします。
                    ダウンロード完了までタブを開いたままにしてください。タブを閉じると転送が中断されます。
                </p>
                <p class="text-xs text-muted m-0">
                    一時ファイルなし。プログレスバーがリアルタイムに更新されます。
                </p>
            </div>
        ...
        <form class="card"
            ...
            <div class="card__header">
                <h2 style="font-size:1.125rem;font-weight:600;margin:0">S3 プリサインド URL にアップロード</h2>
            </div>
            <div class="card__body space-y-2">
                <p class="text-sm text-muted m-0">
                    パネルはアーカイブをディスクに一時保存し、お使いの URL に PUT します。
                    タブを閉じるとアップロードがキャンセルされます。監査ログにこの試行が記録されます。
                </p>
                <label class="label mt-3" for="admin-export-s3-url">プリサインド PUT URL</label>
                <textarea class="textarea"
                          ...
                          required
                          pattern="^https://[^\s]+$"
                          title="S3 クライアントからのプリサインド HTTPS PUT URL を貼り付けてください（例：aws s3 presign --http-method PUT）"
                          placeholder="https://bucket.s3.region.amazonaws.com/path/sbpp-export.zip?X-Amz-..."
                          data-testid="admin-export-s3-url"></textarea>
                <p class="text-xs text-muted m-0 mt-1">
                    HTTPS のみ。短い有効期限（1時間以内）を使用してください。
                    この URL は一時的な書き込み認証情報です。チャットやログに貼り付けないでください。
                </p>
            </div>
            <div class="card__header" style="border-top:1px solid var(--border);border-bottom:0;justify-content:flex-end">
                <button type="submit"
