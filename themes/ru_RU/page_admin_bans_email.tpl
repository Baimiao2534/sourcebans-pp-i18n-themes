...
<section class="p-6" data-testid="banemail-section" style="max-width:48rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">Написать игроку</h1>
        <p class="text-sm text-muted m-0 mt-2">
            Отправляется на <span class="font-mono" data-testid="banemail-addr">{$email_addr|escape}</span>.
        </p>
    </div>
    <form class="card p-6 space-y-4"
        ...
        <div>
            ...
            <textarea class="textarea"
                      ...
                      name="message"
                      rows="8"
                      placeholder="Что вы хотите сообщить игроку?"
        ...
        <div class="flex justify-end gap-2"
            ...
            <button type="button"
                    ...
                    data-testid="banemail-back">Назад</button>
...
(function () {
    ...
    window.CheckEmail = function (type, id) {
        ...
        if (!subject || !subject.value) { setMsg('subject.msg', 'Необходимо указать тему письма.'); err++; }
        else { setMsg('subject.msg', ''); }
        if (!message || !message.value) { setMsg('message.msg', 'Необходимо ввести текст письма.'); err++; }
        ...
        a.call(A.SystemSendMail, {
            ...
            if (!r || r.ok === false) {
                toast('error', 'Ошибка отправки', (r && r.error && r.error.message) || 'Неизвестная ошибка');
                return;
            }
            ...
            toast('success', (msg && msg.title) || 'Письмо отправлено', (msg && msg.body) || 'Письмо успешно отправлено.');
            if (msg && msg.redir) {
