...
<div class="card-tab page-section" id="Edit Admin Server Access">
    <div class="mb-4">
        <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">管理者サーバーアクセスを編集</h1>
        <p class="text-sm text-muted m-0 mt-2">この管理者がゲーム内で管理できるサーバーおよびサーバーグループを選択します。</p>
    </div>
    ...
    {if $row_count < 1}
        <div class="card">
            <div class="card__body">
                <p class="text-sm text-muted m-0"><em>管理者のサーバー権限を設定するには、先にサーバーまたはサーバーグループを追加する必要があります。</em></p>
            </div>
        </div>
    {else}
        <form method="post" action="" class="space-y-4">
            ...
            {if $group_list}
                <div class="card">
                    <div class="card__header">
                        <div>
                            <h3>サーバーグループ</h3>
                            <p>グループを付与すると、そのグループ下のすべてのサーバーがカバーされます。</p>
                        </div>
                    </div>
                    <div class="card__body">
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="btn btn--primary btn--sm" id="editadminserver"
                        data-testid="edit-admin-servers-save"><i data-lucide="save"></i> 変更を保存</button>
            </div>
        </form>
    {/if}
</div>
