    ...
    {if NOT $permission_addserver}
        <div class="card" data-testid="server-add-denied">
            <div class="card__body">
                <h3 style="margin:0 0 0.25rem">アクセス拒否</h3>
                <p class="text-sm text-muted m-0">サーバーを追加する権限がありません。</p>
            </div>
        </div>
    {else}
        <form id="addserver-form"
            ...
            <div class="card__header">
                <div>
                    <h3>{if $edit_server}サーバーを編集{else}サーバーを追加{/if}</h3>
                    <p>ヒント：クエスチョンマークにホバーするとインラインヘルプが表示されます。RCON は任意ですが、ライブコンソールとサーバーごとの管理者マッピングが有効になります。</p>
                </div>
            </div>
            <div class="card__body" style="display:grid;gap:1rem">
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">サーバー IP / ドメイン</span>
                        <input type="text"
                               id="address"
                               name="address"
                        ...
                        <span class="text-xs text-muted">IPv4 / IPv6 / ホスト名。ホスト名はゲームサーバーが接続時に解決します。</span>
                    </label>
                    <label class="block">
                        <span class="label">サーバーポート</span>
                        <input type="number"
                               id="port"
                               name="port"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">デフォルト <code class="font-mono">27015</code>。</span>
                    </label>
                </div>

                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">RCON パスワード</span>
                        <input type="password"
                               id="rcon"
                               name="rcon"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted"><code class="font-mono">server.cfg</code> の <code class="font-mono">rcon_password</code> の隣にあります。</span>
                    </label>
                    <label class="block">
                        <span class="label">RCON を確認</span>
                        <input type="password"
                               id="rcon2"
                               name="rcon2"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">入力ミスを防ぐため再入力してください。</span>
                    </label>
                </div>
                ...
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        ...
                        <select id="mod"
                            ...
                            {if !$edit_server}<option value="-2">MOD を選択…</option>{/if}
                    ...
                    <label class="flex items-center gap-2" style="align-self:end;padding:0.5rem 0">
                        ...
                        <span class="text-sm">公開サーバーリストで有効にする</span>
                    </label>
                </div>

                <fieldset style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem 1rem">
                    <legend class="text-xs font-semibold" style="padding:0 0.5rem">サーバーグループ</legend>
                    <p class="text-xs text-muted m-0 mb-2">このサーバーが管理者を継承する SourceMod グループを選択してください。</p>
                    {if $grouplist|@count == 0}
                        <p class="text-xs text-faint m-0">サーバーグループがまだ定義されていません — 管理者 → グループで作成してください。</p>
                    {else}
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!api || !window.Actions) {
            setError('JS API が読み込まれていません。ページを更新してください。');
        ...
        api.call(window.Actions.ServersAdd, {
            ...
            if (submit) submit.disabled = false;
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'サーバーを追加できませんでした。';
                setError(msg);
                if (window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'サーバー追加に失敗しました', body: msg });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast && d.message) {
                window.SBPP.showToast({ kind: 'success', title: d.message.title || 'サーバーが追加されました', body: d.message.body || '' });
            }
