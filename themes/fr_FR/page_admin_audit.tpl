...
<div class="audit-page">
    <header class="audit-page__header">
        <div>
            <h1 class="audit-page__title">Journal d&rsquo;audit</h1>
            <p class="audit-page__subtitle">Chaque action administrative enregistrée par le panneau ({$total_count} {if $total_count == 1}événement{else}événements{/if}).</p>
        </div>
    </header>
    ...
    <div class="audit-filters">
        <form method="get" action="index.php" role="search" aria-label="Rechercher dans le journal d&rsquo;audit" style="flex:1 1 16rem;max-width:22rem">
            ...
            <input type="search"
                   ...
                   name="search"
                   value="{$search}"
                   placeholder="Rechercher un titre ou un détail…"
                   data-testid="audit-search"
                   aria-label="Rechercher dans le journal d&rsquo;audit">
        </form>

        <form method="get" action="index.php" class="flex items-center gap-2" style="flex-wrap:wrap" aria-label="Filtrer le journal d&rsquo;audit par gravité">
            <input type="hidden" name="p" value="admin">
            <input type="hidden" name="c" value="audit">
            ...
            <div class="empty-state"
                ...
                <h2 class="empty-state__title">
                    {if $current_severity != '' || $search != ''}
                        Aucun événement d&rsquo;audit ne correspond à ces filtres
                    {else}
                        Aucun événement d&rsquo;audit enregistré pour le moment
                    {/if}
                </h2>
                <p class="empty-state__body">
                    {if $current_severity != '' || $search != ''}
                        Essayez un autre terme de recherche ou de gravité, ou effacez les filtres pour voir chaque action administrative enregistrée.
                    {else}
                        Lorsque les administrateurs commenceront à modérer depuis le panneau, chaque action sera enregistrée ici pour révision.
                    {/if}
                </p>
                {if $current_severity != '' || $search != ''}
    ...
    {if $page_count > 1 || $current_page > 1}
        <nav class="audit-pager" aria-label="Pagination du journal d&rsquo;audit">
            <div class="audit-pager__status" aria-live="polite">
                Page {$current_page} sur {$page_count}
            </div>
