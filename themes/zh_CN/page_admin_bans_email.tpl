...
<section class="p-6" data-testid="banemail-section" style="max-width:48rem">
    <div class="mb-6">
        <h1 style="font-size:1.5rem;font-weight:600;margin:0">给玩家发邮件</h1>
        <p class="text-sm text-muted m-0 mt-2">
            发送到 <span class="font-mono" data-testid="banemail-addr">{$email_addr|escape}</span>。
        </p>
    </div>
    <form class="card p-6 space-y-4"
        ...
        {csrf_field}
        <div>
            <label class="label" for="subject">主题</label>
            <input type="text"
                   class="input"
                   id="subject"
                   name="subject"
...
(function () {
    ...
    window.CheckEmail = function (type, id) {
        ...
        if (!subject || !subject.value) { setMsg('subject.msg', '您必须输入邮件的主题。'); err++; }
        else { setMsg('subject.msg', ''); }
        if (!message || !message.value) { setMsg('message.msg', '您必须输入邮件的内容。'); err++; }
        ...
        a.call(A.SystemSendMail, {
            ...
            if (!r || r.ok === false) {
                toast('error', '邮件发送失败', (r && r.error && r.error.message) || '未知错误');
                return;
            }
            ...
            toast('success', (msg && msg.title) || '邮件已发送', (msg && msg.body) || '邮件已发送。');
            if (msg && msg.redir) {
