...
{if NOT $permission_addban}
    <div class="card" data-testid="addban-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">У вас нет прав на добавление банов.</p>
        </div>
    </div>
{else}
    <section class="p-6" data-testid="addban-section" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">Добавить бан</h1>
            <p class="text-sm text-muted m-0 mt-2">
                Бан по SteamID или IP-адресу. Наведите курсор на метки для встроенной справки.
            </p>
        </div>
        <form id="addban-form"
            ...
            <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(14rem,1fr))">
                <div>
                    ...
                    <select class="select"
                        ...
                        <option value="1"{if $prefill_type == 1} selected{/if}>IP-адрес</option>
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <select class="select"
                    ...
                    <optgroup label="Читы">
                        <option value="Aimbot">Аимбот</option>
                        <option value="Antirecoil">Антиотдача</option>
                        <option value="Wallhack">Валлхак</option>
                        <option value="Spinhack">Спинхак</option>
                        <option value="Multi-Hack">Мульти-чит</option>
                        <option value="No Smoke">Без дыма</option>
                        <option value="No Flash">Без флешки</option>
                    </optgroup>
                    <optgroup label="Поведение">
                        ...
                        <option value="Inappropriate Spray">Неподходящий спрей</option>
                        <option value="Inappropriate Language">Неподходящий язык</option>
                        <option value="Inappropriate Name">Неподходящее имя</option>
                        <option value="Ignoring Admins">Игнорирование администраторов</option>
                        <option value="Team Stacking">Намеренное сложение команд</option>
                    </optgroup>
                    {if $customreason}
                        <optgroup label="Свои причины">
                ...
                <div id="dreason" class="mt-2" style="display:none">
                    <textarea class="textarea"
                              ...
                              name="txtReason"
                              rows="4"
                              placeholder="Подробно объясните, почему наложен этот бан."
...
(function () {
    ...
    function validate(type) {
        ...
        if (!nick || !nick.value) { setMsg('nick.msg', 'Необходимо указать никнейм блокируемого игрока'); err++; }
        else { setMsg('nick.msg', ''); }

        var steam = $id('steam');
        if (type === 0) {
            if (!steam || !/(?:STEAM_[01]:[01]:\d+)|(?:\[U:1:\d+\])|(?:\d{17})/.test(steam.value)) {
                setMsg('steam.msg', 'Необходимо ввести корректный STEAM ID или ID сообщества'); err++;
        ...
        if (type === 1) {
            if (!ip || ip.value.length < 7) {
                setMsg('ip.msg', 'Необходимо ввести корректный IP-адрес'); err++;
            } else { setMsg('ip.msg', ''); }
        } else { setMsg('ip.msg', ''); }

        if (!reason) {
            setMsg('reason.msg', 'Необходимо выбрать или указать причину этого бана.'); err++;
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.BansAdd, {
            ...
            setBusy(btn, false);
            if (!r || r.ok === false) {
                toast('error', 'Ошибка добавления бана', (r && r.error && r.error.message) || 'Неизвестная ошибка');
                return;
            }
            ...
            toast('success', (msg && msg.title) || 'Бан добавлен', (msg && msg.body) || 'Бан был успешно добавлен');
            reset();
        });
    });
})();
