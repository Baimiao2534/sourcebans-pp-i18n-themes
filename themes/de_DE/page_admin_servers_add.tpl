    ...
    {if NOT $permission_addserver}
        <div class="card" data-testid="server-add-denied">
            <div class="card__body">
                <h3 style="margin:0 0 0.25rem">Zugriff verweigert</h3>
                <p class="text-sm text-muted m-0">Sie haben keine Berechtigung, Server hinzuzufügen.</p>
            </div>
        </div>
    {else}
        <form id="addserver-form"
            ...
            <div class="card__header">
                <div>
                    <h3>{if $edit_server}Server bearbeiten{else}Server hinzufügen{/if}</h3>
                    <p>Tipp: Fahren Sie mit der Maus über die Fragezeichen für kontextsensitive Hilfe. RCON ist optional, schaltet aber die Live-Konsole und die pro-Server-Administrator-Zuordnung frei.</p>
                </div>
            </div>
            <div class="card__body" style="display:grid;gap:1rem">
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        ...
                        <span class="text-xs text-muted">IPv4 / IPv6 / Hostname. Hostnamen werden vom Spielserver zum Verbindungszeitpunkt aufgelöst.</span>
                    </label>
                    <label class="block">
                        ...
                        <span class="text-xs text-muted">Standard <code class="font-mono">27015</code>.</span>
                    </label>
                </div>

                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">RCON-Passwort</span>
                        <input type="password"
                               id="rcon"
                               name="rcon"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">Finden Sie in <code class="font-mono">server.cfg</code> neben <code class="font-mono">rcon_password</code>.</span>
                    </label>
                    <label class="block">
                        <span class="label">RCON bestätigen</span>
                        <input type="password"
                               id="rcon2"
                               name="rcon2"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">Erneut eingeben, um Tippfehler zu vermeiden.</span>
                    </label>
                </div>
                ...
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        ...
                        <select id="mod"
                            ...
                            {if !$edit_server}<option value="-2">Mod auswählen&hellip;</option>{/if}
                    ...
                    <label class="flex items-center gap-2" style="align-self:end;padding:0.5rem 0">
                        ...
                        <span class="text-sm">Auf der öffentlichen Serverliste aktivieren</span>
                    </label>
                </div>

                <fieldset style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem 1rem">
                    <legend class="text-xs font-semibold" style="padding:0 0.5rem">Servergruppen</legend>
                    <p class="text-xs text-muted m-0 mb-2">Wählen Sie die SourceMod-Gruppen aus, von denen dieser Server Administratoren erbt.</p>
                    {if $grouplist|@count == 0}
                        <p class="text-xs text-faint m-0">Noch keine Servergruppen definiert — besuchen Sie Administratoren → Gruppen, um eine zu erstellen.</p>
                    {else}
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!api || !window.Actions) {
            setError('JS-API nicht geladen; aktualisieren Sie die Seite.');
        ...
        api.call(window.Actions.ServersAdd, {
            ...
            if (submit) submit.disabled = false;
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'Server konnte nicht hinzugefügt werden.';
                setError(msg);
                if (window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'Server hinzufügen fehlgeschlagen', body: msg });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast && d.message) {
                window.SBPP.showToast({ kind: 'success', title: d.message.title || 'Server hinzugefügt', body: d.message.body || '' });
            }
