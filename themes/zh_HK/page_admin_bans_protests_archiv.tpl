...
{if NOT $permission_protests}
    <div class="card" data-testid="protests-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">您沒有檢視封禁申訴的權限。</p>
        </div>
    </div>
{else}
        ...
        {if $protest_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="protests-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">申訴歸檔為空</h2>
                    <p class="empty-state__body">當申訴被接受、拒絕或歸檔後，會移至此處作為紀錄。</p>
                </div>
            </div>
        {else}
                    ...
                    <details class="queue-row ban-row ban-row--expired"
                        ...
                        <div class="p-4" id="ban_details_{$protest.pid}" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                                ...
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    {if $protest.archiv != 2}
                                        ...
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.authid == ""}<span class="text-faint">無 SteamID</span>{else}{$protest.authid|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">IP 位址</dt>
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.ip == 'none' OR $protest.ip == ''}<span class="text-faint">無 IP 位址</span>{else}{$protest.ip|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">執行於</dt>
                                        <dd style="margin:0">{$protest.date|escape}</dd>

                                        <dt class="text-muted">結束日期</dt>
                                        <dd style="margin:0">
                                            {if $protest.ends == 'never'}<span class="text-faint">不適用。</span>{else}{$protest.ends|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">封禁原因</dt>
                                        ...
                                        <dt class="text-muted">封禁者</dt>
                                        <dd style="margin:0">{$protest.admin|escape}</dd>

                                        <dt class="text-muted">伺服器</dt>
                                        <dd class="font-mono" style="margin:0">{$protest.server|escape}</dd>
                                    {/if}

                                    <dt class="text-muted">歸檔者</dt>
                                    <dd style="margin:0">
                                        {if !empty($protest.archivedby)}{$protest.archivedby|escape}{else}<i class="text-faint">管理員已刪除。</i>{/if}
                                    </dd>

                                    <dt class="text-muted">申訴者 IP</dt>
                                    <dd class="font-mono" style="margin:0">{$protest.pip|escape}</dd>

                                    <dt class="text-muted">申訴於</dt>
                                    <dd style="margin:0">{$protest.datesubmitted|escape}</dd>

                                    <dt class="text-muted">訊息</dt>
                            ...
                            {if $protest.commentdata != "None"}
                                <div class="mt-4">
                                    <h4 class="text-xs text-muted" style="font-weight:600;margin:0 0 0.5rem;text-transform:uppercase;letter-spacing:0.06em">評論</h4>
                                    <div class="space-y-3">
                                            ...
                                            <div class="card p-4">
                                                <div class="flex items-center justify-between gap-2 text-xs text-muted mb-2">
                                                    <strong style="color:var(--text);font-weight:600">
                                                        {if !empty($commenta.comname)}{$commenta.comname|escape}{else}<i class="text-faint">管理員已刪除</i>{/if}
                                                    </strong>
                                                ...
                                                {if !empty($commenta.edittime)}
                                                    <div class="text-xs text-faint mt-2">
                                                        最後編輯 {$commenta.edittime|escape} 由
                                                        {if !empty($commenta.editname)}{$commenta.editname|escape}{else}<i>管理員已刪除</i>{/if}
                                                    </div>
                                                {/if}
                                            </div>
                                        {/foreach}
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var msg;
        if (archiv === '2') msg = '要從歸檔中恢復「' + key + '」的封禁申訴嗎？';
        else if (archiv === '1') msg = '要將「' + key + '」的封禁申訴移至歸檔嗎？';
        else msg = '要刪除「' + key + '」的封禁申訴嗎？';
        ...
        a.call(A.ProtestsRemove, { pid: pid, archiv: archiv }).then(function (r) {
            if (!r || r.ok === false) {
                setBusy(btn, false);
                toast('error', '操作失敗', (r && r.error && r.error.message) || '未知錯誤');
                return;
            }
