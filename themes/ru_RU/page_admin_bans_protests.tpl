        ...
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                ...
                <p class="text-sm text-muted m-0 mt-2">
                    Баны, которые игроки просят пересмотреть.
                </p>
            </div>
        ...
        {if $protest_list|@count == 0}
            ...
            <div class="card" data-testid="protests-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">Нет активных апелляций</h2>
                    <p class="empty-state__body">Нечего проверять. Новые апелляции на баны будут появляться здесь по мере их подачи игроками.</p>
                </div>
            </div>
        {else}
                            ...
                            <div class="row-actions">
                                {if $permission_editban}
                                    <button type="button"
                                            ...
                                            data-archiv="1">
                                        <span data-confirm-label>Удалить</span>
                                    </button>
                                {/if}
...
(function () {
    ...
    function arm(btn) {
        ...
        labelEl.textContent = 'Нажмите для подтверждения';
        btn.setAttribute('data-pending', 'true');
        btn.setAttribute('aria-label', 'Нажмите ещё раз в течение 3 секунд, чтобы подтвердить удаление');
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.ProtestsRemove, { pid: pid, archiv: '1' }).then(function (r) {
            ...
            toast('success', 'Апелляция архивирована', 'При необходимости восстановите её со страницы архива апелляций.');
        });
    });
})();
</script>
