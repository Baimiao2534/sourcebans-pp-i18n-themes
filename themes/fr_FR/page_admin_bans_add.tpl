...
{else}
    <section class="p-6" data-testid="addban-section" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">Ajouter un bannissement</h1>
            <p class="text-sm text-muted m-0 mt-2">
                Bannissez par SteamID ou par adresse IP. Survolez les étiquettes pour obtenir de l&rsquo;aide en ligne.
            </p>
        </div>
        <form id="addban-form"
            ...
            <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(14rem,1fr))">
                <div>
                    ...
                    <select class="select"
                        ...
                        <option value="1"{if $prefill_type == 1} selected{/if}>Adresse IP</option>
                    </select>
                </div>
                <div>
...
(function () {
    ...
    function validate(type) {
        ...
        if (!nick || !nick.value) { setMsg('nick.msg', 'Vous devez saisir le pseudo de la personne que vous bannissez'); err++; }
        else { setMsg('nick.msg', ''); }

        var steam = $id('steam');
        if (type === 0) {
            if (!steam || !/(?:STEAM_[01]:[01]:\d+)|(?:\[U:1:\d+\])|(?:\d{17})/.test(steam.value)) {
                setMsg('steam.msg', 'Vous devez saisir un Steam ID ou Community ID valide'); err++;
        ...
        if (type === 1) {
            if (!ip || ip.value.length < 7) {
                setMsg('ip.msg', 'Vous devez saisir une adresse IP valide'); err++;
            } else { setMsg('ip.msg', ''); }
        } else { setMsg('ip.msg', ''); }

        if (!reason) {
            setMsg('reason.msg', 'Vous devez sélectionner ou saisir un motif pour ce bannissement.'); err++;
    ...
    document.addEventListener('click', function (e) {
        ...
        a.call(A.BansAdd, {
            ...
            if (r && r.ok && r.data && r.data.kickit) {
                ...
                var k = r.data.kickit;
                toast('success', 'Bannissement ajouté',
                    'Le bannissement a été ajouté avec succès.');
            ...
            setBusy(btn, false);
            if (!r || r.ok === false) {
                toast('error', 'Échec de l&rsquo;ajout du bannissement', (r && r.error && r.error.message) || 'Erreur inconnue');
                return;
            }
            ...
            toast('success', (msg && msg.title) || 'Bannissement ajouté', (msg && msg.body) || 'Le bannissement a été ajouté avec succès');
            reset();
        });
    });
})();
