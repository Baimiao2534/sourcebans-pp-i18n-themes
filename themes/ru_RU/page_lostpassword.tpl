...
<div class="lostpw">
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                <h3>Забыли пароль?</h3>
                <p>Введите адрес электронной почты, привязанный к учётной записи администратора, и мы отправим вам ссылку для сброса пароля.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                {csrf_field}
                <div>
                    <label class="label" for="lostpw-email">Адрес электронной почты</label>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> Отправить ссылку для сброса
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; Назад ко входу</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (!emailEl.value) {
            toast('warn', 'Требуется email', 'Пожалуйста, введите ваш адрес электронной почты.');
            return;
        }
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || 'Неизвестная ошибка';
                toast('error', 'Ошибка', emsg);
                return;
            }
            var msg = (res.data || {}).message;
            if (msg) {
                toast(mapKind(msg.kind), msg.title || 'Готово', msg.body || '');
            }
        });
    });
}());
