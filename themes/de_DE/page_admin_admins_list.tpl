...
{else}
    <div class="flex items-end justify-between gap-3 mb-4" style="flex-wrap:wrap">
        <div>
            <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">Administratoren
                <span class="text-faint" style="font-weight:400;margin-left:0.375rem" data-testid="admin-count">({$admin_count})</span>
            </h1>
            <p class="text-sm text-muted m-0 mt-2">Klicken Sie auf die Aktionen einer Administrator-Zeile, um Details, Berechtigungen oder Serverzugriff zu bearbeiten.</p>
        </div>
        {if $can_add_admins}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=admins&amp;section=add-admin"
               data-testid="admin-add-cta"><i data-lucide="user-plus"></i> Administrator hinzufügen</a>
        {/if}
    </div>
    ...
    <div data-testid="admin-admins-section-admins">
        ...
        <div class="card" style="overflow:hidden">
            <table class="table" role="table" aria-label="Administratoren">
                <thead>
                    <tr>
                        <th scope="col">Name</th>
                        <th scope="col">Banns</th>
                        <th scope="col">Servergruppe</th>
                        <th scope="col">Web-Gruppe</th>
                        <th scope="col">Immunität</th>
                        <th scope="col">Letzter Besuch</th>
                    ...
                    <tr data-testid="admin-row" data-id="{$admin.aid}">
                        ...
                        <td class="tabular-nums text-muted">
                            <a href="./index.php?p=banlist&advSearch={$admin.aid|escape:'url'}&advType=admin"
                               title="Banns anzeigen">{$admin.bancount}</a>
                            <span class="text-faint"> · </span>
                            <a href="./index.php?p=banlist&advSearch={$admin.aid|escape:'url'}&advType=nodemo"
                               title="Banns ohne Demo anzeigen">{$admin.nodemocount} ohne Demo</a>
                        </td>
                            ...
                            <div class="row-actions" style="white-space:nowrap">
                                {if $can_edit_admins}
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Details für {$admin.user|escape} bearbeiten"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Berechtigungen für {$admin.user|escape} bearbeiten"
                                       data-testid="admin-action-edit-perms">
                                        <i data-lucide="shield" style="width:14px;height:14px"></i>
                                    </a>
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       href="index.php?p=admin&c=admins&o=editservers&id={$admin.aid|escape:'url'}"
                                       title="Serverzugriff bearbeiten"
                                       aria-label="Serverzugriff für {$admin.user|escape} bearbeiten"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Gruppen für {$admin.user|escape} bearbeiten"
                                ...
                                {if $can_delete_admins}
                                    ...
                                    <button type="button" class="btn btn--ghost btn--icon btn--sm"
                                            ...
                                            aria-label="Administrator {$admin.user|escape} löschen"
    ...
    <dialog id="admins-delete-dialog"
        ...
        <form method="dialog" data-testid="admins-delete-form">
            <h2 id="admins-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Administrator löschen</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                Sie sind dabei, <strong data-testid="admins-delete-target">diesen Administrator</strong> dauerhaft zu löschen. Dies kann nicht rückgängig gemacht werden. Der Serverzugriff wird sofort entzogen und die Rehash-Warteschlange geleert.
            </p>
            <label class="label" for="admins-delete-reason">Grund (optional)</label>
            ...
            <textarea class="textarea"
                      ...
                      maxlength="255"
                      autocomplete="off"
                      placeholder="Nur Audit-Protokoll. Leer lassen, um zu überspringen."></textarea>
            <p class="text-xs" data-testid="admins-delete-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                <button type="button" class="btn btn--secondary" data-testid="admins-delete-cancel" value="cancel">Abbrechen</button>
                <button type="submit" class="btn btn--danger" data-testid="admins-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> Administrator löschen
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.AdminsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', 'Administrator gelöscht', ctx.name + ' wurde entfernt.');
            });
        });
