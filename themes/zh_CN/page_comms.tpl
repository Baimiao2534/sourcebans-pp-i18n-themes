...
<div class="p-6 space-y-4" style="max-width:1700px;margin:0 auto;width:100%">
    ...
    <header class="flex items-center justify-between gap-4" style="flex-wrap:wrap" data-testid="comms-header">
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">通信封禁</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <span class="tabular-nums" data-testid="comms-count">{$ban_list|@count|number_format}</span>
                / 共 <span class="tabular-nums">{$total_bans|number_format}</span> 条
            </p>
        </div>
    ...
    <form method="get" action="index.php"
        ...
        <div class="flex gap-3" style="flex-wrap:wrap">
            <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
                ...
                <input class="input input--with-icon"
                       ...
                       placeholder="玩家、SteamID 或原因…"
                       data-testid="comms-search">
            </div>
            <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="comms-server-filter" aria-label="按服务器筛选">
                <option value="">全部服务器</option>
                {foreach $servers as $s}
                    <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
                {/foreach}
            </select>
            <select class="select" name="time" style="width:auto" data-testid="comms-time-filter" aria-label="按时间范围筛选">
        ...
        <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="快捷筛选">
            ...
            <button class="chip" type="submit" name="type" value="mute"
                ...
                <i data-lucide="mic-off" style="width:12px;height:12px"></i> 禁麦
            </button>
            ...
            <button class="chip" type="submit" name="type" value="silence"
                ...
                <i data-lucide="volume-x" style="width:12px;height:12px"></i> 静默
            </button>
        </div>
    </form>
        ...
        <table class="table" data-testid="comms-table">
            <thead>
                <tr>
                    ...
                    <th>玩家</th>
                    <th>SteamID</th>
                    <th class="col-length col-tier-3">时长</th>
                    <th class="col-tier-2">服务器</th>
                    <th class="col-admin col-tier-2">管理员</th>
                    <th class="col-tier-3">开始时间</th>
                            ...
                            <div class="flex items-center gap-3" style="min-width:0">
                                ...
                                <div style="min-width:0">
                                    ...
                                    <a class="font-medium truncate"
                                       ...
                                       data-testid="drawer-trigger">{if $comm.name}{$comm.name|escape}{else}<i class="text-faint">无昵称</i>{/if}</a>
                                    ...
                                    {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                        <div class="text-xs text-faint mt-1" data-testid="comm-unban-meta">
                                            {if !empty($comm.removedby)}由 <span class="font-medium">{$comm.removedby|escape}</span> 解除{if !empty($comm.ureason)}：{/if}{/if}
                                    ...
                                    <details class="ban-comments-inline mt-1"
                                      ...
                                      <summary class="ban-comments-inline__summary"
                                               ...
                                               title="此通信封禁有 {$comm.commentdata|@count} 条评论">
                            ...
                            <div class="row-actions">
                                ...
                                {if $can_unmute_gag && $comm.unmute_url}
                                    ...
                                    <button type="button"
                                        ...
                                        {if $comm.type == 'mute'}解除禁麦{elseif $comm.type == 'gag'}解除禁言{else}解除封禁{/if}
                                    </button>
                    ...
                    <tr>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">没有符合筛选条件的通信封禁</h2>
                                <p class="empty-state__body">尝试其他搜索词、服务器或时间范围，或清除当前筛选以查看所有禁麦 / 禁言记录。</p>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">尚无通信封禁记录</h2>
                                <p class="empty-state__body">从面板或游戏内发布的禁麦和禁言将显示在这里。</p>
                                {if $can_add_comm}
                                <div class="empty-state__actions">
                                    <a class="btn btn--primary btn--sm"
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-sm truncate">
                                    {if $comm.name}{$comm.name|escape}{else}<span class="text-faint">无昵称</span>{/if}
                                </span>
                            ...
                            {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                ...
                                <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="comm-unban-meta-mobile"
                                     title="{if !empty($comm.removedby)}由 {$comm.removedby|escape} 解除{if !empty($comm.ureason)}：{/if}{/if}{if !empty($comm.ureason)}{$comm.ureason|escape}{/if}">
                                    {if !empty($comm.removedby)}由 {$comm.removedby|escape} 解除{if !empty($comm.ureason)}：{/if}{/if}
...
<dialog id="comms-unblock-dialog"
  ...
  <form method="dialog" data-testid="comms-unblock-form">
    ...
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      您为什么要解除封禁 <strong data-testid="comms-unblock-target">该玩家</strong>？此原因将记录在该封禁上并显示在审计日志中。
    </p>
    ...
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      ...
      <button type="submit" class="btn btn--primary" data-testid="comms-unblock-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> 确认
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnmuted(row) {
        ...
        Array.prototype.forEach.call(
            ...
            function (btn) {
                ...
                a.innerHTML = '<i data-lucide="rotate-ccw" style="width:13px;height:13px"></i> 重新应用';
    ...
    function copyForType(type) {
        if (type === 'mute')    return { title: '解除禁麦',         verb: '解除禁麦' };
        if (type === 'gag')     return { title: '解除禁言',         verb: '解除禁言' };
        if (type === 'silence') return { title: '解除静默',         verb: '解除静默' };
    ...
    function openUnblockDialog(ctx) {
        ...
        // rewrite the parent paragraph's first text node to swap "解除封禁"
        // for the type-specific verb without losing the bold name span.
        var p = d.querySelector('p.text-sm.text-muted');
        if (p && p.firstChild && p.firstChild.nodeType === 3) {
            p.firstChild.textContent = '您为什么要' + copy.verb + ' ';
        }
    ...
    document.addEventListener('click', function (e) {
        ...
        if (act === 'comms-delete') {
            if (!window.confirm('删除 "' + name + '" 的封禁？')) return;
            setBusy(btn, true);
            a.call(A.CommsDelete, { bid: Number(bid) }).then(function (r) {
                ...
                decrementCount();
                toast('success', '封禁已删除', name + ' 的封禁已删除。');
            });
            return;
        }
