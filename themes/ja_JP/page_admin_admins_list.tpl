...
{else}
    <div class="flex items-end justify-between gap-3 mb-4" style="flex-wrap:wrap">
        <div>
            <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">管理者
                <span class="text-faint" style="font-weight:400;margin-left:0.375rem" data-testid="admin-count">({$admin_count})</span>
            </h1>
            <p class="text-sm text-muted m-0 mt-2">管理者行のアクションをクリックして、詳細、権限、サーバーアクセスを編集します。</p>
        </div>
        {if $can_add_admins}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=admins&amp;section=add-admin"
               data-testid="admin-add-cta"><i data-lucide="user-plus"></i> 管理者を追加</a>
        {/if}
    </div>
    ...
    <div data-testid="admin-admins-section-admins">
        ...
        <div class="card" style="overflow:hidden">
            <table class="table" role="table" aria-label="管理者">
                <thead>
                    <tr>
                        <th scope="col">名前</th>
                        <th scope="col">BAN</th>
                        <th scope="col">サーバーグループ</th>
                        <th scope="col">Web グループ</th>
                        <th scope="col">免疫</th>
                        <th scope="col">最終アクセス</th>
                            ...
                            <div class="row-actions" style="white-space:nowrap">
                                {if $can_edit_admins}
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="詳細を編集"
                                       aria-label="{$admin.user|escape} の詳細を編集"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="権限を編集"
                                       aria-label="{$admin.user|escape} の権限を編集"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="サーバーアクセスを編集"
                                       aria-label="{$admin.user|escape} のサーバーアクセスを編集"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="グループを編集"
                                       aria-label="{$admin.user|escape} のグループを編集"
                                ...
                                {if $can_delete_admins}
                                    ...
                                    <button type="button" class="btn btn--ghost btn--icon btn--sm"
                                            ...
                                            title="管理者を削除"
                                            aria-label="管理者 {$admin.user|escape} を削除"
    ...
    <dialog id="admins-delete-dialog"
        ...
        <form method="dialog" data-testid="admins-delete-form">
            <h2 id="admins-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">管理者を削除</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                <strong data-testid="admins-delete-target">この管理者</strong>を完全に削除しようとしています。この操作は取り消せません。直ちにサーバーアクセスが取り消され、再ハッシュキューがフラッシュされます。
            </p>
            <label class="label" for="admins-delete-reason">理由（任意）</label>
    ...
    (function () {
        ...
        function openDeleteDialog(ctx) {
            ...
            if (target) target.textContent = ctx.name || ('管理者 #' + ctx.aid);
        ...
        document.addEventListener('click', function (e) {
            ...
            var name = btn.getAttribute('data-name') || ('管理者 #' + aid);
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.AdminsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', '管理者を削除しました', ctx.name + ' は削除されました。');
            });
        });
