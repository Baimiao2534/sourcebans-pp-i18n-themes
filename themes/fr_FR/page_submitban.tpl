...
<section class="p-6" style="max-width:48rem">
    <header class="mb-6">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Signaler un joueur</h1>
        <p class="text-sm text-muted m-0 mt-2">
            Signalez un joueur pour triche, harcèlement ou autre infraction aux règles. Les démos et captures d&rsquo;écran aident les administrateurs à agir plus rapidement.
        </p>
    </header>
    ...
    <form class="card"
        ...
        <div class="card__body space-y-4">
            <div>
                <h2 id="submitban-heading" class="m-0" style="font-size:var(--fs-base);font-weight:600">
                    Détails du joueur signalé
                </h2>
                <p class="text-xs text-muted m-0 mt-2">
                    Indiquez un Steam ID ou une adresse IP (ou les deux). Le pseudo et une description
                    claire de l&rsquo;incident sont obligatoires.
                </p>
            </div>
            ...
            *}
            <div data-testid="submitban-id-or-ip"
                ...
                <p id="submitban-id-or-ip-help"
                   ...
                   data-state="info">
                    <span class="pill"
                        ...
                        1 obligatoire parmi ces champs
                    </span>
                    <span class="text-muted" data-id-or-ip-text>Indiquez un Steam ID ou une adresse IP (ou les deux).</span>
                </p>
                <div>
                    <label class="label" for="submitban-steam">
                        Steam ID du joueur
                        <span class="text-muted text-xs" style="font-weight:400">(ou utilisez l&rsquo;IP ci-dessous)</span>
                    </label>
                    {*
                ...
                <div>
                    <label class="label" for="submitban-ip">
                        ...
                        <span class="text-muted text-xs" style="font-weight:400">(ou utilisez le Steam ID ci-dessus)</span>
                    </label>
                    <input type="text"
                           class="input font-mono"
                           id="submitban-ip"
            ...
            <div>
                <label class="label" for="submitban-name">
                    Pseudo du joueur <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <input type="text"
                       class="input"
                       id="submitban-name"
            ...
            <div>
                <label class="label" for="submitban-reason">
                    Commentaires <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                ...
                <p id="submitban-reason-help" class="text-xs text-muted m-0 mt-2">
                    Soyez précis. &ldquo;triche&rdquo; ne suffit pas. Décrivez ce que vous avez vu, quand et sur quel serveur.
                </p>
            </div>
        </div>
        ...
        <div class="card__body space-y-4" style="border-top:1px solid var(--border)">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    Pour qu&rsquo;un administrateur puisse vous contacter en cas de questions de suivi.
                </p>
            </div>

            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div>
                    <label class="label" for="submitban-reporter-name">Votre nom</label>
                    <input type="text"
                           class="input"
                ...
                <div>
                    <label class="label" for="submitban-reporter-email">
                        Votre e-mail <span style="color:var(--danger)" aria-hidden="true">*</span>
                    </label>
                    <input type="email"
                           class="input"
            ...
            <div>
                <label class="label" for="submitban-server">
                    Serveur <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <select class="select"
                        id="submitban-server"
                        name="server"
                    ...
                    <option value="-1">&mdash; Sélectionner un serveur &mdash;</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"{if $server_selected == $server.sid} selected{/if}>{$server.hostname}</option>
                    {/foreach}
                    <option value="0"{if $server_selected == 0} selected{/if}>Autre serveur / Non listé ici</option>
                </select>
            </div>

            <div>
                <label class="label" for="submitban-demo">Télécharger une démo ou des preuves</label>
                <div class="file-input">
                    ...
                    <span class="text-muted text-sm" data-file-name>Aucun fichier choisi</span>
                </div>
                <p class="text-xs text-muted m-0 mt-2">
                    Facultatif. Formats autorisés : <code class="font-mono">.dem</code>,
        ...
        <div class="card__body flex items-center justify-between gap-2"
            ...
            <p class="text-xs text-muted m-0">
                <span style="color:var(--danger)" aria-hidden="true">*</span> Champ obligatoire
            </p>
            <div class="flex gap-2">
                <a class="btn btn--ghost"
                   href="index.php?p=banlist"
                   data-testid="submitban-cancel">Annuler</a>
...
(function () {
    ...
    function setError(isError) {
        ...
        if (isError) {
            ...
            helpText.textContent = 'Veuillez saisir un Steam ID ou une adresse IP avant de soumettre.';
        } else {
