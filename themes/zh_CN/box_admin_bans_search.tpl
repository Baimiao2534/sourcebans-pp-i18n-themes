...
<form method="get"
    ...
    <div class="card__header">
        <div>
            <h3>高级搜索</h3>
            <p>每行是独立的搜索条件。右侧的 &laquo;搜索&raquo; 按钮仅使用该行的值进行提交。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-bans-name" style="grid-column:1;align-self:end">昵称</label>
            <input class="input"
                   id="search-bans-name"
                   type="text"
                   placeholder="对玩家昵称进行子字符串匹配&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-bans-reason" style="grid-column:1;align-self:end">原因</label>
            <input class="input"
                   id="search-bans-reason"
                   type="text"
                   placeholder="对已记录原因进行子字符串匹配&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">时长</span>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-bans-length-op"
                        data-testid="search-bans-length-op"
                        style="width:5rem">
                    <option value="e"  title="等于">=</option>
                    <option value="h"  title="大于">&gt;</option>
                    <option value="l"  title="小于">&lt;</option>
                    <option value="eh" title="大于或等于">&gt;=</option>
                    <option value="el" title="小于或等于">&lt;=</option>
                </select>
                <select class="select"
                    ...
                    <option value="other">其他时长 (分钟)&hellip;</option>
                </select>
                <input type="text"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-bans-btype">类型</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        {if NOT $hideadminname}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-bans-admin">管理员</label>
                <div class="flex items-center gap-2" style="flex-wrap:wrap">
                    <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-bans-server">服务器</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-bans-server"
                        data-testid="search-bans-server">
                    <option value="0" selected>Web 封禁</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                data-sid="{$server.sid}"
                                data-ip="{$server.ip}"
                                data-port="{$server.port}"
                                data-server-host>加载中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
            <button type="submit"
        ...
        {if $is_admin}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-bans-comment" style="grid-column:1;align-self:end">备注</label>
                <input class="input"
                       id="search-bans-comment"
                       type="text"
                       placeholder="对管理员备注进行子字符串匹配&hellip;"
