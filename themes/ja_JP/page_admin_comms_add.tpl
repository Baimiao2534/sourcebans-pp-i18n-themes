...
{else}
    ...
    <div class="p-6" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">ブロックを追加</h1>
            <p class="text-sm text-muted m-0 mt-2">接続中のすべてのサーバーでプレイヤーをミュート・ギャグ・サイレンスします。</p>
        </div>
        ...
        <form id="addcomm-form" class="card p-6 space-y-4" onsubmit="return false;" data-testid="addcomm-form">
            ...
            <div>
                ...
                <p class="text-xs text-muted mt-1" id="addcomm-steam-help">
                    受け付ける形式：
                    <code>STEAM_0:1:N</code>、<code>[U:1:N]</code> または 17 桁の SteamID64。
                </p>
            ...
            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div>
                    ...
                    <select class="select" id="type" name="type" data-testid="addcomm-type">
                        <option value="1"{if $prefill_type == 1} selected{/if}>ミュート（ボイス）</option>
                        <option value="2"{if $prefill_type == 2} selected{/if}>ギャグ（チャット）</option>
                        <option value="3"{if $prefill_type == 3} selected{/if}>サイレンス（チャットとボイス）</option>
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <select class="select"
                    ...
                    <optgroup label="違反">
                        <option value="Obscene language">わいせつな言葉</option>
                        <option value="Insult players">プレイヤーへの侮辱</option>
                        <option value="Admin disrespect">管理者への無礼</option>
                        <option value="Inappropriate Language">不適切な言葉</option>
                        <option value="Trading">取引行為</option>
                        <option value="Spam in chat/voice">スパム行為</option>
                        <option value="Advertisement">宣伝</option>
                    </optgroup>
                ...
                <div id="dreason" class="mt-2" style="display:none">
                    <textarea class="textarea"
                              ...
                              name="txtReason"
                              rows="4"
                              placeholder="今回のブロック理由を詳しく記載してください。"
            ...
            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                ...
                <button type="button"
                    ...
                    <i data-lucide="mic-off"></i> ブロックを追加
                </button>
            </div>
        </form>
    </div>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (reason === 'other') {
            ...
            if (!reason) {
                setMsg('reason.msg', 'テキストエリアにブロック理由を記入してください。');
        ...
        a.call(A.CommsAdd, {
            ...
            if (r.ok === false) {
                var err = (r.error && r.error.message) || '不明なエラー';
                toast('error', 'ブロックが追加されませんでした', err);
