...
<div class="p-6 space-y-6" style="max-width:1400px;margin:0 auto;width:100%">
    ...
    <header data-testid="dashboard-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">
            {if $dashboard_title}{$dashboard_title}{else}仪表盘{/if}
        </h1>
        <p class="text-sm text-muted m-0 mt-2">服务器上的活动概览。</p>
    </header>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        <div class="card p-5" data-testid="dashboard-stat-total-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">总封禁数</div>
            ...
            <div class="text-xs text-muted">所有时间</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-active-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">活跃封禁</div>
            ...
            <div class="text-xs text-muted">当前生效中</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-comm-blocks">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">通信封禁</div>
            ...
            <div class="text-xs text-muted">禁言 + 禁字</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-servers">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">服务器</div>
            ...
            <div class="text-xs text-muted">已配置</div>
        </div>
    ...
    {* -- Admins-only project announcement strip ----------------------
        ...
        hidden. *}
    {if $announcement}
    <aside class="announcement-strip" data-testid="dashboard-announcement" aria-label="最新公告">
        <details>
            <summary>
            ...
            <a class="announcement-strip__link" data-testid="dashboard-announcement-link"
               href="{$announcement.url}" target="_blank" rel="noopener noreferrer">
                阅读更多 <i data-lucide="external-link" aria-hidden="true" style="width:13px;height:13px"></i>
            </a>
            {/if}
        </details>
    </aside>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))">
        ...
        <section class="card" data-testid="dashboard-recent-bans">
            <div class="card__header">
                ...
                <a class="btn btn--ghost btn--sm" href="?p=banlist">
                    查看全部 <i data-lucide="arrow-right" style="width:14px;height:14px"></i>
                </a>
            </div>
            <div>
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $b.short_name}{$b.short_name}{else}<span class="text-faint">无昵称</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-bans-empty">
                    ...
                    <h4 class="empty-state__title">暂无封禁</h4>
                    <p class="empty-state__body">当管理员开始执法时，执法操作将显示在此处。</p>
                    {if $can_add_ban}
        ...
        <section class="card" data-testid="dashboard-servers">
            <div class="card__header">
                <div>
                    <h3>服务器</h3>
                    <p>{$total_servers|number_format} 已配置</p>
                </div>
            ...
            *}
            <div style="padding:0.5rem"
                ...
                <div class="empty-state" data-testid="dashboard-servers-empty">
                    ...
                    <h4 class="empty-state__title">未配置服务器</h4>
                    <p class="empty-state__body">添加一个服务器，让访客可以查看实时状态并从面板连接。</p>
                    {if $can_add_server}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
        ...
        *}
        <section class="card" data-testid="dashboard-blocked-attempts" data-lognopopup="{if $dashboard_lognopopup}1{else}0{/if}">
            <div class="card__header">
                <div>
                    <h3>最新拦截记录</h3>
                    <p>{$total_blocked|number_format} 次拦截总计</p>
                </div>
            </div>
            <div>
                ...
                <a class="flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-blocked-row" data-id="{$p.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="font-medium text-sm truncate">
                            {if $p.short_name}{$p.short_name}{else}<span class="text-faint">无昵称</span>{/if}
                        </div>
                ...
                <div class="empty-state" data-testid="dashboard-blocked-attempts-empty">
                    ...
                    <h4 class="empty-state__title">暂无拦截记录</h4>
                    <p class="empty-state__body">当被封禁的玩家尝试重新加入时，SourceMod 插件将在此记录每次拦截。</p>
                </div>
                {/foreach}
            </div>
        </section>
        ...
        <section class="card" data-testid="dashboard-recent-comms">
            ...
            <div>
                ...
                <a class="ban-row ban-row--{$c.state} flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-comm-row" data-id="{$c.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $c.short_name}{$c.short_name}{else}<span class="text-faint">无昵称</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-comms-empty">
                    ...
                    <h4 class="empty-state__title">暂无通信封禁</h4>
                    <p class="empty-state__body">从面板或游戏内发布的禁言和禁字将显示在此处。</p>
                    {if $can_add_ban}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
