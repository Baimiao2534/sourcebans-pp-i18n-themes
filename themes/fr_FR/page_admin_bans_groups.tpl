    ...
    <div class="card" data-testid="groupban-denied">
        <div class="card__body">
            <h1 style="font-size:1.25rem;font-weight:600;margin:0">Accès refusé</h1>
                <p class="text-sm text-muted m-0 mt-2">Vous n&rsquo;avez pas la permission d&rsquo;ajouter des bannissements.</p>
        </div>
    </div>
{else}
    {if NOT $groupbanning_enabled}
        <div class="card" data-testid="groupban-disabled">
            <div class="card__body">
                <h1 style="font-size:1.25rem;font-weight:600;margin:0">Fonctionnalité désactivée</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Le bannissement de groupe est désactivé dans
                    <strong>config.enablegroupbanning</strong>.
                    Réactivez-le depuis l&rsquo;onglet Paramètres pour utiliser cette fonction.
                </p>
            </div>
        </div>
    {else}
        <section class="p-6" data-testid="groupban-section" style="max-width:48rem">
            <div class="mb-6">
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">Bannissement de groupe</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Bannir tous les membres d&rsquo;un groupe Steam communautaire en une fois.
                </p>
            </div>
            ...
            {if NOT $list_steam_groups}
                <form id="groupban-form"
                    ...
                    {csrf_field}
                    <div>
                        <label class="label" for="groupurl">URL du groupe Steam communautaire</label>
                        <input type="text"
                    ...
                    <div>
                        <label class="label" for="groupreason">Motif du bannissement de groupe</label>
                        <textarea class="textarea"
                                  id="groupreason"
                                  name="groupreason"
                                  rows="4"
                                  placeholder="Pourquoi ce groupe est-il banni ?"
                    ...
                    <div class="flex justify-end gap-2"
                        ...
                        <button type="button"
                            ...
                            Ajouter un bannissement de groupe
                        </button>
                    </div>
                </form>
            {else}
                <div class="card p-6 space-y-4" data-testid="groupban-from-player">
                    <p class="text-sm text-muted m-0">
                        {if $player_name}
                            Les groupes auxquels appartient <strong class="font-medium">{$player_name|escape}</strong> sont chargés ci-dessous.
                        {else}
                            Tous les groupes dont le joueur est membre sont listés ici.
                        {/if}
                        Cochez ceux que vous souhaitez bannir.
                    </p>

                    <div id="steamGroupsText"
                         name="steamGroupsText"
                         class="text-sm text-muted">Chargement des groupes…</div>
                    ...
                    <div id="steamGroups"
                            ...
                            <thead>
                                <tr>
                                    <th style="width:2.5rem">
                                        ...
                                        title="Tout sélectionner">+</button>
                                    </th>
                                    <th>Groupe</th>
                                </tr>
                            </thead>
                        ...
                        <a class="btn btn--ghost btn--sm"
                           ...
                           data-action="groupban-select-all"
                           name="tickswitchlink"
                           id="tickswitchlink">Tout sélectionner</a>
                ...
                (function (friendid) {
                    sb.ready(function () {
                        sb.api.call(Actions.BansGetGroups, { friendid: friendid }).then(function (r) {
                            ...
                            if (!tbl) return;
                            if (groups.length === 0) {
                                sb.message.error('Erreur', "Une erreur s'est produite lors de la récupération des données du groupe. Peut-être que le joueur n'est membre d'aucun groupe ou que son profil est privé ?", 'index.php?p=banlist');
                                var txt = sb.$id('steamGroupsText');
                                if (txt) txt.innerHTML = '<i>Aucun groupe...</i>';
                                return;
                            }
                            groups.forEach(function (g, i) {
                                ...
                                td2.appendChild(document.createTextNode(' Membres)'));
                            });
