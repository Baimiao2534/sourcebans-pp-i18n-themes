...
<aside class="admin-sidebar"
       ...
       aria-label="{if $sidebar_label == 'Admin sections'}管理員分區{elseif $sidebar_label == 'Bans sections'}封禁分區{elseif $sidebar_label == 'Group sections'}分組分區{elseif $sidebar_label == 'MOD sections'}模組分區{elseif $sidebar_label == 'Server sections'}伺服器分區{elseif $sidebar_label == 'Settings sections'}設定分區{elseif $sidebar_label == 'Page sections'}頁面分區{else}{$sidebar_label}{/if}">
    <details class="admin-sidebar__details" open>
        <summary class="admin-sidebar__summary">
            <span class="admin-sidebar__summary-label">
                ...
                {if $sidebar_label == 'Admin sections'}管理員分區{elseif $sidebar_label == 'Bans sections'}封禁分區{elseif $sidebar_label == 'Group sections'}分組分區{elseif $sidebar_label == 'MOD sections'}模組分區{elseif $sidebar_label == 'Server sections'}伺服器分區{elseif $sidebar_label == 'Settings sections'}設定分區{elseif $sidebar_label == 'Page sections'}頁面分區{else}{$sidebar_label}{/if}
            </span>
            <i data-lucide="chevron-down" class="admin-sidebar__chevron" style="width:14px;height:14px"></i>
        </summary>
        <nav class="admin-sidebar__nav" aria-label="{if $sidebar_label == 'Admin sections'}管理員分區{elseif $sidebar_label == 'Bans sections'}封禁分區{elseif $sidebar_label == 'Group sections'}分組分區{elseif $sidebar_label == 'MOD sections'}模組分區{elseif $sidebar_label == 'Server sections'}伺服器分區{elseif $sidebar_label == 'Settings sections'}設定分區{elseif $sidebar_label == 'Page sections'}頁面分區{else}{$sidebar_label}{/if}">
            {foreach from=$tabs item=tab}
                <a class="sidebar__link admin-sidebar__link"
                   ...
                   {if $tab.slug == $active_tab}aria-current="page"{/if}>
                    ...
                    <span>{if $tab.name == 'Admins list'}管理員列表{elseif $tab.name == 'Add admin'}新增管理員{elseif $tab.name == 'Add ban'}新增封禁{elseif $tab.name == 'Ban protests'}封禁申訴{elseif $tab.name == 'Ban submissions'}封禁舉報{elseif $tab.name == 'Import bans'}匯入封禁{elseif $tab.name == 'Groups'}分組{elseif $tab.name == 'Groups list'}分組列表{elseif $tab.name == 'Add group'}新增分組{elseif $tab.name == 'Mods list'}模組列表{elseif $tab.name == 'Add mod'}新增模組{elseif $tab.name == 'Server list'}伺服器列表{elseif $tab.name == 'Add server'}新增伺服器{elseif $tab.name == 'Server admins'}伺服器管理員{elseif $tab.name == 'Settings'}設定{elseif $tab.name == 'Features'}功能{elseif $tab.name == 'Logs'}日誌{elseif $tab.name == 'Themes'}主題{elseif $tab.name == 'Add comms block'}新增通訊封禁{else}{$tab.name}{/if}</span>
                </a>
            {/foreach}
        </nav>
    </details>
