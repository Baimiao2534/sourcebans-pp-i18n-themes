...
{if NOT $permission_protests}
    <div class="card" data-testid="protests-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">У вас нет прав на просмотр апелляций на баны.</p>
        </div>
    </div>
{else}
        ...
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                ...
                <p class="text-sm text-muted m-0 mt-2">
                    Разрешённые или истёкшие апелляции.
                </p>
            </div>
        ...
        {if $protest_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="protests-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">Архив апелляций пуст</h2>
                    <p class="empty-state__body">Принятые, отклонённые или архивированные апелляции будут перенесены сюда для протокола.</p>
                </div>
            </div>
        {else}
                    ...
                    <details class="queue-row ban-row ban-row--expired"
                        ...
                        <summary>
                            <div class="queue-row__body">
                                <div class="font-medium text-sm truncate" data-testid="protest-archive-row-name">
                                    ...
                                    {else}
                                        <i class="text-faint">бан удалён</i>
                                    {/if}
                                </div>
                        ...
                        <div class="p-4" id="ban_details_{$protest.pid}" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            <div class="text-sm font-medium mb-3">
                                Архивировано: {$protest.archive|escape}
                            </div>
                                ...
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    {if $protest.archiv != 2}
                                        ...
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.authid == ""}<span class="text-faint">SteamID отсутствует</span>{else}{$protest.authid|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">IP-адрес</dt>
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.ip == 'none' OR $protest.ip == ''}<span class="text-faint">IP-адрес отсутствует</span>{else}{$protest.ip|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">Наложен</dt>
                                        <dd style="margin:0">{$protest.date|escape}</dd>

                                        <dt class="text-muted">Дата окончания</dt>
                                        <dd style="margin:0">
                                            {if $protest.ends == 'never'}<span class="text-faint">Не применимо.</span>{else}{$protest.ends|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">Причина бана</dt>
                                        ...
                                        <dt class="text-muted">Забанил</dt>
                                        <dd style="margin:0">{$protest.admin|escape}</dd>

                                        <dt class="text-muted">Сервер</dt>
                                        <dd class="font-mono" style="margin:0">{$protest.server|escape}</dd>
                                    {/if}

                                    <dt class="text-muted">Архивировал</dt>
                                    <dd style="margin:0">
                                        {if !empty($protest.archivedby)}{$protest.archivedby|escape}{else}<i class="text-faint">Администратор удалён.</i>{/if}
                                    </dd>

                                    <dt class="text-muted">IP отправившего апелляцию</dt>
                                    <dd class="font-mono" style="margin:0">{$protest.pip|escape}</dd>

                                    <dt class="text-muted">Подана</dt>
                                    <dd style="margin:0">{$protest.datesubmitted|escape}</dd>

                                    <dt class="text-muted">Сообщение</dt>
                            ...
                            {if $protest.commentdata != "None"}
                                <div class="mt-4">
                                    <h4 class="text-xs text-muted" style="font-weight:600;margin:0 0 0.5rem;text-transform:uppercase;letter-spacing:0.06em">Комментарии</h4>
                                    <div class="space-y-3">
                                            ...
                                            <div class="card p-4">
                                                <div class="flex items-center justify-between gap-2 text-xs text-muted mb-2">
                                                    <strong style="color:var(--text);font-weight:600">
                                                        {if !empty($commenta.comname)}{$commenta.comname|escape}{else}<i class="text-faint">Администратор удалён</i>{/if}
                                                    </strong>
                                                ...
                                                {if !empty($commenta.edittime)}
                                                    <div class="text-xs text-faint mt-2">
                                                        последний раз изменён {$commenta.edittime|escape}, автор:
                                                        {if !empty($commenta.editname)}{$commenta.editname|escape}{else}<i>Администратор удалён</i>{/if}
                                                    </div>
                                                {/if}
                                            </div>
                                        {/foreach}
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var msg;
        if (archiv === '2') msg = 'Восстановить апелляцию на бан «' + key + '» из архива?';
        else if (archiv === '1') msg = 'Переместить апелляцию на бан «' + key + '» в архив?';
        else msg = 'Удалить апелляцию на бан «' + key + '»?';
        ...
        a.call(A.ProtestsRemove, { pid: pid, archiv: archiv }).then(function (r) {
            if (!r || r.ok === false) {
                setBusy(btn, false);
                toast('error', 'Ошибка действия', (r && r.error && r.error.message) || 'Неизвестная ошибка');
                return;
            }
            ...
            toast('success', 'Готово', 'Архив обновлён.');
        });
    });
})();
</script>
