...
{if NOT $permission_protests}
    <div class="card" data-testid="protests-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">Vous n&rsquo;avez pas la permission de consulter les appels de bannissement.</p>
        </div>
    </div>
{else}
        ...
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">
                    Archives des appels de bannissement
        ...
        {if $protest_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="protests-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">L&rsquo;archive des appels est vide</h2>
                    <p class="empty-state__body">Lorsque les appels seront acceptés, refusés ou archivés, ils apparaîtront ici pour archivage.</p>
                </div>
            </div>
        {else}
                    ...
                    <details class="queue-row ban-row ban-row--expired"
                                ...
                                <div class="font-medium text-sm truncate" data-testid="protest-archive-row-name">
                                    {if $protest.archiv != 2}
                                        <a class="link"
                                           ...
                                           title="Afficher le bannissement"
                                           onclick="event.stopPropagation();">{$protest.name|escape}</a>
                                    {else}
                                        <i class="text-faint">bannissement supprimé</i>
                                    {/if}
                                </div>
                        ...
                        <div class="p-4" id="ban_details_{$protest.pid}" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            <div class="text-sm font-medium mb-3">
                                Archivé car {$protest.archive|escape}
                            </div>
                                ...
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    {if $protest.archiv != 2}
                                        ...
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.authid == ""}<span class="text-faint">aucun steamid présent</span>{else}{$protest.authid|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">Adresse IP</dt>
                                        <dd class="font-mono" style="margin:0">
                                            {if $protest.ip == 'none' OR $protest.ip == ''}<span class="text-faint">aucune adresse IP présente</span>{else}{$protest.ip|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">Invoqué le</dt>
                                        <dd style="margin:0">{$protest.date|escape}</dd>

                                        <dt class="text-muted">Date de fin</dt>
                                        <dd style="margin:0">
                                            {if $protest.ends == 'never'}<span class="text-faint">Non applicable.</span>{else}{$protest.ends|escape}{/if}
                                        </dd>

                                        <dt class="text-muted">Motif du bannissement</dt>
                                        ...
                                        <dt class="text-muted">Banni par</dt>
                                    ...
                                    <dd style="margin:0">
                                        {if !empty($protest.archivedby)}{$protest.archivedby|escape}{else}<i class="text-faint">Administrateur supprimé.</i>{/if}
                                    </dd>

                                    <dt class="text-muted">IP de l&rsquo;appelant</dt>
                                    <dd class="font-mono" style="margin:0">{$protest.pip|escape}</dd>

                                    <dt class="text-muted">Appelé le</dt>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var msg;
        if (archiv === '2') msg = 'Restaurer l\'appel de bannissement pour "' + key + '" depuis l\'archive ?';
        else if (archiv === '1') msg = 'Déplacer l\'appel de bannissement pour "' + key + '" vers l\'archive ?';
        else msg = 'Supprimer l\'appel de bannissement pour "' + key + '" ?';
