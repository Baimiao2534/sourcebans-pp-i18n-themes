...
<div class="audit-page">
    <header class="audit-page__header">
        <div>
            <h1 class="audit-page__title">审计日志</h1>
            <p class="audit-page__subtitle">面板记录的每一次管理操作（共 {$total_count} {if $total_count == 1}条事件{else}条事件{/if}）。</p>
        </div>
    </header>
    ...
    <div class="audit-filters">
        <form method="get" action="index.php" role="search" aria-label="搜索审计日志" style="flex:1 1 16rem;max-width:22rem">
        ...
        <form method="get" action="index.php" class="flex items-center gap-2" style="flex-wrap:wrap" aria-label="按严重级别筛选审计日志">
            <input type="hidden" name="p" value="admin">
            <input type="hidden" name="c" value="audit">
        ...
        {foreachelse}
            ...
            <div class="empty-state"
                ...
                <p class="empty-state__body">
                    {if $current_severity != '' || $search != ''}
                        请尝试其他搜索词或严重级别，或清除筛选条件以查看所有已记录的管理操作。
                    {else}
                        管理员从面板开始执行操作后，每一次操作都会被记录在此供查阅。
                    {/if}
                </p>
                {if $current_severity != '' || $search != ''}
    ...
    {if $page_count > 1 || $current_page > 1}
        <nav class="audit-pager" aria-label="审计日志分页">
            <div class="audit-pager__status" aria-live="polite">
                第 {$current_page} 页 / 共 {$page_count} 页
            </div>
