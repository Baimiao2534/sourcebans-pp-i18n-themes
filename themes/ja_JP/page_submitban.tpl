...
<section class="p-6" style="max-width:48rem">
    <header class="mb-6">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">BAN リクエストを送信</h1>
        <p class="text-sm text-muted m-0 mt-2">
            チート、嫌がらせ、その他のルール違反のプレイヤーを報告してください。デモやスクリーンショットは管理者の迅速な対応に役立ちます。
        </p>
    </header>
    ...
    <form class="card"
        ...
        <div class="card__body space-y-4">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    Steam ID または IP アドレス（または両方）を入力してください。ニックネームと出来事の明確な説明は必須です。
                </p>
            </div>
            ...
            *}
            <div data-testid="submitban-id-or-ip"
                ...
                <p id="submitban-id-or-ip-help"
                   ...
                   data-state="info">
                    ...
                    <span class="text-muted" data-id-or-ip-text>Steam ID または IP アドレス（または両方）を入力してください。</span>
                </p>
                <div>
                    <label class="label" for="submitban-steam">
                        プレイヤーの Steam ID
                        <span class="text-muted text-xs" style="font-weight:400">（または下の IP を使用）</span>
                    </label>
                    {*
                ...
                <div>
                    <label class="label" for="submitban-ip">
                        プレイヤーの IP アドレス
                        <span class="text-muted text-xs" style="font-weight:400">（または上の Steam ID を使用）</span>
                    </label>
                    <input type="text"
                           class="input font-mono"
                           id="submitban-ip"
            ...
            <div>
                <label class="label" for="submitban-name">
                    プレイヤーのニックネーム <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <input type="text"
                       class="input"
                       id="submitban-name"
            ...
            <div>
                <label class="label" for="submitban-reason">
                    コメント <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                ...
                <p id="submitban-reason-help" class="text-xs text-muted m-0 mt-2">
                    具体的に記述してください。「チート」だけでは不十分です。何を、いつ、どのサーバーで見たかを説明してください。
                </p>
            </div>
        </div>
        ...
        <div class="card__body space-y-4" style="border-top:1px solid var(--border)">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    管理者が追加の質問のために連絡できるようにするためです。
                </p>
            </div>

            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div>
                    <label class="label" for="submitban-reporter-name">あなたの名前</label>
                    <input type="text"
                           class="input"
                ...
                <div>
                    <label class="label" for="submitban-reporter-email">
                        メールアドレス <span style="color:var(--danger)" aria-hidden="true">*</span>
                    </label>
                    <input type="email"
                           class="input"
            ...
            <div>
                <label class="label" for="submitban-server">
                    サーバー <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <select class="select"
                        id="submitban-server"
                        name="server"
                    ...
                    <option value="-1">&mdash; サーバーを選択 &mdash;</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"{if $server_selected == $server.sid} selected{/if}>{$server.hostname}</option>
                    {/foreach}
                    <option value="0"{if $server_selected == 0} selected{/if}>その他のサーバー / リストにない</option>
                </select>
            </div>

            <div>
                <label class="label" for="submitban-demo">デモまたは証拠をアップロード</label>
                <div class="file-input">
                    ...
                    <span class="text-muted text-sm" data-file-name>ファイルが選択されていません</span>
                </div>
                <p class="text-xs text-muted m-0 mt-2">
                    任意。許可される形式：<code class="font-mono">.dem</code>、
        ...
        <div class="card__body flex items-center justify-between gap-2"
            ...
            <p class="text-xs text-muted m-0">
                <span style="color:var(--danger)" aria-hidden="true">*</span> 必須項目
            </p>
            <div class="flex gap-2">
                <a class="btn btn--ghost"
                   href="index.php?p=banlist"
                   data-testid="submitban-cancel">キャンセル</a>
...
(function () {
    ...
    function setError(isError) {
        ...
        if (isError) {
            ...
            helpText.textContent = '送信前に Steam ID または IP アドレスを入力してください。';
        } else {
