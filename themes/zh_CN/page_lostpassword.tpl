...
<div class="lostpw">
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                <h3>忘记密码？</h3>
                <p>输入与管理员账户关联的电子邮件地址，我们将向您发送重置密码的链接。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                {csrf_field}
                <div>
                    <label class="label" for="lostpw-email">电子邮件地址</label>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> 发送重置链接
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; 返回登录</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || '未知错误';
                toast('error', '错误', emsg);
                return;
            }
