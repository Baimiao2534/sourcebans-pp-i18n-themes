...
{else}
    {if NOT $groupbanning_enabled}
        <div class="card" data-testid="groupban-disabled">
            <div class="card__body">
                <h1 style="font-size:1.25rem;font-weight:600;margin:0">機能が無効です</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    グループ BAN は
                    <strong>config.enablegroupbanning</strong>
                    で無効になっています。この機能を使用するには設定タブから再度有効化してください。
                </p>
            </div>
        </div>
    {else}
        <section class="p-6" data-testid="groupban-section" style="max-width:48rem">
            <div class="mb-6">
                <h1 style="font-size:1.5rem;font-weight:600;margin:0">グループ BAN</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    Steam コミュニティグループの全メンバーを一括で BAN します。
                </p>
            </div>
            ...
            {if NOT $list_steam_groups}
                <form id="groupban-form"
                    ...
                    {csrf_field}
                    <div>
                        <label class="label" for="groupurl">Steam コミュニティグループの URL</label>
                        <input type="text"
                    ...
                    <div>
                        <label class="label" for="groupreason">グループ BAN の理由</label>
                        <textarea class="textarea"
                                  id="groupreason"
                                  name="groupreason"
                                  rows="4"
                                  placeholder="このグループを BAN する理由は何ですか？"
            ...
            {else}
                <div class="card p-6 space-y-4" data-testid="groupban-from-player">
                    <p class="text-sm text-muted m-0">
                        {if $player_name}
                            <strong class="font-medium">{$player_name|escape}</strong> が所属しているグループは以下に読み込まれます。
                        {else}
                            プレイヤーが所属しているすべてのグループがここに表示されます。
                        {/if}
                        BAN したいグループにチェックを入れてください。
                    </p>

                    <div id="steamGroupsText"
                         name="steamGroupsText"
                         class="text-sm text-muted">グループを読み込み中…</div>
                ...
                (function (friendid) {
                    sb.ready(function () {
                        sb.api.call(Actions.BansGetGroups, { friendid: friendid }).then(function (r) {
                            ...
                            if (!tbl) return;
                            if (groups.length === 0) {
                                sb.message.error('エラー', "グループデータの取得中にエラーが発生しました。プレイヤーがグループに所属していないか、プロフィールが非公開の可能性があります。", 'index.php?p=banlist');
                                var txt = sb.$id('steamGroupsText');
                                if (txt) txt.innerHTML = '<i>グループがありません...</i>';
                                return;
                            }
                            groups.forEach(function (g, i) {
                                ...
                                td2.appendChild(document.createTextNode(' メンバー)'));
                            });
