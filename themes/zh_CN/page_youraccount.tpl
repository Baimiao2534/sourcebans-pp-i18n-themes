...
<div class="p-6 space-y-6" style="max-width:64rem;margin:0 auto;width:100%">
    ...
    <header data-testid="account-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">您的账户</h1>
        <p class="text-sm text-muted m-0 mt-2">权限、密码、服务器密码和邮箱。</p>
    </header>
    ...
    *}
                ...
                {if $server_permissions}
                    <div class="permissions-grid permissions-grid--server">
                        <section class="permissions-group"
                            ...
                            <h5 class="permissions-group__title">游戏服务器权限</h5>
                            <ul class="permissions-group__list">
    ...
    <section class="card" data-testid="account-change-password">
        <div class="card__header">
            <div>
                <h3>修改密码</h3>
                <p>至少使用 {$min_pass_len} 个字符{if $min_pass_len != 1}s{/if}。成功修改后您将被登出。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-password-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-current-password">当前密码</label>
                ...
                <div>
                    <label class="label" for="account-new-password">新密码</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-password">确认新密码</label>
                    <input class="input" type="password"
    ...
    <section class="card" data-testid="account-server-password">
        <div class="card__header">
            <div>
                <h3>服务器密码</h3>
                <p>您需要此密码在游戏服务器上使用管理员权限。
                    <a href="https://wiki.alliedmods.net/Adding_Admins_%28SourceMod%29#Passwords"
                       target="_blank" rel="noopener"
                       style="color:var(--accent);text-decoration:underline">SourceMod 文档</a>。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-srv-password-form"
                ...
                {if $srvpwset}
                    <div>
                        <label class="label" for="account-current-srv-password">当前服务器密码</label>
                        <input class="input" type="password"
                ...
                {/if}
                <div>
                    <label class="label" for="account-new-srv-password">新服务器密码</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-srv-password">确认新服务器密码</label>
                    <input class="input" type="password"
    ...
    <section class="card" data-testid="account-change-email">
        <div class="card__header">
            <div>
                ...
                <p>当前地址：
                    ...
                    {else}
                        <span class="text-sm text-muted" data-testid="account-current-email-empty">（未设置）</span>
                    {/if}
                </p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-email-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-email-password">密码</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-email">新邮箱地址</label>
                ...
                <div>
                    <label class="label" for="account-email-confirm">确认新邮箱地址</label>
                    <input class="input" type="email"
...
(function () {
    ...
    function showFieldError(prefix, err, fieldMap) {
        ...
        setMsg(prefix + domId + '-msg', err.message || '无效的值。');
        return true;
    }
    ...
    function flashSuccess(envelope) {
        ...
        showToast('success', m.title || '已保存', m.body || '');
        if (m.redir) {
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (next.length < minLen) {
                setMsg('account-new-password-msg', '密码至少需要 ' + minLen + ' 个字符。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-password-msg', '两次输入的密码不一致。');
                bad = true;
            }
            if (current.length === 0) {
                setMsg('account-current-password-msg', '请输入您的当前密码。');
                bad = true;
            }
            if (bad) return;
    ...
    if (srvForm) {
        srvForm.addEventListener('submit', function (ev) {
            ...
            var bad = false;
            if (next.length < minLen) {
                setMsg('account-new-srv-password-msg', '密码至少需要 ' + minLen + ' 个字符。');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-srv-password-msg', '两次输入的密码不一致。');
                bad = true;
            }
            if (hasExisting && current.length === 0) {
                setMsg('account-current-srv-password-msg', '请输入您的当前服务器密码。');
                bad = true;
            }
            if (bad) return;
            ...
            if (hasExisting) {
                sb.api.call(Actions.AccountCheckSrvPassword, {
                    ...
                    if (!env.data.matches) {
                        setBusy(srvBtn, false);
                        setMsg('account-current-srv-password-msg', '服务器密码不正确。');
                        return;
                    }
                    send();
                });
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', function (ev) {
            ...
            if (email.length === 0) {
                setMsg('account-email-msg', '请输入您的新邮箱地址。');
                bad = true;
            }
            if (confirm.length === 0) {
                setMsg('account-email-confirm-msg', '请确认您的新邮箱地址。');
                bad = true;
            }
            if (!bad && email !== confirm) {
                setMsg('account-email-confirm-msg', '两次输入的邮箱地址不一致。');
                bad = true;
            }
            if (pw.length === 0) {
                setMsg('account-email-password-msg', '请输入您的密码。');
                bad = true;
            }
            if (bad) return;
