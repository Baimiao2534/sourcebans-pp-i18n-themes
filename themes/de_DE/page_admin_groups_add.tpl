...
<div style="max-width:48rem">
    <div class="mb-4">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Gruppe erstellen</h1>
        <p class="text-sm text-muted m-0 mt-2">Wählen Sie einen Namen und einen Typ. Sie können Berechtigungs-Flags im Tab <strong>Gruppen auflisten</strong> bearbeiten, sobald die Gruppe existiert.</p>
    </div>
    <form class="card"
          method="post"
        ...
        <div class="card__body space-y-4">
            <div>
                <label class="label" for="add-group-name">Gruppenname</label>
                <input class="input"
                       id="add-group-name"
                       name="name"
                       data-testid="add-group-name"
                       autocomplete="off"
                       placeholder="z. B. Senior-Administratoren"
                       required>
                <p class="text-xs text-muted m-0 mt-2">Muss eindeutig sein. Keine Kommas.</p>
            </div>

            <div>
                <label class="label" for="add-group-type">Gruppentyp</label>
                <select class="select"
                        id="add-group-type"
                        name="type"
                        data-testid="add-group-type"
                        onchange="SbppGroupsAddTypeChanged(this);">
                    <option value="0">Bitte auswählen &hellip;</option>
                    <option value="1">Web-Administratorgruppe</option>
                    <option value="2">Server-Administratorgruppe</option>
                    <option value="3">Servergruppe</option>
                </select>
                <p class="text-xs text-muted m-0 mt-2">Web-Administrator = Panel-Berechtigungen. Server-Administrator = SourceMod-Zeichen-Flags. Servergruppe = Gruppierung von Spielservern.</p>
            </div>

            <div data-testid="add-group-srvflags-block" id="add-group-srvflags-block" style="display:none">
                <label class="label" for="add-group-srvflags">SourceMod-Flags &amp; Immunität</label>
                <input class="input"
                ...
                <p class="text-xs text-muted m-0 mt-2">SourceMod-Flag-String. Hängen Sie <code>#&lt;immunity&gt;</code> für Immunität an (Standard 0).</p>
            </div>
        </div>
        <div class="card__header" style="border-top:1px solid var(--border);border-bottom:0;justify-content:flex-end">
            <div class="flex gap-2">
                <a class="btn btn--ghost" href="?p=admin&c=groups&section=list" data-testid="add-group-cancel">Abbrechen</a>
                <button class="btn btn--primary" type="submit" data-testid="add-group-submit">Gruppe erstellen</button>
            </div>
        </div>
    </form>
</div>
    ...
    sb.api.call(Actions.GroupsAdd, {
        ...
        if (r.ok === false) {
            ...
            if (window.SBPP && typeof window.SBPP.showToast === 'function') {
                ...
                sb.message.error('Fehler', em);
            }
            return;
        }
        var data = r.data || {};
        var body = (data.message && data.message.body) || 'Gruppe hinzugefügt.';
        var title = (data.message && data.message.title) || 'Gruppe hinzugefügt';
        if (window.SBPP && typeof window.SBPP.showToast === 'function') {
