    ...
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                ...
                <p>Saisissez l&rsquo;adresse e-mail associée à votre compte administrateur et nous vous enverrons un lien pour réinitialiser votre mot de passe.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                {csrf_field}
                <div>
                    <label class="label" for="lostpw-email">Adresse e-mail</label>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> Envoyer le lien de réinitialisation
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; Retour à la connexion</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (!emailEl.value) {
            toast('warn', 'E-mail requis', 'Veuillez saisir votre adresse e-mail.');
            return;
        }
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || 'Erreur inconnue';
                toast('error', 'Erreur', emsg);
                return;
            }
            var msg = (res.data || {}).message;
            if (msg) {
                toast(mapKind(msg.kind), msg.title || 'Terminé', msg.body || '');
            }
        });
    });
}());
