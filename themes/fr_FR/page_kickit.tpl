...
<head>
    ...
    <title>-{if $mode === 'kick'}-Expulser le joueur-{else}-Bannir le joueur-{/if}-</title>
    <script>
    (function () {
        try {
...
<div id="container" class="card" data-testid="kickit-container" data-mode="-{$mode}-">
    <div class="card__header">
        <div>
            <h3>Recherche du joueur sur tous les serveurs&hellip;</h3>
            <p>Chaque ligne est interrogée en direct ; les lignes se mettent à jour au fur et à mesure que les serveurs répondent.</p>
        </div>
    </div>
                ...
                <tr data-testid="kickit-row--{$serv.num}-">
                    ...
                    <td>
                        <div id="srv_-{$serv.num}-"
                             ...
                             data-testid="kickit-status--{$serv.num}-">En attente&hellip;</div>
                    </td>
                </tr>
                -{/foreach}-
            </tbody>
    ...
    (function () {
        ...
        function setCounter(count) {
            ...
            if (srvcount === TOTAL || count === -1) {
                ...
                if (ctl) {
                    ctl.innerHTML = '<font color="green" style="font-size: 12px;"><b>Recherche terminée.</b></font>' + ctl.innerHTML;
        ...
        function processRow(sid, num) {
            sb.api.call(Actions.KickitKickPlayer, { check: CHECK, sid: sid, num: num, type: TYPE, mode: MODE })
                .then(function (r) {
                    if (!r || !r.ok || !r.data) {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>Erreur.</i></span>");
                        setCounter(1);
                        return;
                    }
                    var d = r.data;
                    ...
                    if (d.status === 'no_connect') {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>Impossible de se connecter au serveur.</i></span>");
                        setCounter(1);
                    } else if (d.status === 'kicked') {
                        sb.setHTML('srv_' + num, "<span class='text-xs font-semibold' style='color:var(--success)'><u>Joueur trouvé et expulsé !</u></span>");
                        setCounter(-1);
                    } else {
                        sb.setHTML('srv_' + num, "<span class='text-xs text-muted'>Joueur introuvable.</span>");
                        setCounter(1);
                    }
                });
        }
        ...
        window.addEventListener('load', function () {
            ...
            sb.api.call(Actions.KickitLoadServers, {}).then(function (r) {
                ...
                r.data.servers.forEach(function (s) {
                    if (s.has_rcon) {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-muted">Recherche&hellip;</span>');
                        processRow(s.sid, s.num);
                    } else {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-faint">Pas de mot de passe rcon.</span>');
                        setCounter(1);
                    }
                });
            });
