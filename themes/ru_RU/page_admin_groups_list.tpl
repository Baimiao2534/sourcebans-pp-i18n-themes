...
{if NOT $permission_listgroups}
    <div class="card"><div class="card__body"><p class="text-muted m-0">Доступ запрещён.</p></div></div>
{else}
...
<div class="space-y-6">
    ...
    <section data-testid="web-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Веб-группы администраторов</h1>
                <p class="text-sm text-muted m-0 mt-2">Флаги прав и иммунитет для групп веб-панели. Всего: {$web_group_count}.</p>
            </div>
        </div>
        ...
        {if $web_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-web" data-filtered="false">
                ...
                <h2 class="empty-state__title">Веб-групп администраторов ещё нет</h2>
                <p class="empty-state__body">Веб-группы администраторов объединяют права панели для набора администраторов. Создайте группу, чтобы назначить нескольким администраторам одни и те же флаги сразу.</p>
                    ...
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           ...
                           data-testid="admin-groups-empty-web-add">
                            ...
                            Добавить веб-группу администратора
                        </a>
                    </div>
                {/if}
            </div>
                    ...
                    <a class="admin-groups-master-detail__row flex items-center gap-3 p-4"
                       ...
                       {if $selected_group && $selected_group.gid == $group.gid}aria-current="true"{/if}>
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="text-xs text-muted">{$web_admins[$smarty.foreach.web_group.index]} участник{if $web_admins[$smarty.foreach.web_group.index] != 1}ов{/if}</div>
                ...
                <form class="card"
                    ...
                    <div class="card__header">
                        <div>
                            ...
                            <p>{$selected_group.member_count} участник{if $selected_group.member_count != 1}ов{/if}</p>
                        </div>
                        {if $permission_deletegroup}
                            <button type="button"
                                    class="btn btn--ghost btn--sm"
                                    data-testid="group-delete"
                                    onclick="SbppGroupsDelete({$selected_group.gid}, '{$selected_group.name|escape:'javascript'}', this);">Удалить группу</button>
                        {/if}
                    </div>
                    <div class="card__body space-y-4">
                        ...
                        <div>
                            <label class="label" for="group-name-input">Название группы</label>
                            <input class="input"
                                   id="group-name-input"
                                   name="name"
                        ...
                        <div>
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <label class="label m-0">Флаги прав</label>
                                ...
                                <span class="text-xs text-muted" data-testid="flag-bitmask">{$selected_group.flags} битовая маска</span>
                            </div>
                        ...
                        {if $permission_editgroup}
                            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                                <a class="btn btn--ghost" href="?p=admin&c=groups">Отменить</a>
                                <button class="btn btn--primary"
                                        type="submit"
                                        data-testid="group-save">Сохранить изменения</button>
                            </div>
                        {/if}
                    </div>
                </form>
            {else}
                <div class="card"><div class="card__body"><p class="text-muted m-0">Выберите группу слева, чтобы изменить её флаги.</p></div></div>
            {/if}
        </div>
        {/if}
    </section>
    ...
    <section data-testid="server-admin-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">Серверные группы администраторов</h2>
                <p class="text-sm text-muted m-0 mt-2">Группы администраторов SourceMod (внутриигровые флаги). Всего: {$server_admin_group_count}.</p>
            </div>
        </div>
        ...
        {if $server_admin_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server-admin" data-filtered="false">
                ...
                <h2 class="empty-state__title">Серверных групп администраторов ещё нет</h2>
                <p class="empty-state__body">Серверные группы администраторов содержат char-флаги SourceMod и иммунитет. Создайте группу, чтобы предоставить внутриигровые права администратора набору администраторов.</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                           data-testid="admin-groups-empty-server-admin-add">
                            <i data-lucide="plus" style="width:13px;height:13px"></i>
                            Добавить серверную группу администратора
                        </a>
                    </div>
                {/if}
            </div>
                    ...
                    <article class="card" data-testid="server-admin-group-row" data-id="{$group.id}">
                        <div class="card__header">
                            <div>
                                ...
                                <p>{$server_admins[$smarty.foreach.server_admin_group.index]} участник{if $server_admins[$smarty.foreach.server_admin_group.index] != 1}ов{/if} &middot; иммунитет {$group.immunity}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=srv&id={$group.id|escape:'url'}">Изменить</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.id}, '{$group.name|escape:'javascript'}', 'srv', this);">Удалить</button>
                                {/if}
                            </div>
                        </div>
                        <div class="card__body space-y-3">
                            <div>
                                <div class="text-xs font-semibold text-muted mb-2">Права</div>
                                ...
                                {else}
                                    <p class="text-xs text-muted m-0"><em>Нет</em></p>
                                {/if}
                            </div>
                            {if $server_admins_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">Участники</div>
                                        ...
                                        {foreach from=$server_admins_list[$smarty.foreach.server_admin_group.index] item="server_admin"}
                                            <li class="flex items-center justify-between gap-2 text-sm">
                                                ...
                                                {if $permission_editadmin}
                                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=admins&o=editgroup&id={$server_admin.aid|escape:'url'}">Изменить</a>
                                                {/if}
                                            </li>
                                        {/foreach}
                                    </ul>
                            ...
                            {if $server_overrides_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">Переопределения</div>
    ...
    <section data-testid="server-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">Серверные группы</h2>
                <p class="text-sm text-muted m-0 mt-2">Объединения игровых серверов (без флагов прав). Всего: {$server_group_count}.</p>
            </div>
        </div>
        ...
        {if $server_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server" data-filtered="false">
                ...
                <h2 class="empty-state__title">Серверных групп ещё нет</h2>
                <p class="empty-state__body">Серверные группы объединяют игровые серверы, чтобы можно было назначать администраторов сразу на несколько серверов. Создайте группу, чтобы начать группировку серверов.</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                ...
                {foreach from=$server_list item="group" name="server_group"}
                    <article class="card" data-testid="server-group-row" data-id="{$group.gid}">
                        <div class="card__header">
                            <div>
                                <h3>{$group.name|escape}</h3>
                                <p>{$server_counts[$smarty.foreach.server_group.index]} сервер{if $server_counts[$smarty.foreach.server_group.index] != 1}ов{/if}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=server&id={$group.gid|escape:'url'}">Изменить</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.gid}, '{$group.name|escape:'javascript'}', 'server', this);">Удалить</button>
                                {/if}
                            </div>
                        </div>
                        {*
                        ...
                        <div class="card__body">
                            ...
                            {if empty($group.servers)}
                                <p class="text-xs text-muted m-0" data-testid="server-group-empty">
                                    <em>К этой группе ещё не привязано ни одного сервера.</em>
                                </p>
                            {else}
                                <ul class="space-y-2"
                                        ...
                                        <li class="flex items-center gap-2"
                                            ...
                                            {if !$server.enabled}
                                                <span class="pill pill--offline"
                                                      ...
                                                      title="Отключён — скрыт из публичных списков и пропускается средством гидратации карточек"
                                                      style="flex-shrink:0">Отключён</span>
                                            {/if}
                                        </li>
                                    {/foreach}
                                </ul>
...
function SbppGroupsApplyResponse(r, opts) {
    ...
    var fallback = (opts && opts.defaultTitle) || 'Готово';

    if (r.ok === false) {
        var em = (r.error && r.error.message) || 'Неизвестная ошибка';
        if (window.SBPP && typeof window.SBPP.showToast === 'function') {
            window.SBPP.showToast({ kind: 'error', title: 'Ошибка', body: em });
        } else if (window.sb && window.sb.message) {
            window.sb.message.error('Ошибка', em);
        }
        return;
    }
...
function SbppGroupsSave(event) {
    ...
    sb.api.call(Actions.GroupsEdit, {
        ...
        SbppGroupsApplyResponse(r, { defaultTitle: 'Группа обновлена' });
    });
    return false;
}
    ...
    sb.api.call(Actions.GroupsRemove, { gid: Number(gid), type: 'web' })
        .then(function (r) {
            ...
            if (r && r.ok && (r.data && (r.data.reload || (r.data.message && r.data.message.redir)))) {
                SbppGroupsApplyResponse(r, { defaultTitle: 'Группа удалена' });
                return;
            }
...
(function () {
    ...
    grid.addEventListener('change', function (event) {
        ...
        preview.textContent = SbppFoldFlags(grid) + ' битовая маска';
    });
})();
{/literal}
</script>
