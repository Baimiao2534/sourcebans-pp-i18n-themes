        ...
        <span class="filters-details__summary-label">
            ...
            {if $active_filter_count > 0}
                <span class="filters-details__count"
                      ...
                      aria-label="{$active_filter_count} 件のアクティブフィルター">
                    &middot; {$active_filter_count} 件アクティブ
                </span>
            {/if}
        </span>
...
<form method="get"
    ...
    *}
    <div class="card__header filters-details__header">
        <div>
            <p>以下のフィルターを組み合わせ — サーバーは<strong>すべての</strong>入力済みフィルターに一致する管理者リストに絞り込みます (AND セマンティクス)。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-name" style="grid-column:1;align-self:end">ログイン名</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <input class="input"
                       id="search-admins-name"
                       name="name"
                       type="text"
                       placeholder="パネルログイン名でマッチ&hellip;"
                ...
                <select class="select"
                        ...
                        aria-label="ログイン名のマッチモード"
                        style="width:9rem">
                    <option value="0"{if $active_filter_name_match == '0'} selected{/if}>完全一致</option>
                    <option value="1"{if $active_filter_name_match != '0'} selected{/if}>部分一致</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        aria-label="Steam ID のマッチモード"
                        style="width:9rem">
                    <option value="0"{if $active_filter_steam_match != '1'} selected{/if}>完全一致</option>
                    <option value="1"{if $active_filter_steam_match == '1'} selected{/if}>部分一致</option>
                </select>
            </div>
        </div>
        ...
        {if $can_editadmin}
            <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
                <label class="label" for="search-admins-admemail" style="grid-column:1;align-self:end">メールアドレス</label>
                <div class="flex gap-2" style="flex-wrap:wrap">
                    <input class="input"
                           id="search-admins-admemail"
                           name="admemail"
                           type="email"
                           placeholder="パネルのメールアドレスでマッチ&hellip;"
                    ...
                    <select class="select"
                            ...
                            aria-label="メールアドレスのマッチモード"
                            style="width:9rem">
                        <option value="0"{if $active_filter_admemail_match == '0'} selected{/if}>完全一致</option>
                        <option value="1"{if $active_filter_admemail_match != '0'} selected{/if}>部分一致</option>
                    </select>
                </div>
            </div>
        {/if}
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-webgroup">ウェブグループ</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvadmgroup">SourceMod 管理者グループ</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvgroup">サーバーグループ</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admwebflag">ウェブ権限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admsrvflag">サーバー権限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-server">サーバー</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
                    ...
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                ...
                                data-server-host{if $active_filter_server == $server.sid && $active_filter_server != ''} selected{/if}>読み込み中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
        </div>
        ...
        <div class="flex gap-2 justify-end" style="border-top:1px solid var(--border);padding-top:0.75rem">
            <a class="btn btn--ghost btn--sm"
               ...
               data-testid="search-admins-reset">フィルターをクリア</a>
            <button type="submit"
                    class="btn btn--primary btn--sm"
                    data-testid="search-admins-submit">
                <i data-lucide="search" style="width:14px;height:14px"></i> 検索
            </button>
        </div>
    </div>
</form>
