...
<div class="lostpw">
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                <h3>パスワードをお忘れですか？</h3>
                <p>管理者アカウントに関連付けられたメールアドレスを入力すると、パスワードをリセットするリンクをお送りします。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> リセットリンクを送信
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; ログインに戻る</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (!emailEl.value) {
            toast('warn', 'メールアドレスが必要です', 'メールアドレスを入力してください。');
            return;
        }
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || '不明なエラー';
                toast('error', 'エラー', emsg);
                return;
            }
            var msg = (res.data || {}).message;
            if (msg) {
                toast(mapKind(msg.kind), msg.title || '完了', msg.body || '');
            }
        });
    });
}());
