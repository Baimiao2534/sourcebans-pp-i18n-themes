...
<div class="p-6 space-y-4" style="max-width:1700px;margin:0 auto;width:100%">
    ...
    <header class="flex items-center justify-between gap-4" style="flex-wrap:wrap" data-testid="comms-header">
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">コミュニケーション制限</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <span class="tabular-nums" data-testid="comms-count">{$ban_list|@count|number_format}</span>
                / 全 <span class="tabular-nums">{$total_bans|number_format}</span> 件
            </p>
        </div>
        <div class="flex gap-2">
            ...
            <a class="btn btn--secondary btn--sm"
               ...
               data-testid="toggle-hide-inactive">
                ...
                {if $is_active_only}無効を表示{else}無効を非表示{/if}
            </a>
            {if $can_add_comm}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=comms"
               data-testid="comms-add-button">
                <i data-lucide="plus"></i> コミュニケーション制限を追加
            </a>
            {/if}
        </div>
    </header>
    ...
    <form method="get" action="index.php"
        ...
        <div class="flex gap-3" style="flex-wrap:wrap">
            <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
                ...
                <input class="input input--with-icon"
                       ...
                       placeholder="プレイヤー、SteamID または理由…"
                       data-testid="comms-search">
            </div>
            <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="comms-server-filter" aria-label="サーバーで絞り込み">
                <option value="">すべてのサーバー</option>
                {foreach $servers as $s}
                    <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
                {/foreach}
            </select>
            <select class="select" name="time" style="width:auto" data-testid="comms-time-filter" aria-label="期間で絞り込み">
        ...
        <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="クイックフィルター">
            ...
            <button class="chip" type="submit" name="type" value="mute"
                ...
                <i data-lucide="mic-off" style="width:12px;height:12px"></i> ミュート
            </button>
            <button class="chip" type="submit" name="type" value="gag"
                ...
                <i data-lucide="message-square-off" style="width:12px;height:12px"></i> ギャグ
            </button>
            <button class="chip" type="submit" name="type" value="silence"
                ...
                <i data-lucide="volume-x" style="width:12px;height:12px"></i> サイレンス
            </button>
        </div>
    </form>
        ...
        <table class="table" data-testid="comms-table">
            <thead>
                <tr>
                    ...
                    <th>プレイヤー</th>
                    <th>SteamID</th>
                    <th class="col-length col-tier-3">期間</th>
                    <th class="col-tier-2">サーバー</th>
                    <th class="col-admin col-tier-2">管理者</th>
                    <th class="col-tier-3">開始日時</th>
                            ...
                            <div class="flex items-center gap-3" style="min-width:0">
                                ...
                                <div style="min-width:0">
                                    ...
                                    <a class="font-medium truncate"
                                       ...
                                       data-testid="drawer-trigger">{if $comm.name}{$comm.name|escape}{else}<i class="text-faint">ニックネームなし</i>{/if}</a>
                                    ...
                                    <details class="ban-comments-inline mt-1"
                                      ...
                                      <summary class="ban-comments-inline__summary"
                                               ...
                                               title="{$comm.commentdata|@count} 件のコメント">
                                        ...
                                        <span class="ban-comments-inline__label">コメント</span>
                                      </summary>
                            ...
                            <div class="row-actions">
                                ...
                                {if $can_unmute_gag && $comm.unmute_url}
                                    ...
                                    <button type="button"
                                        ...
                                        {if $comm.type == 'mute'}ミュート解除{elseif $comm.type == 'gag'}ギャグ解除{else}制限を解除{/if}
                                    </button>
                    ...
                    <tr>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">フィルターに一致するコミュニケーション制限はありません</h2>
                                <p class="empty-state__body">別の検索キーワード、サーバー、または期間を試すか、フィルターをクリアしてすべてのミュート/ギャグを表示してください。</p>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">コミュニケーション制限はまだ記録されていません</h2>
                                <p class="empty-state__body">パネルまたはゲーム内から発行されたミュートおよびギャグはここに表示されます。</p>
                                {if $can_add_comm}
                                <div class="empty-state__actions">
                                    <a class="btn btn--primary btn--sm"
        ...
        <div class="ban-cards">
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-sm truncate">
                                    {if $comm.name}{$comm.name|escape}{else}<span class="text-faint">ニックネームなし</span>{/if}
                                </span>
            ...
            {foreachelse}
                ...
                <div class="empty-state" data-testid="comms-empty-mobile" data-filtered="true">
                    ...
                    <h2 class="empty-state__title">フィルターに一致するコミュニケーション制限はありません</h2>
                    <p class="empty-state__body">別の検索キーワードを試すか、フィルターをクリアしてください。</p>
                    <div class="empty-state__actions">
...
<dialog id="comms-unblock-dialog"
  ...
  <form method="dialog" data-testid="comms-unblock-form">
    <h2 id="comms-unblock-dialog-title" data-testid="comms-unblock-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">制限を解除</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      なぜ<strong data-testid="comms-unblock-target">このプレイヤー</strong>の制限を解除するのですか？この理由は制限に対して記録され、監査ログに表示されます。
    </p>
    <label class="label" for="comms-unblock-reason">理由 <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="誤った制限、申し立てが承認された、刑期が終了した、&hellip;"></textarea>
    <p class="text-xs" data-testid="comms-unblock-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="comms-unblock-cancel" value="cancel">キャンセル</button>
...
(function () {
    ...
    function flipRowToUnmuted(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === prev || txt === 'active' || txt === 'permanent' || txt === 'expired'
                ...
                if (lastText) lastText.textContent = ' 解除済み';
                else pill.textContent = '解除済み';
            }
        });
        Array.prototype.forEach.call(
            ...
            function (btn) {
                ...
                a.innerHTML = '<i data-lucide="rotate-ccw" style="width:13px;height:13px"></i> 再適用';
    ...
    function copyForType(type) {
        if (type === 'mute')    return { title: 'ミュート解除',       verb: 'ミュート解除' };
        if (type === 'gag')     return { title: 'ギャグ解除',         verb: 'ギャグ解除' };
        if (type === 'silence') return { title: 'サイレンス解除',     verb: 'サイレンス解除' };
        return                          { title: '制限を解除',       verb: '制限を解除' };
    }
    ...
    function openUnblockDialog(ctx) {
        ...
        if (prompt) prompt.textContent = ctx.name || ('制限 #' + ctx.bid);
        ...
        if (p && p.firstChild && p.firstChild.nodeType === 3) {
            p.firstChild.textContent = 'なぜ' + copy.verb + 'を行いますか： ';
        }
    ...
    document.addEventListener('click', function (e) {
        ...
        var bid = btn.getAttribute('data-bid') || '';
        var name = btn.getAttribute('data-name') || ('制限 #' + bid);
        ...
        if (act === 'comms-delete') {
            if (!window.confirm('「' + name + '」の制限を完全に削除しますか？')) return;
            setBusy(btn, true);
            a.call(A.CommsDelete, { bid: Number(bid) }).then(function (r) {
                ...
                decrementCount();
                toast('success', '制限を削除しました', '「' + name + '」の制限は削除されました。');
            });
            return;
        }
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.CommsUnblock, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            ...
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '不明なエラー';
                showError(msg);
                toast('error', '解除に失敗しました', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnmuted);
            closeUnblockDialog();
            toast('success', '制限を解除しました', ctx.name + ' の制限は解除されました。');
        });
    });
