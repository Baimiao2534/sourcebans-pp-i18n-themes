    ...
    {if NOT $permission_addserver}
        <div class="card" data-testid="server-add-denied">
            <div class="card__body">
                <h3 style="margin:0 0 0.25rem">访问被拒绝</h3>
                <p class="text-sm text-muted m-0">你没有添加服务器的权限。</p>
            </div>
        </div>
    {else}
        <form id="addserver-form"
            ...
            <div class="card__header">
                <div>
                    <h3>{if $edit_server}编辑服务器{else}添加服务器{/if}</h3>
                    <p>提示：将鼠标悬停在问号上可查看内联帮助。RCON 为可选项，但可启用实时控制台和服务器级管理员映射。</p>
                </div>
            </div>
            <div class="card__body" style="display:grid;gap:1rem">
                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">服务器 IP / 域名</span>
                        <input type="text"
                               id="address"
                               name="address"
                        ...
                        <span class="text-xs text-muted">支持 IPv4 / IPv6 / 主机名。主机名由游戏服务器在连接时解析。</span>
                    </label>
                    <label class="block">
                        ...
                        <span class="text-xs text-muted">默认 <code class="font-mono">27015</code>。</span>
                    </label>
                </div>

                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">RCON 密码</span>
                        <input type="password"
                               id="rcon"
                               name="rcon"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">在 <code class="font-mono">server.cfg</code> 中 <code class="font-mono">rcon_password</code> 旁查找。</span>
                    </label>
                    <label class="block">
                        <span class="label">确认 RCON</span>
                        <input type="password"
                               id="rcon2"
                               name="rcon2"
                               class="input font-mono"
                        ...
                        <span class="text-xs text-muted">再次输入以避免输入错误。</span>
                    </label>
                </div>

                <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))">
                    <label class="block">
                        <span class="label">模组</span>
                        <select id="mod"
                                name="mod"
                                class="select"
                                required
                                data-testid="addserver-mod">
                            {if !$edit_server}<option value="-2">选择模组…</option>{/if}
                    ...
                    <label class="flex items-center gap-2" style="align-self:end;padding:0.5rem 0">
                        ...
                        <span class="text-sm">在公开服务器列表中启用</span>
                    </label>
                </div>

                <fieldset style="border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem 1rem">
                    <legend class="text-xs font-semibold" style="padding:0 0.5rem">服务器分组</legend>
                    <p class="text-xs text-muted m-0 mb-2">选择此服务器继承管理员所使用的 SourceMod 分组。</p>
                    {if $grouplist|@count == 0}
                        <p class="text-xs text-faint m-0">尚未定义服务器分组 — 请前往"管理员 → 分组"创建一个。</p>
                    {else}
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        api.call(window.Actions.ServersAdd, {
            ...
            if (submit) submit.disabled = false;
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '无法添加服务器。';
                setError(msg);
                if (window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: '添加服务器失败', body: msg });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast && d.message) {
                window.SBPP.showToast({ kind: 'success', title: d.message.title || '服务器已添加', body: d.message.body || '' });
            }
