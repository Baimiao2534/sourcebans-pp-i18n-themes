...
{else}
    <div class="flex items-end justify-between gap-3 mb-4" style="flex-wrap:wrap">
        <div>
            <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">Администраторы
                <span class="text-faint" style="font-weight:400;margin-left:0.375rem" data-testid="admin-count">({$admin_count})</span>
            </h1>
            <p class="text-sm text-muted m-0 mt-2">Нажмите на действия в строке администратора, чтобы изменить данные, права или доступ к серверам.</p>
        </div>
        {if $can_add_admins}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=admins&amp;section=add-admin"
               data-testid="admin-add-cta"><i data-lucide="user-plus"></i> Добавить администратора</a>
        {/if}
    </div>
    ...
    <div data-testid="admin-admins-section-admins">
        ...
        <div class="card" style="overflow:hidden">
            <table class="table" role="table" aria-label="Администраторы">
                <thead>
                    <tr>
                        <th scope="col">Имя</th>
                        <th scope="col">Баны</th>
                        <th scope="col">Серверная группа</th>
                        <th scope="col">Веб-группа</th>
                        <th scope="col">Иммунитет</th>
                    ...
                    <tr data-testid="admin-row" data-id="{$admin.aid}">
                        ...
                        <td class="tabular-nums text-muted">
                            <a href="./index.php?p=banlist&advSearch={$admin.aid|escape:'url'}&advType=admin"
                               title="Показать баны">{$admin.bancount}</a>
                            <span class="text-faint"> · </span>
                            <a href="./index.php?p=banlist&advSearch={$admin.aid|escape:'url'}&advType=nodemo"
                               title="Показать баны без демо">{$admin.nodemocount} без демо</a>
                        </td>
                            ...
                            <div class="row-actions" style="white-space:nowrap">
                                {if $can_edit_admins}
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Изменить данные для {$admin.user|escape}"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Изменить права для {$admin.user|escape}"
                                       data-testid="admin-action-edit-perms">
                                        <i data-lucide="shield" style="width:14px;height:14px"></i>
                                    </a>
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       href="index.php?p=admin&c=admins&o=editservers&id={$admin.aid|escape:'url'}"
                                       title="Изменить доступ к серверам"
                                       aria-label="Изменить доступ к серверам для {$admin.user|escape}"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       aria-label="Изменить группы для {$admin.user|escape}"
                                ...
                                {if $can_delete_admins}
                                    ...
                                    <button type="button" class="btn btn--ghost btn--icon btn--sm"
                                            ...
                                            aria-label="Удалить администратора {$admin.user|escape}"
    ...
    <dialog id="admins-delete-dialog"
        ...
        <form method="dialog" data-testid="admins-delete-form">
            <h2 id="admins-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Удалить администратора</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                Вы собираетесь навсегда удалить <strong data-testid="admins-delete-target">этого администратора</strong>. Это действие нельзя отменить. Доступ к серверам будет немедленно отозван, а очередь повторного хеширования очищена.
            </p>
            <label class="label" for="admins-delete-reason">Причина (необязательно)</label>
            ...
            <textarea class="textarea"
                      ...
                      maxlength="255"
                      autocomplete="off"
                      placeholder="Только для журнала аудита. Оставьте пустым, чтобы пропустить."></textarea>
            <p class="text-xs" data-testid="admins-delete-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                <button type="button" class="btn btn--secondary" data-testid="admins-delete-cancel" value="cancel">Отмена</button>
                <button type="submit" class="btn btn--danger" data-testid="admins-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> Удалить администратора
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        function openDeleteDialog(ctx) {
            ...
            if (target) target.textContent = ctx.name || ('администратор №' + ctx.aid);
        ...
        document.addEventListener('click', function (e) {
            ...
            var name = btn.getAttribute('data-name') || ('администратор №' + aid);
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.AdminsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', 'Администратор удалён', ctx.name + ' был удалён.');
            });
        });
