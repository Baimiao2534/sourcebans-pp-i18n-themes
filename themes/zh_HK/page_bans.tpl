  ...
  <div class="card__body">
    ...
    <div class="mt-6">
      {foreach from=$othercomments item=com name=othercomments}
        {if $smarty.foreach.othercomments.first}<h3 style="font-size:var(--fs-base);font-weight:600;margin:0 0 0.5rem">其他評論</h3>{/if}
        <div class="mt-4" style="border-top:1px solid var(--border);padding-top:0.75rem">
          <div class="flex items-center justify-between">
            ...
            {if $hideadminname}<i class="text-faint">已隱藏</i>{elseif !empty($com.comname)}<strong>{$com.comname|escape}</strong>{else}<i class="text-faint">已刪除的管理員</i>{/if}
          ...
          <div class="text-xs text-faint mt-2">最後編輯 {$com.edittime} 由 {if $hideadminname}<i class="text-faint">已隱藏</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>已刪除的管理員</i>{/if}</div>
          {/if}
        </div>
      {/foreach}
    </div>
...
<div id="banlist-root" class="p-6 space-y-4" style="max-width:1700px;margin:0 auto" data-loading="false">
  <div class="flex items-center justify-between gap-3" style="flex-wrap:wrap">
    <div>
      ...
      <p class="text-sm text-muted m-0 mt-2"><span class="tabular-nums">{$ban_list|@count}</span> / <span class="tabular-nums">{$total_bans}</span> 筆封禁</p>
    </div>
    <div class="flex gap-2 items-center">
      {if $can_export}
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=steam" title="匯出永久 SteamID 封禁">匯出 Steam</a>
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=ip" title="匯出永久 IP 封禁">匯出 IP</a>
      {/if}
      ...
      <a class="btn btn--secondary btn--sm"
         ...
         data-testid="toggle-hide-inactive">{$hidetext} 非活躍</a>
      {/if}
      {has_access flag=$smarty.const.ADMIN_ADD_BAN}
      <a class="btn btn--primary btn--sm" href="index.php?p=admin&c=bans">新增封禁</a>
      {/has_access}
    </div>
  </div>
  ...
  <form method="get" action="index.php" id="banlist-filters" style="position:sticky;top:3.5rem;z-index:20;background:var(--bg-page);padding:0.75rem 0;border-bottom:1px solid var(--border)">
    ...
    <div class="flex gap-3" style="flex-wrap:wrap">
      <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
        <input class="input" type="search" name="searchText" data-testid="bans-search"
          ...
          placeholder="玩家、SteamID 或 IP&hellip;" aria-label="搜尋封禁">
      </div>
      <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="bans-server-filter" aria-label="按伺服器篩選">
      ...
      <select class="select" name="time" style="width:auto" data-testid="bans-time-filter" aria-label="按時間範圍篩選">
    ...
    <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="按狀態篩選">
      <a class="chip"
      ...
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#f59e0b"></span>活躍
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:var(--zinc-300)"></span>已過期
      </a>
      <a class="chip"
  ...
  <div class="card" style="overflow:hidden">
    ...
    <table class="table">
      <thead>
        ...
        <tr>
          ...
          <th scope="col">原因</th>
          <th scope="col" class="col-tier-2">伺服器</th>
          {if !$hideadminname}<th scope="col" class="col-admin col-tier-2">管理員</th>{/if}
          <th scope="col" class="col-length col-tier-3">時長</th>
          <th scope="col" class="col-banned col-tier-3">封禁時間</th>
          <th scope="col" class="col-status">狀態</th>
          <th scope="col" class="col-actions" aria-label="列操作"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="ban-row ban-row--{$ban.state}" data-state="{$ban.state}" data-id="{$ban.bid}" data-testid="ban-row">
          <td>
            <div class="flex items-center gap-3" style="min-width:0">
              ...
              <a class="font-medium truncate" href="?p=banlist&amp;id={$ban.bid}" data-drawer-href="?p=banlist&amp;c=details&amp;id={$ban.bid}" data-testid="drawer-trigger">{if empty($ban.name)}<i class="text-faint">無暱稱</i>{else}{$ban.name|escape}{/if}</a>
            </div>
            ...
            <details class="ban-comments-inline"
              ...
              <summary class="ban-comments-inline__summary"
                       ...
                       title="{$ban.commentdata|@count} 則評論{if $ban.commentdata|@count != 1}{/if}在此封禁上">
                ...
                <span class="ban-comments-inline__label">則評論{if $ban.commentdata|@count != 1}{/if}</span>
              </summary>
              <ul class="ban-comments-inline__list" data-testid="ban-comments-list">
                ...
                <li class="ban-comments-inline__item" data-testid="ban-comment-item">
                  ...
                  {if !empty($com.edittime)}
                  <div class="ban-comments-inline__edit text-xs text-faint">最後編輯 {$com.edittime} 由 {if $hideadminname}<i class="text-faint">已隱藏</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>已刪除的管理員</i>{/if}</div>
                  {/if}
                </li>
                {/foreach}
              </ul>
          ...
          <td class="text-muted truncate" style="max-width:18rem"
            ...
            {if empty($ban.reason)}<i class="text-faint">無原因</i>{else}{$ban.reason|escape}{/if}
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint mt-1" data-testid="ban-unban-meta">
                {if !empty($ban.removedby)}
                  解封者 <span class="font-medium">{$ban.removedby|escape}</span>{if !empty($ban.ureason)}：{/if}
                {/if}
          ...
          <td class="col-admin col-tier-2 text-muted">
            {if empty($ban.aname)}<i class="text-faint">已刪除</i>{else}{$ban.aname|escape}{/if}
          </td>
          {/if}
          ...
          <td class="col-status">
            <span class="pill pill--{$ban.state}">{if $ban.state === 'active'}活躍{elseif $ban.state === 'permanent'}永久{elseif $ban.state === 'expired'}已過期{elseif $ban.state === 'unbanned'}已解封{else}{$ban.state|capitalize|escape}{/if}</span>
          </td>
        ...
        <tr>
          ...
          <td colspan="10" style="padding:0">
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="true">
              ...
              <h2 class="empty-state__title">沒有封禁符合這些篩選條件</h2>
              <p class="empty-state__body">嘗試不同的搜尋詞或清除篩選條件以查看所有已記錄的封禁。</p>
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="false">
              ...
              <h2 class="empty-state__title">目前沒有封禁紀錄</h2>
              <p class="empty-state__body">管理員開始執法後，執法操作將顯示在這裡。</p>
              {if $can_add_ban}
              <div class="empty-state__actions">
    ...
    <div class="ban-cards">
        ...
        <a class="ban-card__summary flex items-center gap-3 p-4"
          ...
          <div style="flex:1;min-width:0">
            <div class="flex items-center gap-2">
              <span class="font-medium text-sm truncate">{if empty($ban.name)}<i class="text-faint">無暱稱</i>{else}{$ban.name|escape}{/if}</span>
              <span class="pill pill--{$ban.state}">{if $ban.state === 'active'}活躍{elseif $ban.state === 'permanent'}永久{elseif $ban.state === 'expired'}已過期{elseif $ban.state === 'unbanned'}已解封{else}{$ban.state|capitalize|escape}{/if}</span>
            </div>
            ...
            <div class="text-xs text-muted truncate" style="margin-top:0.125rem"
                 title="{if empty($ban.reason)}無原因{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永久{else}{$ban.length_human|escape}{/if}">{if empty($ban.reason)}無原因{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永久{else}{$ban.length_human|escape}{/if}</div>
      ...
      <div class="empty-state" data-testid="banlist-empty-mobile" data-filtered="true">
        ...
        <h2 class="empty-state__title">沒有封禁符合這些篩選條件</h2>
        <p class="empty-state__body">嘗試不同的搜尋詞或清除篩選條件。</p>
        <div class="empty-state__actions">
  ...
  <div class="card">
    <div class="card__body">
      <nav class="text-xs text-muted" aria-label="封禁列表分頁">
...
<dialog id="bans-unban-dialog"
  ...
  <form method="dialog" data-testid="bans-unban-form">
    ...
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      您為什麼要解封 <strong data-testid="bans-unban-target">此玩家</strong>？此原因將記錄在封禁中並顯示在稽核記錄中。
    </p>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="誤判封禁、申訴已接受、刑期已滿、&hellip;"></textarea>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (act === 'bans-delete') {
            ...
            // page load.
            if (!fallback) return;
            if (!window.confirm('永久刪除「' + name + '」的封禁？此操作無法復原。')) return;
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.BansUnban, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            setBusy(submitBtn, false);
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '未知錯誤';
                showError(msg);
                toast('error', '解封失敗', msg);
                return;
            }
