...
{if NOT $permissions_submissions}
    <div class="card" data-testid="submissions-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">BAN 申告を表示する権限がありません。</p>
        </div>
    </div>
{else}
        ...
        {if $submission_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="submissions-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">申告アーカイブは空です</h2>
                    <p class="empty-state__body">申告が承認・却下・アーカイブされると、記録としてここに移動します。</p>
                </div>
            </div>
        {else}
                        ...
                        <div class="p-4" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            ...
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    ...
                                    <dt class="text-muted">送信日時</dt>
                                    <dd style="margin:0">{$sub.submitted|escape}</dd>

                                    <dt class="text-muted">SteamID</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.SteamId == ""}<span class="text-faint">SteamID なし</span>{else}{$sub.SteamId|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">IP</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.sip == ""}<span class="text-faint">IP アドレスなし</span>{else}{$sub.sip|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">理由</dt>
                                    ...
                                    <dt class="text-muted">申告者</dt>
                                    <dd style="margin:0">
                                        {if $sub.subname == ""}<span class="text-faint">名前なし</span>{else}{$sub.subname|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">申告者 IP</dt>
                                    <dd class="font-mono" style="margin:0">{$sub.ip|escape}</dd>

                                    <dt class="text-muted">アーカイブ実行者</dt>
                                    <dd style="margin:0">
                                        {if !empty($sub.archivedby)}{$sub.archivedby|escape}{else}<i class="text-faint">管理者は削除されました。</i>{/if}
                                    </dd>
                                </dl>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var name = btn.dataset.name || ('申告 #' + sid);
        var archiv = btn.dataset.archiv || '0';
        var msg;
        if (archiv === '2') msg = 'BAN 申告「' + name + '」をアーカイブから復元しますか？';
        else if (archiv === '1') msg = 'BAN 申告「' + name + '」をアーカイブに移動しますか？';
        else msg = 'BAN 申告「' + name + '」を削除しますか？';
