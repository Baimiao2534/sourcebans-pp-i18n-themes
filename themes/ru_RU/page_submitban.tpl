...
<section class="p-6" style="max-width:48rem">
    <header class="mb-6">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Подать заявку на бан</h1>
        <p class="text-sm text-muted m-0 mt-2">
            Пожаловаться на игрока за читы, преследование или другие нарушения правил. Демо и скриншоты помогают администраторам действовать быстрее.
        </p>
    </header>
    ...
    <form class="card"
        ...
        <div class="card__body space-y-4">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    Укажите Steam ID или IP-адрес (или оба). Ник и понятное описание инцидента обязательны.
                </p>
            </div>
            ...
            *}
            <div data-testid="submitban-id-or-ip"
                ...
                <p id="submitban-id-or-ip-help"
                   ...
                   data-state="info">
                    <span class="pill"
                        ...
                        1 из этих обязателен
                    </span>
                    <span class="text-muted" data-id-or-ip-text>Укажите Steam ID или IP-адрес (или оба).</span>
                </p>
                <div>
                    <label class="label" for="submitban-steam">
                        Steam ID игрока
                        <span class="text-muted text-xs" style="font-weight:400">(или используйте IP ниже)</span>
                    </label>
                    {*
                ...
                <div>
                    <label class="label" for="submitban-ip">
                        IP-адрес игрока
                        <span class="text-muted text-xs" style="font-weight:400">(или используйте Steam ID выше)</span>
                    </label>
                    <input type="text"
                           class="input font-mono"
                           id="submitban-ip"
            ...
            <div>
                <label class="label" for="submitban-name">
                    Ник игрока <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <input type="text"
                       class="input"
                       id="submitban-name"
            ...
            <div>
                <label class="label" for="submitban-reason">
                    Комментарии <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                ...
                <p id="submitban-reason-help" class="text-xs text-muted m-0 mt-2">
                    Будьте конкретны. &laquo;читерство&raquo; — недостаточно. Опишите, что вы видели, когда и на каком сервере.
                </p>
            </div>
        </div>
        ...
        <div class="card__body space-y-4" style="border-top:1px solid var(--border)">
            <div>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    Чтобы администратор мог связаться с вами для уточнений.
                </p>
            </div>

            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                <div>
                    <label class="label" for="submitban-reporter-name">Ваше имя</label>
                    <input type="text"
                           class="input"
                ...
                <div>
                    <label class="label" for="submitban-reporter-email">
                        Ваш email <span style="color:var(--danger)" aria-hidden="true">*</span>
                    </label>
                    <input type="email"
                           class="input"
            ...
            <div>
                <label class="label" for="submitban-server">
                    Сервер <span style="color:var(--danger)" aria-hidden="true">*</span>
                </label>
                <select class="select"
                        id="submitban-server"
                        name="server"
                    ...
                    <option value="-1">&mdash; Выберите сервер &mdash;</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"{if $server_selected == $server.sid} selected{/if}>{$server.hostname}</option>
                    {/foreach}
                    <option value="0"{if $server_selected == 0} selected{/if}>Другой сервер / Нет в списке</option>
                </select>
            </div>

            <div>
                <label class="label" for="submitban-demo">Загрузить демо или доказательства</label>
                ...
                <p class="text-xs text-muted m-0 mt-2">
                    Необязательно. Разрешённые форматы: <code class="font-mono">.dem</code>,
        ...
        <div class="card__body flex items-center justify-between gap-2"
            ...
            <p class="text-xs text-muted m-0">
                <span style="color:var(--danger)" aria-hidden="true">*</span> Обязательное поле
            </p>
            <div class="flex gap-2">
                <a class="btn btn--ghost"
                   href="index.php?p=banlist"
                   data-testid="submitban-cancel">Отмена</a>
...
(function () {
    ...
    function setError(isError) {
        ...
        if (isError) {
            ...
            helpText.textContent = 'Пожалуйста, введите Steam ID или IP-адрес перед отправкой.';
        } else {
