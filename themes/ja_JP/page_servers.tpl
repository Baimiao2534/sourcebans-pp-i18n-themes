        ...
        <div>
            ...
            <p class="text-sm text-muted m-0 mt-2"
               ...
               data-total="{$server_list|@count}">
                {$server_list|@count} 件設定済み
                <span data-online-count>&middot; <span data-online-num>0</span> 件オンライン</span>
            </p>
        </div>
        {*
        ...
        {if $can_use_context_menu}
            ...
            <p class="text-xs text-muted m-0 servers-rcon-hint"
               ...
               style="max-width:24rem;text-align:right">
                展開されたカードでプレイヤーを右クリックすると、プロフィールの確認、
                SteamID のコピー、キック / BAN / 通信制限が行えます。
                キック / BAN / 通信制限の操作は、RCON 権限を持つサーバーでのみ表示されます。
            </p>
        {/if}
    </header>
    ...
    {if $server_list|@count == 0}
        ...
        <div class="card" data-testid="servers-empty">
            <div class="empty-state">
                ...
                <h2 class="empty-state__title">サーバーが設定されていません</h2>
                <p class="empty-state__body">
                    サーバーを追加すると、プレイヤー、ステータス、リアルタイムの数値が
                    ここに訪問者向けに表示されます。
                </p>
                {if $can_add_server}
        ...
        <article class="card"
            ...
            <div class="card__body">
                ...
                <dl class="grid gap-2 text-xs"
                    ...
                    <dt class="text-muted flex items-center gap-1" style="margin:0">
                        <i data-lucide="map" style="width:13px;height:13px"></i> マップ
                    </dt>
                    <dd class="font-mono text-faint" data-testid="server-map" style="margin:0">&mdash;</dd>

                    <dt class="text-muted flex items-center gap-1" style="margin:0">
                        <i data-lucide="users" style="width:13px;height:13px"></i> プレイヤー
                    </dt>
            ...
            <div id="server-players-{$server.sid}"
                ...
                *}
                ...
                <p class="text-xs text-muted m-0" data-empty-message>現在接続しているプレイヤーはいません。</p>
