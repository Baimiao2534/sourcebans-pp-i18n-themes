...
-{if NOT $permission_rcon}-
    <section class="card" data-testid="rcon-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0">У вас нет RCON-доступа к этому серверу.</p>
        </div>
    </section>
-{else}-
<section class="card" data-testid="rcon-console" data-sid="-{$id}-">
    <div class="card__header">
        <div>
            <h3>Консоль RCON</h3>
            <p>Сервер №-{$id}- · введите <code class="font-mono">clr</code> для очистки · вывод отображается как обычный текст.</p>
        </div>
    </div>
    <div class="card__body">
        -{*
        ...
        *}-
        <pre id="rcon"
             ...
             style="margin:0;background:var(--bg-page);border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem;height:18rem;overflow:auto;font-family:var(--font-mono);font-size:var(--fs-xs);color:var(--text);white-space:pre-wrap;word-break:break-word"><div id="rcon_con">Консоль RCON SourceBans++
==========================================================
Введите команду в поле ниже и нажмите Enter.
Введите 'clr' для очистки консоли.
==========================================================
</div></pre>
        ...
        api.call(window.Actions.ServersSendRcon, { sid: sid, command: command, output: true }).then(function (r) {
            ...
            if (!r || !r.ok || !r.data) {
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'Сбой RCON', body: r.error.message || 'Неизвестная ошибка' });
                }
                return;
            }
            var d = r.data;
            ...
            if (d.kind === 'error') {
                appendLine('> Ошибка: ', d.error || '', 'error');
                out.appendChild(document.createElement('br'));
                scrollToBottom();
                return;
            }
