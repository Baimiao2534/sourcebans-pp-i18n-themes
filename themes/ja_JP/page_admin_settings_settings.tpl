...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">サイト全体の設定。変更は即座に反映されます。</p>
    </div>

    {if NOT $can_web_settings}
        <div class="card">
            <div class="card__body">
                <p class="text-muted">アクセス拒否。<code>ADMIN_WEB_SETTINGS</code> 権限が必要です。</p>
            </div>
        </div>
    {else}
                <form action="?p=admin&amp;c=settings&amp;section=settings" method="post" class="space-y-4" id="form-settings-main">
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>全般</h3><p>サイト識別、パスワードルール、日付。</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="template.title">
                                <label class="label" for="template_title">サイトタイトル</label>
                            ...
                            <div data-testid="setting-row" data-key="template.logo">
                                <label class="label" for="template_logo">ロゴパス</label>
                                    ...
                                    *}
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-warning-template.logo">
                                        <strong>注意:</strong>クロームは現在デフォルトのシールド（<code class="font-mono">images/favicon.svg</code>）をレンダリングしています。保存されたパスがアクティブなテーマのディスク上のファイルとして解決できないためです。<code class="font-mono">themes/&lt;theme&gt;/</code> 配下にファイルが存在するか確認するか、入力をクリアしてカスタマイズを削除してください。
                                    </p>
                                {/if}
                                <p class="settings-fieldset__help"
                                   id="template_logo_help"
                                   data-testid="setting-help-template.logo">
                                    サイドバーとログインページのロゴのパス（テーマディレクトリ <code class="font-mono">themes/&lt;theme&gt;/</code> からの相対パス）。デフォルトは <code class="font-mono">images/favicon.svg</code>。512&times;512 のラスター版は <code class="font-mono">logos/sbpp_logo.png</code> にあります。
                                </p>
                            </div>
                            <div data-testid="setting-row" data-key="config.password.minlength">
                                <label class="label" for="config_password_minlength">最小パスワード長</label>
                            ...
                            <div data-testid="setting-row" data-key="config.dateformat">
                                <label class="label" for="config_dateformat">日付形式 <span class="text-muted text-xs">(<a href="https://www.php.net/manual/en/datetime.format.php" target="_blank" rel="noopener">PHP date()</a>)</span></label>
                            ...
                            <div data-testid="setting-row" data-key="config.debug">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">デバッグモードを有効化</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="card">
                        <fieldset class="settings-fieldset"
                            ...
                            <legend class="settings-fieldset__legend">
                                <span class="settings-fieldset__title">認証</span>
                                <span class="settings-fieldset__hint">セッショントークンの有効期間（分単位）。値を <code>0</code> に設定するとログインパスを無効化します。</span>
                            </legend>
                            <div class="settings-fieldset__body space-y-5">
                                ...
                                *}
                                <div data-testid="setting-row" data-key="auth.maxlife">
                                    <label class="label" for="auth_maxlife">デフォルトログイン</label>
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife">
                                        通常のログインセッションがユーザーをログアウトするまでの時間（分単位）。
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.remember">
                                    <label class="label" for="auth_maxlife_remember">&ldquo; remember me &rdquo; ログイン</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.remember">
                                        ユーザーがログインフォームで &ldquo; remember me &rdquo; チェックボックスをオンにした場合に使用されます。通常、デフォルトセッションよりはるかに長くなります。
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.steam">
                                    <label class="label" for="auth_maxlife_steam">Steam ログイン</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.steam">
                                        &ldquo;Steam で続行&rdquo; ボタンで開かれたセッションの有効期間。
                                    </p>
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>ダッシュボードの紹介</h3><p>公開ランディングページのヘッダーと本文。</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="dash.intro.title">
                                <label class="label" for="dash_intro_title">紹介タイトル</label>
                                ...
                                <label class="label" for="dash_intro_text">
                                    ...
                                    <span class="text-muted text-xs">
                                        ...
                                        (<a href="https://commonmark.org/help/"
                                            target="_blank"
                                            rel="noopener"
                                            data-testid="dash-intro-cheatsheet-link">CommonMark チートシート</a>)。
                                        生の HTML はレンダリング時にエスケープされます。
                                    </span>
                                </label>
                                {*
                                ...
                                *}
                                <div class="dash-intro-editor"
                                    ...
                                    <div class="dash-intro-preview"
                                        ...
                                        <div class="dash-intro-preview__label">プレビュー</div>
                                        {*
                                        ...
                                        *}
                                        <div class="dash-intro-preview__body"
                                            ...
                                            {else}
                                                <p class="text-xs text-muted m-0" data-empty>左に Markdown を入力して、訪問者へのレンダリング結果を確認します。</p>
                                            {/if}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div data-testid="setting-row" data-key="dash.lognopopup">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">ログポップアップを無効化（直接リンクを使用）</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>公開ページ</h3><p>訪問者がアクセスできる追加ページ。</p></div></div>
                        <div class="card__body space-y-3">
                            <div data-testid="setting-row" data-key="config.enableprotest">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">BAN 申立ページを有効化</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablesubmit">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">BAN 提出ページを有効化</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablecomms">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">通信 BAN リストを有効化</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="protest.emailonlyinvolved">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">新規申立時に元 BAN 管理者にのみメール送信</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.defaultpage">
                                <label class="label" for="default_page">デフォルトランディングページ</label>
                                <select class="select" id="default_page" name="default_page">
                                    <option value="0"{if $config_default_page == 0} selected{/if}>ダッシュボード</option>
                                    <option value="1"{if $config_default_page == 1} selected{/if}>BAN リスト</option>
                                    <option value="2"{if $config_default_page == 2} selected{/if}>サーバー</option>
                                    <option value="3"{if $config_default_page == 3} selected{/if}>BAN を提出</option>
                                    <option value="4"{if $config_default_page == 4} selected{/if}>BAN を申立</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>BAN リスト</h3><p>ページネーションと表示設定。</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="banlist.bansperpage">
                                <label class="label" for="banlist_bansperpage">ページあたりの BAN 数</label>
                            ...
                            <div data-testid="setting-row" data-key="banlist.hideadminname">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">公開 BAN 情報から管理者名を非表示</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.hideplayerips">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">公開 BAN 情報からプレイヤー IP を非表示</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.nocountryfetch">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">IP の国情報参照をスキップ</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="bans.customreasons">
                                <label class="label">カスタム BAN 理由</label>
                                <p class="text-xs text-muted m-0 mb-2">各行が BAN 理由ドロップダウンのオプションになります。空白にすると削除されます。</p>
                                <div id="custom-reasons" class="space-y-3">
                                    ...
                                    {/foreach}
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="他の理由を追加…">
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="他の理由を追加…">
                                </div>
                                <button type="button" class="btn btn--ghost btn--sm mt-2" onclick="addCustomReason();">
                                    <i data-lucide="plus"></i> 行を追加
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>SMTP</h3><p>送信メールトランスポート。</p></div></div>
                        <div class="card__body space-y-4">
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <div data-testid="setting-row" data-key="smtp.host">
                                    <label class="label" for="mail_host">ホスト</label>
                                ...
                                <div data-testid="setting-row" data-key="smtp.port">
                                    <label class="label" for="mail_port">ポート</label>
                            ...
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="smtp.user">
                                    <label class="label" for="mail_user">ユーザー名</label>
                                ...
                                <div data-testid="setting-row" data-key="smtp.pass">
                                    <label class="label" for="mail_pass">パスワード <span class="text-muted text-xs">（空欄で現在の値を維持）</span></label>
                            ...
                            <div data-testid="setting-row" data-key="smtp.verify_peer">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">TLS ピア証明書を検証</span>
                                </label>
                            </div>
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="config.mail.from_email">
                                    <label class="label" for="mail_from_email">送信元メール</label>
                                ...
                                <div data-testid="setting-row" data-key="config.mail.from_name">
                                    <label class="label" for="mail_from_name">送信元名</label>
                            ...
                            *}
                            <div data-testid="setting-row" data-key="smtp.test">
                                <label class="label" for="mail_test_to">テストメールを送信</label>
                                <div class="grid gap-2" style="grid-template-columns:1fr auto">
                                    ...
                                    <button type="button"
                                        ...
                                        <i data-lucide="send"></i> テストメールを送信
                                    </button>
                                </div>
                                <p class="text-xs text-muted mt-1">
                                    保存済みの SMTP 認証情報で検証メールを送信します。上記を変更した場合は、まずフォームを保存してください。テストは入力の未保存内容ではなく、永続化された値を使用します。
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="settings-actions flex items-center justify-end gap-2">
                        <button type="button" class="btn btn--secondary btn--sm" onclick="clearCacheBtn();">
                            <i data-lucide="brush"></i> キャッシュをクリア
                        </button>
...
(function () {
    ...
    if (ta && preview && previewBody && window.sb && window.sb.api && window.Actions) {
        var emptyHtml = '<p class="text-xs text-muted m-0" data-empty>左に Markdown を入力して、訪問者へのレンダリング結果を確認します。</p>';
    ...
    function humanizeMinutes(minutes) {
        if (!Number.isFinite(minutes) || minutes <= 0) return '無効';
        var n = Math.floor(minutes);
        if (n < 60) return n === 1 ? '1 分' : n + ' 分';
        if (n < 1440) {
            if (n % 60 === 0) {
                var h = n / 60;
                return h === 1 ? '1 時間' : h + ' 時間';
            }
    ...
    if (testBtn && window.sb && window.sb.api && window.Actions) {
        testBtn.addEventListener('click', function () {
            ...
            if (rawTo === '' || !testTo.checkValidity()) {
                ...
                toast('warn', '受信者を入力', 'テスト送信前に有効なメールアドレスを入力してください。');
                return;
            }
            ...
            window.sb.api.call(window.Actions.SystemTestEmail, { to: rawTo }).then(
                function (r) {
                    ...
                    if (r && r.ok && r.data && typeof r.data.to === 'string') {
                        toast('success', 'テストメールが送信されました',
                            'SMTP テストメッセージが ' + r.data.to + ' に送信されました。受信箱を確認して配信を確認してください。');
                        return;
                    }
                    ...
                    var msg = err.message || 'サーバーから予期しない応答が返されました。';
                    if (code === 'smtp_not_configured') {
                        toast('warn', 'SMTP が設定されていません', msg);
                    } else if (code === 'rate_limited') {
                        toast('warn', 'テストメールが制限されました', msg);
                    } else if (code === 'validation') {
                        toast('warn', '受信者を確認', msg);
                    } else if (code === 'mail_failed') {
                        toast('error', 'テストメール失敗', msg);
                    } else {
                        toast('error', 'テストメール失敗', msg);
                    }
                },
                function () {
                    setBusy(testBtn, false);
                    toast('error', 'テストメール失敗', 'パネル API にアクセスできませんでした。ネットワークを確認して再試行してください。');
                }
            );
        });
    }
