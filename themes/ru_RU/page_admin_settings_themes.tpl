...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Выберите тему, которая будет отображать оформление панели для всех посетителей.</p>
    </div>
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>Установленные темы</h3>
                        <p>Каждая карточка представляет каталог в <code>web/themes/</code> с манифестом <code>theme.conf.php</code>.</p>
                    </div>
                    <div class="text-xs text-muted" style="text-align:right" data-testid="current-theme-summary">
                        Сейчас используется:<br>
                        <strong>{$theme_name}</strong> v{$theme_version} от {$theme_author}{if $theme_link != ''} · <a href="{$theme_link}" target="_blank" rel="noopener" class="text-muted">домашняя страница</a>{/if}<br>
                ...
                <div class="card__body">
                    {if NOT $can_web_settings}
                        <p class="text-xs text-muted m-0 mb-4" style="padding:0.5rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--bg-muted)">
                            ...
                            Вы можете просматривать темы, но для переключения активной темы требуется <code>ADMIN_WEB_SETTINGS</code>.
                        </p>
                    {/if}
                            ...
                            <article class="theme-card"
                                ...
                                <div style="aspect-ratio:250 / 170;background:var(--bg-muted);display:grid;place-items:center;overflow:hidden">
                                    <img src="{$t.screenshot}"
                                         alt="Скриншот темы {$t.name}"
                                         loading="lazy"
                                         style="width:100%;height:100%;object-fit:cover"
                                         onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\'text-muted text-xs\'>Нет скриншота</span>'">
                                </div>
                                <div class="p-4 space-y-3" style="flex:1;display:flex;flex-direction:column">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            {if $t.active}
                                                <span class="pill pill--online" title="Сейчас активна">
                                                    <i data-lucide="check" style="width:0.75rem;height:0.75rem"></i> Активна
                                                </span>
                                            {/if}
                                        </div>
                                        <p class="text-xs text-muted m-0 mt-2">от {$t.author} · v{$t.version}{if $t.link != ''} · <a href="{$t.link}" target="_blank" rel="noopener" class="text-muted" title="Открыть домашнюю страницу темы"><i data-lucide="external-link" style="width:0.75rem;height:0.75rem;vertical-align:-1px"></i> домашняя страница</a>{/if}</p>
                                        ...
                                        {if $can_web_settings}
                                            {if $t.active}
                                                <button type="button" class="btn btn--secondary btn--sm" disabled aria-pressed="true">
                                                    <i data-lucide="check"></i> Используется
                                                </button>
                                            {else}
                                                <button type="button"
                                                    ...
                                                    <i data-lucide="check-circle"></i> Использовать эту тему
                                                </button>
                                            {/if}
                                        {/if}
                                    </div>
...
(function () {
    ...
    window.applyTheme = function (theme) {
        if (!theme) return;
        if (!window.confirm('Переключить тему панели на «' + theme + '»? Каждый посетитель увидит новую тему при следующем запросе.')) return;
