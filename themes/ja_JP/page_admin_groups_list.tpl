...
<div class="space-y-6">
    ...
    <section data-testid="web-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">ウェブ管理者グループ</h1>
                <p class="text-sm text-muted m-0 mt-2">ウェブパネルグループの権限フラグと免責。合計：{$web_group_count}。</p>
            </div>
        </div>
        ...
        {if $web_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-web" data-filtered="false">
                ...
                <h2 class="empty-state__title">ウェブ管理者グループがまだありません</h2>
                <p class="empty-state__body">ウェブ管理者グループは管理者セットのパネル権限をまとめます。複数の管理者に同じフラグを一括で付与するには作成してください。</p>
                    ...
                    <a class="admin-groups-master-detail__row flex items-center gap-3 p-4"
                       ...
                       {if $selected_group && $selected_group.gid == $group.gid}aria-current="true"{/if}>
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="text-xs text-muted">{$web_admins[$smarty.foreach.web_group.index]} メンバー</div>
                ...
                <form class="card"
                    ...
                    <div class="card__header">
                        <div>
                            ...
                            <p>{$selected_group.member_count} メンバー</p>
                        </div>
                        {if $permission_deletegroup}
                            <button type="button"
                                    class="btn btn--ghost btn--sm"
                                    data-testid="group-delete"
                                    onclick="SbppGroupsDelete({$selected_group.gid}, '{$selected_group.name|escape:'javascript'}', this);">グループを削除</button>
                        {/if}
                    </div>
                    <div class="card__body space-y-4">
                        ...
                        <div>
                            <label class="label" for="group-name-input">グループ名</label>
                            <input class="input"
                                   id="group-name-input"
                                   name="name"
                        ...
                        <div>
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <label class="label m-0">権限フラグ</label>
                                ...
                                <span class="text-xs text-muted" data-testid="flag-bitmask">{$selected_group.flags} ビットマスク</span>
                            </div>
                        ...
                        {if $permission_editgroup}
                            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                                <a class="btn btn--ghost" href="?p=admin&c=groups">破棄</a>
                                <button class="btn btn--primary"
                                        type="submit"
                                        data-testid="group-save">変更を保存</button>
                            </div>
                        {/if}
                    </div>
                </form>
            {else}
                <div class="card"><div class="card__body"><p class="text-muted m-0">左側のグループを選択してフラグを編集してください。</p></div></div>
            {/if}
        </div>
        {/if}
    </section>
    ...
    <section data-testid="server-admin-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">サーバー管理者グループ</h2>
                <p class="text-sm text-muted m-0 mt-2">SourceMod 管理者グループ（ゲーム内フラグ）。合計：{$server_admin_group_count}。</p>
            </div>
        </div>
        ...
        {if $server_admin_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server-admin" data-filtered="false">
                ...
                <h2 class="empty-state__title">サーバー管理者グループがまだありません</h2>
                <p class="empty-state__body">サーバー管理者グループは SourceMod 文字フラグと免責を持ちます。管理者セットにゲーム内管理者権限を付与するには作成してください。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                    ...
                    <article class="card" data-testid="server-admin-group-row" data-id="{$group.id}">
                        <div class="card__header">
                            <div>
                                ...
                                <p>{$server_admins[$smarty.foreach.server_admin_group.index]} メンバー &middot; 免責 {$group.immunity}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=srv&id={$group.id|escape:'url'}">編集</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.id}, '{$group.name|escape:'javascript'}', 'srv', this);">削除</button>
                                {/if}
                            </div>
                        </div>
                        <div class="card__body space-y-3">
                            <div>
                                <div class="text-xs font-semibold text-muted mb-2">権限</div>
                                ...
                                {else}
                                    <p class="text-xs text-muted m-0"><em>なし</em></p>
                                {/if}
                            </div>
                            {if $server_admins_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">メンバー</div>
                                        ...
                                        {foreach from=$server_admins_list[$smarty.foreach.server_admin_group.index] item="server_admin"}
                                            <li class="flex items-center justify-between gap-2 text-sm">
                                                ...
                                                {if $permission_editadmin}
                                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=admins&o=editgroup&id={$server_admin.aid|escape:'url'}">編集</a>
                                                {/if}
                                            </li>
                                        {/foreach}
                                    </ul>
                            ...
                            {if $server_overrides_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">オーバーライド</div>
    ...
    <section data-testid="server-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">サーバーグループ</h2>
                <p class="text-sm text-muted m-0 mt-2">ゲームサーバーのグループ化（権限フラグなし）。合計：{$server_group_count}。</p>
            </div>
        </div>
        ...
        {if $server_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server" data-filtered="false">
                ...
                <h2 class="empty-state__title">サーバーグループがまだありません</h2>
                <p class="empty-state__body">サーバーグループはゲームサーバーをまとめ、複数のサーバーに管理者を一括で割り当てられます。サーバーのグループ化を始めるには作成してください。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                ...
                {foreach from=$server_list item="group" name="server_group"}
                    <article class="card" data-testid="server-group-row" data-id="{$group.gid}">
                        <div class="card__header">
                            <div>
                                <h3>{$group.name|escape}</h3>
                                <p>{$server_counts[$smarty.foreach.server_group.index]} サーバー</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=server&id={$group.gid|escape:'url'}">編集</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.gid}, '{$group.name|escape:'javascript'}', 'server', this);">削除</button>
                                {/if}
                            </div>
                        </div>
                        {*
                        ...
                        <div class="card__body">
                            ...
                            {if empty($group.servers)}
                                <p class="text-xs text-muted m-0" data-testid="server-group-empty">
                                    <em>このグループにバインドされたサーバーはまだありません。</em>
                                </p>
                            {else}
                                <ul class="space-y-2"
                                        ...
                                        <li class="flex items-center gap-2"
                                            ...
                                            {if !$server.enabled}
                                                <span class="pill pill--offline"
                                                      ...
                                                      title="無効 — 公開リストから非表示、カードごとのハイドレーションプローブはスキップされます"
                                                      style="flex-shrink:0">無効</span>
                                            {/if}
                                        </li>
                                    {/foreach}
                                </ul>
...
function SbppGroupsApplyResponse(r, opts) {
    ...
    var fallback = (opts && opts.defaultTitle) || '完了';
    ...
    if (r.ok === false) {
        ...
        if (window.SBPP && typeof window.SBPP.showToast === 'function') {
            ...
            window.sb.message.error('エラー', em);
        }
        return;
    }
...
function SbppGroupsSave(event) {
    ...
    sb.api.call(Actions.GroupsEdit, {
        ...
        SbppGroupsApplyResponse(r, { defaultTitle: 'グループが更新されました' });
    });
    return false;
}
    ...
    sb.api.call(Actions.GroupsRemove, { gid: Number(gid), type: 'web' })
        .then(function (r) {
            ...
            if (r && r.ok && (r.data && (r.data.reload || (r.data.message && r.data.message.redir)))) {
                SbppGroupsApplyResponse(r, { defaultTitle: 'グループが削除されました' });
                return;
            }
...
(function () {
    ...
    grid.addEventListener('change', function (event) {
        ...
        preview.textContent = SbppFoldFlags(grid) + ' ビットマスク';
    });
})();
{/literal}
</script>
