...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">すべての訪問者にパネルクロームをレンダリングするテーマを選択します。</p>
    </div>
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>インストール済みテーマ</h3>
                        <p>各カードは <code>web/themes/</code> 配下の <code>theme.conf.php</code> マニフェストを含むディレクトリを表します。</p>
                    </div>
                    <div class="text-xs text-muted" style="text-align:right" data-testid="current-theme-summary">
                        現在使用中:<br>
                        <strong>{$theme_name}</strong> v{$theme_version} 作成者: {$theme_author}{if $theme_link != ''} · <a href="{$theme_link}" target="_blank" rel="noopener" class="text-muted">ホームページ</a>{/if}<br>
                ...
                <div class="card__body">
                    {if NOT $can_web_settings}
                        <p class="text-xs text-muted m-0 mb-4" style="padding:0.5rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--bg-muted)">
                            ...
                            テーマを閲覧できますが、アクティブなテーマを切り替えるには <code>ADMIN_WEB_SETTINGS</code> 権限が必要です。
                        </p>
                    {/if}
                            ...
                            <article class="theme-card"
                                ...
                                <div style="aspect-ratio:250 / 170;background:var(--bg-muted);display:grid;place-items:center;overflow:hidden">
                                    <img src="{$t.screenshot}"
                                         alt="{$t.name} テーマのスクリーンショット"
                                         loading="lazy"
                                         style="width:100%;height:100%;object-fit:cover"
                                         onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\'text-muted text-xs\'>スクリーンショットなし</span>'">
                                </div>
                                <div class="p-4 space-y-3" style="flex:1;display:flex;flex-direction:column">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            {if $t.active}
                                                <span class="pill pill--online" title="現在使用中">
                                                    <i data-lucide="check" style="width:0.75rem;height:0.75rem"></i> 使用中
                                                </span>
                                            {/if}
                                        </div>
                                        <p class="text-xs text-muted m-0 mt-2">作成者: {$t.author} · v{$t.version}{if $t.link != ''} · <a href="{$t.link}" target="_blank" rel="noopener" class="text-muted" title="テーマのホームページを開く"><i data-lucide="external-link" style="width:0.75rem;height:0.75rem;vertical-align:-1px"></i> ホームページ</a>{/if}</p>
                                        ...
                                        {if $can_web_settings}
                                            ...
                                            {else}
                                                <button type="button"
                                                    ...
                                                    <i data-lucide="check-circle"></i> このテーマを使用
                                                </button>
                                            {/if}
                                        {/if}
                                    </div>
...
(function () {
    ...
    window.applyTheme = function (theme) {
        if (!theme) return;
        if (!window.confirm('パネルテーマを "' + theme + '" に切り替えますか？すべての訪問者は次回リクエスト時に新しいテーマを表示します。')) return;
