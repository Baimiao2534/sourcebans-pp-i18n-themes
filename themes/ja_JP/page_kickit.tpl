...
<head>
    ...
    <title>-{if $mode === 'kick'}-プレイヤーをキック-{else}-プレイヤーを BAN-{/if}-</title>
    <script>
    (function () {
        try {
...
<div id="container" class="card" data-testid="kickit-container" data-mode="-{$mode}-">
    <div class="card__header">
        <div>
            <h3>すべてのサーバーでプレイヤーを検索中&hellip;</h3>
            <p>各行はライブでポーリングされ、サーバーの応答に合わせて更新されます。</p>
        </div>
    </div>
                ...
                <tr data-testid="kickit-row--{$serv.num}-">
                    ...
                    <td>
                        <div id="srv_-{$serv.num}-"
                             ...
                             data-testid="kickit-status--{$serv.num}-">待機中&hellip;</div>
                    </td>
                </tr>
                -{/foreach}-
            </tbody>
    ...
    (function () {
        ...
        function setCounter(count) {
            ...
            if (srvcount === TOTAL || count === -1) {
                ...
                if (ctl) {
                    ctl.innerHTML = '<font color="green" style="font-size: 12px;"><b>検索が完了しました。</b></font>' + ctl.innerHTML;
        ...
        function processRow(sid, num) {
            sb.api.call(Actions.KickitKickPlayer, { check: CHECK, sid: sid, num: num, type: TYPE, mode: MODE })
                .then(function (r) {
                    if (!r || !r.ok || !r.data) {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>エラー。</i></span>");
                        setCounter(1);
                        return;
                    }
                    var d = r.data;
                    ...
                    if (d.status === 'no_connect') {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>サーバーに接続できません。</i></span>");
                        setCounter(1);
                    } else if (d.status === 'kicked') {
                        sb.setHTML('srv_' + num, "<span class='text-xs font-semibold' style='color:var(--success)'><u>プレイヤーを発見し、キックしました！</u></span>");
                        setCounter(-1);
                    } else {
                        sb.setHTML('srv_' + num, "<span class='text-xs text-muted'>プレイヤーが見つかりません。</span>");
                        setCounter(1);
                    }
                });
        }
        ...
        window.addEventListener('load', function () {
            ...
            sb.api.call(Actions.KickitLoadServers, {}).then(function (r) {
                ...
                r.data.servers.forEach(function (s) {
                    if (s.has_rcon) {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-muted">検索中&hellip;</span>');
                        processRow(s.sid, s.num);
                    } else {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-faint">RCON パスワードがありません。</span>');
                        setCounter(1);
                    }
                });
            });
