...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">管理员操作和警告的系统日志。</p>
    </div>
                    ...
                    <div class="card__body">
                        ...
                        {if count($log_items) > 0}
                            ...
                            <table class="table table--clickable-rows" data-testid="logs-table">
                                <thead>
                                    <tr>
                                        <th style="width:6rem">类型</th>
                                        <th>事件</th>
                                        <th style="width:14rem">用户</th>
                                ...
                                <tbody>
                                    {foreach from=$log_items item="log"}
                                        ...
                                        <tr data-testid="log-row"
                                            ...
                                            <td>
                                                {if $log.type == 'm'}
                                                    <span class="pill pill--online"><i data-lucide="info" style="width:0.75rem;height:0.75rem"></i> 信息</span>
                                                {elseif $log.type == 'w'}
                                                    <span class="pill pill--active"><i data-lucide="alert-triangle" style="width:0.75rem;height:0.75rem"></i> 警告</span>
                                                {elseif $log.type == 'e'}
                                                    <span class="pill pill--permanent"><i data-lucide="circle-x" style="width:0.75rem;height:0.75rem"></i> 错误</span>
                                                {else}
                                        ...
                                        <tr data-detail-for="{$log.lid}" id="log-detail-{$log.lid}" hidden>
                                            <td colspan="4" style="background:var(--bg-muted)">
                                                <dl class="grid gap-2 text-xs" style="grid-template-columns:8rem 1fr;margin:0">
                                                    ...
                                                    <dt class="text-muted">函数</dt>
                                                    <dd class="font-mono" style="margin:0">{$log.function}</dd>
                                                    <dt class="text-muted">查询</dt>
                            ...
                            {* #1462: paired mobile card surface. The desktop table
                               ...
                               the summary. *}
                            <ul class="log-cards" data-testid="logs-cards" aria-label="系统日志条目">
                                {foreach from=$log_items item="log"}
                                    <li>
                                        <details class="log-card"
                                            ...
                                            <div class="log-card__meta">
                                                <span><span class="text-muted">用户：</span> {$log.user}</span>
                        ...
                        {else}
                            <p class="text-muted text-sm m-0">暂无日志条目。</p>
                        {/if}
                    </div>
                </div>
    {/if}
...
(function () {
    ...
    window.clearLogs = function () {
        if (!window.confirm('清空整个系统日志？此操作不可撤销。')) return;
