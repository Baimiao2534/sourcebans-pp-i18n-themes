...
{else}
    <form method="post"
        ...
        <div class="card">
            <div class="card__header">
                <div>
                    <h3>Добавить мод</h3>
                    <p>Настройте новый игровой мод, который можно назначать банам и серверам.</p>
                </div>
            </div>
            <div class="card__body space-y-4" style="max-width:42rem">
                ...
                <div>
                    <label class="label" for="name">Название мода</label>
                    <input class="input"
                           type="text"
                           id="name"
                           name="name"
                ...
                <div>
                    <label class="label" for="folder">Папка мода</label>
                    <input class="input"
                           type="text"
                           id="folder"
                           name="folder"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        Имя папки на диске (например, <span class="font-mono">cstrike</span> для Counter-Strike: Source).
                    </p>
                    <div id="folder.msg"
                         class="text-xs"
                         style="color:var(--danger);display:none;margin-top:0.25rem"></div>
                ...
                <div>
                    <label class="label" for="steam_universe">Номер вселенной Steam</label>
                    <input class="input"
                           type="number"
                           id="steam_universe"
                           name="steam_universe"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        Первая цифра (X) в <span class="font-mono">STEAM_X:Y:Z</span>, отображаемая этим модом. По умолчанию 0.
                    </p>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox"
                               ...
                               value="1"
                               checked>
                        <span class="text-sm">Включён — назначается банам и серверам.</span>
                    </label>
                </div>

                <div>
                    <label class="label">Иконка мода</label>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xs text-muted" data-testid="addmod-current-icon" hidden>
                            Текущая: <span class="font-mono" data-testid="addmod-current-icon-name"></span>
                        </span>
                    </div>
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        16x16 GIF, PNG или JPG. Открывает всплывающее окно загрузки.
                    </p>
                    <div id="icon.msg"
                         class="text-xs"
                ...
                <div class="flex justify-end gap-2"
                    ...
                    <a class="btn btn--ghost btn--sm"
                       ...
                       data-testid="addmod-cancel">Назад</a>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            var errors = 0;
            if (!name)   { showMsg('name.msg',   'Необходимо указать название мода.'); errors++; }
            if (!folder) { showMsg('folder.msg', "Необходимо указать имя папки мода."); errors++; }
            if (!icon)   { showMsg('icon.msg',   'Необходимо загрузить иконку для мода.'); errors++; }
            if (errors > 0) return;
            ...
            if (!a || !A) {
                ...
                showMsg('name.msg', 'JSON-диспетчер отсутствует. Обновите страницу и повторите попытку.');
                return;
            }
            ...
            a.call(A.ModsAdd, {
                ...
                if (r.ok === false) {
                    ...
                    var slot = field === 'folder' ? 'folder.msg'
                        ...
                        : 'name.msg';
                    showMsg(slot, em);
                    toast('error', 'Не удалось добавить мод', em);
                    return;
                }
                var data = r.data || {};
                var msg = data.message || {};
                toast('success', msg.title || 'Мод добавлен', msg.body || 'Мод был успешно добавлен.');
                ...
                toast('error', 'Не удалось добавить мод', String(err && err.message ? err.message : err));
            });
        });
    })();
    </script>
