...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">管理員操作與警告的系統日誌。</p>
    </div>
                    ...
                    <div class="card__body">
                        ...
                        {if count($log_items) > 0}
                            ...
                            <table class="table table--clickable-rows" data-testid="logs-table">
                                <thead>
                                    <tr>
                                        <th style="width:6rem">類型</th>
                                        <th>事件</th>
                                        <th style="width:14rem">使用者</th>
                                        <th style="width:14rem">日期</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {foreach from=$log_items item="log"}
                                        ...
                                        <tr data-testid="log-row"
                                            ...
                                            <td>
                                                {if $log.type == 'm'}
                                                    <span class="pill pill--online"><i data-lucide="info" style="width:0.75rem;height:0.75rem"></i> 資訊</span>
                                                {elseif $log.type == 'w'}
                                                    <span class="pill pill--active"><i data-lucide="alert-triangle" style="width:0.75rem;height:0.75rem"></i> 警告</span>
                                                {elseif $log.type == 'e'}
                                                    <span class="pill pill--permanent"><i data-lucide="circle-x" style="width:0.75rem;height:0.75rem"></i> 錯誤</span>
                                                {else}
                                        ...
                                        <tr data-detail-for="{$log.lid}" id="log-detail-{$log.lid}" hidden>
                                            <td colspan="4" style="background:var(--bg-muted)">
                                                <dl class="grid gap-2 text-xs" style="grid-template-columns:8rem 1fr;margin:0">
                                                    ...
                                                    <dt class="text-muted">函式</dt>
                                                    <dd class="font-mono" style="margin:0">{$log.function}</dd>
                                                    <dt class="text-muted">查詢</dt>
                            ...
                            {* #1462: paired mobile card surface. The desktop table
                               ...
                               the summary. *}
                            <ul class="log-cards" data-testid="logs-cards" aria-label="系統日誌項目">
                                {foreach from=$log_items item="log"}
                                    <li>
                                        <details class="log-card"
                                            ...
                                            <div class="log-card__meta">
                                                <span><span class="text-muted">使用者：</span> {$log.user}</span>
                                                <span><span class="text-muted">日期：</span> {$log.date_str}</span>
                                            </div>
                        ...
                        {else}
                            <p class="text-muted text-sm m-0">沒有日誌項目。</p>
                        {/if}
                    </div>
                </div>
    {/if}
...
(function () {
    ...
    window.clearLogs = function () {
        if (!window.confirm('清空整個系統日誌？此操作無法復原。')) return;
