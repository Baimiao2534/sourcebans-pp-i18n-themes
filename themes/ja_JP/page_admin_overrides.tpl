...
{if not $permission_addadmin}
    <div class="card">
        <div class="card__body">
            <p class="text-muted m-0">アクセス拒否。</p>
        </div>
    </div>
{else}
    <div class="mb-4">
        <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">コマンドとグループのオーバーライド</h1>
        <p class="text-sm text-muted m-0 mt-2">任意の SourceMod コマンドの実行に必要なフラグを、グローバルまたはグループ別にオーバーライドします。</p>
    </div>
    ...
    {if $overrides_save_success}
        <div class="card mb-4" role="status" style="border-color:var(--success);background:var(--success-bg)">
            <div class="card__body" style="color:#047857" data-testid="overrides-success">
                <strong>保存されました。</strong>オーバーライドの変更が適用されました。
            </div>
        </div>
    {/if}
    ...
    <form method="post" action="index.php?p=admin&amp;c=admins&amp;section=overrides" data-testid="overrides-form">
        ...
        <div class="card mb-4">
            <div class="card__header">
                <div>
                    <h3>既存のオーバーライド</h3>
                    <p>
                        名前を空にすると保存時にそのオーバーライドが削除されます。
                        フラグリファレンスについては AlliedModders Wiki の
                        <a href="https://wiki.alliedmods.net/Overriding_Command_Access_%28SourceMod%29"
                              target="_blank" rel="noopener noreferrer">コマンドアクセスのオーバーライド</a>
                        を参照してください。
                    </p>
                </div>
            </div>
            <div class="card__body">
                <table class="table" data-testid="overrides-table">
                    <thead>
                        <tr>
                            <th style="width:8rem">タイプ</th>
                            <th>名前</th>
                            <th style="width:14rem">フラグ</th>
                        </tr>
                    </thead>
                    <tbody>
                        {foreach from=$overrides_list item=override}
                            <tr data-testid="override-row" data-id="{$override.oid}">
                                <td>
                                    <select class="select" name="override_type[]" aria-label="オーバーライドタイプ">
                                        <option value="command"{if $override.type == "command"} selected="selected"{/if}>コマンド</option>
                                        <option value="group"{if $override.type == "group"} selected="selected"{/if}>グループ</option>
                                    </select>
        ...
        <div class="card mb-4" data-testid="admin-admins-section-add-override">
            <div class="card__header">
                <div>
                    <h3>オーバーライドを追加</h3>
                    <p>タイプを選択し、コマンド/グループ名と管理者が実行に必要なフラグを入力してください。</p>
                </div>
            </div>
            <div class="card__body">
                <div class="grid gap-3" style="grid-template-columns:8rem 1fr 14rem">
                    <div>
                        <label class="label" for="addoverride-type">タイプ</label>
        ...
        <div class="flex justify-end gap-2">
            <a class="btn btn--ghost" href="javascript:history.go(-1);" data-testid="overrides-back">戻る</a>
            <button class="btn btn--primary" type="submit" data-testid="overrides-save">変更を保存</button>
        </div>
    </form>
{/if}
</div>
