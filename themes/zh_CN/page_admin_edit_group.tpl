...
<div class="space-y-4" data-testid="admin-edit-group">
    <div class="mb-4">
        ...
        <p class="text-sm text-muted m-0 mt-2">
            {if $type === 'srv'}服务器分组：SourceMod管理员标志 + 按命令覆盖。
            {else}网页分组：面板侧权限。{/if}
        </p>
    </div>
    ...
    <form id="edit-group-form" class="space-y-4" autocomplete="off"
        ...
        <div class="card">
            ...
            <div class="card__body">
                <label class="label" for="groupname">分组名称</label>
        ...
        {else}
                ...
                <div class="card__body">
                    <table class="table" style="margin:0;font-size:var(--fs-sm)">
                        ...
                        <tbody>
                            ...
                            {/if}
                            <tr><td>预留席位</td><td class="text-center font-mono">a</td><td>预留席位访问权限。</td>
                                <td class="text-center"><input type="checkbox" id="s1" data-sm-flag="a" {if $sm_reserved_slot}checked{/if}></td></tr>
                            <tr><td>通用</td><td class="text-center font-mono">b</td><td>通用管理员；管理员必填。</td>
                            ...
                            <tr><td>击杀</td><td class="text-center font-mono">f</td><td>击杀/伤害其他玩家。</td>
                                <td class="text-center"><input type="checkbox" id="s5" data-sm-flag="f" {if $sm_slay}checked{/if}></td></tr>
                            <tr><td>地图切换</td><td class="text-center font-mono">g</td><td>切换地图或主要游戏功能。</td>
                                <td class="text-center"><input type="checkbox" id="s6" data-sm-flag="g" {if $sm_map}checked{/if}></td></tr>
                            <tr><td>修改cvar</td><td class="text-center font-mono">h</td><td>修改大部分cvar。</td>
                            ...
                            <tr><td>服务器密码</td><td class="text-center font-mono">l</td><td>在服务器上设置密码。</td>
                                <td class="text-center"><input type="checkbox" id="s11" data-sm-flag="l" {if $sm_password}checked{/if}></td></tr>
                            <tr><td>运行RCON命令</td><td class="text-center font-mono">m</td><td>使用RCON命令。</td>
                                <td class="text-center"><input type="checkbox" id="s12" data-sm-flag="m" {if $sm_rcon}checked{/if}></td></tr>
                            <tr><td>启用作弊</td><td class="text-center font-mono">n</td><td>修改sv_cheats或使用作弊命令。</td>
            ...
            <div class="card">
                <div class="card__header"><div>
                    <h3>分组覆盖</h3>
                    <p>
                        分组覆盖允许完全允许或拒绝组成员执行特定命令或命令组。阅读 AlliedModders 维基中的
                        <a href="http://wiki.alliedmods.net/Adding_Groups_%28SourceMod%29" target="_blank" rel="noopener noreferrer">分组覆盖</a>
                        相关内容。
                    </p>
                    <p class="text-sm text-muted m-0 mt-2">
                        清空覆盖的名称将删除它。
                ...
                <div class="card__body">
                    <table class="table" style="margin:0;font-size:var(--fs-sm)" id="overrides-table">
                        <thead><tr>
                            <th style="width:7rem">类型</th>
                            <th>名称</th>
                            <th style="width:7rem">访问</th>
                        </tr></thead>
                        <tbody data-overrides-body>
                                ...
                                <tr data-override-row data-override-id="{$override.id}">
                                    <td>
                                        <select class="input" data-override-field="type">
                                            <option value="command" {if $override.type === 'command'}selected{/if}>命令</option>
                                            <option value="group"   {if $override.type === 'group'}selected{/if}>分组</option>
                                        </select>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <select class="input" data-override-field="access">
                                            <option value="allow" {if $override.access === 'allow'}selected{/if}>允许</option>
                                            <option value="deny"  {if $override.access === 'deny'}selected{/if}>拒绝</option>
                                        </select>
                                    </td>
                                </tr>
                            {/foreach}
                            <tr data-override-new>
                                ...
                                <td><input class="input" type="text" id="new_override_name" placeholder="（留空 = 不新增覆盖）"></td>
                                <td>
        ...
        <div class="flex gap-2">
            <button type="submit" class="btn btn--primary"
                    data-testid="admin-edit-group-save">保存更改</button>
