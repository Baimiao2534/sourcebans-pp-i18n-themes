  ...
  <div class="card__body">
    ...
    <div class="mt-6">
      {foreach from=$othercomments item=com name=othercomments}
        {if $smarty.foreach.othercomments.first}<h3 style="font-size:var(--fs-base);font-weight:600;margin:0 0 0.5rem">その他のコメント</h3>{/if}
        <div class="mt-4" style="border-top:1px solid var(--border);padding-top:0.75rem">
          <div class="flex items-center justify-between">
            ...
            {if $hideadminname}<i class="text-faint">非表示</i>{elseif !empty($com.comname)}<strong>{$com.comname|escape}</strong>{else}<i class="text-faint">削除済み管理者</i>{/if}
          ...
          <div class="text-xs text-faint mt-2">最終編集 {$com.edittime} by {if $hideadminname}<i class="text-faint">非表示</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>削除済み管理者</i>{/if}</div>
          {/if}
        </div>
      {/foreach}
    </div>
...
<div id="banlist-root" class="p-6 space-y-4" style="max-width:1700px;margin:0 auto" data-loading="false">
  <div class="flex items-center justify-between gap-3" style="flex-wrap:wrap">
    <div>
      <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">BAN リスト</h1>
      <p class="text-sm text-muted m-0 mt-2"><span class="tabular-nums">{$ban_list|@count}</span> / 全 <span class="tabular-nums">{$total_bans}</span> 件</p>
    </div>
    <div class="flex gap-2 items-center">
      {if $can_export}
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=steam" title="永続 SteamID BAN をエクスポート">Steam をエクスポート</a>
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=ip" title="永続 IP BAN をエクスポート">IP をエクスポート</a>
      {/if}
      ...
      <a class="btn btn--secondary btn--sm"
         ...
         data-testid="toggle-hide-inactive">{if $hidetext == 'Show'}表示{else}非表示{/if}期限切れ</a>
      {/if}
      {has_access flag=$smarty.const.ADMIN_ADD_BAN}
      <a class="btn btn--primary btn--sm" href="index.php?p=admin&c=bans">BAN を追加</a>
      {/has_access}
    </div>
  </div>
  ...
  <form method="get" action="index.php" id="banlist-filters" style="position:sticky;top:3.5rem;z-index:20;background:var(--bg-page);padding:0.75rem 0;border-bottom:1px solid var(--border)">
    ...
    <div class="flex gap-3" style="flex-wrap:wrap">
      <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
        <input class="input" type="search" name="searchText" data-testid="bans-search"
          ...
          placeholder="プレイヤー、SteamID または IP&hellip;" aria-label="BAN を検索">
      </div>
      <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="bans-server-filter" aria-label="サーバーで絞り込み">
      ...
      <select class="select" name="time" style="width:auto" data-testid="bans-time-filter" aria-label="期間で絞り込み">
        <option value="">すべての期間</option>
        <option value="1d"  {if $filters.time == '1d'}selected{/if}>今日</option>
        <option value="7d"  {if $filters.time == '7d'}selected{/if}>過去 7 日間</option>
        <option value="30d" {if $filters.time == '30d'}selected{/if}>過去 30 日間</option>
      </select>
      <button class="btn btn--secondary btn--sm" type="submit" data-testid="bans-filter-apply">
        <i data-lucide="filter"></i> 適用
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="状態で絞り込み">
      <a class="chip"
         href="index.php?p=banlist{$chip_base_link}"
         data-state-filter=""
         data-testid="filter-chip-all"
         data-active="{if $active_state == ''}true{else}false{/if}"
         aria-current="{if $active_state == ''}true{else}false{/if}">すべて</a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#ef4444"></span>永続
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#f59e0b"></span>有効
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:var(--zinc-300)"></span>期限切れ
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#10b981"></span>BAN 解除済み
      </a>
    </div>
  </form>
  ...
  <div class="card" style="overflow:hidden">
    ...
    <table class="table">
      <thead>
        ...
        <tr>
          ...
          <th scope="col">理由</th>
          <th scope="col" class="col-tier-2">サーバー</th>
          {if !$hideadminname}<th scope="col" class="col-admin col-tier-2">管理者</th>{/if}
          <th scope="col" class="col-length col-tier-3">期間</th>
          <th scope="col" class="col-banned col-tier-3">BAN 日時</th>
          <th scope="col" class="col-status">状態</th>
          <th scope="col" class="col-actions" aria-label="行操作"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="ban-row ban-row--{$ban.state}" data-state="{$ban.state}" data-id="{$ban.bid}" data-testid="ban-row">
          <td>
            <div class="flex items-center gap-3" style="min-width:0">
              ...
              <a class="font-medium truncate" href="?p=banlist&amp;id={$ban.bid}" data-drawer-href="?p=banlist&amp;c=details&amp;id={$ban.bid}" data-testid="drawer-trigger">{if empty($ban.name)}<i class="text-faint">ニックネームなし</i>{else}{$ban.name|escape}{/if}</a>
            </div>
            ...
            <details class="ban-comments-inline"
              ...
              <summary class="ban-comments-inline__summary"
                       ...
                       title="この BAN には {$ban.commentdata|@count} 件のコメントがあります">
                ...
                <span class="ban-comments-inline__label">件のコメント</span>
              </summary>
              <ul class="ban-comments-inline__list" data-testid="ban-comments-list">
                ...
                <li class="ban-comments-inline__item" data-testid="ban-comment-item">
                  ...
                  {if !empty($com.edittime)}
                  <div class="ban-comments-inline__edit text-xs text-faint">最終編集 {$com.edittime} by {if $hideadminname}<i class="text-faint">非表示</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>削除済み管理者</i>{/if}</div>
                  {/if}
                </li>
                {/foreach}
              </ul>
          ...
          <td class="text-muted truncate" style="max-width:18rem"
            ...
            {if empty($ban.reason)}<i class="text-faint">理由なし</i>{else}{$ban.reason|escape}{/if}
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint mt-1" data-testid="ban-unban-meta">
                {if !empty($ban.removedby)}
                  <span class="font-medium">{$ban.removedby|escape}</span> が BAN を解除{if !empty($ban.ureason)}: {/if}
                {/if}
          ...
          <td class="col-admin col-tier-2 text-muted">
            {if empty($ban.aname)}<i class="text-faint">削除済み</i>{else}{$ban.aname|escape}{/if}
          </td>
          {/if}
          ...
          <td class="col-length col-tier-3 tabular-nums text-muted"
              {if $ban.length != 0}title="{$ban.length_human|escape}"{/if}>{if $ban.length == 0}永続{else}{$ban.length_human|escape}{/if}</td>
          ...
          <td class="col-status">
            ...
            <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}有効{elseif $ban.state == 'permanent'}永続{elseif $ban.state == 'expired'}期限切れ{elseif $ban.state == 'unbanned'}BAN 解除済み{else}{$_pill_label|escape}{/if}</span>
          </td>
        ...
        <tr>
          ...
          <td colspan="10" style="padding:0">
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="true">
              ...
              <h2 class="empty-state__title">フィルターに一致する BAN はありません</h2>
              <p class="empty-state__body">別の検索語を試すか、アクティブなフィルターをクリアして記録されたすべての BAN を表示してください。</p>
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="false">
              ...
              <h2 class="empty-state__title">BAN はまだ記録されていません</h2>
              <p class="empty-state__body">管理者がモデレーションを開始すると、執行アクションがここに表示されます。</p>
              {if $can_add_ban}
              <div class="empty-state__actions">
    ...
    <div class="ban-cards">
        ...
        <a class="ban-card__summary flex items-center gap-3 p-4"
          ...
          <div style="flex:1;min-width:0">
            <div class="flex items-center gap-2">
              <span class="font-medium text-sm truncate">{if empty($ban.name)}<i class="text-faint">ニックネームなし</i>{else}{$ban.name|escape}{/if}</span>
              {assign var=_m_pill_label value=$ban.state|capitalize}
              <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}有効{elseif $ban.state == 'permanent'}永続{elseif $ban.state == 'expired'}期限切れ{elseif $ban.state == 'unbanned'}BAN 解除済み{else}{$_m_pill_label|escape}{/if}</span>
            </div>
            ...
            <div class="text-xs text-muted truncate" style="margin-top:0.125rem"
                 title="{if empty($ban.reason)}理由なし{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永続{else}{$ban.length_human|escape}{/if}">{if empty($ban.reason)}理由なし{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}永続{else}{$ban.length_human|escape}{/if}</div>
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="ban-unban-meta-mobile"
                   title="{if !empty($ban.removedby)}{$ban.removedby|escape} が BAN を解除{if !empty($ban.ureason)}: {/if}{/if}{if !empty($ban.ureason)}{$ban.ureason|escape}{/if}">
                {if !empty($ban.removedby)}{$ban.removedby|escape} が BAN を解除{if !empty($ban.ureason)}: {/if}{/if}
      ...
      <div class="empty-state" data-testid="banlist-empty-mobile" data-filtered="true">
        ...
        <h2 class="empty-state__title">フィルターに一致する BAN はありません</h2>
        <p class="empty-state__body">別の検索語を試すか、アクティブなフィルターをクリアしてください。</p>
        <div class="empty-state__actions">
  ...
  <div class="card">
    <div class="card__body">
      <nav class="text-xs text-muted" aria-label="BAN リストのページネーション">
...
<dialog id="bans-unban-dialog"
  ...
  <form method="dialog" data-testid="bans-unban-form">
    <h2 id="bans-unban-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">プレイヤーの BAN を解除</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      なぜ <strong data-testid="bans-unban-target">このプレイヤー</strong> の BAN を解除するのですか？この理由は BAN に記録され、監査ログに表示されます。
    </p>
    <label class="label" for="bans-unban-reason">BAN 解除の理由 <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="誤った BAN、申し立てが承認、刑期満了&hellip;"></textarea>
    <p class="text-xs" data-testid="bans-unban-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="bans-unban-cancel" value="cancel">キャンセル</button>
      <button type="submit" class="btn btn--primary" data-testid="bans-unban-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> BAN 解除
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnbanned(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === 'Active' || txt === 'Permanent' || txt === 'Expired' || txt === '有効' || txt === '永続' || txt === '期限切れ') {
                if (lastText) lastText.textContent = 'BAN 解除済み';
                else pill.textContent = 'BAN 解除済み';
            }
        });
    ...
    document.addEventListener('click', function (e) {
        ...
        if (act === 'bans-delete') {
            ...
            // page load.
            if (!fallback) return;
            if (!window.confirm('「' + name + '」の BAN を完全に削除しますか？この操作は元に戻せません。')) return;
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.BansUnban, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            setBusy(submitBtn, false);
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || '不明なエラー';
                showError(msg);
                toast('error', 'BAN 解除に失敗しました', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnbanned);
            closeUnbanDialog();
            toast('success', 'プレイヤーの BAN を解除しました', ctx.name + ' の BAN を解除しました。');
        });
    });
