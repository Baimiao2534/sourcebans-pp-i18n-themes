...
<form method="get"
    ...
    <div class="card__header">
        <div>
            <h3>進階搜尋</h3>
            <p>每一列都是獨立的搜尋條件。右側的 &laquo;搜尋&raquo; 按鈕僅使用該列的值進行提交。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-name" style="grid-column:1;align-self:end">暱稱</label>
            <input class="input"
                   id="search-comms-name"
                   type="text"
                   placeholder="與玩家暱稱進行子字串比對&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        style="width:9rem">
                    <option value="0" selected>完全比對</option>
                    <option value="1">部分比對</option>
                </select>
            </div>
            <button type="submit"
                    class="btn btn--secondary btn--sm"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-reason" style="grid-column:1;align-self:end">原因</label>
            <input class="input"
                   id="search-comms-reason"
                   type="text"
                   placeholder="與記錄原因進行子字串比對&hellip;"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">日期</span>
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <span class="label" style="grid-column:1;align-self:end">時長</span>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-length-op"
                        data-testid="search-comms-length-op"
                        style="width:5rem">
                    <option value="e"  title="等於">=</option>
                    <option value="h"  title="大於">&gt;</option>
                    <option value="l"  title="小於">&lt;</option>
                    <option value="eh" title="大於或等於">&gt;=</option>
                    <option value="el" title="小於或等於">&lt;=</option>
                </select>
                <select class="select"
                    ...
                    <optgroup label="月">
                        <option value="40320">1 個月</option>
                        <option value="80640">2 個月</option>
                        <option value="120960">3 個月</option>
                        <option value="241920">6 個月</option>
                        <option value="483840">12 個月</option>
                    </optgroup>
                    <option value="other">其他時長（分鐘）&hellip;</option>
                </select>
                <input type="text"
        ...
        {if NOT $hideadminname}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-admin">管理員</label>
                <div class="flex items-center gap-2" style="flex-wrap:wrap">
                    <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
            <label class="label" for="search-comms-server">伺服器</label>
            <div class="flex items-center gap-2" style="flex-wrap:wrap">
                <select class="select"
                        id="search-comms-server"
                        data-testid="search-comms-server">
                    <option value="0" selected>網頁封禁</option>
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                data-sid="{$server.sid}"
                                data-ip="{$server.ip}"
                                data-port="{$server.port}"
                                data-server-host>載入中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
            <button type="submit"
        ...
        {if $is_admin}
            <div class="grid gap-3" style="grid-template-columns:10rem 1fr auto;align-items:end">
                <label class="label" for="search-comms-comment" style="grid-column:1;align-self:end">備註</label>
                <input class="input"
                       id="search-comms-comment"
                       type="text"
                       placeholder="與管理員備註進行子字串比對&hellip;"
