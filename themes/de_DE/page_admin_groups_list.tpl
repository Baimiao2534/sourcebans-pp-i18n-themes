...
{if NOT $permission_listgroups}
    <div class="card"><div class="card__body"><p class="text-muted m-0">Zugriff verweigert.</p></div></div>
{else}
...
<div class="space-y-6">
    ...
    <section data-testid="web-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Web-Administratorgruppen</h1>
                <p class="text-sm text-muted m-0 mt-2">Berechtigungs-Flags und Immunität für Web-Panel-Gruppen. Gesamt: {$web_group_count}.</p>
            </div>
        </div>
        ...
        {if $web_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-web" data-filtered="false">
                ...
                <h2 class="empty-state__title">Noch keine Web-Administratorgruppen</h2>
                <p class="empty-state__body">Web-Administratorgruppen bündeln Panel-Berechtigungen für eine Gruppe von Administratoren. Erstellen Sie eine, um mehreren Administratoren gleichzeitig dieselben Flags zuzuweisen.</p>
                    ...
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           ...
                           data-testid="admin-groups-empty-web-add">
                            ...
                            Web-Administratorgruppe hinzufügen
                        </a>
                    </div>
                {/if}
            </div>
                    ...
                    <a class="admin-groups-master-detail__row flex items-center gap-3 p-4"
                       ...
                       {if $selected_group && $selected_group.gid == $group.gid}aria-current="true"{/if}>
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="text-xs text-muted">{$web_admins[$smarty.foreach.web_group.index]} Mitglied{if $web_admins[$smarty.foreach.web_group.index] != 1}er{/if}</div>
                ...
                <form class="card"
                    ...
                    <div class="card__header">
                        <div>
                            ...
                            <p>{$selected_group.member_count} Mitglied{if $selected_group.member_count != 1}er{/if}</p>
                        </div>
                        {if $permission_deletegroup}
                            <button type="button"
                                    class="btn btn--ghost btn--sm"
                                    data-testid="group-delete"
                                    onclick="SbppGroupsDelete({$selected_group.gid}, '{$selected_group.name|escape:'javascript'}', this);">Gruppe löschen</button>
                        {/if}
                    </div>
                    <div class="card__body space-y-4">
                        ...
                        <div>
                            <label class="label" for="group-name-input">Gruppenname</label>
                            <input class="input"
                                   id="group-name-input"
                                   name="name"
                        ...
                        <div>
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <label class="label m-0">Berechtigungs-Flags</label>
                                ...
                                <span class="text-xs text-muted" data-testid="flag-bitmask">{$selected_group.flags} Bitmaske</span>
                            </div>
                        ...
                        {if $permission_editgroup}
                            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                                <a class="btn btn--ghost" href="?p=admin&c=groups">Verwerfen</a>
                                <button class="btn btn--primary"
                                        type="submit"
                                        data-testid="group-save">Änderungen speichern</button>
                            </div>
                        {/if}
                    </div>
                </form>
            {else}
                <div class="card"><div class="card__body"><p class="text-muted m-0">Wählen Sie links eine Gruppe aus, um ihre Flags zu bearbeiten.</p></div></div>
            {/if}
        </div>
        {/if}
    </section>
    ...
    <section data-testid="server-admin-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">Server-Administratorgruppen</h2>
                <p class="text-sm text-muted m-0 mt-2">SourceMod-Administratorgruppen (In-Game-Flags). Gesamt: {$server_admin_group_count}.</p>
            </div>
        </div>
        ...
        {if $server_admin_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server-admin" data-filtered="false">
                ...
                <h2 class="empty-state__title">Noch keine Server-Administratorgruppen</h2>
                <p class="empty-state__body">Server-Administratorgruppen tragen SourceMod-Zeichen-Flags und Immunität. Erstellen Sie eine, um einer Gruppe von Administratoren In-Game-Administratorrechte zu gewähren.</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                           data-testid="admin-groups-empty-server-admin-add">
                            <i data-lucide="plus" style="width:13px;height:13px"></i>
                            Server-Administratorgruppe hinzufügen
                        </a>
                    </div>
                {/if}
            </div>
                    ...
                    <article class="card" data-testid="server-admin-group-row" data-id="{$group.id}">
                        <div class="card__header">
                            <div>
                                ...
                                <p>{$server_admins[$smarty.foreach.server_admin_group.index]} Mitglied{if $server_admins[$smarty.foreach.server_admin_group.index] != 1}er{/if} &middot; Immunität {$group.immunity}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=srv&id={$group.id|escape:'url'}">Bearbeiten</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.id}, '{$group.name|escape:'javascript'}', 'srv', this);">Löschen</button>
                                {/if}
                            </div>
                        </div>
                        <div class="card__body space-y-3">
                            <div>
                                <div class="text-xs font-semibold text-muted mb-2">Berechtigungen</div>
                                ...
                                {else}
                                    <p class="text-xs text-muted m-0"><em>Keine</em></p>
                                {/if}
                            </div>
                            {if $server_admins_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">Mitglieder</div>
                                        ...
                                        {foreach from=$server_admins_list[$smarty.foreach.server_admin_group.index] item="server_admin"}
                                            <li class="flex items-center justify-between gap-2 text-sm">
                                                ...
                                                {if $permission_editadmin}
                                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=admins&o=editgroup&id={$server_admin.aid|escape:'url'}">Bearbeiten</a>
                                                {/if}
                                            </li>
                                        {/foreach}
                                    </ul>
    ...
    <section data-testid="server-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">Servergruppen</h2>
                <p class="text-sm text-muted m-0 mt-2">Gruppierungen von Spielservern (keine Berechtigungs-Flags). Gesamt: {$server_group_count}.</p>
            </div>
        </div>
        ...
        {if $server_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server" data-filtered="false">
                ...
                <h2 class="empty-state__title">Noch keine Servergruppen</h2>
                <p class="empty-state__body">Servergruppen bündeln Spielserver, sodass Sie Administratoren mehrere Server gleichzeitig zuweisen können. Erstellen Sie eine, um Ihre Server zu gruppieren.</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                ...
                {foreach from=$server_list item="group" name="server_group"}
                    <article class="card" data-testid="server-group-row" data-id="{$group.gid}">
                        <div class="card__header">
                            <div>
                                <h3>{$group.name|escape}</h3>
                                <p>{$server_counts[$smarty.foreach.server_group.index]} Server</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=server&id={$group.gid|escape:'url'}">Bearbeiten</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.gid}, '{$group.name|escape:'javascript'}', 'server', this);">Löschen</button>
                                {/if}
                            </div>
                        </div>
                        {*
                        ...
                        <div class="card__body">
                            ...
                            {if empty($group.servers)}
                                <p class="text-xs text-muted m-0" data-testid="server-group-empty">
                                    <em>Dieser Gruppe sind noch keine Server zugewiesen.</em>
                                </p>
                            {else}
                                <ul class="space-y-2"
                                        ...
                                        <li class="flex items-center gap-2"
                                            ...
                                            {if !$server.enabled}
                                                <span class="pill pill--offline"
                                                      ...
                                                      title="Deaktiviert — in öffentlichen Listen ausgeblendet und vom pro-Karten-Hydratations-Check übersprungen"
                                                      style="flex-shrink:0">Deaktiviert</span>
                                            {/if}
                                        </li>
                                    {/foreach}
                                </ul>
...
function SbppGroupsSave(event) {
    ...
    sb.api.call(Actions.GroupsEdit, {
        ...
        SbppGroupsApplyResponse(r, { defaultTitle: 'Gruppe aktualisiert' });
    });
    return false;
}
    ...
    sb.api.call(Actions.GroupsRemove, { gid: Number(gid), type: 'web' })
        .then(function (r) {
            ...
            if (r && r.ok && (r.data && (r.data.reload || (r.data.message && r.data.message.redir)))) {
                SbppGroupsApplyResponse(r, { defaultTitle: 'Gruppe gelöscht' });
                return;
            }
...
(function () {
    ...
    grid.addEventListener('change', function (event) {
        ...
        preview.textContent = SbppFoldFlags(grid) + ' Bitmaske';
    });
})();
{/literal}
</script>
