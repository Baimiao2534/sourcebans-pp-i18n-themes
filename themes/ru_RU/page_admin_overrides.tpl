...
{if not $permission_addadmin}
    <div class="card">
        <div class="card__body">
            <p class="text-muted m-0">Доступ запрещён.</p>
        </div>
    </div>
{else}
    <div class="mb-4">
        <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">Переопределения команд и групп</h1>
        <p class="text-sm text-muted m-0 mt-2">Переопределяйте флаги, необходимые для выполнения любой команды SourceMod — глобально или для отдельной группы.</p>
    </div>
    ...
    {if $overrides_save_success}
        <div class="card mb-4" role="status" style="border-color:var(--success);background:var(--success-bg)">
            <div class="card__body" style="color:#047857" data-testid="overrides-success">
                <strong>Сохранено.</strong> Изменения переопределений применены.
            </div>
        </div>
    {/if}
    ...
    <form method="post" action="index.php?p=admin&amp;c=admins&amp;section=overrides" data-testid="overrides-form">
        ...
        <div class="card mb-4">
            <div class="card__header">
                <div>
                    <h3>Существующие переопределения</h3>
                    <p>
                        Очистите имя, чтобы удалить переопределение при сохранении.
                        См. <a href="https://wiki.alliedmods.net/Overriding_Command_Access_%28SourceMod%29"
                              target="_blank" rel="noopener noreferrer">переопределение доступа к командам</a>
                        в вики AlliedModders для справки по флагам.
                    </p>
                </div>
            </div>
            <div class="card__body">
                <table class="table" data-testid="overrides-table">
                    <thead>
                        <tr>
                            <th style="width:8rem">Тип</th>
                            <th>Имя</th>
                            <th style="width:14rem">Флаги</th>
                        </tr>
                    </thead>
                    <tbody>
                        {foreach from=$overrides_list item=override}
                            <tr data-testid="override-row" data-id="{$override.oid}">
                                <td>
                                    <select class="select" name="override_type[]" aria-label="Тип переопределения">
                                        <option value="command"{if $override.type == "command"} selected="selected"{/if}>Команда</option>
                                        <option value="group"{if $override.type == "group"} selected="selected"{/if}>Группа</option>
                                    </select>
        ...
        <div class="card mb-4" data-testid="admin-admins-section-add-override">
            <div class="card__header">
                <div>
                    <h3>Добавить переопределение</h3>
                    <p>Выберите тип, укажите имя команды/группы и флаги, необходимые администраторам для её выполнения.</p>
                </div>
            </div>
            <div class="card__body">
                <div class="grid gap-3" style="grid-template-columns:8rem 1fr 14rem">
                    <div>
                        <label class="label" for="addoverride-type">Тип</label>
        ...
        <div class="flex justify-end gap-2">
            <a class="btn btn--ghost" href="javascript:history.go(-1);" data-testid="overrides-back">Назад</a>
            <button class="btn btn--primary" type="submit" data-testid="overrides-save">Сохранить изменения</button>
        </div>
    </form>
{/if}
</div>
