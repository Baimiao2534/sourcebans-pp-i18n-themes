        ...
        <div class="card__body">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <div class="text-xs text-muted">Panneau d'administration</div>
                </div>
            </div>

            <h1 class="m-0" style="font-size:var(--fs-2xl);font-weight:600">Se connecter</h1>
            <p class="text-sm text-muted mt-2">Utilisez vos identifiants administrateur, ou connectez-vous avec Steam.</p>
            ...
            -{if !$normallogin_show and !$steamlogin_show}-
                <div class="toast"
                    ...
                    <div class="text-sm">
                        La connexion est actuellement désactivée. Veuillez contacter l'administrateur du site.
                    </div>
                </div>
            -{/if}-
            ...
            -{if $steamlogin_show}-
                <a class="btn btn--secondary"
                   ...
                   style="width:100%;margin-top:1.5rem">
                    <i data-lucide="gamepad-2" aria-hidden="true"></i> Continuer avec Steam
                </a>
            -{/if}-
            ...
            *}-
            -{if $normallogin_show}-
                <form id="loginForm"
                    ...
                    <div>
                        <label class="label" for="loginUsername">Nom d'utilisateur</label>
                        <input class="input"
                               id="loginUsername"
                               name="username"
                               type="text"
                    ...
                    <div>
                        <label class="label" for="loginPassword">Mot de passe</label>
                        <input class="input"
                               id="loginPassword"
                               name="password"
                               type="password"
                    ...
                    <button class="btn btn--primary"
                        ...
                        Se connecter <i data-lucide="arrow-right" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-4 text-xs" style="text-align:center">
                    <a href="index.php?p=lostpassword" data-testid="login-lostpwd">Mot de passe oublié ?</a>
                </div>
            -{/if}-
        </div>
    </div>
...
(function () {
    ...
    var lockedBody = 'Votre compte est temporairement verrouillé en raison de trop nombreuses tentatives de connexion échouées. '
                   + 'Veuillez réessayer dans environ ' + time + ' minute'
                   + (time === 1 ? '' : 's') + '.';
    var msgs = {
        no_access:    { kind: 'error', title: 'Accès refusé',          body: "Vous n'avez pas la permission d'accéder à cette page. Veuillez vous connecter avec un compte qui y a accès." },
        empty_pwd:    { kind: 'info',  title: 'Mot de passe vide',     body: 'Votre compte a un mot de passe vide défini. Réinitialisez-le via le lien de mot de passe oublié, ou demandez à un administrateur de le faire pour vous.' },
        failed:       { kind: 'error', title: 'Échec de connexion',       body: "Le nom d'utilisateur ou le mot de passe que vous avez fourni est incorrect. Si vous avez oublié votre mot de passe, utilisez le lien de mot de passe oublié ci-dessous." },
        steam_failed: { kind: 'error', title: 'Échec de connexion Steam', body: "La connexion Steam a réussi, mais votre SteamID n'est associé à aucun compte." },
        locked:       { kind: 'error', title: 'Compte verrouillé',     body: lockedBody }
    };
    var msg = msgs[m];
    if (!msg) return;
    function show() {
