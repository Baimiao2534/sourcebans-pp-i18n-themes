    ...
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                ...
                <p>Geben Sie die E-Mail-Adresse Ihres Admin-Kontos ein und wir senden Ihnen einen Link zum Zurücksetzen Ihres Passworts.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                {csrf_field}
                <div>
                    <label class="label" for="lostpw-email">E-Mail-Adresse</label>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> Link zum Zurücksetzen senden
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; Zurück zur Anmeldung</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (!emailEl.value) {
            toast('warn', 'E-Mail erforderlich', 'Bitte geben Sie Ihre E-Mail-Adresse ein.');
            return;
        }
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || 'Unbekannter Fehler';
                toast('error', 'Fehler', emsg);
                return;
            }
            var msg = (res.data || {}).message;
            if (msg) {
                toast(mapKind(msg.kind), msg.title || 'Erledigt', msg.body || '');
            }
        });
    });
}());
