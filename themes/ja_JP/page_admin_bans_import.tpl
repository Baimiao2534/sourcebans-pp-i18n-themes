...
{if NOT $permission_import}
    <div class="card" data-testid="banimport-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">BAN をインポートする権限がありません。</p>
        </div>
    </div>
{else}
    <section class="p-6" data-testid="banimport-section" style="max-width:48rem">
        <div class="mb-6">
            <h1 style="font-size:1.5rem;font-weight:600;margin:0">BAN をインポート</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <code class="font-mono">banned_user.cfg</code> または
                <code class="font-mono">banned_ip.cfg</code> ファイルをアップロードして、
                既存の BAN を SourceBans に一括インポートします。
            </p>
        </div>
        <form id="banimport-form"
            ...
            <div>
                <label class="label" for="importFile">BAN ファイル</label>
            ...
            <label class="flex items-center gap-2 p-2"
                ...
                <div>
                    <span class="text-sm font-medium">Steam コミュニティからプレイヤー名を解決</span>
                    <div class="text-xs text-muted">
                        {if $extreq}
                            インポート中に各コミュニティ ID をオンラインで照会します
                            （<code class="font-mono">banned_user.cfg</code> のみで動作）。
                        {else}
                            <code class="font-mono">allow_url_fopen</code> がこのサーバーで
                            利用できないため無効です。
                        {/if}
                    </div>
                </div>
