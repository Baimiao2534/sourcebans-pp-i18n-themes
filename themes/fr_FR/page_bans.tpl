...
<div class="card" style="max-width:42rem;margin:1.5rem auto">
  <div class="card__header">
    <div>
      <h3>Commentaire {if $commenttype == 'ban'}de bannissement{elseif $commenttype == 'admin'}d'administration{else}{$commenttype|escape}{/if}</h3>
      <p>Visible par les administrateurs ; les fils de commentaires apparaissent aussi publiquement lorsque les commentaires publics sont activés.</p>
    </div>
  </div>
  <div class="card__body">
    <form id="banlist-comment-form" data-bid="{$comment}" data-ctype="{$ctype}" data-cid="{$cid}" data-page="{$page}">
      <label class="label" for="banlist-comment-text">Commentaire</label>
      <textarea class="textarea" id="banlist-comment-text" name="commenttext" rows="6" {if !$canedit}disabled{/if}>{$commenttext}</textarea>
      <div class="flex gap-2 mt-4">
        {if $canedit}
        <button class="btn btn--primary" type="submit">Commentaire {if $commenttype == 'ban'}de bannissement{elseif $commenttype == 'admin'}d'administration{else}{$commenttype|escape}{/if}</button>
        {/if}
    ...
    <div class="mt-6">
      {foreach from=$othercomments item=com name=othercomments}
        {if $smarty.foreach.othercomments.first}<h3 style="font-size:var(--fs-base);font-weight:600;margin:0 0 0.5rem">Autres commentaires</h3>{/if}
        <div class="mt-4" style="border-top:1px solid var(--border);padding-top:0.75rem">
          <div class="flex items-center justify-between">
            ...
            {if $hideadminname}<i class="text-faint">Masqué</i>{elseif !empty($com.comname)}<strong>{$com.comname|escape}</strong>{else}<i class="text-faint">administrateur supprimé</i>{/if}
          ...
          <div class="text-xs text-faint mt-2">dernière modification {$com.edittime} par {if $hideadminname}<i class="text-faint">Masqué</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>administrateur supprimé</i>{/if}</div>
          {/if}
        </div>
      {/foreach}
    </div>
...
<div id="banlist-root" class="p-6 space-y-4" style="max-width:1700px;margin:0 auto" data-loading="false">
  <div class="flex items-center justify-between gap-3" style="flex-wrap:wrap">
    <div>
      <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Liste des bannissements</h1>
      <p class="text-sm text-muted m-0 mt-2"><span class="tabular-nums">{$ban_list|@count}</span> sur <span class="tabular-nums">{$total_bans}</span> bannissements</p>
    </div>
    <div class="flex gap-2 items-center">
      {if $can_export}
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=steam" title="Exporter les bannissements SteamID permanents">Exporter Steam</a>
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=ip" title="Exporter les bannissements IP permanents">Exporter IP</a>
      {/if}
      ...
      <a class="btn btn--secondary btn--sm"
         ...
         data-testid="toggle-hide-inactive">{if $hidetext == 'Show'}Afficher{else}Masquer{/if} inactifs</a>
      {/if}
      {has_access flag=$smarty.const.ADMIN_ADD_BAN}
      <a class="btn btn--primary btn--sm" href="index.php?p=admin&c=bans">Ajouter un bannissement</a>
      {/has_access}
    </div>
  </div>
  ...
  <form method="get" action="index.php" id="banlist-filters" style="position:sticky;top:3.5rem;z-index:20;background:var(--bg-page);padding:0.75rem 0;border-bottom:1px solid var(--border)">
    ...
    <div class="flex gap-3" style="flex-wrap:wrap">
      <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
        <input class="input" type="search" name="searchText" data-testid="bans-search"
          ...
          placeholder="Joueur, SteamID ou IP&hellip;" aria-label="Rechercher des bannissements">
      </div>
      <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="bans-server-filter" aria-label="Filtrer par serveur">
        <option value="">Tous les serveurs</option>
        {foreach $server_list as $s}
          <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
        {/foreach}
      </select>
      <select class="select" name="time" style="width:auto" data-testid="bans-time-filter" aria-label="Filtrer par période">
        <option value="">Toutes les dates</option>
        <option value="1d"  {if $filters.time == '1d'}selected{/if}>Aujourd'hui</option>
        <option value="7d"  {if $filters.time == '7d'}selected{/if}>7 derniers jours</option>
        <option value="30d" {if $filters.time == '30d'}selected{/if}>30 derniers jours</option>
      </select>
      <button class="btn btn--secondary btn--sm" type="submit" data-testid="bans-filter-apply">
        <i data-lucide="filter"></i> Appliquer
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="Filtrer par statut">
      <a class="chip"
         href="index.php?p=banlist{$chip_base_link}"
         data-state-filter=""
         data-testid="filter-chip-all"
         data-active="{if $active_state == ''}true{else}false{/if}"
         aria-current="{if $active_state == ''}true{else}false{/if}">Tous</a>
      <a class="chip"
      ...
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#f59e0b"></span>Actif
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:var(--zinc-300)"></span>Expiré
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#10b981"></span>Débanni
      </a>
    </div>
  </form>
  ...
  <div class="card" style="overflow:hidden">
    ...
    <table class="table">
      <thead>
        ...
        <tr>
          ...
          <th scope="col" class="col-tier-2">Serveur</th>
          {if !$hideadminname}<th scope="col" class="col-admin col-tier-2">Administrateur</th>{/if}
          <th scope="col" class="col-length col-tier-3">Durée</th>
          <th scope="col" class="col-banned col-tier-3">Banni</th>
          <th scope="col" class="col-status">Statut</th>
          <th scope="col" class="col-actions" aria-label="Actions de ligne"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="ban-row ban-row--{$ban.state}" data-state="{$ban.state}" data-id="{$ban.bid}" data-testid="ban-row">
          <td>
            <div class="flex items-center gap-3" style="min-width:0">
              ...
              <a class="font-medium truncate" href="?p=banlist&amp;id={$ban.bid}" data-drawer-href="?p=banlist&amp;c=details&amp;id={$ban.bid}" data-testid="drawer-trigger">{if empty($ban.name)}<i class="text-faint">pas de pseudo</i>{else}{$ban.name|escape}{/if}</a>
            </div>
            ...
            <details class="ban-comments-inline"
              ...
              <summary class="ban-comments-inline__summary"
                       ...
                       title="{$ban.commentdata|@count} commentaire{if $ban.commentdata|@count != 1}s{/if} sur ce bannissement">
                ...
                <span class="ban-comments-inline__label">commentaire{if $ban.commentdata|@count != 1}s{/if}</span>
              </summary>
              <ul class="ban-comments-inline__list" data-testid="ban-comments-list">
                ...
                <li class="ban-comments-inline__item" data-testid="ban-comment-item">
                  <div class="ban-comments-inline__meta">
                    ...
                    {if $hideadminname}
                      <i class="text-faint">Masqué</i>
                    {elseif !empty($com.comname)}
                      <strong>{$com.comname|escape}</strong>
                    {else}
                      <i class="text-faint">administrateur supprimé</i>
                    {/if}
                  ...
                  {if !empty($com.edittime)}
                  <div class="ban-comments-inline__edit text-xs text-faint">dernière modification {$com.edittime} par {if $hideadminname}<i class="text-faint">Masqué</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>administrateur supprimé</i>{/if}</div>
                  {/if}
                </li>
                {/foreach}
              </ul>
          ...
          <td class="text-muted truncate" style="max-width:18rem"
            ...
            {if empty($ban.reason)}<i class="text-faint">aucun motif</i>{else}{$ban.reason|escape}{/if}
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint mt-1" data-testid="ban-unban-meta">
                {if !empty($ban.removedby)}
                  Débanni par <span class="font-medium">{$ban.removedby|escape}</span>{if !empty($ban.ureason)} : {/if}
                {/if}
          ...
          <td class="col-admin col-tier-2 text-muted">
            {if empty($ban.aname)}<i class="text-faint">supprimé</i>{else}{$ban.aname|escape}{/if}
          </td>
          {/if}
          ...
          <td class="col-status">
            ...
            <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}Actif{elseif $ban.state == 'permanent'}Permanent{elseif $ban.state == 'expired'}Expiré{elseif $ban.state == 'unbanned'}Débanni{else}{$_pill_label|escape}{/if}</span>
          </td>
        ...
        <tr>
          ...
          <td colspan="10" style="padding:0">
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="true">
              ...
              <h2 class="empty-state__title">Aucun bannissement ne correspond à ces filtres</h2>
              <p class="empty-state__body">Essayez un autre terme de recherche ou effacez les filtres actifs pour voir tous les bannissements enregistrés.</p>
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="false">
              ...
              <h2 class="empty-state__title">Aucun bannissement enregistré</h2>
              <p class="empty-state__body">Les actions de modération apparaîtront ici dès que les administrateurs commenceront à modérer.</p>
              {if $can_add_ban}
              <div class="empty-state__actions">
    ...
    <div class="ban-cards">
      ...
      <div class="ban-row ban-row--{$ban.state} ban-card"
        ...
        <a class="ban-card__summary flex items-center gap-3 p-4"
          ...
          <div style="flex:1;min-width:0">
            <div class="flex items-center gap-2">
              <span class="font-medium text-sm truncate">{if empty($ban.name)}<i class="text-faint">pas de pseudo</i>{else}{$ban.name|escape}{/if}</span>
              {assign var=_m_pill_label value=$ban.state|capitalize}
              <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}Actif{elseif $ban.state == 'permanent'}Permanent{elseif $ban.state == 'expired'}Expiré{elseif $ban.state == 'unbanned'}Débanni{else}{$_m_pill_label|escape}{/if}</span>
            </div>
            ...
            <div class="text-xs text-muted truncate" style="margin-top:0.125rem"
                 title="{if empty($ban.reason)}aucun motif{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}Permanent{else}{$ban.length_human|escape}{/if}">{if empty($ban.reason)}aucun motif{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}Permanent{else}{$ban.length_human|escape}{/if}</div>
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="ban-unban-meta-mobile"
                   title="{if !empty($ban.removedby)}Débanni par {$ban.removedby|escape}{if !empty($ban.ureason)} : {/if}{/if}{if !empty($ban.ureason)}{$ban.ureason|escape}{/if}">
                {if !empty($ban.removedby)}Débanni par {$ban.removedby|escape}{if !empty($ban.ureason)} : {/if}{/if}
        ...
        <div class="row-actions ban-card__actions">
          ...
          <div class="text-xs text-faint truncate" style="margin-top:0.125rem;display:flex;align-items:center;gap:0.25rem" data-testid="ban-comments-count-mobile">
            <i data-lucide="message-square-text" style="width:11px;height:11px" aria-hidden="true"></i>
            <span class="tabular-nums">{$ban.commentdata|@count}</span>
            <span>commentaire{if $ban.commentdata|@count != 1}s{/if}</span>
          </div>
          {/if}
        </div>
        {/if}
      ...
      <div class="empty-state" data-testid="banlist-empty-mobile" data-filtered="true">
        ...
        <h2 class="empty-state__title">Aucun bannissement ne correspond à ces filtres</h2>
        <p class="empty-state__body">Essayez un autre terme de recherche ou effacez les filtres actifs.</p>
        <div class="empty-state__actions">
  ...
  <div class="card">
    <div class="card__body">
      <nav class="text-xs text-muted" aria-label="Pagination de la liste des bannissements">
...
<dialog id="bans-unban-dialog"
  ...
  <form method="dialog" data-testid="bans-unban-form">
    <h2 id="bans-unban-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Débannir le joueur</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      Pourquoi débannissez-vous <strong data-testid="bans-unban-target">ce joueur</strong> ? Cette raison sera enregistrée et apparaîtra dans le journal d'audit.
    </p>
    <label class="label" for="bans-unban-reason">Motif de débannissement <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="Bannissement par erreur, appel accepté, peine purgée, &hellip;"></textarea>
    <p class="text-xs" data-testid="bans-unban-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="bans-unban-cancel" value="cancel">Annuler</button>
      <button type="submit" class="btn btn--primary" data-testid="bans-unban-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> Débannir
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnbanned(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === 'Active' || txt === 'Permanent' || txt === 'Expired' || txt === 'Actif' || txt === 'Expiré') {
                if (lastText) lastText.textContent = 'Débanni';
                else pill.textContent = 'Débanni';
            }
        });
    ...
    function openUnbanDialog(ctx) {
        ...
        if (target) target.textContent = ctx.name || ('bannissement #' + ctx.bid);
    ...
    document.addEventListener('click', function (e) {
        ...
        var name = actionBtn.getAttribute('data-name') || ('bannissement #' + bid);
        ...
        if (act === 'bans-delete') {
            ...
            // page load.
            if (!fallback) return;
            if (!window.confirm('Supprimer définitivement le bannissement de « ' + name + ' » ? Cette action est irréversible.')) return;
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.BansUnban, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            ...
            if (!r || r.ok === false) {
                ...
                showError(msg);
                toast('error', 'Échec du débannissement', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnbanned);
            closeUnbanDialog();
            toast('success', 'Joueur débanni', ctx.name + ' a été débanni.');
        });
    });
