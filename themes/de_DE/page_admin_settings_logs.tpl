...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Systemprotokoll von Administratoraktionen und Warnungen.</p>
    </div>
                ...
                <div class="card">
                    <div class="card__header">
                        <div>
                            ...
                            <p>Auf eine Zeile klicken zum Aufklappen. Neueste zuerst.</p>
                        </div>
                        {if $can_owner}
                            <button type="button"
                                    ...
                                    onclick="clearLogs();">
                                <i data-lucide="trash-2"></i> Protokoll leeren
                            </button>
                        {/if}
                    </div>
                    <div class="card__body">
                        ...
                        {if count($log_items) > 0}
                            ...
                            <table class="table table--clickable-rows" data-testid="logs-table">
                                <thead>
                                    <tr>
                                        <th style="width:6rem">Typ</th>
                                        <th>Ereignis</th>
                                        <th style="width:14rem">Benutzer</th>
                                    ...
                                    {foreach from=$log_items item="log"}
                                        ...
                                        <tr data-testid="log-row"
                                            ...
                                            <td>
                                                ...
                                                {elseif $log.type == 'w'}
                                                    <span class="pill pill--active"><i data-lucide="alert-triangle" style="width:0.75rem;height:0.75rem"></i> Warnung</span>
                                                {elseif $log.type == 'e'}
                                                    <span class="pill pill--permanent"><i data-lucide="circle-x" style="width:0.75rem;height:0.75rem"></i> Fehler</span>
                                                {else}
                                        ...
                                        <tr data-detail-for="{$log.lid}" id="log-detail-{$log.lid}" hidden>
                                            <td colspan="4" style="background:var(--bg-muted)">
                                                <dl class="grid gap-2 text-xs" style="grid-template-columns:8rem 1fr;margin:0">
                                                    ...
                                                    <dt class="text-muted">Funktion</dt>
                                                    <dd class="font-mono" style="margin:0">{$log.function}</dd>
                                                    <dt class="text-muted">Abfrage</dt>
                            ...
                            {* #1462: paired mobile card surface. The desktop table
                               ...
                               the summary. *}
                            <ul class="log-cards" data-testid="logs-cards" aria-label="Systemprotokoll-Einträge">
                                {foreach from=$log_items item="log"}
                                    <li>
                                        <details class="log-card"
                                            ...
                                            <div class="log-card__meta">
                                                <span><span class="text-muted">Benutzer:</span> {$log.user}</span>
                                                <span><span class="text-muted">Datum:</span> {$log.date_str}</span>
                                            </div>
                        ...
                        {else}
                            <p class="text-muted text-sm m-0">Noch keine Protokolleinträge.</p>
                        {/if}
                    </div>
                </div>
    {/if}
...
(function () {
    ...
    window.clearLogs = function () {
        if (!window.confirm('Das gesamte Systemprotokoll leeren? Diese Aktion kann nicht rückgängig gemacht werden.')) return;
