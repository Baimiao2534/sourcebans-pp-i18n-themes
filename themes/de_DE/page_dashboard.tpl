...
<div class="p-6 space-y-6" style="max-width:1400px;margin:0 auto;width:100%">
    ...
    <header data-testid="dashboard-header">
        ...
        <p class="text-sm text-muted m-0 mt-2">Aktivität auf Ihren Servern.</p>
    </header>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        <div class="card p-5" data-testid="dashboard-stat-total-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Banns gesamt</div>
        ...
        <div class="card p-5" data-testid="dashboard-stat-active-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Aktive Banns</div>
            ...
            <div class="text-xs text-muted">derzeitig gültig</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-comm-blocks">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Kommunikationssperren</div>
            ...
            <div class="text-xs text-muted">Stumm + Sperre</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-servers">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">Server</div>
            ...
            <div class="text-xs text-muted">konfiguriert</div>
        </div>
    ...
    {* -- Admins-only project announcement strip ----------------------
        ...
        hidden. *}
    {if $announcement}
    <aside class="announcement-strip" data-testid="dashboard-announcement" aria-label="Neueste Ankündigung">
        <details>
            <summary>
            ...
            <a class="announcement-strip__link" data-testid="dashboard-announcement-link"
               href="{$announcement.url}" target="_blank" rel="noopener noreferrer">
                Weiterlesen <i data-lucide="external-link" aria-hidden="true" style="width:13px;height:13px"></i>
            </a>
            {/if}
        </details>
    </aside>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))">
        ...
        <section class="card" data-testid="dashboard-recent-bans">
            <div class="card__header">
                <div>
                    <h3>Neueste Banns</h3>
                    <p>Die jüngsten Maßnahmen</p>
                </div>
                <a class="btn btn--ghost btn--sm" href="?p=banlist">
                    Alle ansehen <i data-lucide="arrow-right" style="width:14px;height:14px"></i>
                </a>
            </div>
            <div>
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $b.short_name}{$b.short_name}{else}<span class="text-faint">kein Spitzname</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-bans-empty">
                    ...
                    <h4 class="empty-state__title">Noch keine Banns</h4>
                    <p class="empty-state__body">Maßnahmen erscheinen hier, sobald Administratoren mit der Moderation beginnen.</p>
                    {if $can_add_ban}
        ...
        <section class="card" data-testid="dashboard-servers">
            <div class="card__header">
                <div>
                    <h3>Server</h3>
                    <p>{$total_servers|number_format} konfiguriert</p>
                </div>
            ...
            <div class="text-xs text-muted" style="border-bottom:1px solid var(--border);padding:0.625rem 1.25rem">
                ...
                Öffnen Sie die Server-Seite, um Spieler in Echtzeit zu verwalten.
            </div>
            {/if}
            {*
            ...
            *}
            <div style="padding:0.5rem"
                ...
                <div class="empty-state" data-testid="dashboard-servers-empty">
                    ...
                    <h4 class="empty-state__title">Keine Server konfiguriert</h4>
                    <p class="empty-state__body">Fügen Sie einen Server hinzu, damit Besucher den Live-Status sehen und direkt aus dem Panel beitreten können.</p>
                    {if $can_add_server}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(360px,1fr))">
        ...
        *}
        <section class="card" data-testid="dashboard-blocked-attempts" data-lognopopup="{if $dashboard_lognopopup}1{else}0{/if}">
            <div class="card__header">
                <div>
                    <h3>Neueste blockierte Versuche</h3>
                    <p>{$total_blocked|number_format} Versuche gesamt</p>
                </div>
            </div>
            <div>
                ...
                <a class="flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-blocked-row" data-id="{$p.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="font-medium text-sm truncate">
                            {if $p.short_name}{$p.short_name}{else}<span class="text-faint">kein Spitzname</span>{/if}
                        </div>
                ...
                <div class="empty-state" data-testid="dashboard-blocked-attempts-empty">
                    ...
                    <h4 class="empty-state__title">Noch keine blockierten Versuche</h4>
                    <p class="empty-state__body">Sobald gebannte Spieler versuchen beizutreten, protokolliert das SourceMod-Plugin jeden Versuch hier.</p>
                </div>
                {/foreach}
            </div>
        </section>
        ...
        <section class="card" data-testid="dashboard-recent-comms">
            <div class="card__header">
                <div>
                    <h3>Neueste Kommunikationssperren</h3>
                    <p>Die jüngsten Stummschaltungen und Sperren</p>
                </div>
            ...
            <div>
                ...
                <a class="ban-row ban-row--{$c.state} flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-comm-row" data-id="{$c.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $c.short_name}{$c.short_name}{else}<span class="text-faint">kein Spitzname</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-comms-empty">
                    ...
                    <h4 class="empty-state__title">Noch keine Kommunikationssperren</h4>
                    <p class="empty-state__body">Stummschaltungen und Sperren aus dem Panel oder im Spiel erscheinen hier.</p>
                    {if $can_add_ban}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
