...
{else}
    <form method="post"
        ...
        <div class="card">
            ...
            <div class="card__body space-y-4" style="max-width:42rem">
                ...
                <div>
                    <label class="label" for="name">模組名稱</label>
                    <input class="input"
                           type="text"
                           id="name"
                           name="name"
                ...
                <div>
                    <label class="label" for="folder">模組資料夾</label>
                    <input class="input"
                           type="text"
                           id="folder"
                           name="folder"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        磁碟上的資料夾名稱（例如 Counter-Strike: Source 為 <span class="font-mono">cstrike</span>）。
                    </p>
                    <div id="folder.msg"
                         class="text-xs"
                         style="color:var(--danger);display:none;margin-top:0.25rem"></div>
                ...
                <div>
                    <label class="label" for="steam_universe">Steam universe 編號</label>
                    <input class="input"
                           type="number"
                           id="steam_universe"
                           name="steam_universe"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        此模組所呈現之 <span class="font-mono">STEAM_X:Y:Z</span> 的第一位數字（X）。預設為 0。
                    </p>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox"
                               ...
                               value="1"
                               checked>
                        <span class="text-sm">已啟用 — 可指派給封禁與伺服器。</span>
                    </label>
                </div>

                <div>
                    <label class="label">模組圖示</label>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xs text-muted" data-testid="addmod-current-icon" hidden>
                            目前：<span class="font-mono" data-testid="addmod-current-icon-name"></span>
                        </span>
                    </div>
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        16x16 GIF、PNG 或 JPG。會開啟彈出式上傳器。
                    </p>
                    <div id="icon.msg"
                         class="text-xs"
                ...
                <div class="flex justify-end gap-2"
                    ...
                    <a class="btn btn--ghost btn--sm"
                       ...
                       data-testid="addmod-cancel">返回</a>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            var errors = 0;
            if (!name)   { showMsg('name.msg',   '必須輸入模組名稱。'); errors++; }
            if (!folder) { showMsg('folder.msg', '必須輸入模組的資料夾名稱。'); errors++; }
            if (!icon)   { showMsg('icon.msg',   '必須上傳模組圖示。'); errors++; }
            if (errors > 0) return;
            ...
            if (!a || !A) {
                ...
                showMsg('name.msg', 'JSON 派發器遺失。請重新整理頁面後再試一次。');
                return;
            }
            ...
            a.call(A.ModsAdd, {
                ...
                if (r.ok === false) {
                    ...
                    var slot = field === 'folder' ? 'folder.msg'
                        ...
                        : 'name.msg';
                    showMsg(slot, em);
                    toast('error', '新增模組失敗', em);
                    return;
                }
                var data = r.data || {};
                var msg = data.message || {};
                toast('success', msg.title || '模組已新增', msg.body || '此模組已新增。');
                ...
                toast('error', '新增模組失敗', String(err && err.message ? err.message : err));
            });
        });
    })();
    </script>
