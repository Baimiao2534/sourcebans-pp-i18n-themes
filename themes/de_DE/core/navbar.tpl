...
<aside class="sidebar" id="sidebar" data-mobile-open="false">
    ...
    <nav class="sidebar__nav" role="navigation" aria-label="Hauptnavigation">
        <div class="sidebar__section">
            <div class="sidebar__section-label">Öffentlich</div>
            {foreach from=$navbar item=nav}
                {if $nav.endpoint != 'admin'}
                    <a class="sidebar__link"
                       href="index.php?p={$nav.endpoint}"
                       data-testid="nav-{$nav.endpoint}"
                       title="{if $nav.endpoint == 'home'}Diese Seite zeigt eine Übersicht Ihrer Banns und Server.{elseif $nav.endpoint == 'servers'}Hier können Sie alle Server und deren Status einsehen.{elseif $nav.endpoint == 'banlist'}Hier können Sie alle Banns in der Datenbank einsehen.{elseif $nav.endpoint == 'commslist'}Hier können Sie alle Kommunikationsbanns (Text- und Stummschaltungen) in der Datenbank einsehen.{elseif $nav.endpoint == 'submit'}Hier können Sie Demodateien oder Screenshots von verdächtigen Spielern einreichen. Diese werden von einem Administrator überprüft.{elseif $nav.endpoint == 'protest'}Hier können Sie einen Einspruch gegen Ihren Bann einreichen und erklären, warum die Entsperrung erfolgen sollte.{else}{$nav.description}{/if}"
                       {if $nav.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $nav.endpoint == 'home'}Dashboard{elseif $nav.endpoint == 'servers'}Server{elseif $nav.endpoint == 'banlist'}Bannliste{elseif $nav.endpoint == 'commslist'}Kommunikation{elseif $nav.endpoint == 'submit'}Spieler melden{elseif $nav.endpoint == 'protest'}Bann anfechten{else}{$nav.title}{/if}
                    </a>
                {/if}
            {/foreach}
        </div>
        ...
        {if $isAdmin}
            <div class="sidebar__section">
                <div class="sidebar__section-label">Verwaltung</div>
                {foreach from=$navbar item=nav}
                    {if $nav.endpoint == 'admin'}
                        <a class="sidebar__link"
                           href="index.php?p={$nav.endpoint}"
                           data-testid="nav-{$nav.endpoint}"
                           title="Dies ist das SourceBans-Kontrollzentrum, in dem Sie neue Administratoren einrichten, neue Server hinzufügen usw."
                           {if $nav.state == 'active'}aria-current="page"{/if}>
                ...
                {foreach from=$adminbar item=admin}
                    <a class="sidebar__link"
                       ...
                       {if $admin.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $admin.endpoint == 'admins'}Administratoren{elseif $admin.endpoint == 'servers'}Server{elseif $admin.endpoint == 'bans'}Banns{elseif $admin.endpoint == 'comms'}Kommunikation{elseif $admin.endpoint == 'groups'}Gruppen{elseif $admin.endpoint == 'settings'}Einstellungen{elseif $admin.endpoint == 'mods'}Mods{elseif $admin.endpoint == 'export'}Export{else}{$admin.title}{/if}
                    </a>
                {/foreach}
            </div>
        {/if}
        ...
        {else}
            <a class="sidebar__link"
               ...
               data-testid="nav-login">
                <i data-lucide="log-in"></i> Anmelden
            </a>
        {/if}
    </div>
</aside>
