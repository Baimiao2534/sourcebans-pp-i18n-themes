...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">選擇要為所有訪客呈現面板介面的主題。</p>
    </div>
            <div class="card">
                <div class="card__header">
                    <div>
                        <h3>已安裝的主題</h3>
                        <p>每一張卡片代表 <code>web/themes/</code> 下一個含有 <code>theme.conf.php</code> 資訊清單的目錄。</p>
                    </div>
                    <div class="text-xs text-muted" style="text-align:right" data-testid="current-theme-summary">
                        目前使用中：<br>
                        <strong>{$theme_name}</strong> v{$theme_version} 作者 {$theme_author}{if $theme_link != ''} · <a href="{$theme_link}" target="_blank" rel="noopener" class="text-muted">首頁</a>{/if}<br>
                ...
                <div class="card__body">
                    {if NOT $can_web_settings}
                        <p class="text-xs text-muted m-0 mb-4" style="padding:0.5rem 0.75rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--bg-muted)">
                            ...
                            您可以瀏覽主題，但切換使用中的主題需要 <code>ADMIN_WEB_SETTINGS</code> 權限。
                        </p>
                    {/if}
                            ...
                            <article class="theme-card"
                                ...
                                <div style="aspect-ratio:250 / 170;background:var(--bg-muted);display:grid;place-items:center;overflow:hidden">
                                    <img src="{$t.screenshot}"
                                         ...
                                         onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\'text-muted text-xs\'>無截圖</span>'">
                                </div>
                                <div class="p-4 space-y-3" style="flex:1;display:flex;flex-direction:column">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            {if $t.active}
                                                <span class="pill pill--online" title="目前使用中">
                                                    <i data-lucide="check" style="width:0.75rem;height:0.75rem"></i> 使用中
                                                </span>
                                            {/if}
                                        </div>
                                        <p class="text-xs text-muted m-0 mt-2">作者 {$t.author} · v{$t.version}{if $t.link != ''} · <a href="{$t.link}" target="_blank" rel="noopener" class="text-muted" title="開啟主題首頁"><i data-lucide="external-link" style="width:0.75rem;height:0.75rem;vertical-align:-1px"></i> 首頁</a>{/if}</p>
                                        ...
                                        {if $can_web_settings}
                                            ...
                                            {else}
                                                <button type="button"
                                                    ...
                                                    <i data-lucide="check-circle"></i> 使用此主題
                                                </button>
                                            {/if}
                                        {/if}
                                    </div>
...
(function () {
    ...
    window.applyTheme = function (theme) {
        if (!theme) return;
        if (!window.confirm('要將面板主題切換為「' + theme + '」嗎？所有訪客下次請求時都會看到新主題。')) return;
