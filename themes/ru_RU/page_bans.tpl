...
<div class="card" style="max-width:42rem;margin:1.5rem auto">
  <div class="card__header">
    <div>
      ...
      <p>Виден только администраторам; при включённых публичных комментариях обсуждения также видны обычным пользователям.</p>
    </div>
  </div>
  <div class="card__body">
    <form id="banlist-comment-form" data-bid="{$comment}" data-ctype="{$ctype}" data-cid="{$cid}" data-page="{$page}">
      <label class="label" for="banlist-comment-text">Комментарий</label>
      <textarea class="textarea" id="banlist-comment-text" name="commenttext" rows="6" {if !$canedit}disabled{/if}>{$commenttext}</textarea>
      <div class="flex gap-2 mt-4">
        {if $canedit}
        <button class="btn btn--primary" type="submit">{$commenttype} комментарий</button>
        {/if}
        <button class="btn btn--secondary" type="button" onclick="history.back()">Назад</button>
      </div>
    </form>

    <div class="mt-6">
      {foreach from=$othercomments item=com name=othercomments}
        {if $smarty.foreach.othercomments.first}<h3 style="font-size:var(--fs-base);font-weight:600;margin:0 0 0.5rem">Другие комментарии</h3>{/if}
        <div class="mt-4" style="border-top:1px solid var(--border);padding-top:0.75rem">
          <div class="flex items-center justify-between">
            ...
            {if $hideadminname}<i class="text-faint">Скрыто</i>{elseif !empty($com.comname)}<strong>{$com.comname|escape}</strong>{else}<i class="text-faint">удалённый администратор</i>{/if}
          ...
          <div class="text-xs text-faint mt-2">последнее редактирование {$com.edittime} от {if $hideadminname}<i class="text-faint">Скрыто</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>удалённый администратор</i>{/if}</div>
          {/if}
        </div>
      {/foreach}
    </div>
...
<div id="banlist-root" class="p-6 space-y-4" style="max-width:1700px;margin:0 auto" data-loading="false">
  <div class="flex items-center justify-between gap-3" style="flex-wrap:wrap">
    <div>
      <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Список банов</h1>
      <p class="text-sm text-muted m-0 mt-2"><span class="tabular-nums">{$ban_list|@count}</span> из <span class="tabular-nums">{$total_bans}</span> банов</p>
    </div>
    <div class="flex gap-2 items-center">
      {if $can_export}
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=steam" title="Экспорт постоянных банов по SteamID">Экспорт Steam</a>
      <a class="btn btn--secondary btn--sm" href="exportbans.php?type=ip" title="Экспорт постоянных банов по IP">Экспорт IP</a>
      {/if}
      ...
      <a class="btn btn--secondary btn--sm"
         ...
         data-testid="toggle-hide-inactive">{if $hidetext == 'Show'}Показать{else}Скрыть{/if} неактивные</a>
      {/if}
      {has_access flag=$smarty.const.ADMIN_ADD_BAN}
      <a class="btn btn--primary btn--sm" href="index.php?p=admin&c=bans">Добавить бан</a>
      {/has_access}
    </div>
  </div>
  ...
  <form method="get" action="index.php" id="banlist-filters" style="position:sticky;top:3.5rem;z-index:20;background:var(--bg-page);padding:0.75rem 0;border-bottom:1px solid var(--border)">
    ...
    <div class="flex gap-3" style="flex-wrap:wrap">
      <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
        <input class="input" type="search" name="searchText" data-testid="bans-search"
          ...
          placeholder="Игрок, SteamID или IP&hellip;" aria-label="Поиск банов">
      </div>
      <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="bans-server-filter" aria-label="Фильтр по серверу">
        <option value="">Все серверы</option>
        {foreach $server_list as $s}
          <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
        {/foreach}
      </select>
      <select class="select" name="time" style="width:auto" data-testid="bans-time-filter" aria-label="Фильтр по времени">
        <option value="">За всё время</option>
        <option value="1d"  {if $filters.time == '1d'}selected{/if}>Сегодня</option>
        <option value="7d"  {if $filters.time == '7d'}selected{/if}>Последние 7 дней</option>
        <option value="30d" {if $filters.time == '30d'}selected{/if}>Последние 30 дней</option>
      </select>
      <button class="btn btn--secondary btn--sm" type="submit" data-testid="bans-filter-apply">
        <i data-lucide="filter"></i> Применить
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="Фильтр по статусу">
      <a class="chip"
         href="index.php?p=banlist{$chip_base_link}"
         data-state-filter=""
         data-testid="filter-chip-all"
         data-active="{if $active_state == ''}true{else}false{/if}"
         aria-current="{if $active_state == ''}true{else}false{/if}">Все</a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#ef4444"></span>Постоянные
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#f59e0b"></span>Активные
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:var(--zinc-300)"></span>Истёкшие
      </a>
      <a class="chip"
        ...
        <span class="chip__dot" style="background:#10b981"></span>Разбаненные
      </a>
    </div>
  </form>
  ...
  <div class="card" style="overflow:hidden">
    ...
    <table class="table">
      <thead>
        ...
        <tr>
          ...
          <th scope="col" class="col-tier-2">Сервер</th>
          {if !$hideadminname}<th scope="col" class="col-admin col-tier-2">Админ</th>{/if}
          <th scope="col" class="col-length col-tier-3">Срок</th>
          <th scope="col" class="col-banned col-tier-3">Забанен</th>
          <th scope="col" class="col-status">Статус</th>
          <th scope="col" class="col-actions" aria-label="Действия строки"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="ban-row ban-row--{$ban.state}" data-state="{$ban.state}" data-id="{$ban.bid}" data-testid="ban-row">
          <td>
            <div class="flex items-center gap-3" style="min-width:0">
              ...
              <a class="font-medium truncate" href="?p=banlist&amp;id={$ban.bid}" data-drawer-href="?p=banlist&amp;c=details&amp;id={$ban.bid}" data-testid="drawer-trigger">{if empty($ban.name)}<i class="text-faint">без ника</i>{else}{$ban.name|escape}{/if}</a>
            </div>
            ...
            <details class="ban-comments-inline"
              ...
              <summary class="ban-comments-inline__summary"
                       ...
                       title="{$ban.commentdata|@count} комментарий{if $ban.commentdata|@count != 1}я{/if} к этому бану">
                ...
                <span class="ban-comments-inline__label">комментари{if $ban.commentdata|@count != 1}ев{else}й{/if}</span>
              </summary>
              <ul class="ban-comments-inline__list" data-testid="ban-comments-list">
                ...
                <li class="ban-comments-inline__item" data-testid="ban-comment-item">
                  <div class="ban-comments-inline__meta">
                    ...
                    {if $hideadminname}
                      <i class="text-faint">Скрыто</i>
                    {elseif !empty($com.comname)}
                      <strong>{$com.comname|escape}</strong>
                    {else}
                      <i class="text-faint">удалённый администратор</i>
                    {/if}
                  ...
                  {if !empty($com.edittime)}
                  <div class="ban-comments-inline__edit text-xs text-faint">последнее редактирование {$com.edittime} от {if $hideadminname}<i class="text-faint">Скрыто</i>{elseif !empty($com.editname)}{$com.editname|escape}{else}<i>удалённый администратор</i>{/if}</div>
                  {/if}
                </li>
                {/foreach}
              </ul>
          ...
          <td class="text-muted truncate" style="max-width:18rem"
            ...
            {if empty($ban.reason)}<i class="text-faint">без причины</i>{else}{$ban.reason|escape}{/if}
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint mt-1" data-testid="ban-unban-meta">
                {if !empty($ban.removedby)}
                  Разбанен <span class="font-medium">{$ban.removedby|escape}</span>{if !empty($ban.ureason)}: {/if}
                {/if}
          ...
          <td class="col-admin col-tier-2 text-muted">
            {if empty($ban.aname)}<i class="text-faint">удалён</i>{else}{$ban.aname|escape}{/if}
          </td>
          {/if}
          ...
          <td class="col-length col-tier-3 tabular-nums text-muted"
              {if $ban.length != 0}title="{$ban.length_human|escape}"{/if}>{if $ban.length == 0}Постоянный{else}{$ban.length_human|escape}{/if}</td>
          ...
          <td class="col-status">
            ...
            <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}Активный{elseif $ban.state == 'permanent'}Постоянный{elseif $ban.state == 'expired'}Истёкший{elseif $ban.state == 'unbanned'}Разбанен{else}{$_pill_label|escape}{/if}</span>
          </td>
        ...
        <tr>
          ...
          <td colspan="10" style="padding:0">
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="true">
              ...
              <h2 class="empty-state__title">Нет банов, соответствующих фильтрам</h2>
              <p class="empty-state__body">Попробуйте другой поисковый запрос или сбросьте активные фильтры, чтобы увидеть все записанные баны.</p>
            ...
            <div class="empty-state" data-testid="banlist-empty" data-filtered="false">
              ...
              <h2 class="empty-state__title">Банов пока нет</h2>
              <p class="empty-state__body">Принятые меры появятся здесь, как только администраторы начнут модерацию.</p>
              {if $can_add_ban}
              <div class="empty-state__actions">
    ...
    <div class="ban-cards">
      ...
      <div class="ban-row ban-row--{$ban.state} ban-card"
        ...
        <a class="ban-card__summary flex items-center gap-3 p-4"
          ...
          <div style="flex:1;min-width:0">
            <div class="flex items-center gap-2">
              <span class="font-medium text-sm truncate">{if empty($ban.name)}<i class="text-faint">без ника</i>{else}{$ban.name|escape}{/if}</span>
              {assign var=_m_pill_label value=$ban.state|capitalize}
              <span class="pill pill--{$ban.state}">{if $ban.state == 'active'}Активный{elseif $ban.state == 'permanent'}Постоянный{elseif $ban.state == 'expired'}Истёкший{elseif $ban.state == 'unbanned'}Разбанен{else}{$_m_pill_label|escape}{/if}</span>
            </div>
            ...
            <div class="text-xs text-muted truncate" style="margin-top:0.125rem"
                 title="{if empty($ban.reason)}без причины{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}Постоянный{else}{$ban.length_human|escape}{/if}">{if empty($ban.reason)}без причины{else}{$ban.reason|escape}{/if} &middot; {if $ban.length == 0}Постоянный{else}{$ban.length_human|escape}{/if}</div>
            ...
            {if $ban.state == 'unbanned' && !$hideadminname && (!empty($ban.ureason) || !empty($ban.removedby))}
              <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="ban-unban-meta-mobile"
                   title="{if !empty($ban.removedby)}Разбанен {$ban.removedby|escape}{if !empty($ban.ureason)}: {/if}{/if}{if !empty($ban.ureason)}{$ban.ureason|escape}{/if}">
                {if !empty($ban.removedby)}Разбанен {$ban.removedby|escape}{if !empty($ban.ureason)}: {/if}{/if}
        ...
        <div class="row-actions ban-card__actions">
          ...
          <div class="text-xs text-faint truncate" style="margin-top:0.125rem;display:flex;align-items:center;gap:0.25rem" data-testid="ban-comments-count-mobile">
            <i data-lucide="message-square-text" style="width:11px;height:11px" aria-hidden="true"></i>
            <span class="tabular-nums">{$ban.commentdata|@count}</span>
            <span>комментари{if $ban.commentdata|@count != 1}ев{else}й{/if}</span>
          </div>
          {/if}
        </div>
        {/if}
      ...
      <div class="empty-state" data-testid="banlist-empty-mobile" data-filtered="true">
        ...
        <h2 class="empty-state__title">Нет банов, соответствующих фильтрам</h2>
        <p class="empty-state__body">Попробуйте другой поисковый запрос или сбросьте активные фильтры.</p>
        <div class="empty-state__actions">
  ...
  <div class="card">
    <div class="card__body">
      <nav class="text-xs text-muted" aria-label="Пагинация списка банов">
...
<dialog id="bans-unban-dialog"
  ...
  <form method="dialog" data-testid="bans-unban-form">
    <h2 id="bans-unban-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Разбанить игрока</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      Почему вы разбаниваете <strong data-testid="bans-unban-target">этого игрока</strong>? Причина будет записана в бане и отражена в журнале аудита.
    </p>
    <label class="label" for="bans-unban-reason">Причина разбана <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="Ошибочный бан, апелляция принята, срок истёк&hellip;"></textarea>
    <p class="text-xs" data-testid="bans-unban-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="bans-unban-cancel" value="cancel">Отмена</button>
      <button type="submit" class="btn btn--primary" data-testid="bans-unban-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> Разбанить
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnbanned(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === 'Active' || txt === 'Permanent' || txt === 'Expired' || txt === 'Активный' || txt === 'Постоянный' || txt === 'Истёкший') {
                if (lastText) lastText.textContent = 'Разбанен';
                else pill.textContent = 'Разбанен';
            }
        });
    ...
    function openUnbanDialog(ctx) {
        ...
        if (target) target.textContent = ctx.name || ('бан №' + ctx.bid);
    ...
    document.addEventListener('click', function (e) {
        ...
        var name = actionBtn.getAttribute('data-name') || ('бан №' + bid);
        ...
        if (act === 'bans-delete') {
            ...
            // page load.
            if (!fallback) return;
            if (!window.confirm('Навсегда удалить бан игрока «' + name + '»? Это действие нельзя отменить.')) return;
    ...
    document.addEventListener('submit', function (e) {
        ...
        if (reason === '') {
            ...
            // gate.
            showError('Пожалуйста, оставьте комментарий.');
        ...
        a.call(A.BansUnban, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            setBusy(submitBtn, false);
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'Неизвестная ошибка';
                showError(msg);
                toast('error', 'Не удалось разбанить', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnbanned);
            closeUnbanDialog();
            toast('success', 'Игрок разбанен', ctx.name + ' разбанен.');
        });
    });
