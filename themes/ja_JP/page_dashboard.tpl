...
<div class="p-6 space-y-6" style="max-width:1400px;margin:0 auto;width:100%">
    ...
    <header data-testid="dashboard-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">
            {if $dashboard_title}{$dashboard_title}{else}ダッシュボード{/if}
        </h1>
        <p class="text-sm text-muted m-0 mt-2">サーバー上のアクティビティの概覧。</p>
    </header>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        <div class="card p-5" data-testid="dashboard-stat-total-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">総 BAN 数</div>
            ...
            <div class="text-xs text-muted">全期間</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-active-bans">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">有効な BAN</div>
            ...
            <div class="text-xs text-muted">現在適用中</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-comm-blocks">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">通信制限</div>
            ...
            <div class="text-xs text-muted">ミュート + 発言制限</div>
        </div>

        <div class="card p-5" data-testid="dashboard-stat-servers">
            <div class="flex items-start justify-between">
                <div class="text-xs font-medium text-muted" style="text-transform:uppercase;letter-spacing:0.06em">サーバー</div>
            ...
            <div class="text-xs text-muted">設定済み</div>
        </div>
    ...
    {* -- Admins-only project announcement strip ----------------------
        ...
        hidden. *}
    {if $announcement}
    <aside class="announcement-strip" data-testid="dashboard-announcement" aria-label="最新のお知らせ">
        <details>
            <summary>
            ...
            <a class="announcement-strip__link" data-testid="dashboard-announcement-link"
               href="{$announcement.url}" target="_blank" rel="noopener noreferrer">
                続きを読む <i data-lucide="external-link" aria-hidden="true" style="width:13px;height:13px"></i>
            </a>
            {/if}
        </details>
    </aside>
    ...
    <div class="grid gap-4" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))">
        ...
        <section class="card" data-testid="dashboard-recent-bans">
            <div class="card__header">
                ...
                <a class="btn btn--ghost btn--sm" href="?p=banlist">
                    すべて表示 <i data-lucide="arrow-right" style="width:14px;height:14px"></i>
                </a>
            </div>
            <div>
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $b.short_name}{$b.short_name}{else}<span class="text-faint">ニックネームなし</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-bans-empty">
                    ...
                    <h4 class="empty-state__title">BAN はまだありません</h4>
        ...
        <section class="card" data-testid="dashboard-servers">
            <div class="card__header">
                <div>
                    <h3>サーバー</h3>
                    <p>{$total_servers|number_format} 件設定済み</p>
                </div>
            ...
            <div class="text-xs text-muted" style="border-bottom:1px solid var(--border);padding:0.625rem 1.25rem">
                ...
                リアルタイムでプレイヤーを管理するには、サーバーページを開いてください。
            </div>
            {/if}
            {*
            ...
            *}
            <div style="padding:0.5rem"
                ...
                <div class="empty-state" data-testid="dashboard-servers-empty">
                    ...
                    <h4 class="empty-state__title">サーバーが設定されていません</h4>
                    <p class="empty-state__body">訪問者がライブステータスを確認し、パネルから接続できるようにサーバーを追加してください。</p>
                    {if $can_add_server}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
        ...
        *}
        <section class="card" data-testid="dashboard-blocked-attempts" data-lognopopup="{if $dashboard_lognopopup}1{else}0{/if}">
            <div class="card__header">
                <div>
                    <h3>最新のブロック試行</h3>
                    <p>{$total_blocked|number_format} 件のブロック</p>
                </div>
            </div>
            <div>
                ...
                <a class="flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-blocked-row" data-id="{$p.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="font-medium text-sm truncate">
                            {if $p.short_name}{$p.short_name}{else}<span class="text-faint">ニックネームなし</span>{/if}
                        </div>
                ...
                <div class="empty-state" data-testid="dashboard-blocked-attempts-empty">
                    ...
                    <h4 class="empty-state__title">ブロック試行はまだありません</h4>
                    <p class="empty-state__body">BAN されたプレイヤーが再参加を試みると、SourceMod プラグインが毎回のブロックをここに記録します。</p>
                </div>
                {/foreach}
            </div>
        </section>
        ...
        <section class="card" data-testid="dashboard-recent-comms">
            ...
            <div>
                ...
                <a class="ban-row ban-row--{$c.state} flex items-center gap-3 p-4"
                   ...
                   data-testid="dashboard-comm-row" data-id="{$c.bid}">
                    ...
                    <div class="flex-1" style="min-width:0">
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm truncate">
                                {if $c.short_name}{$c.short_name}{else}<span class="text-faint">ニックネームなし</span>{/if}
                            </span>
                ...
                <div class="empty-state" data-testid="dashboard-recent-comms-empty">
                    ...
                    <h4 class="empty-state__title">通信制限はまだありません</h4>
                    <p class="empty-state__body">パネルまたはゲーム内で発行されたミュートや発言制限はここに表示されます。</p>
                    {if $can_add_ban}
                        <div class="empty-state__actions">
                            <a class="btn btn--primary btn--sm"
