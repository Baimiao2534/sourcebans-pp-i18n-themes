...
{else}
    <div class="flex items-end justify-between gap-3 mb-4" style="flex-wrap:wrap">
        <div>
            <h1 style="font-size:var(--fs-xl);font-weight:600;margin:0">管理員
                <span class="text-faint" style="font-weight:400;margin-left:0.375rem" data-testid="admin-count">({$admin_count})</span>
            </h1>
            <p class="text-sm text-muted m-0 mt-2">點擊管理員列的操作以編輯詳細資料、權限或伺服器存取。</p>
        </div>
        {if $can_add_admins}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=admins&amp;section=add-admin"
               data-testid="admin-add-cta"><i data-lucide="user-plus"></i> 新增管理員</a>
        {/if}
    </div>
    ...
    <div data-testid="admin-admins-section-admins">
        ...
        <div class="card" style="overflow:hidden">
            <table class="table" role="table" aria-label="管理員">
                <thead>
                    <tr>
                        <th scope="col">名稱</th>
                        <th scope="col">封禁</th>
                        <th scope="col">伺服器分組</th>
                        <th scope="col">網頁分組</th>
                        <th scope="col">免禁</th>
                        <th scope="col">最後造訪</th>
                            ...
                            <div class="row-actions" style="white-space:nowrap">
                                {if $can_edit_admins}
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="編輯詳細資料"
                                       aria-label="編輯 {$admin.user|escape} 的詳細資料"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="編輯權限"
                                       aria-label="編輯 {$admin.user|escape} 的權限"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="編輯伺服器存取"
                                       aria-label="編輯 {$admin.user|escape} 的伺服器存取"
                                    ...
                                    <a class="btn btn--ghost btn--icon btn--sm"
                                       ...
                                       title="編輯分組"
                                       aria-label="編輯 {$admin.user|escape} 的分組"
                                ...
                                {if $can_delete_admins}
                                    ...
                                    <button type="button" class="btn btn--ghost btn--icon btn--sm"
                                            ...
                                            title="刪除管理員"
                                            aria-label="刪除管理員 {$admin.user|escape}"
    ...
    <dialog id="admins-delete-dialog"
        ...
        <form method="dialog" data-testid="admins-delete-form">
            <h2 id="admins-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">刪除管理員</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                您即將永久刪除 <strong data-testid="admins-delete-target">此管理員</strong>。此操作無法復原。其伺服器存取權將立即撤銷，且任何重新雜湊佇列將被清除。
            </p>
            <label class="label" for="admins-delete-reason">原因（可選）</label>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.AdminsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', '管理員已刪除', ctx.name + ' 已被移除。');
            });
        });
