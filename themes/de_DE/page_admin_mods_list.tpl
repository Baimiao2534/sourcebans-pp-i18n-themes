...
{else}
    <div class="card">
        <div class="card__header">
            <div>
                <h3>Server-Mods</h3>
                <p><span data-testid="mod-count">{$mod_count}</span> konfiguriert</p>
            </div>
        </div>
        {if $mod_count > 0}
            <table class="table" data-testid="mods-table">
                <thead>
                    <tr>
                        ...
                        <th>Ordner</th>
                        <th><span title="SteamID-Universum (X von STEAM_X:Y:Z)">SU</span></th>
                        <th>Status</th>
                ...
                <tbody>
                    {foreach from=$mod_list item=mod}
                        <tr id="mid_{$mod.mid}" data-testid="mod-row" data-id="{$mod.mid}">
                            ...
                            <td>
                                {if $mod.enabled}
                                    <span class="pill pill--online">Aktiviert</span>
                                {else}
                                    <span class="pill pill--offline">Deaktiviert</span>
                                {/if}
                            </td>
                            {if $permission_editmods || $permission_deletemods}
                                    ...
                                    <div class="flex justify-end gap-2">
                                        {if $permission_editmods}
                                            <a class="btn btn--ghost btn--sm"
                                               ...
                                               data-testid="editmod-link">Bearbeiten</a>
                                        {/if}
                                        {if $permission_deletemods}
                                            ...
                                            <button class="btn btn--ghost btn--sm"
                                                    ...
                                                    aria-label="Mod löschen {$mod.name|escape}">Löschen</button>
                                        {/if}
                                    </div>
                                </td>
                            {/if}
        ...
        {else}
            <div class="card__body">
                <p class="text-muted">Noch keine Mods konfiguriert.</p>
            </div>
        {/if}
    </div>
    ...
    <dialog id="mod-delete-dialog"
        ...
        <form method="dialog" data-testid="mod-delete-form">
            <h2 id="mod-delete-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Mod löschen</h2>
            <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
                Sie sind dabei, <strong data-testid="mod-delete-target">diesen Mod</strong> dauerhaft zu löschen. Dies kann nicht rückgängig gemacht werden. Historische Banns und Kommunikationsblockierungen, die gegen diesen Mod aufgezeichnet wurden, behalten ihre Zeilen, verlieren jedoch ihre Spiel-Zuordnung, sodass in der Bannliste für diese Einträge in der Spalte &bdquo;Server / Mod&ldquo; &bdquo;Unbekannt&ldquo; angezeigt wird.
            </p>
            <label class="label" for="mod-delete-reason">Grund (optional)</label>
            ...
            <textarea class="textarea"
                      ...
                      maxlength="255"
                      autocomplete="off"
                      placeholder="Nur für Audit-Protokoll. Leer lassen zum Überspringen."></textarea>
            <p class="text-xs" data-testid="mod-delete-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
            <div class="flex gap-2 mt-4" style="justify-content:flex-end">
                <button type="button" class="btn btn--secondary" data-testid="mod-delete-cancel" value="cancel">Abbrechen</button>
                <button type="submit" class="btn btn--danger" data-testid="mod-delete-submit" value="confirm">
                    <i data-lucide="trash-2" style="width:13px;height:13px"></i> Mod löschen
                </button>
            </div>
        </form>
    </dialog>
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            a.call(A.ModsRemove, params).then(function (r) {
                ...
                decrementCount();
                closeDeleteDialog();
                toast('success', 'Mod gelöscht', ctx.name + ' wurde entfernt.');
            });
        });
