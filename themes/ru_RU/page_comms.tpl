...
<div class="p-6 space-y-4" style="max-width:1700px;margin:0 auto;width:100%">
    ...
    <header class="flex items-center justify-between gap-4" style="flex-wrap:wrap" data-testid="comms-header">
        <div>
            <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Блокировки связи</h1>
            <p class="text-sm text-muted m-0 mt-2">
                <span class="tabular-nums" data-testid="comms-count">{$ban_list|@count|number_format}</span>
                из <span class="tabular-nums">{$total_bans|number_format}</span> блокировок
            </p>
        </div>
        <div class="flex gap-2">
            ...
            <a class="btn btn--secondary btn--sm"
               ...
               data-testid="toggle-hide-inactive">
                ...
                {if $is_active_only}Показать неактивные{else}Скрыть неактивные{/if}
            </a>
            {if $can_add_comm}
            <a class="btn btn--primary btn--sm"
               href="index.php?p=admin&amp;c=comms"
               data-testid="comms-add-button">
                <i data-lucide="plus"></i> Добавить блокировку
            </a>
            {/if}
        </div>
    </header>
    ...
    <form method="get" action="index.php"
        ...
        <div class="flex gap-3" style="flex-wrap:wrap">
            <div style="flex:1;min-width:14rem;max-width:24rem;position:relative">
                ...
                <input class="input input--with-icon"
                       ...
                       placeholder="Игрок, SteamID или причина…"
                       data-testid="comms-search">
            </div>
            <select class="select" name="server" style="width:auto;min-width:11rem" data-testid="comms-server-filter" aria-label="Фильтр по серверу">
                <option value="">Все серверы</option>
                {foreach $servers as $s}
                    <option value="{$s.sid}" {if $filters.server == $s.sid}selected{/if}>{$s.name|escape}</option>
                {/foreach}
            </select>
            <select class="select" name="time" style="width:auto" data-testid="comms-time-filter" aria-label="Фильтр по времени">
        ...
        <div class="flex items-center gap-2 mt-2 scroll-x" role="group" aria-label="Быстрые фильтры">
        ...
        <table class="table" data-testid="comms-table">
            <thead>
                <tr>
                    ...
                    <th>Игрок</th>
                    <th>SteamID</th>
                    <th class="col-length col-tier-3">Срок</th>
                    <th class="col-tier-2">Сервер</th>
                    <th class="col-admin col-tier-2">Админ</th>
                    <th class="col-tier-3">Начат</th>
                            ...
                            <div class="flex items-center gap-3" style="min-width:0">
                                ...
                                <div style="min-width:0">
                                    ...
                                    <a class="font-medium truncate"
                                       ...
                                       data-testid="drawer-trigger">{if $comm.name}{$comm.name|escape}{else}<i class="text-faint">без ника</i>{/if}</a>
                                    ...
                                    {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                        <div class="text-xs text-faint mt-1" data-testid="comm-unban-meta">
                                            {if !empty($comm.removedby)}Снял <span class="font-medium">{$comm.removedby|escape}</span>{if !empty($comm.ureason)}: {/if}{/if}
                                    ...
                                    <details class="ban-comments-inline mt-1"
                                      ...
                                      <summary class="ban-comments-inline__summary"
                                               ...
                                               title="{$comm.commentdata|@count} комментарий{if $comm.commentdata|@count != 1}я{/if} к этой блокировке">
                                        ...
                                        <span class="ban-comments-inline__label">комментари{if $comm.commentdata|@count != 1}ев{else}й{/if}</span>
                                      </summary>
                            ...
                            <div class="row-actions">
                                ...
                                {if $can_unmute_gag && $comm.unmute_url}
                                    ...
                                    <button type="button"
                                        ...
                                        {if $comm.type == 'mute'}Снять мут{elseif $comm.type == 'gag'}Снять гаг{else}Снять блокировку{/if}
                                    </button>
                    ...
                    <tr>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">Нет блокировок, соответствующих фильтрам</h2>
                                <p class="empty-state__body">Попробуйте другой поисковый запрос, сервер или время, либо сбросьте активные фильтры, чтобы увидеть все записанные муты/гаги.</p>
                        ...
                        <td colspan="9"
                            ...
                            <div class="empty-state">
                                ...
                                <h2 class="empty-state__title">Блокировок пока нет</h2>
                                <p class="empty-state__body">Муты и гаги, выданные с панели или в игре, появятся здесь.</p>
                                {if $can_add_comm}
                                <div class="empty-state__actions">
                                    <a class="btn btn--primary btn--sm"
        ...
        <div class="ban-cards">
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-sm truncate">
                                    {if $comm.name}{$comm.name|escape}{else}<span class="text-faint">без ника</span>{/if}
                                </span>
                            ...
                            {if $comm.state == 'unmuted' && !$hideadminname && (!empty($comm.ureason) || !empty($comm.removedby))}
                                ...
                                <div class="text-xs text-faint truncate" style="margin-top:0.125rem" data-testid="comm-unban-meta-mobile"
                                     title="{if !empty($comm.removedby)}Снял {$comm.removedby|escape}{if !empty($comm.ureason)}: {/if}{/if}{if !empty($comm.ureason)}{$comm.ureason|escape}{/if}">
                                    {if !empty($comm.removedby)}Снял {$comm.removedby|escape}{if !empty($comm.ureason)}: {/if}{/if}
            ...
            {foreachelse}
                ...
                <div class="empty-state" data-testid="comms-empty-mobile" data-filtered="true">
                    ...
                    <h2 class="empty-state__title">Нет блокировок, соответствующих этим фильтрам</h2>
                ...
                <div class="empty-state" data-testid="comms-empty-mobile" data-filtered="false">
                    ...
                    <h2 class="empty-state__title">Блокировки связи пока не записаны</h2>
                    <p class="empty-state__body">Муты и гаги, выданные из панели или в игре, появятся здесь.</p>
                    {if $can_add_comm}
                    <div class="empty-state__actions">
            ...
            {if $pagination.next_url}
                <a class="btn btn--secondary btn--sm"
                   ...
                   data-testid="page-next">
                    Далее <i data-lucide="chevron-right"></i>
                </a>
            {else}
                <span class="btn btn--secondary btn--sm"
                      aria-disabled="true"
...
<dialog id="comms-unblock-dialog"
  ...
  <form method="dialog" data-testid="comms-unblock-form">
    <h2 id="comms-unblock-dialog-title" data-testid="comms-unblock-dialog-title" style="font-size:var(--fs-lg);font-weight:600;margin:0 0 0.25rem">Снять блокировку</h2>
    <p class="text-sm text-muted m-0" style="margin-bottom:0.75rem">
      Почему вы снимаете блокировку с <strong data-testid="comms-unblock-target">этого игрока</strong>? Эта причина записывается против блокировки и отображается в журнале аудита.
    </p>
    <label class="label" for="comms-unblock-reason">Причина <span aria-hidden="true" style="color:var(--danger)">*</span></label>
    ...
    <textarea class="textarea"
              ...
              maxlength="255"
              autocomplete="off"
              placeholder="Ошибочная блокировка, апелляция принята, срок истёк, &hellip;"></textarea>
    <p class="text-xs" data-testid="comms-unblock-error" role="alert" hidden style="color:var(--danger);margin:0.25rem 0 0"></p>
    <div class="flex gap-2 mt-4" style="justify-content:flex-end">
      <button type="button" class="btn btn--secondary" data-testid="comms-unblock-cancel" value="cancel">Отмена</button>
      <button type="submit" class="btn btn--primary" data-testid="comms-unblock-submit" value="confirm">
        <i data-lucide="check" style="width:13px;height:13px"></i> Подтвердить
      </button>
    </div>
  </form>
</dialog>
...
(function () {
    ...
    function flipRowToUnmuted(row) {
        ...
        Array.prototype.forEach.call(row.querySelectorAll('.pill'), function (pill) {
            ...
            if (txt === prev || txt === 'active' || txt === 'permanent' || txt === 'expired') {
                ...
                if (lastText) lastText.textContent = ' Снято';
                else pill.textContent = 'Снято';
            }
        });
        Array.prototype.forEach.call(
            ...
            function (btn) {
                ...
                a.innerHTML = '<i data-lucide="rotate-ccw" style="width:13px;height:13px"></i> Применить снова';
    ...
    function copyForType(type) {
        if (type === 'mute')    return { title: 'Снять мут',     verb: 'снимаете мут' };
        if (type === 'gag')     return { title: 'Снять гаг',      verb: 'снимаете гаг' };
        if (type === 'silence') return { title: 'Снять сайленс',  verb: 'снимаете сайленс' };
        return                          { title: 'Снять блокировку', verb: 'снимаете блокировку' };
    }
    ...
    function openUnblockDialog(ctx) {
        ...
        if (prompt) prompt.textContent = ctx.name || ('блок №' + ctx.bid);
        ...
        if (p && p.firstChild && p.firstChild.nodeType === 3) {
            p.firstChild.textContent = 'Почему вы ' + copy.verb + ' с ';
        }
    ...
    document.addEventListener('click', function (e) {
        ...
        var bid = btn.getAttribute('data-bid') || '';
        var name = btn.getAttribute('data-name') || ('блок №' + bid);
        ...
        if (act === 'comms-delete') {
            if (!window.confirm('Удалить блокировку для «' + name + '»?')) return;
            setBusy(btn, true);
            a.call(A.CommsDelete, { bid: Number(bid) }).then(function (r) {
                ...
                decrementCount();
                toast('success', 'Блокировка удалена', 'Блокировка для ' + name + ' была удалена.');
            });
            return;
        }
    ...
    document.addEventListener('submit', function (e) {
        ...
        a.call(A.CommsUnblock, { bid: Number(ctx.bid), ureason: reason }).then(function (r) {
            ...
            if (!r || r.ok === false) {
                var msg = (r && r.error && r.error.message) || 'Неизвестная ошибка';
                showError(msg);
                toast('error', 'Не удалось снять блокировку', msg);
                return;
            }
            rowsForBid(ctx.bid).forEach(flipRowToUnmuted);
            closeUnblockDialog();
            toast('success', 'Блокировка снята', ctx.name + ' разблокирован.');
        });
    });
