...
-{if NOT $permission_rcon}-
    <section class="card" data-testid="rcon-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0">Sie haben keinen RCON-Zugriff auf diesen Server.</p>
        </div>
    </section>
-{else}-
<section class="card" data-testid="rcon-console" data-sid="-{$id}-">
    <div class="card__header">
        <div>
            <h3>RCON-Konsole</h3>
            <p>Server #-{$id}- · <code class="font-mono">clr</code> eingeben zum Leeren · Ausgabe wird als Klartext gerendert.</p>
        </div>
    </div>
    <div class="card__body">
        -{*
        ...
        *}-
        <pre id="rcon"
             ...
             style="margin:0;background:var(--bg-page);border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem;height:18rem;overflow:auto;font-family:var(--font-mono);font-size:var(--fs-xs);color:var(--text);white-space:pre-wrap;word-break:break-word"><div id="rcon_con">SourceBans++ RCON-Konsole
==========================================================
Befehl unten eingeben und Enter drücken.
'clr' eingeben, um die Konsole zu leeren.
==========================================================
</div></pre>
        ...
        api.call(window.Actions.ServersSendRcon, { sid: sid, command: command, output: true }).then(function (r) {
            ...
            if (!r || !r.ok || !r.data) {
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'RCON fehlgeschlagen', body: r.error.message || 'Unbekannter Fehler' });
                }
                return;
            }
            var d = r.data;
            ...
            if (d.kind === 'error') {
                appendLine('> Fehler: ', d.error || '', 'error');
                out.appendChild(document.createElement('br'));
                scrollToBottom();
                return;
            }
