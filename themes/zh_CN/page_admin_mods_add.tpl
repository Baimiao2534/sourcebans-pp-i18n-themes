...
{else}
    <form method="post"
        ...
        <div class="card">
            ...
            <div class="card__body space-y-4" style="max-width:42rem">
                ...
                <div>
                    <label class="label" for="name">模组名称</label>
                    <input class="input"
                           type="text"
                           id="name"
                           name="name"
                ...
                <div>
                    <label class="label" for="folder">模组文件夹</label>
                    <input class="input"
                           type="text"
                           id="folder"
                           name="folder"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        磁盘上的文件夹名（例如 Counter-Strike: Source 的 <span class="font-mono">cstrike</span>）。
                    </p>
                    <div id="folder.msg"
                         class="text-xs"
                         style="color:var(--danger);display:none;margin-top:0.25rem"></div>
                ...
                <div>
                    <label class="label" for="steam_universe">Steam universe 编号</label>
                    <input class="input"
                           type="number"
                           id="steam_universe"
                           name="steam_universe"
                    ...
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        由该模组渲染的 <span class="font-mono">STEAM_X:Y:Z</span> 中的第一位数字（X）。默认为 0。
                    </p>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox"
                               ...
                               value="1"
                               checked>
                        <span class="text-sm">已启用 — 可分配给封禁和服务器。</span>
                    </label>
                </div>

                <div>
                    <label class="label">模组图标</label>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xs text-muted" data-testid="addmod-current-icon" hidden>
                            当前：<span class="font-mono" data-testid="addmod-current-icon-name"></span>
                        </span>
                    </div>
                    <p class="text-xs text-muted" style="margin-top:0.25rem">
                        16x16 的 GIF、PNG 或 JPG。将打开一个上传弹窗。
                    </p>
                    <div id="icon.msg"
                         class="text-xs"
    ...
    (function () {
        ...
        document.addEventListener('submit', function (e) {
            ...
            var errors = 0;
            if (!name)   { showMsg('name.msg',   '必须填写模组名称。'); errors++; }
            if (!folder) { showMsg('folder.msg', "必须填写模组文件夹名。"); errors++; }
            if (!icon)   { showMsg('icon.msg',   '必须为模组上传图标。'); errors++; }
            if (errors > 0) return;
            ...
            if (!a || !A) {
                ...
                showMsg('name.msg', 'JSON 调度器缺失。请刷新页面后重试。');
                return;
            }
            ...
            a.call(A.ModsAdd, {
                ...
                if (r.ok === false) {
                    ...
                    var slot = field === 'folder' ? 'folder.msg'
                        ...
                        : 'name.msg';
                    showMsg(slot, em);
                    toast('error', '添加模组失败', em);
                    return;
                }
                var data = r.data || {};
                var msg = data.message || {};
                toast('success', msg.title || '模组已添加', msg.body || '模组已添加。');
                ...
                toast('error', '添加模组失败', String(err && err.message ? err.message : err));
            });
        });
    })();
    </script>
