...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Seitenweite Konfiguration. Änderungen werden sofort wirksam.</p>
    </div>
    ...
    {else}
                <form action="?p=admin&amp;c=settings&amp;section=settings" method="post" class="space-y-4" id="form-settings-main">
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Allgemein</h3><p>Seitenidentität, Passwortregeln und Datumsangaben.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="template.title">
                                <label class="label" for="template_title">Seitentitel</label>
                            ...
                            <div data-testid="setting-row" data-key="template.logo">
                                <label class="label" for="template_logo">Logo-Pfad</label>
                                    ...
                                    *}
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-warning-template.logo">
                                        <strong>Achtung:</strong>Das Chrome rendert derzeit das Standardschild (<code class="font-mono">images/favicon.svg</code>), da der gespeicherte Pfad nicht zu einer Datei auf dem Datenträger im aktiven Thema auflöst. Prüfen Sie, dass die Datei unter <code class="font-mono">themes/&lt;theme&gt;/</code> vorhanden ist, oder leeren Sie das Eingabefeld, um die Anpassung zu entfernen.
                                    </p>
                                {/if}
                                <p class="settings-fieldset__help"
                                   id="template_logo_help"
                                   data-testid="setting-help-template.logo">
                                    Pfad zum Seitenleisten- und Anmelde-Logo, relativ zum Themenverzeichnis (<code class="font-mono">themes/&lt;theme&gt;/</code>). Standard ist <code class="font-mono">images/favicon.svg</code>. Eine 512&times;512-Raster-Version liegt unter <code class="font-mono">logos/sbpp_logo.png</code>.
                                </p>
                            </div>
                            <div data-testid="setting-row" data-key="config.password.minlength">
                                <label class="label" for="config_password_minlength">Mindestlänge des Passworts</label>
                            ...
                            <div data-testid="setting-row" data-key="config.dateformat">
                                <label class="label" for="config_dateformat">Datumsformat <span class="text-muted text-xs">(<a href="https://www.php.net/manual/en/datetime.format.php" target="_blank" rel="noopener">PHP date()</a>)</span></label>
                            ...
                            <div data-testid="setting-row" data-key="config.debug">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Debug-Modus aktivieren</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="card">
                        <fieldset class="settings-fieldset"
                            ...
                            <legend class="settings-fieldset__legend">
                                <span class="settings-fieldset__title">Authentifizierung</span>
                                <span class="settings-fieldset__hint">Lebensdauer von Sitzungs-Tokens, gemessen in Minuten. Auf <code>0</code> setzen, um einen Anmeldepfad zu deaktivieren.</span>
                            </legend>
                            <div class="settings-fieldset__body space-y-5">
                                ...
                                *}
                                <div data-testid="setting-row" data-key="auth.maxlife">
                                    <label class="label" for="auth_maxlife">Standardanmeldung</label>
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife">
                                        Wie lange eine reguläre Anmeldesitzung dauert, bevor der Benutzer abgemeldet wird, in Minuten.
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.remember">
                                    <label class="label" for="auth_maxlife_remember">&ldquo;Angemeldet bleiben&rdquo;-Anmeldung</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.remember">
                                        Wird verwendet, wenn der Benutzer das Kontrollkästchen &ldquo;Angemeldet bleiben&rdquo; im Anmeldeformular aktiviert. Typischerweise deutlich länger als die Standardsitzung.
                                    </p>
                                </div>
                                <div data-testid="setting-row" data-key="auth.maxlife.steam">
                                    <label class="label" for="auth_maxlife_steam">Steam-Anmeldung</label>
                                    <input class="input settings-fieldset__input"
                                           type="number" min="0"
                                    ...
                                    <p class="settings-fieldset__help"
                                       ...
                                       data-testid="setting-help-auth.maxlife.steam">
                                        Lebensdauer einer Sitzung, die über die Schaltfläche &ldquo;Mit Steam fortfahren&rdquo; geöffnet wurde.
                                    </p>
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Dashboard-Intro</h3><p>Öffentlicher Header und Text der Landing-Page.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="dash.intro.title">
                                <label class="label" for="dash_intro_title">Intro-Titel</label>
                                ...
                                <label class="label" for="dash_intro_text">
                                    ...
                                    <span class="text-muted text-xs">
                                        ...
                                        (<a href="https://commonmark.org/help/"
                                            target="_blank"
                                            rel="noopener"
                                            data-testid="dash-intro-cheatsheet-link">CommonMark-Spickzettel</a>).
                                        Rohes HTML wird beim Rendern maskiert.
                                    </span>
                                </label>
                                {*
                                ...
                                *}
                                <div class="dash-intro-editor"
                                    ...
                                    <div class="dash-intro-preview"
                                         ...
                                         data-loading="false"
                                         aria-label="Markdown-Vorschau">
                                        <div class="dash-intro-preview__label">Vorschau</div>
                                        {*
                                        ...
                                        *}
                                        <div class="dash-intro-preview__body"
                                            ...
                                            {else}
                                                <p class="text-xs text-muted m-0" data-empty>Geben Sie links Markdown ein, um zu sehen, wie es für Besucher gerendert wird.</p>
                                            {/if}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div data-testid="setting-row" data-key="dash.lognopopup">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Log-Popup deaktivieren (stattdessen direkte Links verwenden)</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Öffentliche Seiten</h3><p>Welche zusätzlichen Seiten Besucher erreichen können.</p></div></div>
                        <div class="card__body space-y-3">
                            <div data-testid="setting-row" data-key="config.enableprotest">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Bann-Einspruchsseite aktivieren</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablesubmit">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Bann-Einsendeseite aktivieren</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.enablecomms">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Comm-Bannliste aktivieren</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="protest.emailonlyinvolved">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Bei neuen Einsprüchen nur den ursprünglichen Bann-Administrator per E-Mail benachrichtigen</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="config.defaultpage">
                                <label class="label" for="default_page">Standard-Landing-Page</label>
                                <select class="select" id="default_page" name="default_page">
                                    ...
                                    <option value="1"{if $config_default_page == 1} selected{/if}>Bannliste</option>
                                    <option value="2"{if $config_default_page == 2} selected{/if}>Server</option>
                                    <option value="3"{if $config_default_page == 3} selected{/if}>Bann einreichen</option>
                                    <option value="4"{if $config_default_page == 4} selected{/if}>Bann anfechten</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>Bannliste</h3><p>Paginierung und Sichtbarkeit.</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="banlist.bansperpage">
                                <label class="label" for="banlist_bansperpage">Banns pro Seite</label>
                            ...
                            <div data-testid="setting-row" data-key="banlist.hideadminname">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Administratornamen in öffentlichen Bann-Infos ausblenden</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.hideplayerips">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Spieler-IPs in öffentlichen Bann-Infos ausblenden</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="banlist.nocountryfetch">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Länderabfrage für IPs überspringen</span>
                                </label>
                            </div>
                            <div data-testid="setting-row" data-key="bans.customreasons">
                                <label class="label">Benutzerdefinierte Banngründe</label>
                                <p class="text-xs text-muted m-0 mb-2">Jede Zeile wird zu einer Option im Dropdown für Banngründe. Zum Entfernen leer lassen.</p>
                                <div id="custom-reasons" class="space-y-3">
                                    ...
                                    {/foreach}
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="Weiteren Grund hinzufügen…">
                                    <input class="input" type="text" name="bans_customreason[]" placeholder="Weiteren Grund hinzufügen…">
                                </div>
                                <button type="button" class="btn btn--ghost btn--sm mt-2" onclick="addCustomReason();">
                                    <i data-lucide="plus"></i> Zeile hinzufügen
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>SMTP</h3><p>Ausgehender E-Mail-Transport.</p></div></div>
                        <div class="card__body space-y-4">
                            ...
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="smtp.user">
                                    <label class="label" for="mail_user">Benutzername</label>
                                ...
                                <div data-testid="setting-row" data-key="smtp.pass">
                                    <label class="label" for="mail_pass">Passwort <span class="text-muted text-xs">(leer lassen, um das aktuelle beizubehalten)</span></label>
                            ...
                            <div data-testid="setting-row" data-key="smtp.verify_peer">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">TLS-Peer-Zertifikat verifizieren</span>
                                </label>
                            </div>
                            <div class="grid gap-4" style="grid-template-columns:1fr 1fr">
                                <div data-testid="setting-row" data-key="config.mail.from_email">
                                    <label class="label" for="mail_from_email">Absender-E-Mail</label>
                                ...
                                <div data-testid="setting-row" data-key="config.mail.from_name">
                                    <label class="label" for="mail_from_name">Absendername</label>
                            ...
                            *}
                            <div data-testid="setting-row" data-key="smtp.test">
                                <label class="label" for="mail_test_to">Test-E-Mail senden</label>
                                <div class="grid gap-2" style="grid-template-columns:1fr auto">
                                    ...
                                    <button type="button"
                                        ...
                                        <i data-lucide="send"></i> Test-E-Mail senden
                                    </button>
                                </div>
                                <p class="text-xs text-muted mt-1">
                                    Sendet eine Verifizierungs-E-Mail über die gespeicherten SMTP-Zugangsdaten. Speichern Sie das Formular
                                    zuerst, falls Sie oben etwas geändert haben — der Test verwendet die persistenten Werte,
                                    nicht den ungespeicherten Inhalt der Eingabefelder.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    *}
                    <div class="settings-actions flex items-center justify-end gap-2">
                        <button type="button" class="btn btn--secondary btn--sm" onclick="clearCacheBtn();">
                            <i data-lucide="brush"></i> Cache leeren
                        </button>
...
(function () {
    ...
    window.addCustomReason = function () {
        ...
        inp.placeholder = 'Weiteren Grund hinzufügen…';
        box.appendChild(inp);
    };
    ...
    if (ta && preview && previewBody && window.sb && window.sb.api && window.Actions) {
        var emptyHtml = '<p class="text-xs text-muted m-0" data-empty>Geben Sie links Markdown ein, um zu sehen, wie es für Besucher gerendert wird.</p>';
    ...
    function humanizeMinutes(minutes) {
        if (!Number.isFinite(minutes) || minutes <= 0) return 'deaktiviert';
        var n = Math.floor(minutes);
        if (n < 60) return n === 1 ? '1 Minute' : n + ' Minuten';
        if (n < 1440) {
            if (n % 60 === 0) {
                var h = n / 60;
    ...
    if (testBtn && window.sb && window.sb.api && window.Actions) {
        testBtn.addEventListener('click', function () {
            ...
            if (rawTo === '' || !testTo.checkValidity()) {
                ...
                toast('warn', 'Empfänger eingeben', 'Geben Sie vor dem Senden des Tests eine gültige E-Mail-Adresse ein.');
                return;
            }
            ...
            window.sb.api.call(window.Actions.SystemTestEmail, { to: rawTo }).then(
                function (r) {
                    ...
                    if (r && r.ok && r.data && typeof r.data.to === 'string') {
                        toast('success', 'Test-E-Mail gesendet',
                            'Eine SMTP-Testnachricht wurde an ' + r.data.to + ' gesendet. Prüfen Sie den Posteingang, um die Zustellung zu bestätigen.');
                        return;
                    }
                    ...
                    var msg = err.message || 'Der Server hat eine unerwartete Antwort zurückgegeben.';
                    if (code === 'smtp_not_configured') {
                        toast('warn', 'SMTP nicht konfiguriert', msg);
                    } else if (code === 'rate_limited') {
                        toast('warn', 'Test-E-Mail gedrosselt', msg);
                    } else if (code === 'validation') {
                        toast('warn', 'Empfänger prüfen', msg);
                    } else if (code === 'mail_failed') {
                        toast('error', 'Test-E-Mail fehlgeschlagen', msg);
                    } else {
                        toast('error', 'Test-E-Mail fehlgeschlagen', msg);
                    }
                },
                function () {
                    setBusy(testBtn, false);
                    toast('error', 'Test-E-Mail fehlgeschlagen', 'Panel-API nicht erreichbar. Netzwerk prüfen und erneut versuchen.');
                }
            );
        });
    }
