...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">Системный журнал действий и предупреждений администраторов.</p>
    </div>
                ...
                <div class="card">
                    <div class="card__header">
                        <div>
                            ...
                            <p>Нажмите на строку, чтобы развернуть. Сначала новые.</p>
                        </div>
                        {if $can_owner}
                            <button type="button"
                                    ...
                                    onclick="clearLogs();">
                                <i data-lucide="trash-2"></i> Очистить журнал
                            </button>
                        {/if}
                    </div>
                    <div class="card__body">
                        ...
                        {if count($log_items) > 0}
                            ...
                            <table class="table table--clickable-rows" data-testid="logs-table">
                                <thead>
                                    <tr>
                                        <th style="width:6rem">Тип</th>
                                        <th>Событие</th>
                                        <th style="width:14rem">Пользователь</th>
                                        <th style="width:14rem">Дата</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {foreach from=$log_items item="log"}
                                        ...
                                        <tr data-testid="log-row"
                                            ...
                                            <td>
                                                {if $log.type == 'm'}
                                                    <span class="pill pill--online"><i data-lucide="info" style="width:0.75rem;height:0.75rem"></i> Инфо</span>
                                                {elseif $log.type == 'w'}
                                                    <span class="pill pill--active"><i data-lucide="alert-triangle" style="width:0.75rem;height:0.75rem"></i> Предупр.</span>
                                                {elseif $log.type == 'e'}
                                                    <span class="pill pill--permanent"><i data-lucide="circle-x" style="width:0.75rem;height:0.75rem"></i> Ошибка</span>
                                                {else}
                                        ...
                                        <tr data-detail-for="{$log.lid}" id="log-detail-{$log.lid}" hidden>
                                            <td colspan="4" style="background:var(--bg-muted)">
                                                <dl class="grid gap-2 text-xs" style="grid-template-columns:8rem 1fr;margin:0">
                                                    <dt class="text-muted">Подробности</dt>
                                                    ...
                                                    <dt class="text-muted">Функция</dt>
                                                    <dd class="font-mono" style="margin:0">{$log.function}</dd>
                                                    <dt class="text-muted">Запрос</dt>
                            ...
                            {* #1462: paired mobile card surface. The desktop table
                               ...
                               the summary. *}
                            <ul class="log-cards" data-testid="logs-cards" aria-label="Записи системного журнала">
                                {foreach from=$log_items item="log"}
                                    <li>
                                        <details class="log-card"
                                            ...
                                            <div class="log-card__meta">
                                                <span><span class="text-muted">Пользователь:</span> {$log.user}</span>
                                                <span><span class="text-muted">Дата:</span> {$log.date_str}</span>
                                            </div>
                        ...
                        {else}
                            <p class="text-muted text-sm m-0">Нет записей в журнале.</p>
                        {/if}
                    </div>
                </div>
    {/if}
...
(function () {
    ...
    window.clearLogs = function () {
        if (!window.confirm('Очистить весь системный журнал? Это действие необратимо.')) return;
