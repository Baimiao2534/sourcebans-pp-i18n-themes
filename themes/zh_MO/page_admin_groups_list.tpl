...
{if NOT $permission_listgroups}
    <div class="card"><div class="card__body"><p class="text-muted m-0">存取被拒。</p></div></div>
{else}
...
<div class="space-y-6">
    ...
    <section data-testid="web-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">網頁管理員分組</h1>
                <p class="text-sm text-muted m-0 mt-2">網頁面板分組的權限旗標與免疫。總計：{$web_group_count}。</p>
            </div>
        </div>
        ...
        {if $web_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-web" data-filtered="false">
                ...
                <h2 class="empty-state__title">尚無網頁管理員分組</h2>
                <p class="empty-state__body">網頁管理員分組可將面板權限綁定給一組管理員。建立一個分組即可一次為多名管理員指派相同的旗標。</p>
                    ...
                    <a class="admin-groups-master-detail__row flex items-center gap-3 p-4"
                       ...
                       {if $selected_group && $selected_group.gid == $group.gid}aria-current="true"{/if}>
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="text-xs text-muted">{$web_admins[$smarty.foreach.web_group.index]} 位成員</div>
                ...
                <form class="card"
                    ...
                    <div class="card__header">
                        <div>
                            ...
                            <p>{$selected_group.member_count} 位成員</p>
                        </div>
                        {if $permission_deletegroup}
                            <button type="button"
                                    class="btn btn--ghost btn--sm"
                                    data-testid="group-delete"
                                    onclick="SbppGroupsDelete({$selected_group.gid}, '{$selected_group.name|escape:'javascript'}', this);">刪除分組</button>
                        {/if}
                    </div>
                    <div class="card__body space-y-4">
                        ...
                        <div>
                            <label class="label" for="group-name-input">分組名稱</label>
                            <input class="input"
                                   id="group-name-input"
                                   name="name"
                        ...
                        <div>
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <label class="label m-0">權限旗標</label>
                                ...
                                <span class="text-xs text-muted" data-testid="flag-bitmask">{$selected_group.flags} 位元遮罩</span>
                            </div>
                        ...
                        {if $permission_editgroup}
                            <div class="flex justify-end gap-2" style="border-top:1px solid var(--border);padding-top:0.75rem">
                                <a class="btn btn--ghost" href="?p=admin&c=groups">放棄變更</a>
                                <button class="btn btn--primary"
                                        type="submit"
                                        data-testid="group-save">儲存變更</button>
                            </div>
                        {/if}
                    </div>
                </form>
            {else}
                <div class="card"><div class="card__body"><p class="text-muted m-0">在左側選擇分組以編輯其旗標。</p></div></div>
            {/if}
        </div>
        {/if}
    </section>
    ...
    <section data-testid="server-admin-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">伺服器管理員分組</h2>
                <p class="text-sm text-muted m-0 mt-2">SourceMod 管理員分組（遊戲內旗標）。總計：{$server_admin_group_count}。</p>
            </div>
        </div>
        ...
        {if $server_admin_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server-admin" data-filtered="false">
                ...
                <h2 class="empty-state__title">尚無伺服器管理員分組</h2>
                <p class="empty-state__body">伺服器管理員分組帶有 SourceMod 字元旗標與免疫。建立一個分組即可將遊戲內管理員權限授予一組管理員。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                    ...
                    <article class="card" data-testid="server-admin-group-row" data-id="{$group.id}">
                        <div class="card__header">
                            <div>
                                ...
                                <p>{$server_admins[$smarty.foreach.server_admin_group.index]} 位成員 &middot; 免疫 {$group.immunity}</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=srv&id={$group.id|escape:'url'}">編輯</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.id}, '{$group.name|escape:'javascript'}', 'srv', this);">刪除</button>
                                {/if}
                            </div>
                        </div>
                        <div class="card__body space-y-3">
                            <div>
                                <div class="text-xs font-semibold text-muted mb-2">權限</div>
                                ...
                                {else}
                                    <p class="text-xs text-muted m-0"><em>無</em></p>
                                {/if}
                            </div>
                            {if $server_admins_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">成員</div>
                                        ...
                                        {foreach from=$server_admins_list[$smarty.foreach.server_admin_group.index] item="server_admin"}
                                            <li class="flex items-center justify-between gap-2 text-sm">
                                                ...
                                                {if $permission_editadmin}
                                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=admins&o=editgroup&id={$server_admin.aid|escape:'url'}">編輯</a>
                                                {/if}
                                            </li>
                                        {/foreach}
                                    </ul>
                            ...
                            {if $server_overrides_list[$smarty.foreach.server_admin_group.index]}
                                <div>
                                    <div class="text-xs font-semibold text-muted mb-2">覆寫</div>
    ...
    <section data-testid="server-groups-section">
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h2 style="font-size:var(--fs-xl);font-weight:600;margin:0">伺服器分組</h2>
                <p class="text-sm text-muted m-0 mt-2">遊戲伺服器的群組（無權限旗標）。總計：{$server_group_count}。</p>
            </div>
        </div>
        ...
        {if $server_group_count == 0}
            ...
            <div class="empty-state" data-testid="admin-groups-empty-server" data-filtered="false">
                ...
                <h2 class="empty-state__title">尚無伺服器分組</h2>
                <p class="empty-state__body">伺服器分組可將多個遊戲伺服器綁在一起，讓您一次為多部伺服器指派管理員。建立一個分組即可開始整理您的伺服器。</p>
                {if $permission_addgroup}
                    <div class="empty-state__actions">
                        <a class="btn btn--primary btn--sm"
                           href="?p=admin&amp;c=groups&amp;section=add"
                ...
                {foreach from=$server_list item="group" name="server_group"}
                    <article class="card" data-testid="server-group-row" data-id="{$group.gid}">
                        <div class="card__header">
                            <div>
                                <h3>{$group.name|escape}</h3>
                                <p>{$server_counts[$smarty.foreach.server_group.index]} 部伺服器</p>
                            </div>
                            <div class="flex gap-1">
                                {if $permission_editgroup}
                                    <a class="btn btn--ghost btn--sm" href="index.php?p=admin&c=groups&o=edit&type=server&id={$group.gid|escape:'url'}">編輯</a>
                                {/if}
                                {if $permission_deletegroup}
                                    <button type="button" class="btn btn--ghost btn--sm" onclick="SbppServerGroupsDelete({$group.gid}, '{$group.name|escape:'javascript'}', 'server', this);">刪除</button>
                                {/if}
                            </div>
                        </div>
                        {*
                                        ...
                                        <li class="flex items-center gap-2"
                                            ...
                                            {if !$server.enabled}
                                                <span class="pill pill--offline"
                                                      ...
                                                      title="已停用 — 從公開列表隱藏，且會被每卡片 hydration 探查跳過"
                                                      style="flex-shrink:0">已停用</span>
                                            {/if}
                                        </li>
                                    {/foreach}
                                </ul>
...
function SbppGroupsSave(event) {
    ...
    sb.api.call(Actions.GroupsEdit, {
        ...
        SbppGroupsApplyResponse(r, { defaultTitle: '分組已更新' });
    });
    return false;
}

function SbppGroupsDelete(gid, name, btn) {
    if (!confirm('刪除分組「' + name + '」？')) return;
    ...
    sb.api.call(Actions.GroupsRemove, { gid: Number(gid), type: 'web' })
        .then(function (r) {
            ...
            if (r && r.ok && (r.data && (r.data.reload || (r.data.message && r.data.message.redir)))) {
                SbppGroupsApplyResponse(r, { defaultTitle: '分組已刪除' });
                return;
            }
...
(function () {
    ...
    grid.addEventListener('change', function (event) {
        ...
        preview.textContent = SbppFoldFlags(grid) + ' 位元遮罩';
    });
})();
{/literal}
</script>
