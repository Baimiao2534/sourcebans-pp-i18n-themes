...
<div class="lostpw">
    <div class="card lostpw__card">
        <div class="card__header">
            <div>
                <h3>忘記密碼？</h3>
                <p>請輸入與您管理員帳號關聯的電子郵件地址，我們會寄送重設密碼的連結給您。</p>
            </div>
        </div>
        <div class="card__body">
            <form id="lostpw-form" class="space-y-3" method="post" action="index.php?p=lostpassword" novalidate>
                {csrf_field}
                <div>
                    <label class="label" for="lostpw-email">電子郵件地址</label>
                ...
                <button id="lostpw-submit"
                    ...
                    <i data-lucide="mail"></i> 寄送重設連結
                </button>
            </form>
            <p class="text-xs text-muted mt-4" style="text-align:center">
                <a href="index.php?p=login" style="color:var(--accent)">&larr; 返回登入</a>
            </p>
        </div>
    </div>
</div>
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        sb.api.call(Actions.AuthLostPassword, { email: emailEl.value }).then(function (res) {
            ...
            if (res.ok === false) {
                var emsg = (res.error && res.error.message) || '未知錯誤';
                toast('error', '錯誤', emsg);
                return;
            }
            var msg = (res.data || {}).message;
            if (msg) {
                toast(mapKind(msg.kind), msg.title || '完成', msg.body || '');
            }
        });
    });
}());
