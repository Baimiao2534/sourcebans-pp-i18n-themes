...
<div class="login-shell">
    <div class="card login-shell__card">
        <div class="card__body">
            ...
            <h1 class="m-0" style="font-size:var(--fs-2xl);font-weight:600">登入</h1>
            <p class="text-sm text-muted mt-2">使用您的管理員憑證，或以 Steam 登入。</p>
            ...
            -{if $steamlogin_show}-
                <a class="btn btn--secondary"
                   ...
                   style="width:100%;margin-top:1.5rem">
                    <i data-lucide="gamepad-2" aria-hidden="true"></i> 使用 Steam 繼續
                </a>
            -{/if}-
            ...
            *}-
            -{if $normallogin_show}-
                <form id="loginForm"
                    ...
                    <div>
                        <label class="label" for="loginUsername">使用者名稱</label>
                        <input class="input"
                               id="loginUsername"
                               name="username"
                               type="text"
                    ...
                    <button class="btn btn--primary"
                        ...
                        登入 <i data-lucide="arrow-right" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-4 text-xs" style="text-align:center">
                    <a href="index.php?p=lostpassword" data-testid="login-lostpwd">忘記密碼？</a>
                </div>
            -{/if}-
        </div>
    </div>
...
(function () {
    ...
    var lockedBody = '您的帳號因登入失敗次數過多而暫時鎖定。'
                   + '請於大約 ' + time + ' 分鐘後再試一次。';
    var msgs = {
        no_access:    { kind: 'error', title: '無存取權限',          body: '您沒有存取此頁面的權限。請使用具有存取權限的帳號登入。' },
        empty_pwd:    { kind: 'info',  title: '密碼為空',     body: '您的帳號設定了空白密碼。請透過忘記密碼連結重設，或請管理員協助處理。' },
        failed:       { kind: 'error', title: '登入失敗',       body: '您提供的使用者名稱或密碼不正確。若忘記密碼，請使用下方的忘記密碼連結。' },
        steam_failed: { kind: 'error', title: 'Steam 登入失敗', body: 'Steam 登入成功，但您的 SteamID 未與任何帳號關聯。' },
        locked:       { kind: 'error', title: '帳號已鎖定',     body: lockedBody }
    };
    var msg = msgs[m];
    if (!msg) return;
    function show() {
