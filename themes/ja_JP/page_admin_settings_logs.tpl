...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">管理者操作と警告のシステムログ。</p>
    </div>
                    ...
                    <div class="card__body">
                        ...
                        {if count($log_items) > 0}
                            ...
                            <table class="table table--clickable-rows" data-testid="logs-table">
                                <thead>
                                    <tr>
                                        <th style="width:6rem">タイプ</th>
                                        <th>イベント</th>
                                        <th style="width:14rem">ユーザー</th>
                                        <th style="width:14rem">日付</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {foreach from=$log_items item="log"}
                                        ...
                                        <tr data-testid="log-row"
                                            ...
                                            <td>
                                                {if $log.type == 'm'}
                                                    <span class="pill pill--online"><i data-lucide="info" style="width:0.75rem;height:0.75rem"></i> 情報</span>
                                                {elseif $log.type == 'w'}
                                                    <span class="pill pill--active"><i data-lucide="alert-triangle" style="width:0.75rem;height:0.75rem"></i> 警告</span>
                                                {elseif $log.type == 'e'}
                                                    <span class="pill pill--permanent"><i data-lucide="circle-x" style="width:0.75rem;height:0.75rem"></i> エラー</span>
                                                {else}
                                        ...
                                        <tr data-detail-for="{$log.lid}" id="log-detail-{$log.lid}" hidden>
                                            <td colspan="4" style="background:var(--bg-muted)">
                                                <dl class="grid gap-2 text-xs" style="grid-template-columns:8rem 1fr;margin:0">
                                                    ...
                                                    <dt class="text-muted">関数</dt>
                                                    <dd class="font-mono" style="margin:0">{$log.function}</dd>
                                                    <dt class="text-muted">クエリ</dt>
                            ...
                            {* #1462: paired mobile card surface. The desktop table
                               ...
                               the summary. *}
                            <ul class="log-cards" data-testid="logs-cards" aria-label="システムログエントリ">
                                {foreach from=$log_items item="log"}
                                    <li>
                                        <details class="log-card"
                                            ...
                                            <div class="log-card__meta">
                                                <span><span class="text-muted">ユーザー:</span> {$log.user}</span>
                                                <span><span class="text-muted">日付:</span> {$log.date_str}</span>
                                            </div>
                        ...
                        {else}
                            <p class="text-muted text-sm m-0">ログエントリがありません。</p>
                        {/if}
                    </div>
                </div>
    {/if}
...
(function () {
    ...
    window.clearLogs = function () {
        if (!window.confirm('システムログ全体をクリアしますか？この操作は元に戻せません。')) return;
