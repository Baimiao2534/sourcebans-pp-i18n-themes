...
{if NOT $permissions_submissions}
    <div class="card" data-testid="submissions-archive-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0 mt-2">您沒有檢視封禁舉報的權限。</p>
        </div>
    </div>
{else}
        ...
        {if $submission_list_archiv|@count == 0}
            ...
            <div class="card" data-testid="submissions-archive-empty">
                <div class="empty-state">
                    ...
                    <h2 class="empty-state__title">舉報歸檔為空</h2>
                    <p class="empty-state__body">當舉報被接受、拒絕或歸檔後，會移至此處作為紀錄。</p>
                </div>
            </div>
        {else}
                        ...
                        <div class="p-4" style="background:var(--bg-muted);border-top:1px solid var(--border)">
                            ...
                            <div class="grid gap-4" style="grid-template-columns:2fr 1fr">
                                <dl class="text-sm" style="margin:0;display:grid;grid-template-columns:auto 1fr;gap:0.375rem 0.75rem">
                                    ...
                                    <dt class="text-muted">提交於</dt>
                                    <dd style="margin:0">{$sub.submitted|escape}</dd>

                                    <dt class="text-muted">SteamID</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.SteamId == ""}<span class="text-faint">無 SteamID</span>{else}{$sub.SteamId|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">IP</dt>
                                    <dd class="font-mono" style="margin:0">
                                        {if $sub.sip == ""}<span class="text-faint">無 IP 位址</span>{else}{$sub.sip|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">原因</dt>
                                    ...
                                    <dt class="text-muted">舉報者</dt>
                                    <dd style="margin:0">
                                        {if $sub.subname == ""}<span class="text-faint">無名稱</span>{else}{$sub.subname|escape}{/if}
                                    </dd>

                                    <dt class="text-muted">舉報者 IP</dt>
                                    <dd class="font-mono" style="margin:0">{$sub.ip|escape}</dd>

                                    <dt class="text-muted">歸檔者</dt>
                                    <dd style="margin:0">
                                        {if !empty($sub.archivedby)}{$sub.archivedby|escape}{else}<i class="text-faint">管理員已刪除。</i>{/if}
                                    </dd>
                                </dl>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var name = btn.dataset.name || ('舉報 #' + sid);
        var archiv = btn.dataset.archiv || '0';
        var msg;
        if (archiv === '2') msg = '要從歸檔中恢復「' + name + '」的封禁舉報嗎？';
        else if (archiv === '1') msg = '要將「' + name + '」的封禁舉報移至歸檔嗎？';
        else msg = '要刪除「' + name + '」的封禁舉報嗎？';
