...
<aside class="admin-sidebar"
       ...
       aria-label="{if $sidebar_label == 'Admin sections'}管理者セクション{elseif $sidebar_label == 'Bans sections'}BAN セクション{elseif $sidebar_label == 'Group sections'}グループセクション{elseif $sidebar_label == 'MOD sections'}MOD セクション{elseif $sidebar_label == 'Server sections'}サーバーセクション{elseif $sidebar_label == 'Settings sections'}設定セクション{elseif $sidebar_label == 'Page sections'}ページセクション{else}{$sidebar_label}{/if}">
    <details class="admin-sidebar__details" open>
        <summary class="admin-sidebar__summary">
            <span class="admin-sidebar__summary-label">
                ...
                {if $sidebar_label == 'Admin sections'}管理者セクション{elseif $sidebar_label == 'Bans sections'}BAN セクション{elseif $sidebar_label == 'Group sections'}グループセクション{elseif $sidebar_label == 'MOD sections'}MOD セクション{elseif $sidebar_label == 'Server sections'}サーバーセクション{elseif $sidebar_label == 'Settings sections'}設定セクション{elseif $sidebar_label == 'Page sections'}ページセクション{else}{$sidebar_label}{/if}
            </span>
            <i data-lucide="chevron-down" class="admin-sidebar__chevron" style="width:14px;height:14px"></i>
        </summary>
        <nav class="admin-sidebar__nav" aria-label="{if $sidebar_label == 'Admin sections'}管理者セクション{elseif $sidebar_label == 'Bans sections'}BAN セクション{elseif $sidebar_label == 'Group sections'}グループセクション{elseif $sidebar_label == 'MOD sections'}MOD セクション{elseif $sidebar_label == 'Server sections'}サーバーセクション{elseif $sidebar_label == 'Settings sections'}設定セクション{elseif $sidebar_label == 'Page sections'}ページセクション{else}{$sidebar_label}{/if}">
            {foreach from=$tabs item=tab}
                <a class="sidebar__link admin-sidebar__link"
                   ...
                   {if $tab.slug == $active_tab}aria-current="page"{/if}>
                    ...
                    <span>{if $tab.name == 'Admins list'}管理者一覧{elseif $tab.name == 'Add admin'}管理者を追加{elseif $tab.name == 'Add ban'}BAN を追加{elseif $tab.name == 'Ban protests'}BAN 異議申し立て{elseif $tab.name == 'Ban submissions'}BAN 通報{elseif $tab.name == 'Import bans'}BAN をインポート{elseif $tab.name == 'Groups'}グループ{elseif $tab.name == 'Groups list'}グループ一覧{elseif $tab.name == 'Add group'}グループを追加{elseif $tab.name == 'Mods list'}MOD 一覧{elseif $tab.name == 'Add mod'}MOD を追加{elseif $tab.name == 'Server list'}サーバー一覧{elseif $tab.name == 'Add server'}サーバーを追加{elseif $tab.name == 'Server admins'}サーバー管理者{elseif $tab.name == 'Settings'}設定{elseif $tab.name == 'Features'}機能{elseif $tab.name == 'Logs'}ログ{elseif $tab.name == 'Themes'}テーマ{elseif $tab.name == 'Add comms block'}通信制限を追加{else}{$tab.name}{/if}</span>
                </a>
            {/foreach}
        </nav>
    </details>
