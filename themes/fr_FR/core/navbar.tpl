            ...
            {foreach from=$navbar item=nav}
                {if $nav.endpoint != 'admin'}
                    <a class="sidebar__link"
                       ...
                       title="{if $nav.endpoint == 'home'}Cette page affiche un aperçu de vos bannissements et serveurs.{elseif $nav.endpoint == 'servers'}Vous pouvez consulter ici tous les serveurs et leur statut.{elseif $nav.endpoint == 'banlist'}Vous pouvez consulter ici tous les bannissements de la base de données.{elseif $nav.endpoint == 'commslist'}Vous pouvez consulter ici tous les blocages de communication (muet texte et vocal) de la base de données.{elseif $nav.endpoint == 'submit'}Vous pouvez soumettre ici des démos ou captures d'écran de joueurs suspectés de triche. Les soumissions seront examinées par un administrateur.{elseif $nav.endpoint == 'protest'}Vous pouvez déposer ici une demande de contestation de votre bannissement et expliquer pourquoi vous devriez être débanni.{else}{$nav.description}{/if}"
                       {if $nav.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $nav.endpoint == 'home'}Tableau de bord{elseif $nav.endpoint == 'servers'}Serveurs{elseif $nav.endpoint == 'banlist'}Liste des bannissements{elseif $nav.endpoint == 'commslist'}Blocages communication{elseif $nav.endpoint == 'submit'}Signaler un joueur{elseif $nav.endpoint == 'protest'}Contester un bannissement{else}{$nav.title}{/if}
                    </a>
                {/if}
            {/foreach}
        </div>
                ...
                {foreach from=$navbar item=nav}
                    {if $nav.endpoint == 'admin'}
                        <a class="sidebar__link"
                           href="index.php?p={$nav.endpoint}"
                           data-testid="nav-{$nav.endpoint}"
                           title="Ceci est le panneau de contrôle de SourceBans, vous pouvez y configurer de nouveaux administrateurs, ajouter des serveurs, etc."
                           {if $nav.state == 'active'}aria-current="page"{/if}>
                ...
                {foreach from=$adminbar item=admin}
                    <a class="sidebar__link"
                       ...
                       {if $admin.state == 'active'}aria-current="page"{/if}>
                        ...
                        {if $admin.endpoint == 'admins'}Administrateurs{elseif $admin.endpoint == 'servers'}Serveurs{elseif $admin.endpoint == 'bans'}Bannissements{elseif $admin.endpoint == 'comms'}Communications{elseif $admin.endpoint == 'groups'}Groupes{elseif $admin.endpoint == 'settings'}Paramètres{elseif $admin.endpoint == 'mods'}Modules{elseif $admin.endpoint == 'export'}Export{else}{$admin.title}{/if}
                    </a>
                {/foreach}
            </div>
        {/if}
