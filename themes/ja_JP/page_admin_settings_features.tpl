...
<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted m-0 mt-2">オプション機能と統合。</p>
    </div>
    ...
    {else}
                <form action="?p=admin&amp;c=settings&amp;section=features" method="post" class="space-y-4">
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>BAN</h3><p>公開エクスポート、KickIt、グループ/フレンド BAN。</p></div></div>
                        <div class="card__body space-y-4">
                            ...
                            <div data-testid="setting-row" data-key="config.enablegroupbanning">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">Steam グループ BAN</span>
                                </label>
                                <p class="settings-fieldset__help" id="enable_groupbanning_help" data-testid="setting-help-config.enablegroupbanning">
                                    Steam コミュニティグループの全メンバーを BAN します。
                                    {if NOT $steamapi}<br><span style="color:var(--warning)"><code>config.php</code> に Steam Web API キーが必要です。</span>{/if}
                                </p>
                            </div>
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>ログイン</h3><p>管理者に公開するログイン方法。</p></div></div>
                        <div class="card__body space-y-4">
                    ...
                    <div class="card">
                        <div class="card__header"><div><h3>コメント</h3><p>BAN / 申告に対する管理者コメントの表示設定。</p></div></div>
                        <div class="card__body space-y-4">
                    ...
                    *}
                    <div class="card">
                        <div class="card__header"><div><h3>プライバシー</h3><p>リリース優先順位付けに役立つ匿名テレメトリ。</p></div></div>
                        <div class="card__body space-y-4">
                            <div data-testid="setting-row" data-key="telemetry.enabled">
                                <label class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium">匿名テレメトリ</span>
                                </label>
                                <p class="settings-fieldset__help" id="telemetry_enabled_help" data-testid="setting-help-telemetry.enabled">
                                    1 日 1 回、パネルバージョン、PHP / DB / OS 種別、カウント（管理者、サーバー、BAN、通信ブロック、30 日間の申告/申し立て）、有効な機能を匿名 ping として送信します。ホスト名、IP、管理者名、SteamID、BAN 理由は送信<strong>されません</strong>。ping の重複排除用にランダムなインストール ID が含まれます。
                                    <a href="https://sbpp.github.io/updating/1-8-to-2-0/#anonymous-telemetry" target="_blank" rel="noopener noreferrer">完全なペイロードを見る</a>。
                                    無効化するとランダム ID がクリアされ、再有効化時に新しい ID が発行されます。
                                </p>
                            </div>
                        </div>
                    </div>
