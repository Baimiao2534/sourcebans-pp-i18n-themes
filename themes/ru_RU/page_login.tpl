        ...
        <div class="card__body">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <div class="text-xs text-muted">Панель администратора</div>
                </div>
            </div>

            <h1 class="m-0" style="font-size:var(--fs-2xl);font-weight:600">Войти</h1>
            <p class="text-sm text-muted mt-2">Используйте учётные данные администратора или войдите через Steam.</p>
            ...
            -{if !$normallogin_show and !$steamlogin_show}-
                <div class="toast"
                    ...
                    <div class="text-sm">
                        Вход в систему отключён. Обратитесь к администратору сайта.
                    </div>
                </div>
            -{/if}-
            ...
            -{if $steamlogin_show}-
                <a class="btn btn--secondary"
                   ...
                   style="width:100%;margin-top:1.5rem">
                    <i data-lucide="gamepad-2" aria-hidden="true"></i> Войти через Steam
                </a>
            -{/if}-

            -{if $steamlogin_show and $normallogin_show}-
                <div class="login-shell__divider" aria-hidden="true">
                    <span>Или по учётным данным</span>
                </div>
            -{/if}-
            ...
            *}-
            -{if $normallogin_show}-
                <form id="loginForm"
                    ...
                    <div>
                        <label class="label" for="loginUsername">Имя пользователя</label>
                        <input class="input"
                               id="loginUsername"
                               name="username"
                               type="text"
                    ...
                    <button class="btn btn--primary"
                        ...
                        Войти <i data-lucide="arrow-right" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-4 text-xs" style="text-align:center">
                    <a href="index.php?p=lostpassword" data-testid="login-lostpwd">Забыли пароль?</a>
                </div>
            -{/if}-
        </div>
    </div>
...
(function () {
    ...
    var lockedBody = 'Ваша учётная запись временно заблокирована из-за слишком большого количества неудачных попыток входа. '
                   + 'Повторите попытку примерно через ' + time + ' мин.';
    var msgs = {
        no_access:    { kind: 'error', title: 'Нет доступа',          body: 'У вас нет прав для доступа к этой странице. Войдите с учётной записью, имеющей доступ.' },
        empty_pwd:    { kind: 'info',  title: 'Пустой пароль',         body: 'Для вашей учётной записи установлен пустой пароль. Сбросьте его по ссылке восстановления пароля или обратитесь к администратору.' },
        failed:       { kind: 'error', title: 'Ошибка входа',          body: 'Указанные имя пользователя или пароль неверны. Если вы забыли пароль, воспользуйтесь ссылкой восстановления ниже.' },
        steam_failed: { kind: 'error', title: 'Ошибка входа через Steam', body: 'Вход через Steam выполнен успешно, но ваш SteamID не связан ни с одной учётной записью.' },
        locked:       { kind: 'error', title: 'Учётная запись заблокирована', body: lockedBody }
    };
    var msg = msgs[m];
    if (!msg) return;
    function show() {
