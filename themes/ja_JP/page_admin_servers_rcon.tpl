...
-{if NOT $permission_rcon}-
    <section class="card" data-testid="rcon-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0">このサーバーの RCON アクセス権がありません。</p>
        </div>
    </section>
-{else}-
<section class="card" data-testid="rcon-console" data-sid="-{$id}-">
    <div class="card__header">
        <div>
            <h3>RCON コンソール</h3>
            <p>サーバー #-{$id}- · <code class="font-mono">clr</code> でクリア · 出力はプレーンテキストで表示されます。</p>
        </div>
    </div>
    <div class="card__body">
        -{*
        ...
        *}-
        <pre id="rcon"
             ...
             style="margin:0;background:var(--bg-page);border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem;height:18rem;overflow:auto;font-family:var(--font-mono);font-size:var(--fs-xs);color:var(--text);white-space:pre-wrap;word-break:break-word"><div id="rcon_con">SourceBans++ RCON コンソール
==========================================================
下のボックスにコマンドを入力して Enter を押してください。
「clr」と入力するとコンソールをクリアします。
==========================================================
</div></pre>
        ...
        api.call(window.Actions.ServersSendRcon, { sid: sid, command: command, output: true }).then(function (r) {
            ...
            if (!r || !r.ok || !r.data) {
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'RCON 失敗', body: r.error.message || '不明なエラー' });
                }
                return;
            }
            var d = r.data;
