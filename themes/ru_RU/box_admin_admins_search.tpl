        ...
        <span class="filters-details__summary-label">
            ...
            {if $active_filter_count > 0}
                <span class="filters-details__count"
                      ...
                      aria-label="{$active_filter_count} активный фильтр{if $active_filter_count != 1}а{/if}">
                    &middot; активных: {$active_filter_count}
                </span>
            {/if}
        </span>
...
<form method="get"
    ...
    *}
    <div class="card__header filters-details__header">
        <div>
            <p>Комбинируйте любые из фильтров ниже — сервер сузит список администраторов до строк, соответствующих <strong>каждому</strong> заполненному фильтру (семантика И).</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-name" style="grid-column:1;align-self:end">Имя входа</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <input class="input"
                       id="search-admins-name"
                       name="name"
                       type="text"
                       placeholder="Совпадение с именем входа панели&hellip;"
                ...
                <select class="select"
                        ...
                        aria-label="Режим совпадения имени входа"
                        style="width:9rem">
                    <option value="0"{if $active_filter_name_match == '0'} selected{/if}>Точное совпадение</option>
                    <option value="1"{if $active_filter_name_match != '0'} selected{/if}>Частичное совпадение</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        aria-label="Режим совпадения Steam ID"
                        style="width:9rem">
                    <option value="0"{if $active_filter_steam_match != '1'} selected{/if}>Точное совпадение</option>
                    <option value="1"{if $active_filter_steam_match == '1'} selected{/if}>Частичное совпадение</option>
                </select>
            </div>
        </div>
        ...
        {if $can_editadmin}
            <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
                <label class="label" for="search-admins-admemail" style="grid-column:1;align-self:end">Эл. почта</label>
                <div class="flex gap-2" style="flex-wrap:wrap">
                    <input class="input"
                           id="search-admins-admemail"
                           name="admemail"
                           type="email"
                           placeholder="Совпадение с эл. почтой панели&hellip;"
                    ...
                    <select class="select"
                            ...
                            aria-label="Режим совпадения эл. почты"
                            style="width:9rem">
                        <option value="0"{if $active_filter_admemail_match == '0'} selected{/if}>Точное совпадение</option>
                        <option value="1"{if $active_filter_admemail_match != '0'} selected{/if}>Частичное совпадение</option>
                    </select>
                </div>
            </div>
        {/if}
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-webgroup">Веб-группа</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvadmgroup">Группа администраторов SourceMod</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvgroup">Группа серверов</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admwebflag">Веб-права</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admsrvflag">Серверные права</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-server">Сервер</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
                    ...
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                ...
                                data-server-host{if $active_filter_server == $server.sid && $active_filter_server != ''} selected{/if}>Загрузка&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
        </div>
        ...
        <div class="flex gap-2 justify-end" style="border-top:1px solid var(--border);padding-top:0.75rem">
            <a class="btn btn--ghost btn--sm"
               ...
               data-testid="search-admins-reset">Очистить фильтры</a>
            <button type="submit"
                    class="btn btn--primary btn--sm"
                    data-testid="search-admins-submit">
                <i data-lucide="search" style="width:14px;height:14px"></i> Поиск
            </button>
        </div>
    </div>
</form>
...
(function () {
    ...
    Array.prototype.forEach.call(form.querySelectorAll('option[data-server-host]'), function (opt) {
        ...
        sb.api.call(Actions.ServersHostPlayers, { sid: sid, trunchostname: 70 }).then(function (r) {
            if (!r || !r.ok || !r.data) {
                opt.textContent = 'Не в сети (' + ip + ':' + port + ')';
                return;
            }
            var d = r.data;
            if (d.error === 'connect') {
