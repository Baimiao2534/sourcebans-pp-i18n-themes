    ...
    {if NOT $permission_list}
        <div class="card" data-testid="server-list-denied">
            <div class="card__body">
                ...
                <p class="text-sm text-muted m-0">Sie haben keine Berechtigung, Server aufzulisten.</p>
            </div>
        </div>
    {else}
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                ...
                <p class="text-sm text-muted m-0 mt-2">
                    ...
                    {if $server_count == 1}registrierter Server{else}registrierte Server{/if}.
                    Live-Hostname / Karte / Spielerzahlen werden nach dem ersten Rendering geladen.
                </p>
            </div>
        ...
        {if $server_count == 0}
            <div class="card" data-testid="server-list-empty">
                <div class="card__body text-sm text-muted">
                    ...
                    {if $permission_addserver}<a href="index.php?p=admin&amp;c=servers&amp;section=add">Jetzt hinzufügen</a>, um zu beginnen.{/if}
                </div>
            </div>
        {else}
            {*
                            ...
                            {if $server.enabled}
                                ...
                                *}
                                <span class="pill pill--offline"
                                    ...
                                    <span data-status-label>Lädt</span>
                                </span>
                            {else}
                                <span class="pill pill--offline" title="Deaktiviert (in öffentlichen Listen ausgeblendet)">Deaktiviert</span>
                            {/if}
                        </header>
                        ...
                        *}
                        <dl class="text-xs text-muted" style="margin:0;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:0.25rem 0.5rem">
                            ...
                            *}
                            ...
                            <dd style="margin:0">{if isset($server.mod_name)}{$server.mod_name|escape}{else}<span class="text-faint">unbekannt</span>{/if}</dd>
                            <dt style="font-weight:500;color:var(--text)">Spieler</dt>
                            <dd style="margin:0" data-testid="server-players">—</dd>
                            <dt style="font-weight:500;color:var(--text)">Karte</dt>
        ...
        {if $permission_addserver}
            <div class="text-xs text-muted mt-4" data-testid="server-list-mapimg-hint">
                Möchten Sie einen Karten-Screenshot hochladen? Legen Sie ihn unter
                <code class="font-mono">web/images/maps/</code> ab und verwenden Sie den Kartennamen als Dateinamen
                (z. B. <code class="font-mono">de_dust2.jpg</code>).
            </div>
        {/if}
    {/if}
</section>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (!window.confirm('Server ' + label + ' löschen?\n\nDies entfernt den Server-Eintrag und alle Gruppen-/Administrator-Zuordnungen. Von diesem Server protokollierte Banns bleiben erhalten.')) {
            return;
        }
        ...
        api.call(window.Actions.ServersRemove, { sid: sid }).then(function (r) {
            if (!r || r.ok === false) {
                ...
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'Löschen fehlgeschlagen', body: r.error.message || 'Unbekannter Fehler' });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast) {
                window.SBPP.showToast({ kind: 'success', title: 'Server gelöscht', body: label });
            }
        });
    });
})();
