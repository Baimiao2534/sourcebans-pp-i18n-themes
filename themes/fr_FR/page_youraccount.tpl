...
<div class="p-6 space-y-6" style="max-width:64rem;margin:0 auto;width:100%">
    ...
    <header data-testid="account-header">
        <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">Votre compte</h1>
        <p class="text-sm text-muted m-0 mt-2">Permissions, mot de passe, mot de passe serveur et e-mail.</p>
    </header>
    ...
    *}
    <section class="card" data-testid="account-permissions">
        <div class="card__header">
            <div>
                ...
                <p>Les droits actuellement accordés à votre compte administrateur.</p>
            </div>
        </div>
                ...
                {else}
                    <p class="permissions-empty"
                       data-testid="account-permissions-web-empty">
                        <em>Aucune permission web accordée.</em>
                    </p>
                {/if}
            </div>
                ...
                {if $server_permissions}
                    <div class="permissions-grid permissions-grid--server">
                        <section class="permissions-group"
                            ...
                            <h5 class="permissions-group__title">Droits du serveur de jeu</h5>
                            <ul class="permissions-group__list">
                ...
                {else}
                    <p class="permissions-empty"
                       data-testid="account-permissions-server-empty">
                        <em>Aucun droit SourceMod accordé.</em>
                    </p>
                {/if}
            </div>
        </div>
    ...
    <section class="card" data-testid="account-change-password">
        <div class="card__header">
            <div>
                ...
                <p>Utilisez au moins {$min_pass_len} caractère{if $min_pass_len != 1}s{/if}. Vous serez déconnecté après un changement réussi.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-password-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-current-password">Mot de passe actuel</label>
                ...
                <div>
                    <label class="label" for="account-new-password">Nouveau mot de passe</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-password">Confirmer le nouveau mot de passe</label>
                    <input class="input" type="password"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-cancel">Annuler</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-save">Enregistrer</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-server-password">
        <div class="card__header">
            <div>
                <h3>Mot de passe serveur</h3>
                <p>Vous en avez besoin sur le serveur de jeu pour utiliser vos droits d&rsquo;administrateur.
                    <a href="https://wiki.alliedmods.net/Adding_Admins_%28SourceMod%29#Passwords"
                       target="_blank" rel="noopener"
                       style="color:var(--accent);text-decoration:underline">Documentation SourceMod</a>.</p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-srv-password-form"
                ...
                {if $srvpwset}
                    <div>
                        <label class="label" for="account-current-srv-password">Mot de passe serveur actuel</label>
                        <input class="input" type="password"
                ...
                {/if}
                <div>
                    <label class="label" for="account-new-srv-password">Nouveau mot de passe serveur</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-confirm-srv-password">Confirmer le nouveau mot de passe serveur</label>
                    <input class="input" type="password"
                ...
                {if $srvpwset}
                    <label class="flex items-center gap-2 text-sm">
                        ...
                        <span>Supprimer le mot de passe serveur existant (vide le champ).</span>
                    </label>
                {/if}
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-srv-cancel">Annuler</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-srv-save">Enregistrer</button>
                </div>
            </form>
        </div>
    </section>
    ...
    <section class="card" data-testid="account-change-email">
        <div class="card__header">
            <div>
                <h3>Changer l&rsquo;e-mail</h3>
                <p>Adresse actuelle :
                    {if $email}
                        <span class="font-mono text-sm" data-testid="account-current-email">{$email}</span>
                    {else}
                        <span class="text-sm text-muted" data-testid="account-current-email-empty">(non définie)</span>
                    {/if}
                </p>
            </div>
        </div>
        <div class="card__body">
            <form id="account-email-form"
                ...
                {csrf_field}
                <div>
                    <label class="label" for="account-email-password">Mot de passe</label>
                    <input class="input" type="password"
                ...
                <div>
                    <label class="label" for="account-email">Nouvelle adresse e-mail</label>
                ...
                <div>
                    <label class="label" for="account-email-confirm">Confirmer la nouvelle adresse e-mail</label>
                    <input class="input" type="email"
                ...
                <div class="flex justify-end gap-2">
                    <button class="btn btn--secondary" type="reset" data-testid="account-email-cancel">Annuler</button>
                    <button class="btn btn--primary" type="submit" data-testid="account-email-save">Enregistrer</button>
                </div>
            </form>
        </div>
    </section>
...
(function () {
    ...
    function showFieldError(prefix, err, fieldMap) {
        ...
        setMsg(prefix + domId + '-msg', err.message || 'Valeur invalide.');
        return true;
    }
    ...
    function flashSuccess(envelope) {
        ...
        showToast('success', m.title || 'Enregistré', m.body || '');
        if (m.redir) {
    ...
    function flashFailure(envelope) {
        var msg = (envelope && envelope.error && envelope.error.message)
            ...
            : 'La demande a échoué. Veuillez réessayer.';
    ...
    if (pwForm) {
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (next.length < minLen) {
                setMsg('account-new-password-msg', 'Le mot de passe doit comporter au moins ' + minLen + ' caractères.');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-password-msg', 'Les mots de passe ne correspondent pas.');
                bad = true;
            }
            if (current.length === 0) {
                setMsg('account-current-password-msg', 'Saisissez votre mot de passe actuel.');
                bad = true;
            }
            if (bad) return;
    ...
    if (srvForm) {
        srvForm.addEventListener('submit', function (ev) {
            ...
            var bad = false;
            if (next.length < minLen) {
                setMsg('account-new-srv-password-msg', 'Le mot de passe doit comporter au moins ' + minLen + ' caractères.');
                bad = true;
            }
            if (confirm !== next) {
                setMsg('account-confirm-srv-password-msg', 'Les mots de passe ne correspondent pas.');
                bad = true;
            }
            if (hasExisting && current.length === 0) {
                setMsg('account-current-srv-password-msg', 'Saisissez votre mot de passe serveur actuel.');
                bad = true;
            }
            if (bad) return;
            ...
            if (hasExisting) {
                sb.api.call(Actions.AccountCheckSrvPassword, {
                    ...
                    if (!env.data.matches) {
                        setBusy(srvBtn, false);
                        setMsg('account-current-srv-password-msg', 'Mot de passe serveur incorrect.');
                        return;
                    }
                    send();
                });
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', function (ev) {
            ...
            if (email.length === 0) {
                setMsg('account-email-msg', 'Saisissez votre nouvelle adresse e-mail.');
                bad = true;
            }
            if (confirm.length === 0) {
                setMsg('account-email-confirm-msg', 'Confirmez votre nouvelle adresse e-mail.');
                bad = true;
            }
            if (!bad && email !== confirm) {
                setMsg('account-email-confirm-msg', 'Les adresses e-mail ne correspondent pas.');
                bad = true;
            }
            if (pw.length === 0) {
                setMsg('account-email-password-msg', 'Saisissez votre mot de passe.');
                bad = true;
            }
            if (bad) return;
