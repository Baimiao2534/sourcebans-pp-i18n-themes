        ...
        <span class="filters-details__summary-label">
            ...
            {if $active_filter_count > 0}
                <span class="filters-details__count"
                      ...
                      aria-label="{$active_filter_count} 个活动筛选器">
                    &middot; {$active_filter_count} 个活动
                </span>
            {/if}
        </span>
...
<form method="get"
    ...
    *}
    <div class="card__header filters-details__header">
        <div>
            <p>组合下方任意筛选器 — 服务器将管理员列表缩小为匹配<strong>每个</strong>已填筛选器的行 (AND 逻辑)。</p>
        </div>
    </div>

    <div class="card__body space-y-3">
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-name" style="grid-column:1;align-self:end">登录名</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <input class="input"
                       id="search-admins-name"
                       name="name"
                       type="text"
                       placeholder="匹配面板登录名&hellip;"
                ...
                <select class="select"
                        ...
                        aria-label="登录名匹配模式"
                        style="width:9rem">
                    <option value="0"{if $active_filter_name_match == '0'} selected{/if}>精确匹配</option>
                    <option value="1"{if $active_filter_name_match != '0'} selected{/if}>部分匹配</option>
                </select>
            </div>
        </div>
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            ...
            <div class="flex gap-2" style="flex-wrap:wrap">
                ...
                <select class="select"
                        ...
                        aria-label="Steam ID 匹配模式"
                        style="width:9rem">
                    <option value="0"{if $active_filter_steam_match != '1'} selected{/if}>精确匹配</option>
                    <option value="1"{if $active_filter_steam_match == '1'} selected{/if}>部分匹配</option>
                </select>
            </div>
        </div>
        ...
        {if $can_editadmin}
            <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
                <label class="label" for="search-admins-admemail" style="grid-column:1;align-self:end">邮箱</label>
                <div class="flex gap-2" style="flex-wrap:wrap">
                    <input class="input"
                           id="search-admins-admemail"
                           name="admemail"
                           type="email"
                           placeholder="匹配面板邮箱&hellip;"
                    ...
                    <select class="select"
                            ...
                            aria-label="邮箱匹配模式"
                            style="width:9rem">
                        <option value="0"{if $active_filter_admemail_match == '0'} selected{/if}>精确匹配</option>
                        <option value="1"{if $active_filter_admemail_match != '0'} selected{/if}>部分匹配</option>
                    </select>
                </div>
            </div>
        {/if}
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-webgroup">Web 组</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvadmgroup">SourceMod 管理员组</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-srvgroup">服务器组</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admwebflag">Web 权限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-admsrvflag">服务器权限</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
        ...
        <div class="grid gap-3" style="grid-template-columns:12rem 1fr;align-items:end">
            <label class="label" for="search-admins-server">服务器</label>
            <div class="flex gap-2" style="flex-wrap:wrap">
                <select class="select"
                    ...
                    {foreach from=$server_list item="server"}
                        <option value="{$server.sid}"
                                ...
                                data-server-host{if $active_filter_server == $server.sid && $active_filter_server != ''} selected{/if}>加载中&hellip; ({$server.ip}:{$server.port})</option>
                    {/foreach}
                </select>
            </div>
        </div>
        ...
        <div class="flex gap-2 justify-end" style="border-top:1px solid var(--border);padding-top:0.75rem">
            <a class="btn btn--ghost btn--sm"
               ...
               data-testid="search-admins-reset">清除筛选器</a>
            <button type="submit"
