...
-{if NOT $permission_rcon}-
    <section class="card" data-testid="rcon-denied">
        <div class="card__body">
            ...
            <p class="text-sm text-muted m-0">你没有此服务器的 RCON 访问权限。</p>
        </div>
    </section>
-{else}-
<section class="card" data-testid="rcon-console" data-sid="-{$id}-">
    <div class="card__header">
        <div>
            <h3>RCON 控制台</h3>
            <p>服务器 #-{$id}- · 输入 <code class="font-mono">clr</code> 清屏 · 输出以纯文本渲染。</p>
        </div>
    </div>
    <div class="card__body">
        -{*
        ...
        *}-
        <pre id="rcon"
             ...
             style="margin:0;background:var(--bg-page);border:1px solid var(--border);border-radius:var(--radius-md);padding:0.75rem;height:18rem;overflow:auto;font-family:var(--font-mono);font-size:var(--fs-xs);color:var(--text);white-space:pre-wrap;word-break:break-word"><div id="rcon_con">SourceBans++ RCON 控制台
==========================================================
在下方输入框中输入命令并按回车。
输入 'clr' 清空控制台。
==========================================================
        ...
        api.call(window.Actions.ServersSendRcon, { sid: sid, command: command, output: true }).then(function (r) {
            ...
            if (!r || !r.ok || !r.data) {
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: 'RCON 失败', body: r.error.message || '未知错误' });
                }
                return;
            }
            var d = r.data;
