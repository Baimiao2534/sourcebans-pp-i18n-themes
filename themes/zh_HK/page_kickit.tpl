...
<head>
    ...
    <title>-{if $mode === 'kick'}-踢出玩家-{else}-封禁玩家-{/if}-</title>
    <script>
    (function () {
        try {
...
<div id="container" class="card" data-testid="kickit-container" data-mode="-{$mode}-">
    <div class="card__header">
        <div>
            <h3>正在所有伺服器上搜尋玩家&hellip;</h3>
            <p>每一列均為即時輪詢，伺服器回應後該列會隨之更新。</p>
        </div>
    </div>
                ...
                <tr data-testid="kickit-row--{$serv.num}-">
                    ...
                    <td>
                        <div id="srv_-{$serv.num}-"
                             ...
                             data-testid="kickit-status--{$serv.num}-">等待中&hellip;</div>
                    </td>
                </tr>
                -{/foreach}-
            </tbody>
    ...
    (function () {
        ...
        function setCounter(count) {
            ...
            if (srvcount === TOTAL || count === -1) {
                ...
                if (ctl) {
                    ctl.innerHTML = '<font color="green" style="font-size: 12px;"><b>搜尋完成。</b></font>' + ctl.innerHTML;
        ...
        function processRow(sid, num) {
            sb.api.call(Actions.KickitKickPlayer, { check: CHECK, sid: sid, num: num, type: TYPE, mode: MODE })
                .then(function (r) {
                    if (!r || !r.ok || !r.data) {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>錯誤。</i></span>");
                        setCounter(1);
                        return;
                    }
                    var d = r.data;
                    ...
                    if (d.status === 'no_connect') {
                        sb.setHTML('srv_' + num, "<span class='text-xs' style='color:var(--danger)'><i>無法連線至伺服器。</i></span>");
                        setCounter(1);
                    } else if (d.status === 'kicked') {
                        sb.setHTML('srv_' + num, "<span class='text-xs font-semibold' style='color:var(--success)'><u>已找到並踢出玩家！</u></span>");
                        setCounter(-1);
                    } else {
                        sb.setHTML('srv_' + num, "<span class='text-xs text-muted'>未找到玩家。</span>");
                        setCounter(1);
                    }
                });
        }
        ...
        window.addEventListener('load', function () {
            ...
            sb.api.call(Actions.KickitLoadServers, {}).then(function (r) {
                ...
                r.data.servers.forEach(function (s) {
                    if (s.has_rcon) {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-muted">搜尋中&hellip;</span>');
                        processRow(s.sid, s.num);
                    } else {
                        sb.setHTML('srv_' + s.num, '<span class="text-xs text-faint">無 rcon 密碼。</span>');
                        setCounter(1);
                    }
                });
            });
