    ...
    {if NOT $permission_list}
        <div class="card" data-testid="server-list-denied">
            <div class="card__body">
                ...
                <p class="text-sm text-muted m-0">サーバーを一覧表示する権限がありません。</p>
            </div>
        </div>
    {else}
        <div class="flex items-center justify-between gap-4 mb-4" style="flex-wrap:wrap">
            <div>
                <h1 style="font-size:var(--fs-2xl);font-weight:600;margin:0">サーバー</h1>
                <p class="text-sm text-muted m-0 mt-2">
                    <span data-testid="server-count" id="srvcount">{$server_count}</span>
                    件の登録済みサーバー。
                    ライブホスト名 / マップ / プレイヤー数は初回描画後にロードされます。
                </p>
            </div>
        ...
        {if $server_count == 0}
            <div class="card" data-testid="server-list-empty">
                <div class="card__body text-sm text-muted">
                    設定済みのサーバーはまだありません。
                    {if $permission_addserver}<a href="index.php?p=admin&amp;c=servers&amp;section=add">追加</a>して開始してください。{/if}
                </div>
            </div>
        {else}
            {*
                    ...
                    <article class="card{if !$server.enabled} card--disabled{/if}"
                            ...
                            {if $server.enabled}
                                ...
                                *}
                                <span class="pill pill--offline"
                                    ...
                                    <span data-status-label>読み込み中</span>
                                </span>
                            {else}
                                <span class="pill pill--offline" title="無効（公開リストで非表示）">無効</span>
                            {/if}
                        </header>
                        ...
                        *}
                        <dl class="text-xs text-muted" style="margin:0;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:0.25rem 0.5rem">
                            ...
                            *}
                            <dt style="font-weight:500;color:var(--text)">サーバー ID</dt>
                            <dd style="margin:0;display:flex;align-items:center;gap:0.25rem" data-testid="server-id">
                                ...
                                <button class="btn btn--ghost btn--icon btn--xs"
                                        ...
                                        title="サーバー ID をコピー"
                                        aria-label="サーバー ID をコピー">
                                    <i data-lucide="copy" style="width:12px;height:12px"></i>
                                </button>
                            </dd>
                            <dt style="font-weight:500;color:var(--text)">MOD</dt>
                            <dd style="margin:0">{if isset($server.mod_name)}{$server.mod_name|escape}{else}<span class="text-faint">不明</span>{/if}</dd>
                            <dt style="font-weight:500;color:var(--text)">プレイヤー</dt>
                            <dd style="margin:0" data-testid="server-players">—</dd>
                            <dt style="font-weight:500;color:var(--text)">マップ</dt>
        ...
        {if $permission_addserver}
            <div class="text-xs text-muted mt-4" data-testid="server-list-mapimg-hint">
                マップのスクリーンショットをアップロードしますか？
                <code class="font-mono">web/images/maps/</code> にマップ名をファイル名として
                （例：<code class="font-mono">de_dust2.jpg</code>）配置してください。
            </div>
        {/if}
    {/if}
</section>
...
(function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        var label = btn.dataset.label || ('サーバー #' + sid);
        if (!Number.isFinite(sid) || sid <= 0) return;
        if (!window.confirm(label + ' を削除しますか？\n\nサーバーエントリおよびすべてのグループ/管理者マッピングが削除されます。このサーバーからの BAN 記録は保持されます。')) {
            return;
        }
        ...
        api.call(window.Actions.ServersRemove, { sid: sid }).then(function (r) {
            if (!r || r.ok === false) {
                ...
                if (r && r.error && window.SBPP && window.SBPP.showToast) {
                    window.SBPP.showToast({ kind: 'error', title: '削除に失敗しました', body: r.error.message || '不明なエラー' });
                }
                return;
            }
            ...
            if (window.SBPP && window.SBPP.showToast) {
                window.SBPP.showToast({ kind: 'success', title: 'サーバーが削除されました', body: label });
            }
        });
    });
})();
