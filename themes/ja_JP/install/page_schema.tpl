...
<p class="lead">
    データベース <code>{$val_database}</code>（文字セット <code>{$charset}</code>）に
    SourceBans++ テーブルを作成しています。
</p>
...
{if $success}
    <div class="install-alert install-alert--ok"
        ...
        <strong>{$tables_created}</strong> 個のテーブルを作成しました。管理者アカウントの作成に進んでください。
    </div>
{else}
    <div class="install-alert install-alert--error"
         role="alert"
         data-testid="install-schema-error">
        <strong>テーブルの作成に失敗しました。</strong>
        {$errors_text}
        データベースステップに戻り、データベースユーザーの認証情報と権限
        （CREATE、ALTER、INDEX、INSERT）を確認してください。
    </div>
{/if}
